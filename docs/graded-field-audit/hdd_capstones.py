...
"""HELD DECISIONS (HD), 2026-09-22. Writes the guarded capstone migrations.
  ...
  python3 hd_capstones.py [--container hdd-scratch] [--check]
...
def load_specs():
    ...
    for p in sorted(glob.glob(os.path.join(SPECS, '*.json'))):
        s = json.load(open(p))
        if s.get('builder') != 'hdd':   # one generator per HD unit, as the waves do
            continue
        if s.get('kind') != 'rekey':
            continue
        # `fields_file` (HD fluid): a tier's `rekey_keys` keep their key, label,
        # unit and tol and take the expected value the fields file holds, which
        # its keys script regenerates from the vendored engine
        if s.get('fields_file'):
            rows = json.load(open(os.path.join(REPO, s['fields_file'])))
            for tier, t in s['tiers'].items():
                t['fields'] = {}
                for k in t.get('rekey_keys', []):
                    f = [r for r in rows if r['tier'] == tier and r['key'] == k]
                    if len(f) != 1:
                        sys.exit(f"REFUSED: {s['course']}/{tier}: {k} is in {s['fields_file']} {len(f)} times")
                    f = f[0]
                    t['fields'][k] = {'key': k, 'label': f['label'], 'unit': f['unit'],
                                      'expected': f['expected'], 'tol': f['tol']}
        out[s['course']] = s
    return out
...
def new_row(course, tier, t, cap):
    ...
    for f in cap['fields']:
        ...
        fields.append({'key': nf['key'], 'tol': nf['tol'] if isinstance(nf['tol'], int) else float(nf['tol']), 'unit': nf['unit'], 'label': nf['label'],
    ...
    for i, tier in enumerate(tiers):
        ...
        for f in cap['fields']:
            ...
            if n:
                ...
                lines.append(f"--       -> {n['key']} '{n['label']}' {n['unit']}, expected {float(n['expected'])!r}, tol {n['tol'] if isinstance(n['tol'], int) else float(n['tol'])!r}")
        for a, b in t.get('prompt_edits', []):
