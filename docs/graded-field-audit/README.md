...
- hydraulics beginner `hb_tau_y_Pa` equals the light mud's measured 3 rpm stress (4 dial degrees) in the rheology explorer's default residual table, because the capstone mud's 2 x theta3 less theta6 is also 4 degrees. Fixing it needs a re-key. **Re-keyed in HD** (`20261030c_hd_hydraulics.sql`, see "Held decisions (HD)" below).
...
- **mbal** (A beginner; B intermediate, advanced): beginner moves to the ISAN tank, stated in the brief and typed into the tank explorer's Your tank mode. The stripped tiers work a teaching Fetkovich aquifer (180 degrees, 150 md, reD 4), reD 3 for the pD asymptote and an Ahmed 11-1 variant at N 9.2 MMstb; the aquifer explorer opens on the teaching aquifer (the book's printed column shows only at the published geometry), the pD explorer on reD 10, and the tank explorer gains a Dake Exercise 9.2 mode that opens with no aquifer. Residual finding: the 3 MMSTB tolerance on the Dake oil in place admits Dake's own published least squares fit (310.2 MMSTB) and the one-parameter history match.
- **welltest** (A beginner; B intermediate, advanced): beginner moves to the OBODO-4 buildup, a file of 40 shut-in pressures generated by the engine's own buildup superposition and opened in the buildup explorer's Your test mode. The stripped tiers needed panel defaults only (no lesson printed a graded value): the diagnostic explorer opens on the drawdown. Found: the learning page header printed the fault late-line permeability to one decimal, inside that intermediate field's tolerance; it now prints a whole number.
...
## HD (held decisions, 2026-09-22)
The owner's held decisions on the follow-on residuals ("proceed to implement your recommendations"). `hd_capstones.py` generates `migrations/20261030<x>_hd_<course>.sql` from `hd/<course>.json` on a scratch replay of the post-W2-W6 state; each file applies after the whole W2-W6 batch. `normalize.py` records each fix with `wave: hd`, so `audit.py --post --wave ... --wave hd` checks it on the replay and the same dump without `--wave hd` is RED.
- **mbal advanced `dake_ooip_mmstb`** (`20261030a_hd_mbal.sql`, a tightening, tol 3 -> 0.5 MMSTB, attempts guard with an empty D5 allowlist). The W5c residual: tol 3 passed Dake's published least squares fit (310.2) and the course's history match (310.1986). After W5c the only route is the tank explorer's Dake mode with the finite Carter-Tracy aquifer, which prints 307.22141 against the key 307.2214095537205 (the baseline source "prompt-given" meant lesson-given; the brief never stated the value). 0.5 is half a whole MMSTB: 307 passes, and 310.2, 312 and the whole 95 percent interval of the history match fail. Gate: `src/components/course/panels/mbal/hdDakeTol.test.jsx` renders the panel on the route and takes every other number from the vendored engine and its fixture.
## Held decisions (HD), 2026-09-22

Owner instruction 2026-09-22: "Proceed to implement your recommendations for the held decisions". Each unit is a spec `hd/<course>.json` (wave `hd`); `hd_capstones.py` writes its guarded migration `migrations/20261030<x>_hd_<course>.sql` from a local scratch in the post-W2-W6 state, and `normalize.py` `hd_shipped()` records each re-key with `wave: hd` and `closes_leak`, chaining the fix it already carried as `prior`. The files apply after the whole W2-W6 batch. `audit.py --post ... --wave w5d --wave hd` checks the HD state; without `--wave hd` it checks the post-W2-W6 state.

| Unit | File | Kind | What moves |
|---|---|---|---|
| hydraulics beginner | `20261030c_hd_hydraulics.sql` | rekey | `hb_tau_y_Pa` (2.04161424376 Pa, tol 5e-4) becomes `hb_n`, the Herschel-Bulkley flow behaviour index of the same capstone mud (0.7269815055935842, tol 5e-7, pl_n's tolerance; the typed Three models view prints it at 8 dp). |

**hydraulics beginner.** The capstone mud's yield, (2 x 5 - 6) = 4 dial degrees, is the light mud's 3 rpm reading, so the rheology explorer's default residual table printed the graded 2.04161 Pa with the Your mud boxes blank. Keeping the quantity would need a new mud, which moves all eighteen fields on three tiers, so the field moves to the same mud's Herschel-Bulkley n: it is fitted once the yield stress is taken off the 600 and 300 rpm stresses (the Bingham yield point there gives n = 1), and only the typed mud prints it. `panelCapstoneGuard.test.js` drops W4a's pinned exception and adds the controls: the blank-box sweep finds the old yield on the light mud and not the new n, the typed mud prints the new n, no lesson or panel source carries it. The other beginner fields keep their keys: the same sweep finds no other beginner hit (the one remaining pinned exception is intermediate, a swab EMW read as a velocity in mm/s). Lesson changed: `src/content/courses/hydraulics/beginner/m06-the-associate-reading/l02-working-the-capstone.md`.
## HD (held decisions): nodal lift gas typed column, 2026-09-22
Owner, 2026-09-22: "Proceed to implement your recommendations for the held decisions." The nodal intermediate lift gas fields `liftgas_valve_pwf_psia` and `liftgas_mid_pmf_psia` had no route: W2 left them for a W4 panel mode (a 20-step Cullender and Smith march with z at trial pressures) and no W4 part built it.
- **Panel.** The Outflow explorer (`panels/nodal/VlpExplorer.jsx`) gains "Your gas column, typed" (mode `typedcolumn`): tubing head pressure, gravity, measured and vertical depth, both temperatures and the step count as boxes, marched by `typedGasColumn` in `nodalLab.js`, one call to the vendored `cullenderSmithBhp`. It prints the bottom and midpoint pressures with `plain(v, 6)`. The view opens on the BONNY-7 teaching column and the panel still opens on "The J, taken apart".
- **Gate.** `panels/nodal/yourCase.test.js`: the brief's column typed as printed reproduces both keys at 6 dp (`typedRouteMiss`), the default and the rendered opening views land on no graded answer (D2 rules 1 to 3 and the lab's own leak guard), and the negative controls (two station default, vertical depth typed as the measured one, a 2 dp print, the brief column planted as a default, a graded answer planted in the page) are all caught.
- **Brief.** `hd_typed_capstones.py` writes `migrations/20261030e_hd_nodal.sql` from `hd/nodal.json` (prompt only, content-addressed on the post-W2-W6 row, refuses without the W2-W6 state; no attempts guard, nothing re-grades). It joins the W2-W6 batch after every file in it.
- **Audit.** `normalize.py` folds `hd/*.json` in as wave `hd` (source `nextgen-panel`, class none, `case_mode`, `prompt_append`). `audit.py --post --wave hd` checks the pointer is on the brief; without `--wave hd` a brief that already carries it is RED, so a prompt-only wave can be told from the state before it. Selftest controls cover both. The two fields leave `display` / `no_route` (display 11 to 9, no_route 6 to 4).
## HD (held decisions): fluid advanced recut, 2026-09-22
- **fluid advanced, the labTune Bo fallback fix** (engines #235, lead decision Option A). The engine's Bo fallback, used when the model is two phase at the lab reservoir conditions, returned a null Bo at random, so the Good Oil regression met its penalty residual and every tuned figure the tier taught and graded was an artefact of it. With the fix re-vendored: `20261030d_hd_fluid.sql` (kind rekey) moves the six W5b keys' expected values in place onto the fixed engine's (key, label, unit and tol unchanged; attempts guard, empty D5 allowlist), from `hd/fluid/fields.json`, which `hd/fluid/keys.mjs` regenerates and `panelCapstoneGuard.test.js` checks (`w5b/fluid.fields.json` stays the W5b state, so W5b's fix is the `prior`). `20261030f_hd_fluid_quiz.sql` (kind quiz, generated by `hd_quiz.py` from `hd/quiz/fluid.json`) retires and re-inserts the seven served questions that printed the old figures, in the W6 slot pattern, every figure rendered by `hd/fluid/figures.mjs`. The advanced tuning lessons follow the engine (`src/components/course/panels/fluid/tuningLessonFigures.test.js`), and the W6 fluid finals (`20261029_w6_fluid.sql`, unapplied) were regenerated in place on the fixed engine.
