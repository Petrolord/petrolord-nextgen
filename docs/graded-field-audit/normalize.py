...
# HELD DECISIONS (HD, owner 2026-09-22): typed "your case" panel modes for the
# fields no earlier wave routed, from hd/<course>.json (hd_typed_capstones.py).
# The W4b shape: a field the typed mode prints at the graded precision moves to
# source `nextgen-panel`, class `none`, with `case_mode`; the brief gains one
# pointer sentence (the spec's own 20261030<x>_hd_<course>.sql, prompt only),
# carried as `prompt_append` so `audit.py --post --wave hd` checks it is on the
# brief, and checks it is absent when hd is not named. No key, expected value or
# tolerance moves.
def hd_typed_shipped():
    out = {}
    for p in sorted(glob.glob(os.path.join(HERE, 'hd', '*.json'))):
        s = json.load(open(p))
        if s.get('builder') != 'hde':
            continue
        course = s['course']
        for tier, t in s['tiers'].items():
            for key, k in t.get('keys', {}).items():
                ref = f"{k['panel']}:{k['line']}"
                out[(course, tier, key)] = {
                    'migration': s['migration'], 'wave': 'hd', 'decided': True, 'class': 'none',
                    'prompt_append': t['pointer'].strip(),
                    'case_mode': {'panel': k['panel'], 'mode': k['mode'], 'test': k['test']},
                    'annot_update': {
                        'source': 'nextgen-panel',
                        'source_ref': f"{ref} ({k['mode']} mode: the case typed as the prompt states it)",
...
# HELD DECISIONS (HD, 2026-09-22), generated by hd_capstones.py from hd/<course>.json.
# A spec re-keys named fields in place (the rest of the tier is untouched). Each
# baseline key records its replacement with `wave: hd`; an earlier fix on the same
# key rides along as `prior`, so a dry run without --wave hd still checks the
# earlier state. The replacement is printed nowhere a learner reads first
# (`closes_leak`, proved by the course's leak gate).
def hdb_shipped(prev):
    sys.path.insert(0, HERE)
    import hdb_capstones as hd
    out = {}
    for course, s in sorted(hd.load_specs().items()):
        # a key an earlier wave re-keyed, traced to the baseline key it replaced
        back = {(c, t, v['rekey']['key']): k for (c, t, k), v in prev.items()
                if c == course and isinstance(v, dict) and 'rekey' in v}
        for tier, t in s['tiers'].items():
            for live_key, new_key in t['rekey'].items():
                base_key = back.get((course, tier, live_key), live_key)
                nf = hd.engine_field(s, tier, new_key)
                ann = dict(t.get('annot', {}).get(new_key, {}))
                printed = ann.pop('default_state_prints', False) or ann.pop('lesson_prints', False)
                e = {'migration': s['migration'], 'wave': 'hd', 'decided': True, 'closes_leak': not printed,
                     'rekey': {'key': nf['key'], 'label': nf['label'], 'unit': nf['unit'],
                               'expected': float(nf['expected']), 'tol': float(nf['tol'])},
                     'rekey_annot': {**ann, 'class': 'none', 'owner_decision': False, 'leak': bool(printed)}}
                p = prev.get((course, tier, base_key))
                if p:
                    e['prior'] = {x: v for x, v in p.items() if x not in ('decided', 'class', 'annot_update')}
                out[(course, tier, base_key)] = e
    return out


overlay(hdb_shipped(SHIPPED))


# HELD DECISIONS (HD), 2026-09-22, generated by hdd_capstones.py from hd/<course>.json.
# A re-key replaces live fields in place. HD fluid (engines #235, the labTune Bo
# fallback fix) keeps each W5b key, label, unit and tol and moves its expected
# value onto the fixed engine's; the live key is traced back to the baseline key
# W5b re-keyed, and W5b's fix rides along as `prior`, so `audit.py --post`
# without `--wave hd` still checks the post-W2-W6 state.
def hdd_shipped(prev):
    sys.path.insert(0, HERE)
    import hdd_capstones as hd
    out = {}
    for course, s in sorted(hd.load_specs().items()):
        back = {(c, t, v['rekey']['key']): k for (c, t, k), v in prev.items()
                if c == course and isinstance(v, dict) and 'rekey' in v}
        for tier, t in s['tiers'].items():
            for live_key, nf in t.get('fields', {}).items():
                base_key = back.get((course, tier, live_key), live_key)
                ann = dict(t.get('annot', {}).get(nf['key'], {}))
                printed = ann.pop('default_state_prints', False) or ann.pop('lesson_prints', False)
                e = {'migration': s['migration'], 'wave': 'hd', 'decided': True, 'closes_leak': True,
                     'rekey': {'key': nf['key'], 'label': nf['label'], 'unit': nf['unit'],
                               'expected': float(nf['expected']), 'tol': float(nf['tol'])},
                     'rekey_annot': {**ann, 'class': 'none', 'owner_decision': False, 'leak': bool(printed)}}
                p = prev.get((course, tier, base_key))
