    ...
    for slug, prefix, stem, name in COURSES:
        doc = {
            ...
            'method': (f'OLD is the row the APPLIED deep seed migrations/{stem}_<tier>_deep.sql wrote: the generator '
                       f'rebuilds the 132 VALUES lines from the banks before the recut (OLD_REF) and requires them to be that file\'s '
                       'VALUES block byte for byte. NEW is what the same generator writes from the recut banks '
                       '(NEW_REF). No production access: OLD was not compared with a dump of the live rows; each '
