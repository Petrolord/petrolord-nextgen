// THE D5 CAPSTONE CASE FILES are the data the three capstones are set on, and
// they ship in the zip (src/content/capstone-cases/appliedai). This suite proves
...
// engine's own inputs (tools/course-waves/appliedai/gen_course.py running
// d5_capstone.mjs through THIS repository's vendored engine), so a case file
...
const CASES = path.join(ROOT, 'src/content/capstone-cases/appliedai');
const WAVE = mirrorDir('appliedai');

describe('THE D5 CAPSTONE CASE FILES', () => {
  it('are byte-identical to the committed generator run through this repository\'s vendored engine', () => {
    const out = fs.mkdtempSync(path.join(os.tmpdir(), 'd5cases-'));
    ...
    execFileSync('python3', [path.join(WAVE, 'gen_course.py')], {
      ...
      env: {
        ...
        D5_WAVE: WAVE,
        D5_REPO: ROOT,
        D5_ENGINES: path.join(ROOT, 'packages/engines'),
        D5_TOLERANCE: path.join(HERE, 'gradedTolerance.js'),
        D5_COURSE_OUT: course,
        D5_CASES_OUT: out,
      },
    });
    ...
    const mig = fs.readdirSync(path.join(ROOT, 'migrations')).find((f) => /_d5_appliedai_course\.sql$/.test(f));
    expect(fs.readFileSync(path.join(ROOT, 'migrations', mig), 'utf8')).toBe(fs.readFileSync(course, 'utf8'));
    fs.rmSync(out, { recursive: true, force: true });
    console.log(`[appliedai cases] ${committed.length} case files and the course migration regenerate byte for byte`);
  }, 120000);
  ...
  it('reach the capstone card only: no panel and no lab imports them', () => {
    ...
    const page = fs.readFileSync(path.join(ROOT, 'src/pages/apps/AppliedaiLearningPage.jsx'), 'utf8');
    expect(page).toMatch(/@\/content\/capstone-cases\/appliedai/);
    expect(page).toMatch(/<CapstoneCaseFiles files=\{APPLIEDAI_CASE_FILES\[tier\]\}/);
  });
});
