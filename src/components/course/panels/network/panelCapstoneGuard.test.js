...
//
// PD7's capstone is a different gathering system under different conditions
// from anything in this lab, and networkLab.js carries no capstone material at
// all: there is no CAP, no graded field, no `capstoneValues`, nothing for a
...
// gathering systems can be different in every input and still land on the same
// number, and the grader does not care which system produced a figure. So this
// gate is arithmetic and not provenance: it takes the wave's graded field
// list, builds a forbidden neighbourhood around every graded answer, and
// checks EVERY number the lab exposes and EVERY number the shipped teaching
// digest prints against all of them.
//
...
// of the value. A trunk mass is accepted within 0.0089 POUNDS A DAY of its
// answer, not within 0.0089 per cent of it, and a gap fraction within 8.4e-9
// of its answer. Read that function before touching the numbers below: a guard
...
// is still a very small target: ten times 8.4e-9 is 8.4e-8 on a fraction, and
// ten times 0.00037 is 0.0037 on a mass rate.
//
...
//   x1000        lb/d read as thousands of lb/d, ft read as kft, psi read as
//                millipsi
//   x0.001       the same shift the other way
//   x100         a FRACTION read as a PERCENTAGE. This is the live confusion
//                in this domain and the wave's own leak check carries it for
//                that reason: a conservation gap is graded as a fraction and
//                every lesson prints it as a percentage.
//   x0.01        the same shift the other way
//
// The band travels with the value under every restatement, because the
// tolerance is absolute: a band of 0.0089 lb/d restated in thousands is
// 0.0000089.
//
// THE GUARD IS DIMENSION BLIND, deliberately. The grader compares numbers and
// never asks what they were a measurement of, so neither does this. A psia, a
// lb/d and a stb/d are all just numbers to it.
...
const WAVE_DIR = '/root/pd-wip-network';
 ...
 */
const GRADED_FIELD_NAME = ['OBI', 'AFU'].join('');
 ...
 * it, and it is in this domain's list because a conservation gap is graded as
 * a fraction and taught as a percentage.
 */
...
/** The number token the wave's own leak guard, pd7_leakcheck.mjs, scans with. */
...
describe.skipIf(!fieldsAvailable)('THE LEAK GATE: no teaching number may be a graded capstone answer', () => {
  ...
  it('and so is the learning page, which reads the same lab', () => {
    const page = path.join(HERE, '..', '..', '..', '..', 'pages', 'apps', 'NetworkLearningPage.jsx');
    if (!fs.existsSync(page)) return;
    const text = fs.readFileSync(page, 'utf8');
    const hits = [];
    (text.match(DIGEST_NUMBER) || []).forEach((tok) => {
      const hit = leakGuardHit(Number(tok), targets);
      if (hit) hits.push(`NetworkLearningPage.jsx prints ${tok}, inside ${hit.tier}/${hit.key} (${hit.tag})`);
    });
    expect(hits).toEqual([]);
  });
});
...
describe.skipIf(!fieldsAvailable || !digestAvailable)('and neither does the shipped teaching digest', () => {
  ...
  it('NO NUMBER THE DIGEST PRINTS lands in any forbidden neighbourhood', () => {
    ...
    // prose. The wave's own pd7_leakcheck.mjs makes the same statement in the
    // same five unit shiftings; this repeats it inside the suite, so it
...
describe('the guard itself is honest about when it cannot run', () => {
  it('says so rather than passing silently when the wave directory is absent', () => {
    ...
    // without /root/pd-wip-network the two blocks above skip, and this one
    // records why.
    if (!fieldsAvailable) {
  ...
  it('the lab carries no capstone surface at all, which is the other half of the guarantee', () => {
    ...
    expect(named, `networkLab.js has grown a capstone surface: ${named.join(', ')}`).toEqual([]);
  });
  ...
  it('and the teaching network is named instead, which is the case a lesson may quote', () => {
    expect(L.TEACHING_NETWORK_NAME).toBe('AGBADA WEST');
    expect(L.teachingConditions().name).toBe('AGBADA WEST');
  });
});
