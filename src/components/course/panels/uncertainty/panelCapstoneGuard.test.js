...
//
// The lab's leak gate in uncertaintyLab.test.js walks the TEACHING exports'
// return values. It says nothing about the PANELS, and the panels import the
// lab directly. The lab exports its whole capstone surface (UMUNEDE,
// UMUNEDE_BELIEF, UMUNEDE_SEED, umunedeRuns, umunedeCapstoneFields and friends)
...
//   UMUNEDE                        ->  ISIALA, OKPOMA, NTEJE
//   UMUNEDE_BELIEF                 ->  BELIEF, NARROW_OPEX
//   UMUNEDE_SEED, UMUNEDE_MC       ->  DEFAULT_SEED, DEFAULT_MC_SEED, APP_MC
//   umunedeRuns                    ->  ledger / breakevenRun / scenarioBuilderMonteCarlo
//   umunedeCapstone*               ->  nothing; a panel never needs a graded answer
...
const LEARNING_PAGE = path.resolve(HERE, '../../../../pages/apps/UncertaintyLearningPage.jsx');

const panelSources = fs
  .readdirSync(HERE)
  .filter((f) => f.endsWith('.jsx') && !f.endsWith('.test.jsx'))
  .map((f) => ({ file: f, text: fs.readFileSync(path.join(HERE, f), 'utf8') }))
  .concat([{ file: 'UncertaintyLearningPage.jsx', text: fs.readFileSync(LEARNING_PAGE, 'utf8') }]);
...
describe('THE PANEL GUARD: no panel may read the capstone', () => {
  it('there are panels to check, so a rename cannot silently empty this gate', () => {
    expect(panelSources.map((s) => s.file).sort()).toEqual([
      'BreakevenExplorer.jsx', 'RiskExplorer.jsx', 'ScreeningExplorer.jsx', 'UncertaintyLearningPage.jsx',
    ]);
  ...
  it('the lab names every capstone export in CAPSTONE_ONLY_EXPORTS, so the grep list cannot go stale', () => {
    const named = Object.keys(L).filter((k) => /umunede/i.test(k));
    ...
    L.CAPSTONE_ONLY_EXPORTS.forEach((k) => {
      ...
      expect(k, `${k} does not carry the capstone name`).toMatch(/umunede/i);
    });
  });

  it('NEGATIVE CONTROL: the grep finds a capstone name and a graded answer when one is planted', () => {
    const planted = `import { umunedeCapstoneValues } from './uncertaintyLab';\nconst x = ${JSON.parse(fs.readFileSync(FIELDS_JSON, 'utf8'))[4][2].toPrecision(6)};`;
    expect(L.CAPSTONE_ONLY_EXPORTS.filter((name) => new RegExp(`\\b${name}\\b`).test(planted))).toEqual(['umunedeCapstoneValues']);
    const graded = JSON.parse(fs.readFileSync(FIELDS_JSON, 'utf8')).map((f) => f[2].toPrecision(6).replace(/0+$/, ''));
  ...
  panelSources.forEach(({ file, text }) => {
    ...
    it(`${file} names no umunede reader and never says the field's name`, () => {
      const hits = [...text.matchAll(/umunede\w*/gi)].map((x) => x[0]);
