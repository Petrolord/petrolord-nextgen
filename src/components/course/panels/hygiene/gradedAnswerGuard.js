// EVERY STRING SHAPE A GRADED H2 CAPSTONE ANSWER CAN REACH A LEARNER AS.
//
...
//
// AND ONE THING ABOUT THIS WAVE'S ANSWER KEY THAT THE GUARD REPORTS RATHER THAN
// HIDES. Some of the eighteen H2 values already fit inside nine significant
...
// instead of twice. panelCapstoneGuard.test.js SEARCHES for a probe field that
// can distinguish a nine-only matcher from this one rather than taking the first.
...
export const allRenderings = (fields) => {
  if (!Array.isArray(fields) || fields.length !== 18) {
    throw new Error(`[hygiene guard] expected eighteen graded fields, got ${Array.isArray(fields) ? fields.length : typeof fields}`);
  }
