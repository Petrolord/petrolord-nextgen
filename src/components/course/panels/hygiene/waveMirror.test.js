// THE H2 MIRROR GATE: the committed wave inputs against the live wave
// directory, and a STATEMENT when there is no live one.
//
// WHY. The committed copy under tools/course-waves/hygiene is what every H2
...
describe('THE H2 MIRROR GATE', () => {
  it('the wave is registered with a live directory, an inputs list and pins', () => {
    expect(entry, 'hygiene is not registered in tools/course-waves/waves.json').toBeTruthy();
    expect(entry.course).toBe('H2');
    ...
    console.log(`[hygiene mirror] ${entry.inputs.length} committed inputs, 2 pinned by sha256`);
  });
  ...
  it('SAYS WHICH JOB IT DID, so a vacuous run is visible in the output', () => {
    console.log(LIVE
      ? `[hygiene mirror] a live wave directory is present at ${LIVE}: byte-comparing all ${entry.inputs.length} inputs`
      : `[hygiene mirror] no live wave directory: asserting the committed copy at ${MIRROR} is internally whole`);
  ...
  it('the committed copy is byte-identical to the live wave directory, or there is none and it says so', () => {
    if (!LIVE) {
      ...
      console.log('[hygiene mirror] no live wave directory to compare against, which is the CI case. '
    ...
    console.log(`[hygiene mirror] ${entry.inputs.length} inputs byte-identical against ${LIVE}`);
  });
  ...
  it('THE STATEMENT WHEN THERE IS NO LIVE COPY: the committed digest is whole and agrees with wave.json', () => {
    ...
    // THIS ENGINE HAS NO REPAIR HISTORY, so no section title may read as one.
    expect(headings.filter((h) => /USED TO DO|REPAIR HISTORY/i.test(h))).toEqual([]);
    // AND THE HEAT DECISION IS STATED IN THE DIGEST, with both of its reasons.
    expect(digest, 'the digest no longer states the heat decision').toMatch(/^THE HEAT DECISION\. No graded field passes through the NIOSH RAL or REL equation/m);
    expect(digest).toContain('the equation constants and the WBGT weights are checked for transcription only');
    expect(digest).toContain("NIOSH's own worked example disagrees with its own equation");
    console.log(`[hygiene mirror] committed digest: ${lines} lines, ${headings.length} contiguous sections, `
      + `no history section, and the heat decision stated with both reasons`);
  });
  ...
  it('THE STATEMENT WHEN THERE IS NO LIVE COPY: the committed answer key is eighteen fields that the tolerance derivation spells', () => {
    ...
    console.log('[hygiene mirror] committed answer key: 18 fields, six a tier, every key spelled by gradedTolerance.js');
  });
  ...
  it('THE STATEMENT WHEN THERE IS NO LIVE COPY: precision.json classifies every graded field', () => {
    ...
    console.log(`[hygiene mirror] committed precision.json: ${Object.keys(precision).length} quantity `
  ...
  it('THE STATEMENT WHEN THERE IS NO LIVE COPY: every committed generator that spells a graded field name is present', () => {
    ...
    console.log(`[hygiene mirror] ${gens.length} committed generators spell all ${fields.length} graded field names`);
  });
  ...
  it('THE STATEMENT WHEN THERE IS NO LIVE COPY: the committed digest carries none of the eighteen graded answers', () => {
    ...
    console.log(`[hygiene mirror] committed digest swept against 18 graded answers at five renderings each, 0 leaks`);
  });
});
