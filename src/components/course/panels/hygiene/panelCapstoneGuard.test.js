// A PANEL MAY NOT REACH INTO THE H2 CAPSTONE, AND THE GUARD MUST CATCH THE FULL
...
const EXPECTED_SOURCES = [
  'HeatStressExplorer.jsx',
  'NoiseDosimeterExplorer.jsx',
  'ProtectionChemicalsExplorer.jsx',
 ...
 */
const LEARNING_PAGE = 'HygieneLearningPage.jsx';
...
describe('THE H2 PANEL GUARD: no source in this directory may carry a graded capstone answer', () => {
  it('reads its inputs through the course-wave resolver and says which copy it read', () => {
    ...
    console.log(`[hygiene guard] read fields.json from ${where}: ${waveDir(WAVE_NAME)}`);
  });
  ...
  it('the guard holds four shapes, and every field contributes at least the full double', () => {
    ...
    GRADED_FIELDS.forEach(([, key]) => {
      ...
      // A TERMINATING value renders as ONE string in all four shapes (an 8-hour
      // TWA of decimal inputs divides by 8 and always terminates), and the
      // deduplication keeps that one string. It is still searched; what matters
      // is that no distinct shape was dropped, which is asserted instead.
      const value = FIELDS.find((f) => f[1] === key)[2];
      const raw = new Set([String(value), value.toPrecision(12).replace(/0+$/, '').replace(/\.$/, ''),
        value.toPrecision(9).replace(/0+$/, '').replace(/\.$/, ''), value.toFixed(PRINTED_DECIMALS[gradedClassOf(key).cls])]
        .filter((t) => t.includes('.') && !t.includes('e') && t.length >= MIN_CHARS));
      expect(new Set(mine.map((x) => x.text)), key).toEqual(raw);
      expect(mine.length, key).toBeGreaterThanOrEqual(1);
    });
    // A field is NOT required to contribute a distinct nine shape. Some of the
    // eighteen H2 values already fit inside nine significant digits, so their
    ...
    console.log(`[hygiene guard] ${r.length} searchable renderings over 18 graded fields, `
  ...
  it('PLANT TWO: the guard catches the FULL FLOAT shape, and a nine-only matcher provably does not', () => {
    ...
    console.log(`[hygiene guard] both plants proved on ${key}: the four-shape guard catches the full `
  ...
  it('PLANT THREE: the guard catches the PRINTED shape a learner would be told to quote', () => {
    const r = allRenderings(FIELDS);
    const row = FIELDS.find(([, k]) => k === 'osioka_adjusted_limit_ppm');
    expect(row).toBeTruthy();
    const printed = renderingsOf(row[1], row[2]).find((x) => x.shape === 'printed');
    expect(printed).toBeTruthy();
    expect(leaksIn(`An adjusted limit of ${printed.text} ppm`, r).map((h) => h.shape)).toContain('printed');
  });
  ...
  it('THE TOLERANCE IS MADE IN ONE PLACE, and it is max and never min', () => {
    ...
    expect(Object.keys(PRINTED_DECIMALS).length).toBeGreaterThanOrEqual(7);
    console.log('[hygiene guard] all eighteen tolerances are max(stated, printed floor), and the '
  ...
  it('NEGATIVE CONTROL: a capstone CONDITION import is caught even after the keys are removed', () => {
    const planted = "import { UTOROGU_RECORD, OSIOKA_SHIFT } from './conditions';";
    let rest = planted;
    GRADED_FIELDS.forEach(([, key]) => { rest = rest.split(key).join(' GRADEDKEY '); });
    const hits = [...rest.matchAll(/\b(utorogu|amukpe|osioka)\w*/gi)].map((x) => x[0]);
    expect([...new Set(hits)].sort()).toEqual(['OSIOKA_SHIFT', 'UTOROGU_RECORD']);
  });
