// THE H4 MIRROR GATE: the committed wave inputs against the live wave
// directory, and a STATEMENT when there is no live one.
//
// WHY. The committed copy under tools/course-waves/consequence is what every H4
...
import { renderingsOf } from './gradedAnswerGuard.js';

const WAVE_NAME = 'consequence';
const entry = WAVES[WAVE_NAME];
const MIRROR = mirrorDir(WAVE_NAME);
const LIVE = liveWaveDir(WAVE_NAME);
const sha = (p) => crypto.createHash('sha256').update(fs.readFileSync(p)).digest('hex');

describe('THE H4 MIRROR GATE', () => {
  it('the wave is registered with a live directory, an inputs list and pins', () => {
    expect(entry, 'consequence is not registered in tools/course-waves/waves.json').toBeTruthy();
    expect(entry.course).toBe('H4');
    expect(['foundation', 'full']).toContain(entry.kit);
    ...
    console.log(`[consequence mirror] ${entry.inputs.length} committed inputs, 2 pinned by sha256`);
  });
  ...
  it('SAYS WHICH JOB IT DID, so a vacuous run is visible in the output', () => {
    console.log(LIVE
      ? `[consequence mirror] a live wave directory is present at ${LIVE}: byte-comparing all ${entry.inputs.length} inputs`
      : `[consequence mirror] no live wave directory: asserting the committed copy at ${MIRROR} is internally whole`);
  ...
  it('the committed copy is byte-identical to the live wave directory, or there is none and it says so', () => {
    if (!LIVE) {
      ...
      console.log('[consequence mirror] no live wave directory to compare against, which is the CI case. '
    ...
    console.log(`[consequence mirror] ${entry.inputs.length} inputs byte-identical against ${LIVE}`);
  });
  ...
  it('THE STATEMENT WHEN THERE IS NO LIVE COPY: the committed digest is whole and agrees with wave.json', () => {
    ...
    console.log(`[consequence mirror] committed digest: ${lines} lines, ${headings.length} contiguous sections, no history section`);
  });
  ...
  it('THE STATEMENT WHEN THERE IS NO LIVE COPY: the committed answer key is eighteen fields that the tolerance derivation spells', () => {
    ...
    console.log('[consequence mirror] committed answer key: 18 fields, six a tier, every key spelled by gradedTolerance.js');
  });
  ...
  it('THE STATEMENT WHEN THERE IS NO LIVE COPY: precision.json classifies every graded field', () => {
    ...
    console.log(`[consequence mirror] committed precision.json: ${Object.keys(precision).length} quantity `
  ...
  it('THE STATEMENT WHEN THERE IS NO LIVE COPY: every committed generator that spells a graded field name is present', () => {
    ...
    console.log(`[consequence mirror] ${gens.length} committed generators spell all ${fields.length} graded field names`);
  });

  it('THE STATEMENT WHEN THERE IS NO LIVE COPY: the committed digest carries none of the eighteen graded answers', () => {
    // The same check gate_capstone_leak.mjs runs in the wave directory, run here
    ...
    const fields = JSON.parse(fs.readFileSync(waveInput(WAVE_NAME, 'fields.json'), 'utf8'));
    const leaks = [];
    // The renderings are the four shapes gradedAnswerGuard.js derives, at the
    // decimals the course PRINTS each class to. A six or four decimal rendering
    // of a PFDavg of a few parts in ten thousand is a prefix of unrelated
    // twelve decimal figures, so it would report arithmetic as a leak.
    let searched = 0;
    fields.forEach(([, key, value]) => {
      renderingsOf(key, value).forEach(({ text }) => {
        searched += 1;
        if (digest.includes(text)) leaks.push(`${key} as ${text}`);
      });
    });
    expect(searched).toBeGreaterThanOrEqual(36);
    expect(leaks, 'the committed digest carries a graded capstone answer').toEqual([]);
    console.log(`[consequence mirror] committed digest swept against 18 graded answers at ${searched} renderings, 0 leaks`);
  });
});
