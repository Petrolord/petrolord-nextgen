...
//
// PD5's capstone is a different well under different conditions from anything
// in this lab, and gasWellLab.js carries no capstone material at all: there is
// no CAP, no graded well, no `capstoneValues`, nothing for a panel author to
// reach for by mistake. That is the first half of the guarantee and it is a
// design decision rather than a check.
//
...
// the lab exposes and EVERY number the shipped teaching digest prints against
// all of them.
//
...
// of the value. `plunger_max_slug_ft` is accepted within 0.0011 FEET of
// 1041.93..., not within 0.0011 per cent of it, and
// `plunger_slug_hydrostatic_psi` within 0.000082 PSI. Read that function before
// touching the numbers below: a guard whose threshold is inferred rather than
// read comes out thousands of times too wide and passes everything.
//
...
// is still a very small target: ten times 0.000082 psi is 0.00082 psi, and ten
// times 0.0000042 is 0.000042 on a gas density.
//
...
//   x1000             Mscf/d read as scf/d, scf read as Mscf, ft read as kft
//   x0.001            the same shift the other way
//   the psi identity  a pressure or a hydrostatic restated in pascals, and back
//                     out of pascals. The oracle for this domain works in
//                     pascals and metres throughout and the wave declares
//                     6894.757293168 pascals per psi as one of its three
//                     constants, so a psi figure carried into SI and printed
//                     there is the same graded answer wearing another unit.
//                     Six of the eighteen graded fields are pressures or
//                     hydrostatics, so this is not a hypothetical restatement.
//
// The band travels with the value under every restatement, because the
// tolerance is absolute: a band of 0.0011 ft restated in thousands is
// 0.0000011.
//
...
const WAVE_DIR = '/root/pd-wip-gaswell';
...
/** The number token the wave's own leak guard, pd5_leakcheck.mjs, scans with. */
...
describe.skipIf(!fieldsAvailable)('THE LEAK GATE: no teaching number may be a graded capstone answer', () => {
  ...
  it('NO NUMBER THE LAB EXPOSES lands in any forbidden neighbourhood', () => {
    ...
    expect(quantities.length).toBeGreaterThan(1500);
...
describe.skipIf(!fieldsAvailable || !digestAvailable)('and neither does the shipped teaching digest', () => {
  ...
  it('NO NUMBER THE DIGEST PRINTS lands in any forbidden neighbourhood', () => {
    // The 78 lessons were written from this file, so a leak here is a leak in
    // prose that is already shipped. The wave's own pd5_leakcheck.mjs makes the
    // same statement; this repeats it inside the suite so it travels with the
    // panels rather than with the working directory.
    const numbers = digestNumbers();
    // the wave's own pd5_leakcheck.mjs counted 2448 on the cut it ran against,
    // so anything far under that means this file stopped reading the digest
  ...
  it('and every number the lab exposes is a number the digest also prints, or is derived from ones that are', () => {
    // Not an equality: the lab carries booleans, key lists and a few ratios a
    // panel needs that the digest does not print. What this pins is that the
    // lab has not grown a NEW numeric source the wave's own leak check never
    // saw, which is the way a leak would arrive after the digest was cut.
    const digest = new Set(digestNumbers().map((n) => n.value));
    const printed = L.teachingNumbers().filter((v) => digest.has(v));
    expect(printed.length).toBeGreaterThan(400);
  });
});
...
describe('the guard itself is honest about when it cannot run', () => {
  it('says so rather than passing silently when the wave directory is absent', () => {
    ...
    // /root/pd-wip-gaswell the two blocks above skip, and this one records why.
    if (!fieldsAvailable) {
  ...
  it('the lab carries no capstone surface at all, which is the other half of the guarantee', () => {
    ...
    const forbidden = /^(CAP$|CAP_|CAPSTONE_|capstone|okpara|okari|leakGuard)/;
    const named = Object.keys(L).filter((n) => forbidden.test(n));
    expect(named, `gasWellLab.js has grown a capstone surface: ${named.join(', ')}`).toEqual([]);
  });
});
