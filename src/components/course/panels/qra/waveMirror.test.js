// THE H5 MIRROR GATE: the committed wave inputs against the live wave
// directory, and a STATEMENT when there is no live one.
//
// WHY. The committed copy under tools/course-waves/qra is what every H5
...
describe('THE H5 MIRROR GATE', () => {
  it('the wave is registered with a live directory, an inputs list and pins', () => {
    ...
    console.log(`[qra mirror] ${entry.inputs.length} committed inputs, 2 pinned by sha256`);
  });
  ...
  it('SAYS WHICH JOB IT DID, so a vacuous run is visible in the output', () => {
    console.log(LIVE
      ? `[qra mirror] a live wave directory is present at ${LIVE}: byte-comparing all ${entry.inputs.length} inputs`
      : `[qra mirror] no live wave directory: asserting the committed copy at ${MIRROR} is internally whole`);
  ...
  it('the committed copy is byte-identical to the live wave directory, or there is none and it says so', () => {
    if (!LIVE) {
      ...
      console.log('[qra mirror] no live wave directory to compare against, which is the CI case. '
    ...
    console.log(`[qra mirror] ${entry.inputs.length} inputs byte-identical against ${LIVE}`);
  });
  ...
  it('THE STATEMENT WHEN THERE IS NO LIVE COPY: the committed digest is whole and agrees with wave.json', () => {
    ...
    // THE COURSE TEACHES NO REPAIR HISTORY, so no section may frame itself as
    // one, and every section names an owner module.
    ...
    console.log(`[qra mirror] committed digest: ${lines} lines, ${headings.length} contiguous sections, no history section`);
  });
  ...
  it('THE STATEMENT WHEN THERE IS NO LIVE COPY: the committed answer key is eighteen fields that the tolerance derivation spells', () => {
    ...
    console.log('[qra mirror] committed answer key: 18 fields, six a tier, every key spelled by gradedTolerance.js');
  });
  ...
  it('THE STATEMENT WHEN THERE IS NO LIVE COPY: precision.json classifies every graded field', () => {
    ...
    console.log(`[qra mirror] committed precision.json: ${Object.keys(precision).length} quantity `
  ...
  it('THE STATEMENT WHEN THERE IS NO LIVE COPY: every committed generator that spells a graded field name is present', () => {
    ...
    console.log(`[qra mirror] ${gens.length} committed generators spell all ${fields.length} graded field names`);
  });
  ...
  it('THE STATEMENT WHEN THERE IS NO LIVE COPY: the committed digest carries none of the eighteen graded answers', () => {
    ...
    console.log(`[qra mirror] committed digest swept against 18 graded answers at ${searched} renderings, 0 leaks`);
  });
});
