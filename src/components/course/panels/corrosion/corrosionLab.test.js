...
const buildSections = () => {
  ...
  w(`| \`ph2sPsia\` | ${e6(s2.ph2sPsia)} | the same, converted by the engine's exact bar to psia factor |`);
  ...
  w();
  w(`ONE OF THOSE FACTORS IS TRUNCATED AND THE ENGINE'S OWN IS NOT. The studio divides a psig pressure by 14.5038 to reach bar. The engine exports the exact factor, which is ${e12(s24.barPsiaExact)}, exact by the definition of the bar and of the pound-force, and measured here out of the psia the engine reports for a 1 bar partial pressure. The studio's divisor is therefore larger than the exact factor by ${expo(s24.divisorFraction)} as a fraction, so a pressure converted through the studio and a pressure converted through the engine's own factor differ in the sixth significant figure.`);
  w();
  w(`WHAT THAT IS WORTH, measured rather than argued. The shipped defaults through the studio's divisor give a total pressure of ${e12(s24.studioPTotalBar)} bar and a rate of ${e12(s24.studioRateMmYr)} mm/yr. Through the engine's exact factor they give ${e12(s24.exactPTotalBar)} bar and ${e12(s24.exactRateMmYr)} mm/yr. The two rates differ by ${expo(s24.rateFraction)} as a fraction, which is far below anything a screening decision turns on and far above zero.`);
  w();
  ...
  w();
  w(`AND THE SAME TRUNCATION ONCE REACHED A HINT A USER READ. The sour threshold was labelled 0.05 psia and it is ${e12(s24.sourPsia)} psia; a threshold of exactly 0.05 psia would be ${e12(s24.claimedSourPsiaAsBar)} bar, which is ${e6(s24.belowThresholdPct)} percent below the threshold the engine actually uses. The engine now derives and prints both numbers, and the vendored gate asserts the psia value is not 0.05.`);
  w();
...
describe('THE PROSE SWEEP: the lab\'s own comments are swept for claims the code no longer makes', () => {
  it('no comment claims a count, a name or a behaviour the code contradicts', () => {
    ...
    // the comment says fifteen exports, and section 3 prints that sentence
    expect(readDigest()).toContain('THE ENGINE EXPORTS FIFTEEN OF THESE');
  });
