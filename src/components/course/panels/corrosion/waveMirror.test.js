// THE FC9 MIRROR GATE: the committed wave inputs against the live wave
// directory, and a STATEMENT when there is no live one.
//
// WHY. The committed copy under tools/course-waves/corrosion is what every FC9
...
describe('THE FC9 MIRROR GATE', () => {
  it('the wave is registered with a live directory, an inputs list and pins', () => {
    expect(entry, 'corrosion is not registered in tools/course-waves/waves.json').toBeTruthy();
    expect(entry.course).toBe('FC9');
    ...
    console.log(`[corrosion mirror] ${entry.inputs.length} committed inputs, 2 pinned by sha256`);
  });
  ...
  it('SAYS WHICH JOB IT DID, so a vacuous run is visible in the output', () => {
    console.log(LIVE
      ? `[corrosion mirror] a live wave directory is present at ${LIVE}: byte-comparing all ${entry.inputs.length} inputs`
      : `[corrosion mirror] no live wave directory: asserting the committed copy at ${MIRROR} is internally whole`);
  ...
  it('the committed copy is byte-identical to the live wave directory, or there is none and it says so', () => {
    if (!LIVE) {
      ...
      console.log('[corrosion mirror] no live wave directory to compare against, which is the CI case. '
    ...
    console.log(`[corrosion mirror] ${entry.inputs.length} inputs byte-identical against ${LIVE}`);
  });
  ...
  it('THE STATEMENT WHEN THERE IS NO LIVE COPY: the committed digest is whole and agrees with wave.json', () => {
    ...
    // the one history section is the LAST one and says so in its own title
    expect(headings[headings.length - 1]).toMatch(/USED TO DO|REPAIR HISTORY/i);
    expect(headings.slice(0, -1).filter((h) => /USED TO DO|REPAIR HISTORY/i.test(h))).toEqual([]);
    // AND THE WITHDRAWAL IS IN SECTION 2 AND IS FRAMED AS CURRENT, not as
    // history. This wave's whole history position rests on the distinction: the
    // region is gone permanently and the engine declares its absence in two
    // returned fields today, so a digest that let section 2 read as repair
    // history would teach a learner that it is coming back.
    expect(headings[1], 'SECTION 2 is no longer the withdrawal').toMatch(/THE WITHDRAWAL/);
    expect(digest, 'SECTION 2 no longer declares itself as current behaviour')
      .toContain('This section is about behaviour the engine SHIPS TODAY, which is a refusal to answer. It is not repair history');
    console.log(`[corrosion mirror] committed digest: ${lines} lines, ${headings.length} contiguous sections, `
      + `history in section ${numbers[numbers.length - 1]} and nowhere else`);
  });
  ...
  it('THE STATEMENT WHEN THERE IS NO LIVE COPY: the committed answer key is eighteen fields that the tolerance derivation spells', () => {
    ...
    console.log('[corrosion mirror] committed answer key: 18 fields, six a tier, every key spelled by gradedTolerance.js');
  });
  ...
  it('THE STATEMENT WHEN THERE IS NO LIVE COPY: precision.json classifies every graded field', () => {
    ...
    console.log(`[corrosion mirror] committed precision.json: ${Object.keys(precision).length} quantity `
  ...
  it('THE STATEMENT WHEN THERE IS NO LIVE COPY: every committed generator that spells a graded field name is present', () => {
    ...
    expect(gens.length).toBeGreaterThanOrEqual(10);
    ...
    console.log(`[corrosion mirror] ${gens.length} committed generators spell all ${fields.length} graded field names`);
  });
  ...
  it('THE STATEMENT WHEN THERE IS NO LIVE COPY: the committed digest carries none of the eighteen graded answers', () => {
    ...
    console.log(`[corrosion mirror] committed digest swept against 18 graded answers at five renderings each, 0 leaks`);
  });
});
