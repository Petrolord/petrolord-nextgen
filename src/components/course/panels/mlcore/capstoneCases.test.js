// THE D2 CAPSTONE CASE FILES are the data the three capstones are set on, and
// they ship in the zip (src/content/capstone-cases/mlcore). This suite proves
...
// engine's own inputs (tools/course-waves/mlcore/gen_course.py running
// d2_capstone.mjs through THIS repository's vendored engine), so a case file
...
const CASES = path.join(ROOT, 'src/content/capstone-cases/mlcore');
const WAVE = mirrorDir('mlcore');

describe('THE D2 CAPSTONE CASE FILES', () => {
  it('are byte-identical to the committed generator run through this repository\'s vendored engine', () => {
    const out = fs.mkdtempSync(path.join(os.tmpdir(), 'd2cases-'));
    ...
    execFileSync('python3', [path.join(WAVE, 'gen_course.py')], {
      ...
      env: {
        ...
        D2_WAVE: WAVE,
        D2_REPO: ROOT,
        D2_ENGINES: path.join(ROOT, 'packages/engines'),
        D2_TOLERANCE: path.join(HERE, 'gradedTolerance.js'),
        D2_COURSE_OUT: course,
        D2_CASES_OUT: out,
      },
    });
    ...
    const mig = fs.readdirSync(path.join(ROOT, 'migrations')).find((f) => /_d2_mlcore_course\.sql$/.test(f));
    expect(fs.readFileSync(path.join(ROOT, 'migrations', mig), 'utf8')).toBe(fs.readFileSync(course, 'utf8'));
    fs.rmSync(out, { recursive: true, force: true });
    console.log(`[mlcore cases] ${committed.length} case files and the course migration regenerate byte for byte`);
  }, 120000);
  ...
  it('reach the capstone card only: no panel and no lab imports them', () => {
    ...
    const page = fs.readFileSync(path.join(ROOT, 'src/pages/apps/MlCoreLearningPage.jsx'), 'utf8');
    expect(page).toMatch(/@\/content\/capstone-cases\/mlcore/);
    expect(page).toMatch(/<CapstoneCaseFiles files=\{MLCORE_CASE_FILES\[tier\]\}/);
  });
});
