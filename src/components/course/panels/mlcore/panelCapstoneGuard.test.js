// THE D2 CAPSTONE GUARD over everything a learner sees in the app: the teaching
...
// fields.json and the inputs from the committed d2_capstone.mjs, run in a child
...
const PAGE = path.resolve(HERE, '../../../../pages/apps/MlCoreLearningPage.jsx');
const EXPECTED_SOURCES = [
  'DiagnoseExplorer.jsx', 'FitExplorer.jsx', 'ValidateExplorer.jsx', 'ekeneWells.json', 'mlcoreLab.js', 'panelBits.jsx',
];
const FIELDS = JSON.parse(fs.readFileSync(waveInput('mlcore', 'fields.json'), 'utf8'));
const INPUTS = JSON.parse(execFileSync('node', [path.join(mirrorDir('mlcore'), 'd2_capstone.mjs'), '--inputs'], {
  encoding: 'utf8', maxBuffer: 1e8, stdio: ['ignore', 'pipe', 'ignore'],
  env: { ...process.env, D2_ENGINES: path.join(ROOT, 'packages/engines'), D2_TOLERANCE: path.join(HERE, 'gradedTolerance.js') },
}));

/**
 * Every numeric series in the three capstone fields: one per well and
 * continuous channel in depth order, one per well-level attribute across the
 * wells. The 0 and 1 PAY label is left out, because a run of three labels
 * matches any example array a panel shows.
 */
const SERIES = [];
const SCALARS = [];
const CHANNELS = ['GR', 'RHOB', 'NPHI', 'RT', 'CALI', 'DT', 'PHIC', 'depth'];
Object.values(INPUTS).forEach(({ field, stated }) => {
  ...
  ['easting', 'northing', 'kb', 'mudWeight', 'top'].forEach((a) => SERIES.push(field.wells.map((w) => w[a])));
...
const NAMES = /\b(akpara|oboria|isuama)\b/i;
...
const sources = () => {
  ...
  return { panelFiles, texts: [...panelFiles.map((f) => [f, fs.readFileSync(path.join(HERE, f), 'utf8')]), ['MlCoreLearningPage.jsx', fs.readFileSync(PAGE, 'utf8')]] };
};
const scalarsIn = (text) => SCALARS.filter((h) => new RegExp(`(?<![\\d.])${h}(?![\\d])`).test(text));
const runsIn = (text) => RUNS.filter((s) => text.includes(s));

describe('THE D2 CAPSTONE GUARD', () => {
  it('reads eighteen answers and the three datasets, and derives enough to search for', () => {
    ...
    expect(Object.keys(INPUTS)).toEqual(['AKPARA', 'OBORIA', 'ISUAMA']);
    ...
    console.log(`[mlcore guard] ${r.length} renderings of 18 answers searched, ${skipped.length} shapes skipped as too short or exponential, `
  ...
  it('NEGATIVE CONTROL: every shape of an answer, a name, a run and a scalar is caught when planted', () => {
    ...
    expect('<p>Isuama</p>').toMatch(NAMES);
    const rhob = INPUTS.OBORIA.field.rows.filter((r) => r.well === 'OBORIA-2').map((r) => r.RHOB);
