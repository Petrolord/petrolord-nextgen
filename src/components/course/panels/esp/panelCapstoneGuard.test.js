...
//
// espLab.js exports its whole capstone surface (CAP, capstoneValues, every
// okari* reader, the leak guard) beside its teaching surface, in one module,
// because the grader, the lab's own tests and the migration headers all have to
// read ONE derivation of OKARI-9. A panel imports that same module.
//
...
// okariCableStudy() is the cable sensitivity study, okariSeam() is the load
// fraction seam, okariTurndownLadder() is the turndown ladder, and every one of
// them runs the GRADED well. A panel built on any of them would print graded
// answers to the learner sitting the assessment, and every other gate in this
// suite would stay green, because no lesson and no bank would have changed.
//
...
//   okariCurve            ->  vendorCurveFit / VENDOR_CURVE
//   okariCase             ->  goldenDesign / teachingWellCase / allCases
...
//
// espLab.test.js carries the same grep as part of the lab's own gate. This file
// exists so the guard travels with the PANELS: it is the file a panel author
// opens, it fails on its own if a panel reaches over the line, and it survives
// any future split of the lab test.
...
const capstoneNamesIn = (text) => [...new Set([
  ...
  // the naming rule is the guard, so anything called okari* is caught even if
...
describe('THE PANEL GUARD: no ESP panel may read the capstone', () => {
  ...
  it('every capstone-only export still exists under the name the grep looks for', async () => {
    const L = await import('./espLab.js');
  ...
  it('THE GREP IS LIVE: it catches a source that reaches into the capstone', () => {
    const bad = "import { okariCableStudy, CAP } from './espLab.js';";
    expect(capstoneNamesIn(bad).sort()).toEqual(['CAP', 'okariCableStudy']);
    // and it does not fire on the teaching mirrors
    const good = "import { teachingCableFlipStudy, goldenDesign } from './espLab.js';";
