// THE EC9 CAPSTONE GUARD over everything a learner sees in the app: the
...
// committed joa_capstone.mjs, run in a child process against this
...
const PAGE = path.resolve(HERE, '../../../../pages/apps/JoaLearningPage.jsx');
const EXPECTED_SOURCES = [
  'AccountCalculator.jsx', 'AgreementCalculator.jsx', 'RecoveryCalculator.jsx', 'joaLab.js', 'panelBits.jsx',
];
...
Object.values(INPUTS).forEach((o) => {
  addString(o.label);
  walkNums(o);
  if (o.cashCalls) ['forecast', 'actual'].forEach((col) => SERIES.push(o.cashCalls.months.map((m) => m[col])));
  if (o.carry) ['cost', 'entitlement'].forEach((col) => SERIES.push(o.carry.years.map((y) => y[col])));
  if (o.psc) ['grossRevenue', 'capex', 'opex'].forEach((col) => SERIES.push(o.psc.years.map((y) => y[col])));
  if (o.backIn) SERIES.push(o.backIn.years.map((y) => y.entitlement));
  if (o.soleRisk) ['grossValue', 'deductions'].forEach((col) => SERIES.push(o.soleRisk.years.map((y) => y[col])));
});
...
const NAMES = /\b(idumu|okwelle|abiama)\b/i;
...
const sources = () => {
  ...
  return { panelFiles, texts: [...panelFiles.map((f) => [f, fs.readFileSync(path.join(HERE, f), 'utf8')]), ['JoaLearningPage.jsx', fs.readFileSync(PAGE, 'utf8')]] };
};
...
describe('THE EC9 CAPSTONE GUARD', () => {
  it('reads eighteen answers and the three cases, and derives enough to search for', () => {
    ...
    expect(Object.keys(INPUTS)).toEqual(['IDUMU', 'OKWELLE', 'ABIAMA']);
    ...
    console.log(`[joa guard] ${r.length} renderings of 18 answers searched, ${skipped.length} shapes skipped as too short or exponential, `
  ...
  it('NEGATIVE CONTROL: every shape of an answer, a name, a string, a run and a scalar is caught when planted', () => {
    ...
    expect('<p>Abiama</p>').toMatch(NAMES);
    expect(stringsIn(`const q = '${INPUTS.OKWELLE.label}';`).length).toBe(1);
    const fc = INPUTS.OKWELLE.cashCalls.months.map((m) => m.forecast);
    expect(runsIn(`values: [${fc.slice(1, 4).join(', ')}]`).length).toBeGreaterThan(0);
