...
const buildDigest = () => {
  ...
  w('| mist extractor | psig | base K | derated K | K used | derated | floored |');
  w('| --- | --- | --- | --- | --- | --- | --- |');
  s3.atPressure.forEach((k) => w(`| ${k.internalsId} | ${e6(k.pPsig)} | ${e6(k.kBase)} | ${e6(k.kDerated)} | ${e6(k.k)} | ${k.derated} | ${k.floored} |`));
  ...
  s3.published.forEach((c) => w(`- ${c.name}: K ${e6(c.k)}, the rule gives ${e6(c.kDerated)}, derated ${c.derated}, floored ${c.floored}.`));
  w();
  w(`How close a derated K can sit to the floor without saying so: ${s3.nearFloor.name} returns K ${e6(s3.nearFloor.k)} with derated ${s3.nearFloor.derated} and floored ${s3.nearFloor.floored}, which is ${e6(s3.nearFloor.gapAboveFloorDerived)} above the floor of ${e6(s3.nearFloor.floor)} (derived: the two figures on this row). The rule takes 0.01 off K every 100 psi, so 50 psig more of operating pressure puts that vessel on the floor, and the return carries no flag for sitting on the edge of the range where the rule of thumb stops meaning anything.`);
  w();
