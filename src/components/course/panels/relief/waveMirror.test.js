// THE FC5 MIRROR GATE: the committed wave inputs against the live wave
// directory, and a STATEMENT when there is no live one.
//
// WHY. The committed copy under tools/course-waves/relief is what every FC5
...
describe('THE FC5 MIRROR GATE', () => {
  it('the wave is registered with a live directory, an inputs list and pins', () => {
    expect(entry, 'relief is not registered in tools/course-waves/waves.json').toBeTruthy();
    expect(entry.course).toBe('FC5');
    expect(entry.kit).toBe('full');
    expect(entry.inputs.length).toBeGreaterThanOrEqual(12);
    ...
    console.log(`[relief mirror] ${entry.inputs.length} committed inputs, 2 pinned by sha256`);
  });
  ...
  it('SAYS WHICH JOB IT DID, so a vacuous run is visible in the output', () => {
    console.log(LIVE
      ? `[relief mirror] a live wave directory is present at ${LIVE}: byte-comparing all ${entry.inputs.length} inputs`
      : `[relief mirror] no live wave directory: asserting the committed copy at ${MIRROR} is internally whole`);
  ...
  it('the committed copy is byte-identical to the live wave directory, or there is none and it says so', () => {
    if (!LIVE) {
      ...
      console.log('[relief mirror] no live wave directory to compare against, which is the CI case. '
    ...
    console.log(`[relief mirror] ${entry.inputs.length} inputs byte-identical against ${LIVE}`);
  });
  ...
  it('THE STATEMENT WHEN THERE IS NO LIVE COPY: the committed digest is whole and agrees with wave.json', () => {
    ...
    console.log(`[relief mirror] committed digest: ${lines} lines, ${headings.length} contiguous sections, `
  ...
  it('THE STATEMENT WHEN THERE IS NO LIVE COPY: the committed answer key is eighteen fields that the tolerance derivation spells', () => {
    ...
    console.log('[relief mirror] committed answer key: 18 fields, six a tier, every key spelled by gradedTolerance.js');
  });
  ...
  it('THE STATEMENT WHEN THERE IS NO LIVE COPY: the committed digest carries none of the eighteen graded answers', () => {
    ...
    console.log(`[relief mirror] committed digest swept against 18 graded answers at five renderings each, 0 leaks`);
  });
});
