// EVERY STRING SHAPE A GRADED CAPSTONE ANSWER CAN REACH A LEARNER AS.
//
...
// numeric sweep, which walks values rather than text. That boundary is printed
// by the gate rather than assumed, so nobody has to guess what was skipped.
...
export const allRenderings = (fields) => {
  if (!Array.isArray(fields) || fields.length !== 18) {
    throw new Error(`[relief guard] expected eighteen graded fields, got ${Array.isArray(fields) ? fields.length : typeof fields}`);
  }
