// THE D4 CAPSTONE CASE FILES are the data the three capstones are set on, and
// they ship in the zip (src/content/capstone-cases/forecastml). This suite proves
...
// engine's own inputs (tools/course-waves/forecastml/gen_course.py running
// d4_capstone.mjs through THIS repository's vendored engine), so a case file
...
const CASES = path.join(ROOT, 'src/content/capstone-cases/forecastml');
const WAVE = mirrorDir('forecastml');

describe('THE D4 CAPSTONE CASE FILES', () => {
  it('are byte-identical to the committed generator run through this repository\'s vendored engine', () => {
    const out = fs.mkdtempSync(path.join(os.tmpdir(), 'd4cases-'));
    ...
    execFileSync('python3', [path.join(WAVE, 'gen_course.py')], {
      ...
      env: {
        ...
        D4_WAVE: WAVE,
        D4_REPO: ROOT,
        D4_ENGINES: path.join(ROOT, 'packages/engines'),
        D4_TOLERANCE: path.join(HERE, 'gradedTolerance.js'),
        D4_COURSE_OUT: course,
        D4_CASES_OUT: out,
      },
    });
    ...
    const mig = fs.readdirSync(path.join(ROOT, 'migrations')).find((f) => /_d4_forecastml_course\.sql$/.test(f));
    expect(fs.readFileSync(path.join(ROOT, 'migrations', mig), 'utf8')).toBe(fs.readFileSync(course, 'utf8'));
    fs.rmSync(out, { recursive: true, force: true });
    console.log(`[forecastml cases] ${committed.length} case files and the course migration regenerate byte for byte`);
  }, 120000);
  ...
  it('reach the capstone card only: no panel and no lab imports them', () => {
    ...
    const page = fs.readFileSync(path.join(ROOT, 'src/pages/apps/ForecastmlLearningPage.jsx'), 'utf8');
    expect(page).toMatch(/@\/content\/capstone-cases\/forecastml/);
    expect(page).toMatch(/<CapstoneCaseFiles files=\{FORECASTML_CASE_FILES\[tier\]\}/);
  });
});
