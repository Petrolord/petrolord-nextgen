...
//
// The lab's leak gate in rotatingLab.test.js walks the TEACHING exports'
// return values. It says nothing about the PANELS, and the panels import the
// lab directly. The lab exports its whole capstone surface (ESCRAVOS_POINTS,
// BONGA_SUCTION, BONNY, capstoneRuns, capstoneFields and friends) beside its
...
//   ESCRAVOS_POINTS, ESCRAVOS_SYSTEM  ->  OKONO_POINTS, OKONO_SYSTEM, twoCurves
//   ESCRAVOS_SG, ESCRAVOS_EFFICIENCY  ->  OKONO_SG, OKONO_EFFICIENCY, powerHeadPressure
//   BONGA_POINTS, BONGA_SYSTEM        ->  OKONO_FRICTION_SYSTEM, dutyPointSolved
//   BONGA_SUCTION                     ->  OKONO_SUCTION, suctionSide, marginAndRule
//   BONGA_SPEED_RATIO                 ->  OKONO_SPEED_SWEEP, speedAndTrim
//   BONGA_N_PARALLEL                  ->  OKONO_PARALLEL_COUNTS, twoPumpsAndWater
//   BONNY                             ->  SOKU, SOKU_STAGE, stageIsNotAPump, trainAndCooling
//   BONNY_HEAT_RATE_BTU_HP_HR         ->  SOKU_HEAT_RATE_BTU_HP_HR, machineDriverFuel
//   capstoneRuns                      ->  the seventeen section readers
//   capstone*                         ->  nothing; a panel never needs a graded answer
import { describe, it, expect } from 'vitest';
import * as fs from 'node:fs';
import * as path from 'node:path';
import { fileURLToPath } from 'node:url';
import * as L from './rotatingLab.js';

const HERE = path.dirname(fileURLToPath(import.meta.url));
const FIELDS_JSON = '/root/fc-wip-rotating/fields.json';
const LEARNING_PAGE = path.resolve(HERE, '../../../../pages/apps/RotatingLearningPage.jsx');

const panelSources = fs
  .readdirSync(HERE)
  .filter((f) => f.endsWith('.jsx') && !f.endsWith('.test.jsx'))
  .map((f) => ({ file: f, text: fs.readFileSync(path.join(HERE, f), 'utf8') }))
  .concat([{ file: 'RotatingLearningPage.jsx', text: fs.readFileSync(LEARNING_PAGE, 'utf8') }]);
...
describe('THE PANEL GUARD: no panel may read the capstone', () => {
  it('there are panels to check, so a rename cannot silently empty this gate', () => {
    expect(panelSources.map((s) => s.file).sort()).toEqual([
      'CompressorExplorer.jsx', 'PumpExplorer.jsx', 'RotatingLearningPage.jsx', 'SuctionExplorer.jsx',
    ]);
  ...
  it('the lab names every capstone export in CAPSTONE_ONLY_EXPORTS, so the grep list cannot go stale', () => {
    const named = Object.keys(L).filter((k) => /escravos|bonga|bonny|capstone/i.test(k));
    ...
    L.CAPSTONE_ONLY_EXPORTS.forEach((k) => {
      ...
      expect(k, `${k} does not carry a capstone name`).toMatch(/escravos|bonga|bonny|capstone/i);
    });
  });
  ...
  it('NEGATIVE CONTROL: the grep finds a capstone name and a graded answer when one is planted', () => {
    ...
    const planted = `import { capstoneValues } from './rotatingLab';\nconst x = ${graded[0]};`;
  ...
  panelSources.forEach(({ file, text }) => {
    ...
    it(`${file} names no capstone machine and never says a capstone field's name`, () => {
      const hits = [...text.matchAll(/(?:escravos|bonga|bonny)\w*/gi)].map((x) => x[0]);
    ...
    it(`${file} computes no rotating-equipment quantity of its own`, () => {
      // Every number a panel shows comes through the lab, which comes through
      // the vendored engines. A panel that imported an engine directly could
