...
**Module 2.** Six regimes, four characteristic derivative slopes, and a required ORDER: storage first, near-well geometry early, boundaries late. A slope band on its own is necessary and not sufficient, because a transition between two regimes has a well defined local slope that usually falls inside one: a steep fall between regimes lands in the recharge band and a slow rise between plateaus lands in the bilinear band. The classifier applies three ordering rules, which remove eight such labels across this course's seven fixtures, and one remains that ordering cannot reach, because a first segment can be a fracture's linear flow or a storage roll-off and only the pressure curve separates them. A label needs five things before it is a diagnosis, and the software supplies three of them.
