...
The friction factor is different. It already returns an object with a regime field in it, so its refusal rides in the field that was there anyway. The regime comes back as "invalid", which is a word no valid flow ever produces, and the friction factor beside it is a NaN like the other two. The table above prints that row as JSON, so the null standing in it is already the encoder at work.
...
Serialise any of these three and all three print as null, because that is what encoding a NaN does. The engine returned NaN in every one of them. Null is what printing it produced, and not one of the three is a null the engine chose.

So in a log line, an API response or a saved case file, all three look the same. All three are a number that is not a number, and nothing in the record says so. The regime field is the only thing that distinguishes the third, which is another reason that design is the better one.
...
Name the three returns that sit outside the refusal contract and give what each one returns. Then say why the two bare numbers had no alternative, why the friction factor's refusal is the tidiest of the three, and explain what happens to all three when they are serialised and what a reader must not conclude from it.
