...
That note is why the course computes every inventory on "IPCC AR6 GWP100, fossil methane". The digest says so directly: the course uses that set because the engine counts escaped carbon as methane, as the methane note states. The other three sets are printed beside it.

## The same Igbogene on the non-fossil value

The digest rebuilds the inventory on the AR6 non-fossil set to show what the choice of methane value moves:
...
Both sets are AR6 and both are the 100-year horizon. The only difference between them is which methane value they hold, and the Igbogene total on the non-fossil set is 42502.520 tCO2e. The digest's arithmetic prints its total less the course set as -443.257 tCO2e. Only the label's last words tell the two sets apart, and the engine states the label on every result as gwpSetLabel. The methane tonnes are the same on both rows: 16.306 t from the flare and 142.000 t vented. Scope 2 stays at 12915.000.
...
Self check: on AR6 fossil methane, 29.8, the total is 42945.777 tCO2e. On AR6 non-fossil methane, 27, it is 42502.520 tCO2e, and the digest prints the total less the course set as -443.257 tCO2e. The report, the horizon and every activity are the same, so the methane value the set holds is the whole of the difference. The flare's methane line moves from 485.922 to 440.265 and the vented line from 4231.600 to 3834.000.
