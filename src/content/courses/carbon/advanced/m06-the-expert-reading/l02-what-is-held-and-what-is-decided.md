...
## The held items on the Agbor records

Two of the held items reach the Agbor inventory this tier's curve and path are built on. The flare source passed to the curve, 7562.133 tCO2e, is the Flaring (CO2) line of 6189.848 tCO2e and the Flaring (unburned CH4) line of 1372.285 tCO2e (SECTION 21). The methane line exists because of H3: every escaped carbon atom is counted as methane, and it is converted to CO2e on IPCC AR6 GWP100, fossil methane. The Agbor inventory table in SECTION 21 prints five lines, and none of them is a nitrous oxide line. H4 says why a combustion N2O figure would need a line of its own with an emission factor.

H1 reaches every figure in tCO2e. The Agbor baseline of 56100.276 tCO2e is on the course's set. SECTION 8 prints the Igbogene inventory on all four sets and shows that only the methane lines move. The Agbor inventory is printed on one set only, and this course prints no other total for it.

## The rules in force for this tier

| rule | where it prints |
| --- | --- |
| A negative abatement, a blank capital cost, a blank rate with capital, and a rate outside (-1, 1) are refused; blank savings and running costs are 0 and named | SECTION 19 |
| A curve with a claim above what its source emits returns meetsTarget none and says why | SECTION 21 |
| A baseline that is not positive is refused, and a measure with no start year is named | SECTION 22 |
...
Read H1 and the inputs row of the held items, and the SECTION 19 rule on blank capital, blank rates and blank running figures. Say what the three, read together, show about which boxes the engine refuses, which it fills and names, and which choices it leaves to the user.
