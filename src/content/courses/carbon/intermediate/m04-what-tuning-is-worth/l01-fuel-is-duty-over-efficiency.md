...
The method then gives the saving as one ratio, (target - current) / target, with the target efficiency on the bottom. The course adds how the ratio is used: the engine multiplies a saving fraction by the annual fuel it is given. The fraction is therefore a fraction of that annual fuel figure.

## The Isiokpo heater, tuned

The lab prints the result for the heater, with the current reading of 5.5 percent oxygen, a target of 2.8 percent, a declared minimum safe stack oxygen of 2 percent and 410000 GJ of fuel a year on LHV. The floor and the fuel figure are both invented for this course.
...
The first row of the output is basis LHV. The saving is a saving on the LHV basis because both efficiencies were on LHV, and the engine's basis refusal that module three's last lesson read stops a pair on mixed bases from reaching this rule at all. The 410000 GJ a year is stated on LHV too, so the fuel figure and the fraction share a basis.

## Exercise

Read the lab's tuning output table and the engine's method sentence. Say which of the two efficiencies sits on the bottom of the engine's ratio, what the duty has to do with that choice, and what fuelSavingFraction 0.0164448058 is a fraction of.
