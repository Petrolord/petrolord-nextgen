# The ratio and the shortcut

Lesson one read the first sentence of the engine's method. This lesson reads the second, which names a way the same saving is worked out wrongly, and sets the engine's figure beside the wrong one so the size of the error is on the page.
