...
The lab prints the engine's comparison warning on LHV, verbatim: "This efficiency is on LHV. An efficiency on the other basis is a different number for the same heater and the two must not be compared."
...
The lab shows the first claim directly:
...
The tuning saving of module four takes a current efficiency and a target efficiency. The lab prints what the engine answers when they come on different bases:
...
Where the comparison is allowed, the engine carries the basis through. The lab prints the tuning saving on LHV with basis LHV among its outputs. It also prints the saving on HHV, computed from the two HHV efficiencies of this module, as a fuelSavingPercent of 1.6476 against the LHV figure of 1.6445. Each is a saving on its own basis.

The basis box is checked as well. The engine refuses a basis of "gross" and a blank one. The refusal for "gross", verbatim:
...
Read the four efficiencies of this module and the refusal in the lab for a current efficiency on LHV and a target on HHV. Say which two pairs of efficiencies can be set against each other, what the lab's difference of 8.4741 percentage points is a difference between, and why that figure is no measure of what tuning the heater is worth.
