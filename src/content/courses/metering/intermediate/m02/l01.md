...
This is the reason the regime word and the choked flag are separate returns. A tool that reported only the choked flag would call every row above the crossing acceptable, which covers five of the nine rows of this march, three of which the engine itself labels with a cavitation word.
