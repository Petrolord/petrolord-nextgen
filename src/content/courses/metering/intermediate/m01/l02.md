...
On this valve the allowable drop reads 179.220032 psi on every row of the march, from an outlet pressure of 200.000000 psia down to an outlet pressure of 20.000000 psia. That is the first thing to notice about it. The stated drop changes on every row, because the process is being asked for a different outlet pressure each time. The allowable drop holds still, because it belongs to the valve and to the fluid rather than to the operating point.

It is built from a pressure recovery factor, which is a property of the trim, and a liquid critical pressure ratio factor, which for the BELEMA fluid the engine returns as 0.892161.
