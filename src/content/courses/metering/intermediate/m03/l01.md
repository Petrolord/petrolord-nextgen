...
Two things follow from the terminal ratio being a ratio. The first is that it is dimensionless, so it can be quoted for a valve style without knowing anything about the service. The second is that the pressure it corresponds to depends entirely on the inlet pressure, which is why a gas valve has to be checked at every inlet condition the plant can reach rather than at the design case alone.
