...
The engine evaluates the equation. It is the stated criterion, it is reproducible at any rate, and a figure can only ever approximate it. The golden pins both example values as errata: cases the engine must stay OUTSIDE the printed tolerance of. An engine edited to agree with the figure's 27.800000 C would turn the suite red.

That pin tests the engine's behaviour. It leaves the constants where they were. The equation's own inputs, 56.7 and 11.5 for the NIOSH heat REL, 59.9 and 14.1 for the RAL, are still the NIOSH 2016-106 section 8.1 equation, checked for transcription only, and the worked example cannot rescue them because it was never computed from them.
