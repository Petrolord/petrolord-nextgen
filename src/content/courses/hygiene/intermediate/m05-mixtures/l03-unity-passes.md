...
A boundary rule matters only for inputs that land exactly on the boundary, and in field measurement that almost never happens by chance. It happens when figures are rounded. An index computed from rounded concentrations can land on 1.000000 where the unrounded figures would not, in either direction. So the boundary rule is less a question of chemistry than a question of record keeping. Carry the terms at full precision, and the chance of landing on the line becomes negligible.

## What the flag does and does not tell you

The flag is a comparison, and a verdict word is never graded in this course. An index of 1.000000 passes by the rule and sits at the very edge of the allowance. An index of 0.925000 also passes and leaves some room. A report that writes "passes" for both has lost the one thing the reader needed, which is how close each mixture is to its line. Quote the index to the precision the engine prints and let the reader see the margin.
