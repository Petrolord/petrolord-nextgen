# What an application must carry

The reuse record is only as good as the applications behind it. The engine refuses to record an application that cannot be checked, and each refusal names the missing piece.

{{panel:rc-review-explorer}}

## What every application starts with

An application is recorded with a target type, which says what kind of thing the lesson was applied to, and an outcome: adopted, adapted or rejected. The refusals below show what else each kind of application must carry.

## Into a Suite register

The two targets that are Suite registers, "Risk register" and "Management of change", need the record's key:

- ALLOWED, a risk register target with the risk named.
...
A procedure, a course or a standard lives outside the Suite, so the engine asks for a reference a person can follow:

- REFUSED, a procedure with no reference: Name what changed (the procedure, the course, the standard) so somebody can go and look at it.

## A rejection needs its reasoning

A rejection changes nothing, and it must carry the reason:
...
Two more refusals guard the fields every application needs:
