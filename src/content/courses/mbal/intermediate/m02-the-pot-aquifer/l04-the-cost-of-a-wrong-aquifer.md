...
|---|---|---|
| oil in place | negative (yours to read off the panel) | 12139208.1074968 stb |
| aquifer water in place $W$ | 42890161.1573930 rb | none |
| R-squared | above 0.999 | 1 |
...
A negative oil in place, more than the whole tank away from the truth, on a tank the closed model reads perfectly. A water drive index above one, crediting water with 104 percent of everything withdrawn while the oil expansion supplies a negative amount. A cumulative influx of 331452.684714500 rb against a withdrawal of 317926.842484584 rb, so the model claims 13525.8422299160 more reservoir barrels of water arrived than volume of any kind left the tank.

And an R-squared above 0.999. Those two facts together are the lesson: an excellent looking fit and a physically impossible answer, from the same six surveys, with nothing in the warnings list.
...
reservoir barrels per psi, which the Associate tier already computed as Ekene's rock and connate water response per psi. The engine's slope comes back well above that, and the gap is the slope error.
...
$$\Delta N = \Delta(\text{slope}) \times \bar{x}$$

and on Ekene that product is larger than the whole tank, which is how the answer goes negative.
...
**The wrong model.** Move to "Pot aquifer (not needed here)": the oil in place goes negative, R-squared stays above 0.999, the water drive index passes one, and the mechanism and strength tiles read strong_water_drive and strong. Nothing is flagged.

**The part most learners miss.** With the pot model still selected, read the survey table's last column. It still says 12139208.1 at every survey, because $F$, $E_t$ and their ratio belong to the data and do not know which model you picked. The panel is showing the truth and the model's answer at once, more than a whole tank apart.

## The two numbers the capstone grades

The Professional capstone asks for the oil in place when a pot aquifer is forced on Ekene, and for the R-squared of that fit. Both come from the second pass above, read from the "OOIP from the slope" and "R-squared" tiles with the pot setting selected, with tolerances of 20000 stb and 0.0001. What matters is reading them from the pot setting rather than the none setting, and entering the oil in place as a negative number rather than assuming a sign error somewhere.
...
Rebuild the honest oil in place yourself from the honest slope and the centroid:

$$N = 19998567.202735189 - 113.174770971432 \times 69444.444444444904 = 12139208.1074968 \ \text{stb}$$

the true tank. Now put a slope a couple of hundred reservoir barrels per psi larger through the same line and the intercept goes negative: that is all the pot run did, and it is invisible on any plot of these six points.
...
Obtain the two capstone numbers from the panel, write them down with their tolerances, and work out from the centroid line above what slope the pot run must have fitted to return that oil in place.
