...
**Result and convention.** Oil in place [from your run] MMSTB, cumulative influx [from your run] MMrb, fit statistic above 0.9999. Drive indices at the final survey in the gross withdrawal convention, summing a fraction of a percent short of one. This history produced no water, so the net convention gives identical figures and the shortfall is a residual in the influx solution rather than a denominator artefact. It is inside the tool's closure threshold of 0.05, and is reported rather than rounded to 1.00.
