...
Every ORASHI finding carries an age in days. The age is derived, and it is read against the as-of date 2026-10-15 while the finding is open. Once it is closed or voided, it stops. This lesson reads the age and the overdue flag of each finding and each action, and one difference between two modules' audit overdue rules.
...
Why stop the clock? The age of a closed finding answers a fixed question: how long did it take to close? That answer should not grow with every day the register is read. The age of an open finding answers a different question: how long has it been waiting? Both are useful, and one column carries both because the status says which question each row answers.
...
## An audit overdue rule that differs
...
The two modules agree on every row except Reported. The digest prints both, and SECTION 21 also prints that the two AUDIT_TRANSITIONS tables agree on every status. It does not say why the overdue rules differ at Reported. Read each app's value in its own app, and do not carry one into the other.

## Exercise

At 2026-10-15, read the age, open and overdue columns for all five findings. Say which ages will change if the as-of date moves and which will not, and why. Then read the Reported row of the isAuditOverdue table and say what each module returns for a Reported audit whose planned end has passed.
