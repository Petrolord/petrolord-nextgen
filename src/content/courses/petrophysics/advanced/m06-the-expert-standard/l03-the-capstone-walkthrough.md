...
The Expert capstone is the graded practical that closes the petrophysics ladder. Like the two capstones before it, it is set on a well of its own, IKPO-3, with its own water sample and SP reading, asks for numbers the app computes live once you open that well, grades them server-side within stated tolerances, and certifies you when all of them land. Download the well's LAS file from the capstone card, open it with **Open a LAS file**, and type the zones and water leg the brief states. This lesson walks the same brief on the typewell, the worked example, so that nothing in it surprises you; your capstone numbers come from IKPO-3.

## The brief, on the typewell
...
## The six graded numbers, worked on the typewell

1. **Sample $R_w$ at formation temperature (Arps): 0.0499 ohm.m**, tolerance 0.00005. The temperature correction from module 2.
2. **$R_{we}$ from the SP quicklook: 0.0498 ohm.m**, tolerance 0.00005. The SSP conversion from module 3.
3. **Water-leg mean $S_w$ with the Arps $R_w$: 0.9991**, tolerance 0.0005. The validation check from module 4.
...
Open the Petrophysics app in Learning Mode on the Expert tier. The Expert panel opens on the typewell's givens and runs the engine live on whichever well is open; type the capstone brief's own givens for the capstone:
...
* The water-leg card evaluates Archie with the corrected $R_w$ across the open well's water leg (2075 to 2078 m on the typewell) and reports the mean.
...
Read each value from the panel into the corresponding capstone field. The tolerances are wide enough that engine-computed values pass exactly and hand-computed values pass if you carry enough significant figures: five for the two $R_w$ routes. What the tolerances will not forgive is a wrong route: an uncorrected $R_w$ in field 1, or a booking with the wrong value, misses by far more than the band.
