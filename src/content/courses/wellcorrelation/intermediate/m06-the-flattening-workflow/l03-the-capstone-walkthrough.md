...
| Shallowest displayed depth, flattened on TOP_SAND at 1480 m | m | 0.5 |
...
**5. Shallowest displayed depth, flattened on TOP_SAND at 1480 m, in m, tolerance 0.5.** This is the one reading taken at a second setting. Select TOP_SAND as the flattening top, set the datum to 1480 m, and read the Shallowest displayed tile. By hand, give every well its new shift, 1480 minus its own TOP_SAND pick, add that shift to each of its picks, and take the smallest result. The panel prints whole metres and every pick is a whole metre, so the half metre forgives a slip of the eye and nothing more. Put the panel back on TOP_A at 1450 m before you read field 6.
...
One panel, two datums, six readings. Fields 1 through 4 and field 6 come from the flattened view with TOP_A at 1450 m. Field 5 comes from the same panel flattened on TOP_SAND at 1480 m.

## Field 5 moves with the datum

Field 5 is the one reading that tests whether the shift rule travels. On TOP_A at 1450 m the shallowest displayed pick is the datum itself, because every TOP_A lands on it and TOP_A is the shallowest top in every well. Flatten on TOP_SAND and that stops being true. Every TOP_SAND lands on the datum, and every TOP_A draws above it by that well's own A-to-SAND interval, so the shallowest pick on the panel is a TOP_A, and which well owns it depends on the intervals.

Two wrong answers sit on that panel. One is the datum, 1480 m, which is where the flattening top draws in every well. The other is the TOP_A of a well with a thinner interval, which draws a few metres lower. The half metre of tolerance separates both of them from the answer.
...
Open the panel, flatten on TOP_A at 1450 m, and locate fields 1 to 4 and 6 in capstone order. Then flatten on TOP_SAND at 1480 m for field 5 before you submit anything.

{{panel:wc-flatten-explorer}}

## Exercise

Without opening the panel, list the six graded fields in capstone order with the unit and tolerance of each, and say where you would read each one. Then answer in two sentences: which field is read at a second datum, and which two fields would you get wrong by reporting a number that is genuinely on the panel.

Self-check: Ekene-4 flattening shift in m at tolerance 0.01, which is -80; Ekene-2 TOP_SAND displayed depth in m at tolerance 0.01, which is 1503; Ekene-4 TOP_A to TOP_SAND interval in m at tolerance 0.01, which is 60; A-to-SAND growth range as max minus min in m at tolerance 0.01, which is 14; the shallowest displayed depth flattened on TOP_SAND at 1480 m in m at tolerance 0.5, read on the panel at that setting; and displayed depth span of the section in m at tolerance 0.01, which is 150. Fields 1 to 4 and field 6 are read on the flattened panel with TOP_A at 1450 m, and field 5 on the same panel flattened on TOP_SAND at 1480 m. Field 5 is the one read at a second datum. The two fields most easily filled with a real but wrong number are the shift, where 80 is on the panel as a magnitude while -80 is the answer, and the span, where 1600 and 1450 are both on the panel while 150 is the answer.
