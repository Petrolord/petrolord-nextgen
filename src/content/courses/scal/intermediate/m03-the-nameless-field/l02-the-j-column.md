...
Every row of the lab table is multiplied by the same lab factor from lesson 1. The engine's `computeJTable` returns, at full double precision, the rows below. The top row, $S_w = 0.2$, is the capstone's row, so its engine value is left for you to compute:

| $S_w$ | $P_c$ (psi) | engine $J$ | book prints |
|---|---|---|---|
| 0.2 | 1.75 | yours to compute | 0.169 |
...
Read the pairs across. At every saturation the book's entry is the engine's value rounded to three decimals: 0.1016393518832642 becomes 0.102, 0.07259953705947443 rounds up to 0.073, and so on. There is no disagreement anywhere. There is only precision.

The engine value at $S_w = 0.2$, all seventeen digits of it, is your second capstone value, graded at a tolerance of 0.0005. The tolerance is set so that both legitimate versions of the number, the full one and the book's three-decimal print, pass, and only actual errors fail.

## Worked example: the middle row

Do $S_w = 0.4$ end to end. The printed $P_c$ is 1.05 psi.

$$J(0.4) = \text{lab factor} \times 1.05 = 0.1016393518832642$$
...
Second, your colleague reports "J at 0.6 is 0.073" and asks you to recover the Pc that produced it. Divide 0.073 by the lab factor, compare the result with the true 0.75 psi, and state in one sentence why the recovered value misses, and by how much.
