...
## The two chains at Sw 0.4

Chain one, the book's: printed J of 0.102, times the reservoir factor.

$$0.102 \times 9.191758209219469 = 0.9375593373403855 \ \text{psi}$$

Chain two, full precision end to end: the engine's J of 0.1016393518832642, times the same factor.

$$0.1016393518832642 \times 9.191758209219469 = 0.93424434705274 \ \text{psi}$$

The difference is 0.0033 psi, about 0.35 percent of the value. At $S_w = 0.2$, the capstone's row, the same two chains differ by a similar amount, and the capstone field `res_pc_sw02` is graded at a tolerance of 0.002 psi against the PRINTED chain, so the full-precision answer, correct arithmetic on correct inputs, FAILS the field. That is by design, and the design is teaching you something real: a published table is a specific computational object, and reproducing it means reproducing its inputs, rounding included.
...
|---|---|---|---|
| 0.2 | the capstone's row | yours to compute | positive |
...
You can bound the chain gap before computing anything. The print holds three decimals, so the largest rounding in J is half a unit in the third place, 0.0005. Multiplied by the reservoir factor of about 9.19, that is at most about 0.0046 psi on any row. The observed gaps, a few thousandths of a psi at the worst, all sit inside that bound. This thirty-second estimate is worth doing whenever you inherit a printed table: it tells you in advance how much disagreement is explainable by rounding alone, so anything larger must be a real discrepancy.
