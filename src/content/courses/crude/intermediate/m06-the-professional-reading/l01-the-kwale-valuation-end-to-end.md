...
Module 1 built the blend's own curve at every temperature either crude measured, 14 points, by weighting volume percents on volume and never averaging temperatures.

Module 2 read that curve. T50 interpolated is 587.3184 F, with the grid reading and the averaged midpoints printed beside it as readings the engine does not use. Watson K at T50 is 11.8135, labelled screening, because T50 stands in for the mean average boiling point the studio does not compute. That is held item C13, a stated limit.
...
Module 4 priced them. Yield fraction times price per barrel of product gives value per barrel of crude, summing to a gross of 74.2412 $/bbl. The loss at 0.8 percent comes off on the product side, before the costs, as a named term of 0.5939 $/bbl. Processing and freight come off per barrel of crude. The netback is 64.9473 $/bbl, complete, with nothing assumed zero.
...
The engine's reports make each step checkable. Do the yields close? Here they do. Is the valuation complete, with nothing assumed zero? Here it is. Was the loss taken on the product side, before the costs? The loss appears as its own term directly after gross. Does the blend's netback match the volume-weighted netbacks of its crudes? The digest prints 0.0000. A valuation that passes all four has been built on the right basis at every step.
...
Its cut set is the vacuum refinery's, so its yields run to vacuum gasoil and vacuum residue. Its stability screen basis reads api-contrast, which the Associate tier taught: with no SARA supplied, the screen falls back to the gravity rule of thumb, which can raise a flag and can never clear one.

A learner who opens the studio sees this table first. Nothing in it is Kwale's. Before reading any figure on the page, read which pair, which cut set and which prices it rests on.

## Exercise

Read the Kwale table from blend API to differential. For each of these five figures, the T50, the Naphtha yield, the gross product value, the loss value and the differential, name the module that built it and the basis or rule it was built on. Then say what the default table's stability screen basis tells you about the SARA behind it.
