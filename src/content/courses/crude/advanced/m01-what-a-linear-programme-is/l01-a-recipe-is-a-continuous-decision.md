...
A recipe is a list of volumes, one per component. The kernel's contract puts each volume between two bounds, lo <= x <= hi, and it asks nothing more of it: no whole number of barrels and no round share of the tank. The optimizer's Apapa recipe, which module three reads line by line, puts 3284.6899 bbl of Reformate and 784.1881 bbl of Isomerate into an 8000 bbl cargo. The engine prints fractional volumes like those, and that is what continuous means here: a volume can sit anywhere between its bounds.

## Why the decision is linear

Three facts make the problem linear in those volumes.

First, cost. The course states the rule under the Apapa recipe: "Each component's cost $ is its volume bbl times its cost $/bbl, and the total is their sum." That is a linear objective.

Second, the batch. The volumes must add up to the cargo. That is one linear equation.

Third, the specifications. A blended property is a weighted average, a sum over components divided by another sum over components. A ratio looks nonlinear, yet a limit on a ratio rearranges into a linear row once both sides are multiplied through by the denominator. Module two shows the step exactly, in the course's own form: the row is linear in the volumes.
