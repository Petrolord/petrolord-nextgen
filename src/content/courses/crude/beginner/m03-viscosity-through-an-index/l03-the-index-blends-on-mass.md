...
The two bases put different weights on the same crude. Module two printed them for the export blend: Egbema Medium carries a mass fraction of 0.3654 against a volume fraction of 0.3500, and Obigbo Light 0.6346 against 0.6500. The index on mass weights each crude's index by its mass fraction, and the index on volume weights it by its volume fraction. The same four index values, averaged with different weights, give a different average, and each average inverts to a different viscosity.
...
Read the Asarama Heavy and Ubie Condensate row. Quote the viscosity with the index on mass, the viscosity with the index on volume and the difference column. Then read the export blend row the same way. Say which of the two rows prints the larger difference, and say which basis the engine names and which basis ASTM D7152 uses, without saying which is right.
