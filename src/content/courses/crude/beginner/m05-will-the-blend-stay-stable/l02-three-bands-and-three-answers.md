...
The middle band is the one to understand best. Stable null is its own answer. It is not a weak yes and it is not a weak no. It says the index sits where blends go either way, and the screen cannot decide. The engine names the tests that can: ASTM D7112 and D7157, laboratory methods that measure the stability of the actual mixture.

A result of null must never be shown as a tick. A dashboard that turns null into green has converted "we do not know" into "safe". In the assay explorer, no verdict is drawn as its own state.
...
A stable verdict lets a terminal commingle with a routine check. An uncertain verdict sends a sample to the laboratory before any tank is committed. An unstable verdict says do not commingle without a lab test. The screen narrows the laboratory's work to the blends that need it. It never replaces the laboratory.
