...
"Volume is taken as conserved" is an assumption, and the engine states it as one, in the same sentence that says mass is conserved. The engine reports nothing about how far a real blend departs from it, so this course does not either. Every blend in this course is computed under the stated assumption, and every figure you read is the engine's answer under it. When you quote a blend's specific gravity, the assumption travels with it.
