...
A refinery plan is built in one place and read in many: the refinery, the trading desk, the head office, the lender's engineer. A schedule date must mean the same day to all of them. This lesson reads the digest's test of that, ABUA's schedule built in seven time zones, with the period start 2027-03-01 handed over two ways.

{{panel:refinery-plan-explorer}}

## The string, in seven zones

The digest builds ABUA's schedule in seven time zones, each in its own process, with the period start passed as the string "2027-03-01". It prints the first and last event dates, and whether every one of the 41 dates matches the UTC run:
...
Every row reads true. The zones span both sides of Greenwich, from Pacific/Kiritimati to Pacific/Pago_Pago, and the digest's header adds that the period crosses the North American spring clock change. None of it moves a date. Built anywhere, the schedule starts on 2027-03-01 and ends on 2027-03-31.

## A date object at local midnight

The digest then hands over the same period start a second way: as a Date built at local midnight, new Date(2027, 2, 1). The engine reads a Date's UTC calendar day, so the answer depends on the zone:
