...
Every date in ABUA's schedule is counted from one date: the period start, 2027-03-01. This lesson reads why the period start is an input the schedule must be given, what happens when it is left out, and why this course passes it as a string.
...
The Associate tier's first module listed the exports that read the machine clock when an argument is left out. In refineryPlanning there is one: cascadeToSchedule. The course names the argument that stops it: cascadeToSchedule reads it when periodStart is left out.
...
This course is explicit about how it avoids that: every schedule in it is cascaded from 2027-03-01, passed to the engine as that YYYY-MM-DD string, and the course refuses to print a line from a call that did not pass it. Nothing in the course read the machine clock.

## Why the period start is a string

A date can reach the engine as a string or as a date object built by the calling program. They behave differently, and Lesson 4 shows how. The short version is in the next lesson's closing line on zones:
