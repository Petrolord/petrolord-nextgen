...
Revenue, crude cost and variable opex are the year's money on the crude run: what the products sell for, what the crude costs to buy and what it costs to process. Fixed opex is the yearly figure the gross margin leaves out, 7500000.00. Capex carries the capital of 64000000.00, spread evenly across the construction years: 32000000.00 in year 0 and 32000000.00 in year 1.

Every column is in US dollars except crude run, which is in barrels.
...
The producing rows shown are identical. The plant runs the same barrels at the same prices and costs in year 2, year 3 and year 21. That is the screen's simplification: one year, repeated. It keeps the valuation traceable, and it means the streams carry no ramp up after start, no price path, no change in yields with age, and no turnaround year.
...
Change the crude cost on the panel and watch the crude cost column move while the revenue column stays still. Change a product price and watch revenue move while the crude cost column stays still. Change the capacity and watch both move, together with crude run. The fixed opex column moves only when you change the fixed operating cost itself.
