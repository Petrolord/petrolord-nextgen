...
**The readings, worked at 15 Hz.** The panel opens at 15 Hz. There the tuning thickness tile reads 26 ms; read it from the tile, not from the apex of the plotted curve. The amplitude at tuning is the tuning tile's amplitude. The isolated level is the thick end of this same run, so it belongs to this panel state and not to a separate one; divide the tuning amplitude by it, or read the quotient straight off the "Amplitude relative to isolated" tile with the thickness set to the tuning thickness. A bed thinner than tuning is read by setting the thickness selector to it and reading the "Amplitude there" tile, printed to ten decimals.

**State one: 25 Hz.** Set the frequency to 25 Hz and read the tuning thickness, the amplitude at tuning and the ratio together while the panel is in that state.
...
**Reporting the isolated level where the ratio is asked for.** The two tiles sit next to each other. The graded field is the quotient, not the level, and not the tuning amplitude either.

**Reading the 40 Hz tuning thickness off the 25 Hz curve.** Both curves have an apex that looks flat at chart scale. Confirm the frequency tile reads 40 before recording it.
...
Three relationships hold across the fields. The tuning amplitude divided by the ratio must return the thick end level, close to 0.08. The frequency times the tuning thickness must land near 390 to 400 Hz ms at each frequency, because the tuning thickness scales as the inverse of the frequency (module 4). And the modelled thickness must sit on the 2 ms grid step nearest the theoretical one. At 15 Hz: $15 \times 26 = 390$, and 26 is the grid step nearest 25.99.
