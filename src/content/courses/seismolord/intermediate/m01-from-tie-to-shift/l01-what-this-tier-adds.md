...
You will meet the same construction repeatedly. The observed seismic the lessons work is the 25 Hz synthetic from the teaching well, arriving 6 ms late in TWT. The lag was planted deliberately, which means the scan's answer can be checked against a known truth rather than accepted on trust. The next lesson explains why that design is honest and where its honesty ends. The capstone's observed trace is built the same way with a lag its brief withholds, so the scan, not the lessons, has to find it.
...
Self-check: the second question is the important one. A placed synthetic reports a shift and a quality, because the shift alone tells you what was done and says nothing about whether it was worth doing. A shift of 6 ms in TWT that produces poor agreement and a shift of 6 ms that produces excellent agreement are the same instruction and different pieces of evidence, and only the quality number separates them. On the first part, the dependency that matters most is convolution: if you do not accept that a trace sample is a sum of overlapping wavelet copies, then modules 4 and 5, which explain both the amplitude change and the time change entirely by how far those copies reach, have nothing to stand on.
