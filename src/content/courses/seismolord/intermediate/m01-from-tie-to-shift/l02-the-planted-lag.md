...
The engine builds the synthetic from the teaching well at a 25 Hz Ricker wavelet, on a 2 ms sample grid in two-way time. It then makes the observed trace by copying that synthetic forward by three samples. Three samples at 2 ms each is 6 ms, so every event in the observed trace sits 6 ms later in TWT than the same event in the synthetic. The first three samples of the observed trace have nothing to copy into them and are left as gaps.

That is the entire construction. The observed trace is the synthetic and nothing else, moved down the time axis by 6 ms. There is no noise added, no second wavelet, no processing, no change of polarity, no missing reflector, and no lateral geology. The planted lag is 6 ms in TWT, the scan is not told what it is, and the correct answer therefore exists in advance. The capstone's trace is made the same way with a different lag, which its brief does not state.
...
The reading habits generalise. In module 3 you will find that at zero lag, meaning an unshifted tie, this exercise still returns a correlation of 0.771383. That is a number many people would look at and accept, while the tie sits a full 6 ms wrong in TWT. The lesson that a plausible correlation is not evidence of alignment is a real lesson about real data, and it happens to be visible here with unusual clarity.
...
One habit to build now. The planted lag is 6 ms and the shift the scan reports is 6 ms, and both are differences in two-way time. Whenever you write one down, write the unit and say that it is a shift in TWT. A bare 6 in a notebook is a number that will mislead somebody later, including you, because depth shifts, TWT shifts and one-way time shifts all get written as a small number of the same magnitude and only the label separates them.
