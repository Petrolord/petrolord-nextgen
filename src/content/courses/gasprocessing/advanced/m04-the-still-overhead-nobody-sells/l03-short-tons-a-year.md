...
The engine reports the still overhead in two units at once, pounds a day and short tons a year. The step between them looks like a triviality and it is the clearest example in the module of a constant that can only be measured as a group.

{{panel:fc-water-explorer}}

## Two units, one balance

On OBIAFU at 180.000000 ppmv and an absorbed fraction of 0.150000 the engine returns 405.8358 lb a day and 74.065024 short tons a year. Both come from the same mole balance. Neither is a conversion a reader has to perform.

Reporting both is deliberate. A pounds a day figure is what an operator recognises on a shift. A short tons a year figure is what a permit or an emissions return is written in. Handing over one and asking the reader to convert is how a factor of the wrong size gets introduced.

## The group, and why it is one
...
When a module reports one quantity in two units, ask whether the second came out of the same balance or was converted afterwards, and whether the factor between them is visible. Here it is measured, from two printed answers a reader can reproduce.
