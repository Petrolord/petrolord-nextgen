...
A negative rate returns negligible, and that one is different in kind. It is an answer rather than a refusal, and it is the most reassuring word in the vocabulary, produced from an input that cannot be a corrosion rate. What the engine returns is the behaviour of the door itself, and it says nothing about what any caller hands it. It is the shape to look for: a function that answers where it could decline, with the least alarming answer available.
