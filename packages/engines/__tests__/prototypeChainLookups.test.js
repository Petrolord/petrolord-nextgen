 ...
 *
 * NEXTGEN LOCAL VARIANT (packages/engines/VENDOR.json, group
 * 8-local-test-variant). Canonical 16fd6c9's file, minus: (1) every case and
 * import for the wellsite domain, which NextGen deliberately does not vendor
 * (excludedDomains); (2) the welldata/lasParse LAS 3.0 block case, whose target
 * code the held lasParse.js blob does not have; (3) one assertion on the
 * gasWellLoading refusal `code` field, which is not part of the #231 repair.
 * Every other case is canonical byte for byte, and it runs against the #231
 * own-property hunks backported onto the held Group 3, 4 and 5 blobs.
 */
...
test('peerReview.canTransition refuses instead of throwing', () => {
  each((k) => {
...
test('production/gasWellLoading refuses an inherited correlation instead of returning ok with a NaN velocity', () => {
  each((k) => {
    ...
    // NEXTGEN LOCAL VARIANT: canonical also asserts r.code === 'unknownCorrelation'.
    // The `code` field is a separate canonical change the held Group 3 blob does
    // not carry; only the #231 own-property hunk was backported onto it.
    expect(typeof r.error).toBe('string');
    noPrototypeLeak(r);
  });
});
...
// NEXTGEN LOCAL VARIANT: canonical's welldata/lasParse case (a LAS 3.0 block
// named after an inherited member) is removed. The vendored lasParse.js is a
// held Group 5 blob that predates LAS 3.0 block parsing, so it has no `blocks`
// map for #231's own-property hunk to guard; the case lands with that path's
// own re-vendor.
