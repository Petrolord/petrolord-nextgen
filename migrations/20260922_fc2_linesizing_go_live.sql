...
begin
  ...
  if abs(v_erosional_velocity_fts * sqrt(52.3) - 120) > 1e-9
     or abs(v_erosional_velocity_fts * sqrt(52.3) - 100) < 1
     or abs(v_erosional_velocity_fts * sqrt(52.3) - 125) < 1
     or abs(v_erosional_velocity_fts * sqrt(52.3) - 175) < 1 then
    raise exception 'FC2 go-live refused: the erosional velocity implies a c factor of %, which is not the site figure of 120 the prompt states', v_erosional_velocity_fts * sqrt(52.3);
  end if;
