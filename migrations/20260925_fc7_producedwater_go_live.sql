...
begin
  ...
  select count(*), string_agg(c.tier || '/' || (f->>'key'), ', ')
   ...
   where c.app_slug = 'producedwater' and (f->>'key') like '%' || frag || '%';
  if v_graded <> 0 then
  ...
  select count(*), string_agg(c.tier || '/' || (f->>'key'), ', ')
   ...
   where c.app_slug = 'producedwater' and lower(f->>'label') like '%' || frag || '%';
  if v_graded <> 0 then
