...
q(3, "Asked to check a sonic log in a unit it does not list, the range check refuses. Which field does it name?",
 "`unit`, and the message lists us/ft and us/m and adds that units are never converted here.",
 ["`channel`, and the message lists the ten channels the engine knows, sonic among them.",
  "`values`, and the message says the values cannot be read without a unit the engine lists.",
  "`unit`, and the message says the engine converted the values to us/ft and asks the caller to confirm."],
 "The engine's own words are: unit must be one of us/ft, us/m for channel sonic: units are never converted here. The field is `unit`. The `channel` refusal is the one for an unlisted channel, and the engine converts nothing, so there is no conversion to confirm."),
