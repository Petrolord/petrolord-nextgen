...
q(0, "What does the engine return as the MAD of a series?",
 ...
 "The median and MAD module defines the MAD as the raw median of |x - median|. The 1.4826 scale belongs to the Hampel window, which multiplies the raw MAD by it; an average of distances from the mean is a different statistic that one glitch would drag.")
...
q(3, "Which two gauge statistics stay the same when the glitch at entry 7 is left out?",
 ...
 "The median and MAD module prints both rows: the median is 212.500000 and the MAD 0.100000 either way. The mean moves from 215.250000 to 212.488889 and the sample SD from 8.732220 to 0.126930 when entry 7 goes.")
...
q(2, "The engine multiplies by 0.6745 in the modified z-score. What is that constant?",
 ...
 ["The reciprocal of Tukey's fence multiplier 1.5, carried to four decimals.",
 ...
 "The engine's export list gives `MODIFIED_Z_SCALE` 0.674500 with Iglewicz and Hoaglin (1993), as NIST prints it, as its source, and the median and MAD module says the engine uses 0.6745 as printed. The reciprocal of 1.4826 is 0.674491, a different number, and the constant was not fitted to any dataset.")
...
q(0, "On the EKENE-7 core plugs the median is 0.215500 and the MAD 0.007500. What does the modified z-score report?",
 ...
 "The median and MAD module prints the core row: largest |M| 5.890633, entry 8 flagged. 2.985356 is the ordinary z of the same plug. Entry 2 is flagged only in the copy with a second high plug, and fourteen plugs raise no refusal.")
...
q(0, "The series 5, 5, 5, 6, 7 has a spread, yet its median absolute deviation is zero. What makes it zero?",
 ...
 ["Every present value is the same, which is the only way a MAD can ever reach zero.",
  "Five values are too few for a MAD, so the engine sets it to zero by rule on a short series.",
  "The mean and the median coincide, so every deviation cancels out."],
 "The median of 5, 5, 5, 6, 7 is 5 and three of the five values equal it, so three of the five absolute deviations are 0 and the middle one is 0. The MAD is 0 and the modified z-score refuses. The values 6 and 7 show that a spread in the data does not prevent it; the length of the series and the mean play no part, and the engine sets nothing by rule.")
...
q(2, "What is the engine's own label for a value whose modified z-score is beyond 3.5?",
 ...
 "The course vocabulary records the engine's word for the modified z as \"potential outlier\", from Iglewicz and Hoaglin as NIST prints them. A flag is a question about the value; invalid is the range check's language, and planted defects are the generator's list.")
...
q(0, "With 1 / 1.4826 in place of 0.6745, the gauge glitch would read 186.159450 against the engine's 186.162000. What does that do to the flag?",
 ...
 "The median and MAD module derives 186.159450 with 0.674491 against the engine's 186.162000. Both are enormous next to 3.5, so the flag at entry 7 stands. Nothing about entry 1 or any refusal changes with the constant.")
...
q(3, "Why does the modified z-score catch the gauge glitch that the ordinary z-score missed?",
 ...
 "The median and MAD module: median 212.500000 and MAD 0.100000 in both rows, while the sample SD falls from 8.732220 to 0.126930 without entry 7. The threshold is 3.5, higher than 3, the modified z uses no standard deviation, and every present value takes part.")
...
q(2, "The modified z-score and the Hampel window both build a spread from the raw MAD. How does each use it?",
 ...
 "The median and MAD module gives M = 0.6745 (x - median) / MAD with the raw MAD, and says Hampel's scale, 1.4826 x MAD, is the same idea from the other side. The engine's export list gives the two constants separately, `MODIFIED_Z_SCALE` 0.674500 and `HAMPEL_MAD_SCALE` 1.482600.")
...
q(3, "The mean of the ten gauge readings is 215.250000. What is the mean of the nine readings left once entry 7 is removed?",
 ...
 ["212.500000",
  "215.250000",
  "212.400000"],
 "The median and MAD module prints the derived mean with entry 7 left out as 212.488889, computed by lib/stats `mean`. 212.500000 is the median of either set, and 215.250000 is the mean with the glitch still in.")
...
q(1, "Once the glitch is dropped, how widely do the nine ordinary gauge readings scatter by the sample SD?",
 ...
 ["0.100000",
  "8.732220",
  "8.284111"],
 "The median and MAD module prints 0.126930 for the nine readings. 0.100000 is the MAD, 8.732220 the sample SD with the glitch in, and 8.284111 the population SD of all ten. One reading takes the sample SD from 0.126930 to 8.732220.")
...
q(1, "Where does the Hampel scale 1.4826, the other constant that works with a raw MAD, come from?",
 ...
 "The engine's export list gives `HAMPEL_MAD_SCALE` 1.482600 with the petrophysics conditioning engine's despikeHampel as its source. Iglewicz and Hoaglin supply 0.6745 and the threshold 3.5, Tukey's multiplier is 1.5, and no constant was fitted to Ekene data.")
...
q(2, "A value has a modified z-score of exactly 3.5. Is it labelled?",
 ...
 "The median and MAD module labels a potential outlier when |M| > 3.5. The comparison is strict, as it is for the z-score, the Tukey fences and the Hampel threshold, so a value exactly on 3.5 carries no label and no refusal is raised.")
