...
q(2, "What is Grubbs' G on the fourteen EKENE-7 core plugs with the one fractured plug?",
 ...
 ["2.507321",
  "3.611249",
  "5.890633"],
 "Section 22 prints G 2.985356, which equals the largest |z| of Section 17. 2.507321 is the two-sided critical value at alpha 0.05, 3.611249 is the t point used to build it, and 5.890633 is the plugs' largest modified z-score.")
...
q(0, "For the two-sided test on the fourteen plugs at alpha 0.05, which t point does the engine use to build the critical value?",
 ...
 "Section 22: t is the upper alpha / (2N) point of Student's t on N - 2 degrees of freedom, and the table prints the tail probability 0.001786 and t 3.611249 for N = 14. Dividing alpha by 2N is the correction for testing the largest of N values.")
...
q(3, "The two-plug copy of the core set faces the same critical value, 2.507321, as the one-plug set. Why?",
 ...
 "Section 22: the critical value depends on n, alpha and the side only; the values the rows were run on do not enter it. Both sets hold fourteen plugs, tested two-sided at 0.05.")
...
q(1, "With a second high plug at entry 2, what does Grubbs' test report on the fourteen plugs?",
 ...
 "Section 22's masking row: sample SD 0.024800, G 2.275359, critical 2.507321, reject false, suspect entry 8. 2.371654 is the one-sided critical at n = 14, and the engine runs the test with no refusal.")
...
q(2, "In the masking example, what moves G from 2.985356 down to 2.275359?",
 ...
 "Section 22: with a second high plug the sample SD rises from 0.019859 to 0.024800, and G falls to 2.275359. The critical value stays 2.507321, the suspect stays entry 8, and the generalised ESD test is not built.")
...
q(0, "What is the one-sided critical value for Grubbs' test at n = 14 and alpha 0.05?",
 ...
 ["2.507321",
  "3.474396",
  "2.031652"],
 "Section 22's table prints 2.371654 one-sided and 2.507321 two-sided at n = 14. 3.474396 is the largest G fourteen values can produce, and 2.031652 is the one-sided critical at n = 8.")
...
q(3, "How does the two-sided Grubbs critical value at alpha 0.05 move across the table's sample sizes, from 6 to 30?",
 ...
 "Section 22 prints 1.887145 at n = 6 rising to 2.908473 at n = 30. The largest possible G runs from 2.041241 to 5.294651 over the same rows, above the critical value at every n.")
...
q(2, "At ten values the z-score cannot pass 3, since its ceiling is 2.846050. Can Grubbs' test reject at ten values and alpha 0.05, two-sided?",
 ...
 "Section 22 prints the two-sided critical 2.289954 and the largest possible G 2.846050 at n = 10. G shares the ceiling, and the bar it must clear is 2.289954 in place of 3, so a rejection is reachable one-sided (2.176068) or two-sided.")
...
q(1, "What does the engine offer for a sample that may hold several outliers?",
 ...
 "Section 22: the test is for ONE outlier; NIST points to the generalised ESD test for several, which the engine does not build. Section 31 repeats it in the list of what is not built, beside the robust covariance.")
...
q(0, "Oil sand entry 60 reads z -1.074527 on density and -1.645167 on neutron, and neither channel's z-score flags anything. Why does `mahalanobis` flag it at 22.397696?",
 ...
 "Section 23: each value alone is ordinary, and the pair sits off the trend; the correlation, -0.836264, is what the distance knows. The covariance is the sample one (n - 1), and a density and a neutron inside their sand ranges are what the generator planted.")
...
q(1, "At alpha 0.05 the cutoff on 2 degrees of freedom is 5.991465. Which oil sand rows are flagged?",
 ...
 "Section 23: at 0.050000 two rows are flagged. Entry 62's d^2 of 6.581737 is above 5.991465 and entry 60's 22.397696 is too; entry 56, at 5.442663, comes in only at alpha 0.100000 with the cutoff 4.605170.")
...
q(3, "What covariance does `mahalanobis` use, and what does its basis block call it?",
 ...
 "Section 23: d^2 uses the classical mean and the SAMPLE covariance S (n - 1), and a robust covariance is not built. Section 31 quotes the label \"sample covariance (n - 1), classical (not robust)\".")
