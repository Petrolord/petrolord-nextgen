...
q(1, "How many samples does a Hampel window hold at halfWindow 3, away from the ends of the series?",
 ...
 ["3",
  "6",
  "4"],
 "The Hampel module: the window is 2 x halfWindow + 1 samples centred on the sample, so halfWindow 3 gives 7. Four is what the window at entry 0 holds once it is truncated at the top of the log.")
...
q(3, "At entry 70 of the gamma ray the window MAD is 7.700000 and the engine returns a threshold of 34.248060. How is that threshold built?",
 ...
 "The Hampel module flags a sample when |x - window median| > nSigma x 1.4826 x MAD of the window, and the threshold column prints 34.248060 for entry 70's MAD of 7.700000 at nSigma 3. The 1.4826 is `HAMPEL_MAD_SCALE`, the petrophysics conditioning engine's value.")
...
q(0, "Entry 145 is flagged at a deviation of only 7.240000 gAPI, while entry 70 needs far more. Why?",
 ...
 "The Hampel module prints entry 145 with window MAD 0.830000 and threshold 3.691674, against entry 70's 7.700000 and 34.248060. nSigma is 3 on every row, entry 145 sits in the water sand far from either end, and the rule is the same on both sides of the median.")
...
q(2, "On the whole gamma ray channel the z-score flags nothing: entry 70 reads z 1.058062 and entry 170 reads 0.900269. Why does the global rule miss both spikes?",
 ...
 "The Hampel module: a sand spike to 95.420000 gAPI looks like shale to a global mean and SD. The ceiling rises with n, 4.248529 already at 20 values, the spikes were added to the sand, the z test is on |z|, and no rule skips another's flags.")
...
q(3, "How many of the eleven Hampel flags on the gamma ray at halfWindow 3 and nSigma 3 are planted defects?",
 ...
 "The Hampel module: entries 70 and 170 are the planted spikes, and the other 9 are not planted defects. Each of those 9 is a sample farther from its window median than three scaled MADs of a seven-sample window, which is what the rule flags.")
...
q(1, "Which setting in the Hampel table flags the fewest samples on the gamma ray?",
 ...
 "The Hampel module prints 11, 8, 6, 11, 3 and 4 flags down the table; halfWindow 5 at nSigma 5 gives the smallest count, 3. halfWindow 10 at nSigma 3 gives 4, halfWindow 3 at nSigma 5 gives 6, and halfWindow 5 at nSigma 3 gives 11.")
...
q(0, "Does any setting in the Hampel table flag the two planted spikes and nothing else?",
 ...
 "The Hampel module's settings table reads true in the both-spikes column on every row, and its smallest flag count is 3, at halfWindow 5 and nSigma 5. halfWindow 10 at nSigma 3 flags 4, so every setting flags at least one sample beyond the two spikes.")
...
q(2, "What does the engine return for gamma ray entry 0 at halfWindow 3?",
 ...
 "The Hampel module: at entry 0 the window is truncated to 4 samples, `windowCount` 4, which is at least three, so it is judged and not flagged. The engine truncates at the ends and pads nothing.")
...
q(1, "On the EKENE-7 density, entry 80 is the first sample of the twelve-sample gap. What does `hampel` return for it?",
 ...
 "The Hampel module: entry 80 is missing and is not judged (`judged` false). Missing values never enter a window, and the engine's export list says the engine does not fill a value. Entry 79, beside the gap, is the one whose window holds 4 present samples.")
...
q(3, "On 2.3, null, null, 2.9, null, null, 2.31 with halfWindow 1, what does the engine report for entry 3?",
 ...
 "The Hampel module: entry 3 has 1 present sample in its window and `judged` false, because a window with fewer than three present samples is not judged. Missing values never enter a window, and the engine raises no refusal here.")
...
q(0, "The golden case hampel-on-the-threshold runs -1, 0, 1.482600, 1, 0 at halfWindow 2 and nSigma 1. Which entry does it flag?",
 ...
 ["Entry 2",
  "Entries 2 and 4",
  "None"],
 "The Hampel module: entry 2 sits exactly on its threshold (window median 0, MAD 1, threshold 1.482600) and is not flagged, since the comparison is strict; the case flags entry 4.")
...
q(2, "Hampel flags gamma ray entry 170 and returns a `cleaned` series. What happens to the original reading of 90.590000?",
 ...
 "The Hampel module gives the replacement as the window median, 38.410000 for entry 170. The engine's export list: the engine does not fill, repair or delete a value, and `hampel` returns a cleaned series beside its flags; choosing to use it is the caller's decision.")
...
q(1, "Asked for a Hampel screen with halfWindow set to 0, how does the engine respond?",
 ...
 "The refusal table lists the refusal: \"halfWindow must be a whole number, 1 or more (the window is 2 x halfWindow + 1 samples)\". The engine refuses by name and does not fall back to a halfWindow of 3.")
...
q(3, "The decision inside `hampel` is imported from the petrophysics engine's `despikeHampel`, whose own entry point turns a null into 0. What does that do to this engine's results?",
 ...
 "The engine notes: `hampel` converts missing values to NaN before the call, so this engine is unaffected, and the Hampel module confirms missing values never enter a window. The density gap is handled without a refusal, entry 80 simply not judged.")
