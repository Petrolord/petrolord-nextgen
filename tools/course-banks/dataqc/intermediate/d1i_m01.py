...
q(2, "Which standard deviation does `zScores` divide by when the caller leaves the choice unset?",
 ...
 "The z-score module states z = (x - mean) / s with the sample standard deviation (n - 1) by default, as NIST/SEMATECH 1.3.5.17 defines it. The population SD is available only when the caller asks for it, the MAD belongs to the modified z-score, and the engine never picks a spread by the result it gives.")
...
q(3, "With the sample standard deviation, what is the largest absolute z that any value in a series of n values can reach?",
 ...
 "The z-score module gives the ceiling (n - 1) / sqrt(n) for the sample SD. The ceiling sqrt(n - 1) belongs to the population SD, which the engine names in its basis block when that choice is made. However far out a value sits, it drags the mean and widens the SD, and the engine caps nothing at the threshold.")
...
q(1, "On the ten gauge readings the engine returns `maxPossibleAbsZ` 2.846050 and `thresholdReachable` false, with 0 flags at 3. What does that zero say?",
 ...
 "The z-score module: at n = 10 no value can pass |z| > 3 with the sample SD, however wild. A zero from a test that could not fire is silence and carries no verdict. Entry 7 was in the call and reached 2.845783, and the engine returned a result with no refusal.")
...
q(1, "In the ceiling table, what is the smallest sample size at which a z beyond 3 becomes reachable with the sample SD?",
 ...
 "The table reads false at 10 values, ceiling 2.846050, and true at 11, ceiling 3.015113. At 12 it is true as well, and 20 values is well past the point where 3 can be reached.")
...
q(3, "The caller switches the gauge call to the population standard deviation. What changes?",
 ...
 "The mean is 215.250000 in both columns of the z-score module; only the spread changes. Dividing by n gives the smaller SD, 8.284111, so every z grows, and entry 7 reaches 2.999718, still short of 3, with 0 flags.")
...
q(0, "Nine zeros and a one are run with `sd: 'population'` at a stated threshold of 2.9. What ceiling does the engine report?",
 ...
 "The engine notes: the ceiling follows the chosen standard deviation. With the population SD the ten values reach 3.000000, `thresholdReachable` is true and 1 flag fires at 2.9. The sample SD on the same values stops at 2.846050; the other two figures belong to other rows.")
...
q(2, "EKENE-7's fourteen core plugs could reach a z of 3.474396. The fractured plug at entry 8 reaches 2.985356 and nothing is flagged. Why?",
 ...
 "At fourteen values 3 is reachable, so the ceiling is not the cause. The z-score module prints the sample SD at 0.019859 with all fourteen and 0.010574 with entry 8 left out: the plug widens the yardstick that measures it. The default stays the sample SD, and entry 8 carries the largest z, 2.985356.")
...
q(3, "Measured against the mean and sample SD of the other thirteen plugs, where does the fractured plug at entry 8 sit?",
 ...
 ["2.985356",
  "2.066042",
  "5.890633"],
 "The z-score module derives 6.038080 against the other thirteen. Measured against a spread that includes itself the plug reads 2.985356, which is what `zScores` returns. 2.066042 is the largest z among the thirteen, and 5.890633 is the modified z-score.")
...
q(0, "With the fractured plug left out, what do the thirteen remaining plugs return?",
 ...
 "The z-score module prints the second row: 0.217154, 0.010574 and 2.066042, and no flag. The full-set figures are 0.221714 and 0.019859. The 6.038080 belongs to entry 8 itself against the thirteen, and a series of thirteen raises no refusal.")
...
q(2, "A report states that the gauge glitch sits 'almost three sigma out'. What does this course require the report to add?",
 ...
 "The course vocabulary makes sigma a standard deviation with its source named: sample SD, 1.4826 x MAD or another stated source. Here the z was measured against the sample SD, 8.732220. The course uses no P label at all, and a percentage of the mean is a different quantity.")
...
q(3, "The gauge glitch reaches 2.999718 against the population SD. Why is it still not flagged at a threshold of 3?",
 ...
 "The z-score module flags a value when |z| > 3. 2.999718 is below that, and the population column shows 0 flags. The engine accepts the population SD and compares the unrounded figure; the population ceiling at ten values is 3.000000.")
...
q(2, "Twenty daily rates are screened with `zScores` at 3 and the sample SD. Can any of them be flagged?",
 ...
 "The ceiling table prints 4.248529 at 20 values, reachable at 3. 2.846050 is the ten-value row, the ceiling first passes 3 at eleven values, and 3.000000 is the population ceiling for the ten-value case in the engine notes.")
