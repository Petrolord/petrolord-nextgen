...
q(0, "NIST/SEMATECH 1.3.5.17.1 prints Grubbs' G on its uranium example as 2.4687, and the engine reads 2.468765. What differs?",
 ...
 "Section 31: truncated rather than rounded; rounded to four decimals the engine reads 2.4688. G uses the sample SD, as NIST defines it. The engine keeps full precision and compares 2.468765 with its critical value of 2.031652, and the test rejects either way, as NIST says. The note is about a published page.")
...
q(2, "NIST 6.3.2.4 prints the EWMA lower limit as 47.4115, and the engine reads 47.411568. Where does the difference come from?",
 ...
 "Section 31's row for 6.3.2.4 names the early rounding of the factor. The engine keeps full precision, and its upper limit, 52.588432, agrees with the printed 52.5884. Both limits in the example are the asymptotic pair, the engine takes s as given, and L is 3 on both sides.")
...
q(2, "What covariance does the engine's Mahalanobis distance use, and how does its basis block describe it?",
 ...
 "Section 23: each complete row's d^2 uses the classical mean and the SAMPLE covariance S (n - 1), and a robust covariance is not built. Section 31 quotes the label. The classical estimates are themselves pulled by outliers, which is why the label says so. The engine has no median covariance.")
...
q(0, "With `sd: 'population'`, the stated series of nine zeros and a one is run through zScores at threshold 2.9. What does the engine return?",
 ...
 "Section 31: the ceiling follows the chosen standard deviation. With the population SD it is sqrt(n - 1), 3.000000 at ten values, the threshold 2.9 is reachable, and the one reaches z 3.000000 and is flagged. With the sample SD the same ten values reach 2.846050 at most. The spread is not zero, since one value differs.")
...
q(3, "A draft quality note says entry 7 of EKENE-3's gauge readings \"is an outlier\". What does the course's vocabulary ask of that sentence?",
 ...
 "Section 32: an outlier is a value a STATED RULE flags; say which rule, and the engine's word for the modified z is potential outlier. Section 18 prints entry 7's modified z as 186.162000. The z-score does not flag it: 2.845783 is under 3, and at ten readings no z can pass 3. Missing is null, undefined or NaN, and a flag is a question about the value; the engine deletes nothing.")
...
q(0, "A quality note wants to report the gamma ray's upper quartile on EKENE-7's water sand. How does the course's vocabulary say it should be named?",
 ...
 "Section 32: a quantile at a stated probability by a stated rule (R6, R7 or R8); no P label is used anywhere in this course, since P labels differ between exceedance and non-exceedance conventions. Section 19 prints the third quartile as 38.642500 by R7, 38.737500 by R6 and 38.725833 by R8, so the rule changes the value.")
...
q(3, "The cumulative oil flag's reason, in the engine's words, reads \"cumulative falls from 1338506.2 at entry 67 to 1331009.5\". Which day is entry 67, and which day is flagged?",
 ...
 "Section 11: the engine counts entries from 0, so entry 67 in the reason is day 68. Day 69 is missing, so day 70 is compared with the last present value, day 68, and the `drop` field is 7496.700000 bbl. Entries are positions, one per day, and a missing value keeps its position.")
...
q(2, "On EKENE-3, day 47's oil rate of -18.500000 bbl/d is flagged negative-rate. What does the flag tell the reader about the value?",
 ...
 "Section 1: the engine does not decide whether a flagged value is wrong; a flag is a rule that fired, with its reason. Section 2 states day 47 is an allocation back-out booked as -18.5 bbl/d, a real entry. The engine does not fill, repair or delete a value, and a negative number is present: missing is null, undefined or NaN.")
