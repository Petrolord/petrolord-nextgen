...
q(0, "A `cusumChart` call on EKENE-3 passes target, sigma, k 0.5 and h 4, and says nothing about their units. What does the engine return?",
 ...
 "`units` is required, and the engine refuses and names the two choices in its own words. The rule of thumb in the message is advice, and the engine does not read the numbers to guess which unit was meant. The refusal names `units`, the field that is missing; k and h are present and valid numbers.")
...
q(0, "EKENE-3's S_hi passes h on day 8 at 26.197812. What does the engine do with the upper sum on day 9?",
 ...
 "No reset after a signal; the sum carries on from where it stood. S_hi on day 9 is day 8's 26.197812 plus day 9's 608.000000 less the target and less k, and the table prints 20.930780, above 15.096251, marked high. Both sums update on every day, and S_lo is 1.492969 on day 9 alongside it.")
...
q(1, "NIST 6.3.2.3's design line, h = (2 / delta^2) ln((1 - beta) / alpha) k, is fed the page's own alpha 0.0027, beta 0.01, delta 1 and k 0.317500. What does it give, and what does the engine do about it?",
 ...
 "The course derives 3.749328 and 4.189476 from the page's own inputs; the table uses 4.195900. The course lists a CUSUM design helper from alpha and beta as not built: k and h are inputs with a stated unit. The engine takes the h it is given, reproduces the printed table from it, and takes no alpha.")
...
q(2, "From day 16 to day 40 on EKENE-3, each chart with its own settings, how many low signals does each chart raise?",
 ...
 "The course prints 1, 11 and 20 low signals from day 16 to day 40. 5, 15 and 28 are the days with any signal, high or low, over all forty days, which includes the glitch. The individuals chart does signal low once, on day 22, where a single day falls past three sigma.")
...
q(2, "Which of the three charts signals on day 8, the planted gauge glitch?",
 ...
 "The course's column 'signals on day 8' reads true for all three, and all three see the glitch. The EWMA signals high on days 8, 9, 13 and 14 and the CUSUM holds the glitch in S_hi from day 8 to day 16. The individuals chart signals on a single day past three sigma, which is exactly the glitch.")
...
q(3, "Someone leaves the reference value out of a CUSUM call that names its unit as sigma and gives h 4. How does the engine answer?",
 ...
 "NO ENGINE DEFAULT FOR K OR H. A call without either is refused and the field is named, in the engine's own words. The rule of thumb in the units refusal is advice, and 0.317500 belongs to the NIST example. A k of zero is a legal choice the caller can make, and the engine does not make it on the caller's behalf.")
