...
# D1 Expert m05, Designing a QC policy.
# Figures from digest Section 30 (the defaults and whose choice each is, the
# order of checks, the sonic refusal), Section 13 (the phase-sum tolerance),
# Section 14 (the slow drift), Section 12 (the water cut tolerance), Section 11
# (a tolerance that hides a keying error), Section 10 (the index defaults),
# Section 20 (R7 and R6 fences), Section 3 (the sentinel), Section 2 (the
# planted defects) and Section 1 (what the engine does not do).

q(1, "A QC policy for EKENE-3's pressure must decide whether completeness runs before the control charts. What settles it?",
 "The charts refuse a series with a gap, naming the entry, so completeness has to run first",
