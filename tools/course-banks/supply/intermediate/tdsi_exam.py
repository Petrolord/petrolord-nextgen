...
q(2, "A consultant quotes IBAFO's 3.6000 erlangs and says the rack needs 3.6000 bays. What is the flaw?",
 ...
 "Offered load = arrivals per hour / (60 / load minutes), and the bay count is not in it; utilisation = offered load / bays. The bay count must be a whole number: 2.5 bays is refused with \"The number of bays must be a whole number, one or more.\" The engine does not round it.")
...
q(3, "With only 3 bays against IBAFO's traffic, the mean queue column reads none. Why?",
 "A queue that grows without limit has no average length.",
 ["The mean queue at 3 bays is zero, and the engine prints a zero as none.",
  "No truck queues when all 3 bays are loading.",
  "Its Erlang B cannot be derived above one, so no queue can follow."],
 "At 3 bays the utilisation is 1.200000, the rack is stable false and the probability of waiting is 1.000000, so every arriving truck waits. The engine prints none for both the mean wait and the mean queue, and where it states the reason it says the queue grows without limit.")
...
q(2, "How does tankFarmCover count the IBAFO farm's ullage?",
 ...
 "Ullage is counted the same way as pumpable stock, tank by tank: each tank's capacity less its stock, never below zero, summed over the tanks, 11767.300 m3. The heel is not in the ullage formula; it enters working capacity and pumpable stock.")
...
q(3, "The IBAFO farm turns 44.1108 times a year. Which formula does the engine use?",
 ...
 "Turns a year = daily throughput x 365 / working capacity, 2640.000 m3 a day over 21845.000 m3, which is capacity less heel. Pumpable stock over daily throughput is the days of cover, 3.8345.")
...
q(0, "IBAFO's invented costs are 2.35 USD/m3 variable and 9400.00 USD fixed. Which margin does the engine print with both typed?",
 ...
 ["14388.00 USD",
  "20592.00 USD",
  "1.89 USD"],
 "The margin takes both invented costs off the revenue, and with both typed assumedZero reads none and the margin 4988.00 USD. Leaving the fixed cost blank lifts it to 14388.00 USD; the other two options are the revenue and a per m3 column.")
...
q(1, "Using the course's synthetic factor of 850 kg CO2e a tonne, what emissions does the IBAFO period's loss produce?",
 ...
 ["1.481061 kg CO2e per tonne",
  ...
  "none"],
 "The synthetic 850 kg CO2e a tonne multiplies the loss tonnes, and the case with factor and density supplied prints the product as 2913.7320 kg CO2e. The other two numbers are a weight and an intensity.")
...
q(2, "The IBAFO period prints three carbon cases: factor and density supplied, no emission factor, and no density. Which of them print a margin of 4988.00 USD?",
 "All three cases, whatever the carbon side reads.",
...
q(3, "At 330000.00 litres a day on the IBAFO lane, what does fleetSizing print?",
 ...
 "The first row of the demand sweep, 330000.00 litres a day, needs 10.000000 trips, which 16 trucks cover at a fleet utilisation of 0.940897. The other utilisations and 31 trucks belong to larger demands.")
...
q(1, "The IBAFO lane types 1.75 queueing hours a cycle, and the rack prints a mean wait of 47.2652 minutes. How are the two linked in the engine?",
 ...
 "The lane types its own 1.75 h queueing into the cycle, and the rack's mean wait prints 47.2652 minutes. The cycle is round trip / average speed + load + discharge + queue hours, with no rack figure in it, and the rack's load time is 24 minutes.")
...
q(2, "IBAFO prints three utilisations: the rack's 0.900000, the forecourt's 0.878289 and the fleet's 0.991038. Which of them is not a queue figure?",
 ...
 "The rack and the forecourt are both rackQueue, with bays or nozzles, and utilisation = offered load / bays. The fleet's trucks are the ceiling of trips needed over trips a truck a day, and fleetSizing reports the spare the ceiling buys; no queue enters it.")
...
q(2, "The station's dead stock of 2500.00 litres and IB-T3's heel of 150.000 m3 play which common role?",
 "Volume taken off the capacity before the usable or working room is counted.",
 ["Free water that the dip less the water height removes.",
  "Ullage kept empty so that the next receipt has room to discharge into the tank.",
...
q(2, "Which two notes say a carbon figure is absent because no emission factor was supplied?",
...
q(3, "The daily count of 216 trucks at the rack sits beside days of cover of 3.8345. Why does the farm not use the truck count?",
 ...
 ["The count is per hour, and the farm needs a day.",
  "The farm counts its liftings in trucks per hour.",
  "The count is an Erlang B figure."],
 "The farm table labels its divisor daily throughput (liftings), in m3, and with no daily throughput the farm gives days of cover none. The rack's 216 is labelled trucks per day at this arrival rate, and its inputs are arrivals, load minutes and bays, with no volume.")
