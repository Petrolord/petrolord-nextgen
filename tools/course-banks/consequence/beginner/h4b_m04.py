...
q(0,
  ...
  "The course explains: in a 50 m2 bund the spill would stand, derived, 0.600000 m deep behind a 0.5 m wall, and the engine refuses on `spillVolumeM3` in the quoted words. A refusal carries no number, so there is no warned result and no truncated pool. The engine has no spreading model, so it cannot spread the overflow either.")
...
q(2,
  ...
  "The course explains: the engine gives a diameter of 42.445659 m and an area of 1415.000000 m2, and the book prints its values truncated. 43.701937 m and 1500.000000 m2 are the 30 m3 teaching spill at 0.02 m, a different volume. The engine returns its full value, and nothing about pi or a warning is involved.")
...
q(3,
  ...
  "The course lists `containment` as UNCONFINED_STATED_THICKNESS on every row of the thickness table and CONFINED for the bund. The label says the thickness was stated, because nothing spreads. An overtopping spill is refused, so there is no OVERTOPPED result, and CONFINED belongs to a bund floor.")
...
q(1,
  ...
  "The course shows the rate as 0.451595 kg/s, the evaporation flux 0.005749887804 kg/(m2 s) times the pool area 78.539816 m2. 0.010163820387 is the mass transfer coefficient in m/s. The model string ends \"x A\", so the area enters. 0.672653 kg/s is the 5 m/s row; the argument is the wind at 10 m, `windSpeed10mMS`.")
...
q(0,
  ...
  "The mass transfer coefficient is km = 0.004786 u10^0.78 (2r)^-0.11 Sc^-0.67, and the course says the rate grows with the wind to the power 0.78. The vapour pressure is a stated input the engine never adjusts, so there is no cooling and no cap. The power is 0.78, and the engine has no spreading model.")
...
q(2,
  ...
  ["0.67, the exponent in the correlation, and a constant of 0.78, the exponent on the wind.",
   ...
   "1, and no constant."],
  "The course explains: the Schmidt number defaults to 0.8, the Yellow Book's value for gases and vapours in general, and `MACKAY_MATSUGU_C` is 0.004786. The 0.67 and 0.78 are exponents in km = 0.004786 u10^0.78 (2r)^-0.11 Sc^-0.67. 0.010163820387 m/s is a result of the correlation for one pool.")
...
q(3,
  ...
  "The course explains: two refusals draw the model's edge, and a vapour pressure at or above ambient is refused on `vapourPressurePa` in the quoted words, because Mackay and Matsugu is a model for a pool below its boiling point. A refusal carries no number, so nothing is capped or computed. The calm air message is the other refusal.")
