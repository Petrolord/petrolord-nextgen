...
q(0,
 ...
 "The course states t in MINUTES for every toxic preset, and the course's unit table pairs exposureMinutes with a toxic exposure. Seconds belong to the thermal probits' exposureTimeS. No preset takes hours, and the two sources differ in their concentration unit, never in their time unit."),
...
q(2,
 ...
 "The course explains: the Purple Book presets (pb-) are in mg/m3 and the Lees presets, as OSD/30 prints them, are in ppm. So lees-chlorine takes ppm with a = -8.29, b = 0.92 and n = 2. The g/m3 figure is the plume case of the course's worked example, and no preset converts between the two sources."),
...
q(0,
 ...
 "The engine converts ppm to mg/m3 through the ideal gas molar volume at the stated temperature, and a colder gas packs more mass into a cubic metre, so the mg/m3 figure and the toxic load rise. The coefficients, the time and the molar mass are not changed by the temperature."),
...
q(0,
 ...
 ["144642.857143, which is the toxic load of the time weighted mean held for the whole time",
  "64.285714",
  "25000.000000"],
 "`toxicDose` sums C^n dt over the history, \"D = sum(C^n dt)\", and the engine returns 211500.000000. 144642.857143 is the load of the time weighted mean, 64.285714, held for the whole time; 64.285714 is the mean concentration itself, and 25000.000000 is the Lees chlorine load at 50 ppm for 10 minutes."),
...
q(2,
 ...
 "The course explains: the Purple Book reads its P from Table 5.1, which is why the golden allows 0.002 absolute. The CDF is good to about 1.5e-7, which is why probabilities print to six decimals. The Purple Book toxic coefficients are never graded, so no tolerance of theirs is widened, and the plume is not in this comparison."),
...
q(3,
 ...
 "The course says 24.45 is the rounding of 24.465404 L/mol and the golden gates it at one part in a thousand. 24.055117 and 22.413970 L/mol are the engine's molar volumes at 293.15 and 273.15 K, and the engine uses R_J_MOL_K, 8.314462618, throughout."),
...
q(0,
 ...
 "The course lists the empty history refusal on the field `history` with the message quoted. The zero load message belongs to `probit`, the target concentration message to `plumeDistanceToConcentration`, and the probability message to `probabilityToProbit`."),
