...
q(2,
 ...
 "Section 29: the Purple Book reads its P from Table 5.1, which is why the golden allows 0.002 absolute. The CDF is good to about 1.5e-7, which is why probabilities print to six decimals. The Purple Book toxic coefficients are ungraded apart from this one case, and the plume is not in this comparison."),
...
q(1,
 ...
 "Section 29 describes the Lees columns as quoted, 13 substances at two probabilities and two durations, 52 values, all reproduced through the engine's inverse within one percent or one ppm. That is the evidence that makes the Lees toxic coefficients gradable. The columns are in ppm, and none was dropped."),
