...
q(0,
 ...
 "The engine's method is the bisection quoted, run on Z, after which the distance follows from Z times the cube root of the charge. The fit has no algebraic inverse in the engine, the Kingery-Bulmash fits are not in the engine at all, as the course records, and no Newton step on the distance is used."),
...
q(2,
 ...
 "The course states that every row round trips through the forward fit, so 30.090623 m returns the 50000 Pa it was searched for, to within the rounding of the printed distance. 19626.657230 Pa and 121853.141070 Pa are the forward rows at 50 m and 20 m, and 100000 Pa is another target altogether."),
...
q(0,
 ...
 ["800 and 0.049, as the paper prints them in its formula, since the golden is labelled PUBLISHED from that paper",
  ...
  "800 and 0.048"],
 "The course explains: the paper prints 800 and 0.049, but its column follows 808 and 0.048, and those are the constants in the engine's model string. Using the printed pair, or either mixed pair, would fail to reproduce the column that is the evidence."),
...
q(3,
 ...
 "The course says the column is the evidence and the golden is PUBLISHED because its numbers reproduce. The engine does not use the printed formula, the five values come from the 2020 paper's table, and 3.09e-6 is the difference for one case only; cbu-0.5kg-5m sits at 1.70e-4."),
...
q(3,
 ...
 ["cbu-1kg-5m, which the course shows at 6.47e-5",
  "cbu-0.1kg-2m, which the course shows at 5.73e-5",
  "cbu-0.1kg-1m, which the course shows at 3.09e-6"],
...
q(2,
 ...
 "A vapour cloud is not TNT, and the course says the yield factor carries the whole difference. The multi-energy method is named by the course as out of the engine because its charts are graphical and no published curve fit was found, so no congestion factor exists. The TNT blast energy is always the denominator."),
...
q(1,
 ...
 "The course shows the column at an ambient of 101.325 kPa and says the validation record names the table as secondary. 100000 Pa is one of BONGA's targets, the table prints no per case ambient, and 6894.757293168361 is `PA_PER_PSI`, a pascal count per psi."),
