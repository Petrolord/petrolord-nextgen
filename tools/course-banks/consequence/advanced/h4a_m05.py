...
q(3,
 "For the lees-chlorine preset the engine's one percent toxic load is 149793.673220. What does the exact probit give, derived in the course?",
 "149793.533322",
...
q(0,
 ...
 ["Grades it at six decimals like every other figure the course shows, since the forward CDF is good to about 1.5e-7",
 ...
 "The course ends: a thermal dose or toxic load read back from a probability is quoted to the figures the approximation supports, and this course never grades one. The forward accuracy does not carry through the inverse, the rounding edge cells are kept apart rather than graded, and OSD/30's printed figures are the evidence the engine is checked against; they set no rounding for a read-back value."),
...
q(1,
 ...
 "The course explains: the forward direction is good to about 1.5e-7 in probability, which is why the course prints probabilities to six decimals. Table 5.1 prints probits to two decimals, the lees-chlorine loads differ by 0.139897, and OSD/30 prints lethal doses such as 960 and 2380."),
...
q(2,
 ...
 "The course gives the reason quoted and the remedy: a two phase model outside this engine for a flashing liquid. The engine has no flash factor, never relabels a liquid as a gas, and the course names no offshore frequency argument. None of the items the course lists as out of the engine is taught as computed."),

q(0,
 "An instantaneous release needs a puff. What does the course record about it?",
 ...
 "The course lists the instantaneous puff as not in the engine because its sigmas were not in a source read. The plume is continuous and has no duration input, a single route quantity is one the engine does compute, and the multi-energy method is itself absent."),
...
q(3,
 ...
 "The course explains: only the rural Briggs set is here, because a sourced urban sigma_y was not available. There is no urban flag, 0.00015 is the misprint ALOHA corrects in class D, and class F is the most stable rural class, a weather state that has nothing to do with buildings."),
...
q(1,
 ...
 ["A jet fire is a pool fire with a tilt of zero, which `poolFireSolidFlame` already covers when it is given a zero wind at the hole",
  "A jet fire burns too briefly to harm the people exposed, so the engine leaves it out as negligible",
  "Jet fires fall outside the Yellow Book"],
 "The course gives the reason quoted, and the analyst uses a jet fire model elsewhere. A pool fire is not a jet, nothing in the validation record calls a jet fire negligible, and the course says nothing about the Yellow Book lacking the subject."),
...
q(3,
 ...
 "The course records the reason quoted. The conference column is evidence for Kinney and Graham and was never run against Kingery-Bulmash, the engine has no ground burst option, and the only thing borrowed from the Kingery-Bulmash compilation is the judged span of Z from 0.05 to 40."),

q(2,
 "Lung, eardrum and structural probits do not appear in `OVERPRESSURE_PROBITS`. What does the course say?",
 ...
 "The course names lung, eardrum and structural probits as out of the engine because their source was not read. The hsc preset is a single fatality probit from OSD/30 Equation 4a, the course lists no such single route quantity, and the fit returns a peak side-on overpressure with no impulse."),
...
q(0,
 ...
 "The course explains: the point source model and the setbacks it implies live in the facilities engines, this engine does not re-expose them, and its validation record carries a test that asserts it exports none. The solid flame functions are this course's own model and return no setback. Individual risk and the F-N curve are a different seam."),
...
q(2,
 ...
 "The course's row for the Lees toxic coefficients reads 'the OSD/30 columns' and 'yes'. Bagster, Burgess and Thomas in still air each read 'nothing: the transcription alone' and 'no'. A single route quantity is taught and never carries a graded answer."),
...
q(0,
 ...
 "The course records one published worked case, carbon monoxide, and says every other substance rests on the transcription, so the row reads 'no'; every graded toxic probit uses a Lees preset. The Purple Book presets take mg/m3, the OSD/30 columns test the Lees presets, and each preset names its source."),
