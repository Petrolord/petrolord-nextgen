...
# H4 Professional m05: transmissivity and the heat flux.
# Digest section 19 (Bagster and its band), section 20 (the heat flux as a
# product, ERHA end to end, the distance search and its states) and section
# 22 (the seams: point source heat radiation, frequencies and emissions, all
# named as other courses' subjects and never computed).

q(1,
 "With a water vapour partial pressure of 1500 Pa and a path of 30 m from the flame surface, what transmissivity does the Bagster fit return?",
 "0.770127",
 ["0.850165","0.723552","0.71474"],
 "tau = 2.02 (pw x)^-0.09 at pw x = 45000.000000 N/m gives 0.770127. 0.850165 is the 10 m path and 0.723552 the 60 m path; a longer path lets less through. 0.71474 is the transmissivity the Yellow Book pool fire reads from Hottel charts, a stated value from another source.")

q(3,
 "At the same 1500 Pa, a caller asks Bagster for the transmissivity over a path of only 5 m. What does the engine return?",
 "A refusal naming pathLengthM, because pw x of 7500.000000 N/m lies below the fit's band",
...
q(0,
 ...
 "The flame surface and the target",
...
q(2,
 ...
 ["Each one uses Bagster at the stated humidity, and is graded at a looser tolerance",
  ...
  "Each one uses Bagster, cross-checked against the Hottel chart values"],
 "Nothing but the transcription catches a Bagster mistake, so a single route quantity is taught and never carries a graded answer, and every capstone that needs a transmissivity STATES it. No tolerance is loosened for Bagster, the air is never simply left out, and the Hottel chart value in the Yellow Book example is one stated input, never a check on the fit.")

q(2,
 "ERHA runs end to end: the heptane burning flux, Thomas with a 4 m/s wind, the tilt at nu 0.000015 m2/s, the soot surface emissive power and a stated transmissivity of 0.8. What heat flux in W/m2 reaches a target 40 m from the pool centre?",
 "10412.965226",
 ["3699.568657","4582.518673","12500"],
 "The chain gives SEP 52025.691247 W/m2, Fmax 0.250188055557 and q = SEP x F x tau = 10412.965226 W/m2 at 40 m. 3699.568657 is the 60 m row. 4582.518673 is the Yellow Book benzene heat flux at 100 m from its centre. 12500 is a stated target heat flux for the distance search, never a computed one.")

q(0,
 "On the same ERHA chain, which Fmax does the engine use for a target 100 m from the pool centre?",
 "0.022463264091",
 ["0.008332606429","0.088888022627","0.250188055557"],
 "The ERHA table prints Fmax 0.022463264091 at 100 m, giving 934.933474 W/m2. 0.008332606429 is the 150 m row, 0.088888022627 the 60 m row and 0.250188055557 the 40 m row.")

q(3,
 "Why does the ERHA chain print a refusal field instead of a heat flux for a target 25 m from the pool centre?",
 "The tilted flame reaches over the target, so the view factor call refuses with tiltDeg",
...
q(1,
 ...
 "36.451220",
 ["26.451220","53.796232","43.796232"],
 "The search returns REACHED at 36.451220 m from the centre. 26.451220 m is the same point measured from the pool edge, ten metres less by the radius. 53.796232 m and 43.796232 m are the centre and edge distances for the 5000 W/m2 target.")

q(1,
 "Asked for the distance to 35000 W/m2 on that chain, what state does the search return?",
 "NOT_REACHED, with the largest heat flux it found",
...
q(0,
 ...
 "A root search would walk out of the Bagster band, so the engine requires a transmissivity stated in (0, 1]",
...
q(2,
 ...
 "D/2 with no elongation of the base in the wind, which matches the Yellow Book's own worked step",
...
q(3,
 ...
 "Fmax, the most exposed target orientation",
...
q(0,
 ...
 "In the Facilities courses on relief and flare systems and on layout; this course grades only the solid flame",
...
q(3,
 ...
 "They belong to the later quantitative risk course, and this course never computes or grades one",
...
q(1,
 ...
 "They are other courses' subjects: this course models an accidental release and nothing routine",
