...
q(2,
 ...
 "The course says what IS published and reproduced is the criterion side: the Bevi points on the line and the printed R2P2 point. The CCPS and HSE worked examples were not available. The area identity is a self-consistency check with no published source behind it, and the Purple Book prints no numeric example for the Fd fractions."),
...
q(2,
 ...
 "The course records the planted factor as the reason it never grades a number that passes through the Fd rules: a wrong value shared by the engine and its oracle agrees with itself. It says nothing is wrong with today's factor. The fractions are for societal risk and are not a vulnerability factor, and the oracle shares the same transcription, so it is no escape."),
...
q(1,
 ...
 "The course says the engine applies the cap without refusing and states the time used: a toxic exposure of 60 minutes is computed at 30, and the toxic case belongs to the consequence side of the seam. No refusal is raised, the cap is applied by the engine itself, and nothing rescales a stated probability of death."),

q(2,
 "Purple Book Appendix 6.B prints Pd = 0.381, where the whole chain, as the golden records it, gives 0.380294556093. What explains the gap, and what should a reproduction do?",
 ...
 "The course says the whole chain gives 0.380294556093, which rounds to 0.380, and the printed 0.381 follows only from the rounded ECW of 86.2; both routes reproduce the printed contribution of 7e-9 to two significant figures. The gap describes the SOURCE. It is the source that uses R = 361 m for a point at 360.555 m, and the gap is fully explained."),

q(3,
 "Which of these does the engine decline to do, by its own design?",
 ...
 "The course lists re-grading nothing the consequence or facilities courses own among what the engine does not do: it exports none of their functions. A zero Pd simply contributes zero, the curve is compared with the R2P2 point at N = 50, and a VPF is refused only when it is missing or not above 0."),
...
q(0,
 ...
 "No source the engine read gives a vulnerability factor for individual risk; the indoor fractions are for societal risk, so the default is 1 and the result returns the factor used beside each place's contribution.",
