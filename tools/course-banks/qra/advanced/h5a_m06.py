...
q(1,
 ...
 "The course says an individual risk in the TOLERABLE region is not acceptable until the demonstration is made; the engine sets `alarpDemonstrationRequired` there. TOLERABLE is no synonym for acceptable, the public limit is not the worker test, and ALARP asks for reduction as low as reasonably practicable, which need not reach the broadly acceptable region."),
...
q(3,
 ...
 "The course's note lists the measures adopted and the ones rejected as grossly disproportionate, and says a measure rejected on cost is still weighed against good practice, which the engine does not know. Dropping it hides the decision. The DF is a stated choice to be justified, never tuned to pass a measure, and the largest reasonable cost is a limit, which prescribes no spend."),
...
q(3,
 ...
 "The course shows EXCEEDS, worst ratio 18.000000 at N = 300 against the Dutch line, and BELOW at 0.001000 against the R2P2 point, which has no slope. The Dutch line is a historical published comparison because Bevi was repealed. 0.014910 is the worst ratio against the analyst's own slope minus one line, and the TOUCHES case is a different curve built to sit on the line."),
...
q(1,
 ...
 "The course says R2P2's limits are guidelines, to be applied with judgement, and the note carries the judgement: the region, the measures weighed and what the engine did not know. A band word alone settles nothing. The risk matrix belongs to the risk and change course and this course never scores one, and rounding a figure toward a band edge is the opposite of computing from the inputs."),
...
q(2,
 ...
 "The course says the analyst states the VPF and its year and justifies it: the engine has no default VPF, and current HSE figures were not found on a live page, so neither exported value is a recommendation. A lower VPF lowers the benefit, which makes a measure look worse value, so it is the less cautious choice for the people protected."),
...
q(0,
 ...
 "The course lists no published F-N worked example among what the engine does not know, and the analyst never cites an F-N figure as published-verified. The other three describe gaps that do not exist: N need not be whole, a threshold value is banded by the engine's own lower band rule, and the engine takes a benefit growth rate as an input."),
...
q(1,
 ...
 "The course says a measure rejected on cost is still weighed against good practice, which the engine does not know. The engine bands the individual risk against the stated thresholds, returns the ratio and the verdict, and always forms the ICAF over undiscounted fatalities prevented, which is its declared choice."),
