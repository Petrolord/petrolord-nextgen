...
q(0,
 ...
 "The course says R2P2 defers extrapolation to other N to a reference the engine did not read, so the engine gives the point NO SLOPE and compares the curve at N = 50 only. R2P2 prints no slope at all, flat or otherwise. The engine has no aversion weighting of its own; aversion enters only through the slope of a line a caller or preset gives. It borrows nothing from the Dutch line, which is a separate preset from another source.")
...
q(0,
 ...
 "The course defines EXCEEDS as at least one corner strictly above. A single corner is enough, so every corner above is not needed. A corner on the line within the snap is AT_LINE and leaves the curve at TOUCHES, so a ratio of exactly one does not count. The state compares frequencies at corners and never uses the expected fatalities.")
...
q(0,
 ...
 "The course says a corner within 1e-9 relative of the line is AT_LINE, the same width as BOUNDARY_SNAP. An absolute 1e-9 per year would swallow the whole of a line near 1e-9 at N = 1000. 1e-6 would call corners on the line that are measurably off it. One per cent is no figure the engine uses.")
...
q(3,
 ...
 "The course defines BELOW as every checked corner strictly below the line or point. One corner above makes the curve EXCEED, and one on the line makes it TOUCH. The corner with the largest N is only one of the checks. N = 1 lies outside the Dutch line's range, and the state reads every corner in range.")
...
q(2,
 ...
 "The course's state words table gives AT_LINE as the state of one corner on the line and TOUCHES as the overall state when no corner is above and at least one is on the line. Swapping them misreads the table. ON_LINE is no state word. A curve with no corner above cannot EXCEED, since EXCEEDS needs a corner strictly above.")
