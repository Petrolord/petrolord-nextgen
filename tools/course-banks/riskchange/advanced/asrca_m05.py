...
q(2, "The application counts in the lessons summary carry a stated limit. Which one does the course list among the held items?",
 ...
 "The lesson counts beside them, lessonsApplied and lessonsUnapplied, cover visible lessons only. The application counts do not narrow the same way, and the course lists that as a held item taught as a limit and graded nowhere.")

q(3, "A peer review summary is handed a comment whose review is missing from the list of reviews. What does the course hold it does?",
 ...
 "The summary keeps a finished review's comments out of the open and blocking counts only when it can see the review. A comment it cannot place is counted as open work, and the course lists that as a held item.")
...
q(2, "The course measures peerReview_cases.json by reading it. What does it record for cases, cases carrying a repaired marker, and functions exercised?",
 "184, 38 and 17.",
 ["345, 19 and 26.",
  "184, 17 and 38.",
  "291, 69 and 15."],
...
q(2, "A golden case and the oracle bridge both check the engine. On which records does each check it?",
 ...
 "The bridge, oracle_bridge.py, replays every engine answer in the course, IKANG and ONNE included. A defect that shows on only one record slips past a check that never meets it, which is why both exist.")
...
q(3, "A change has finished, and one of its actions still reads \"Open\". What do openActions and overdueActions do with that action today?",
 ...
 "On 2026-10-01 the summary skips AC-06 and AC-07, whose changes, ES-06 in the Closed stage and ES-09 \"Cancelled\", are finished and locked, and a probe on the \"Rejected\" ES-11 counts its open action nowhere. AC-09, whose change is not in the register, still counts, and the course holds that as a limit.")
...
q(0, "Today, how does the peer review summary count an unresolved \"Critical\" comment that sits on a \"Cancelled\" review?",
 ...
 "A \"Cancelled\" review is locked, and nobody can move a comment on it, so a work count that included one could never fall. The summary lists such a comment in every column that describes the log and leaves it out of the counts of work still owed. The course's case is C-10 on IK-04.")
