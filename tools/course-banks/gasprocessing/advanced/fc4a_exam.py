...
q(2, "How was the water overhead of 1100.000000000000 Btu a lb measured out of the engine rather than read from its export?",
 "By running the reboiler at a circulation ratio of one gallon per pound with no reflux, where the vaporization term is the overhead alone and nothing else.",
 ["By dividing the overhead term at a reflux ratio of 0.250000 by the water a gallon carries, which leaves the constant alone.",
  "By running the BTEX mole balance at a unit absorbed fraction, where the still overhead in pounds a day is the constant.",
  "By comparing the reboiler duty against the golden's, since the golden builds the same overhead from a steam table value."],
 "To measure a constant, ask the engine a question whose answer is that constant and nothing else. At one gallon per pound a gallon carries one pound of water, and with no reflux the overhead is charged once, so the term reads the Btu a lb directly. A reflux ratio of 0.250000 adds a quarter on top, and the BTEX still overhead is aromatics.")
