...
q(0, "Only four range guards in this module are enforced and may be taught as enforced. Which set?",
...
q(1, "A reader wants to know whether this engine actually uses the value it exports for a constant. What answers that, and what does not?",
 ...
 ["Reading the export answers it, because an exported name is the value the functions close over and there is no second copy anywhere in a module of 775 lines.",
  ...
  "Neither answers it, which is why the digest prints both columns."],
...
q(3, "Fifteen of the thirty pinned constants have no export at all. What is the practical consequence?",
 "For those fifteen the measurement is the only column there is, which is the case the technique was built for.",
 ["For those fifteen the pin compares the golden against the digest generator instead, so the engine is out of the comparison and the pin is weaker by one copy.",
  "For those fifteen no pin is possible, so they are carried by the constant-free invariants and by the route independence the golden's own file describes.",
  "For those fifteen the digest reads the literals out of the source."],
 "The two reaction constants, the reaction fugacity exponent, the two fugacity coefficient constants, the mass-transfer coefficient and both of its exponents, the three scale constants, the pH slope, the Blasius pair and the laminar constant are all reached by measurement alone.")
q(0, "The route table gives four of its eight routes an empty cannot-check column, and the course grades four families of field. What connects the two lists?",
 "Both are arithmetic over quantities the caller supplies rather than a correlation with fitted constants in it, so the two lists are one property seen twice.",
 ["Both were chosen from the golden's seventeen whole cases, which is the block the digest compares route by route and the block the graded fields are taken from.",
  "Both are pinned in the digest against a literal in a third file, and a pinned quantity is one a route can check and a graded field is permitted to reach through.",
...
q(3, "The category bands were left where they are and the H2S screening threshold was left where it was. What reason covers both decisions?",
 ...
 ["Both numbers are pinned in the digest against a literal in a third file, and a pinned number cannot be moved without the gate separating the two columns.",
...
q(2, "The sequence that chooses a binding constraint never reads the category word. Why is that the right design?",
 ...
 ["The word is not available at the point the sequence runs, because the category is computed after the binding constraint in the order the screening door walks.",
  "The word is withheld on two of the six shipped streams, and a step that could not fire on every stream would leave those two screenings with no constraint at all.",
...
q(3, "A reader quotes a pressure in bar off the studio screen and a colleague quotes one from a direct engine call. Why might the two differ?",
 "The studio divides a psig pressure by 14.5038 and the engine's exact factor is 14.503773800722, so the two conversions separate in the sixth significant figure.",
 ...
 "The digest lists every one of the studio's conversions for exactly this reason: a learner reading a number off the app needs to know which layer produced it. Every graded condition in this course is stated in the engine's units.")
...
q(1, "Before the repair thirteen defects planted one at a time left the validation suite entirely green. What did the repair put in place of the routes that missed them?",
 ...
 "Section 20 lists the routes with what each one cannot check, section 3 carries 30 pins measured out of the behaviour, and sections 5, 6, 7 and 13 carry the invariants. Naming which is which is what makes the coverage readable.")
...
q(2, "The corrosion inhibitor warning trigger is 0.100000 percentage points of shortfall today and the pre-repair guard fired above 90 percent efficiency. What did the change of quantity buy?",
 ...
 ["A guard that fires more often, which is what makes the warning visible at the shipped defaults where the previous one had been silent about the availability.",
  "A guard with a pinned trigger, which is what lets the digest measure it.",
...
q(0, "The category bands are both pinned and held. What does each of those two words buy?",
 ...
 "Twelve golden category rows and a measurement by bisection establish that the edges are where the digest says they are. Neither establishes where the numbers came from, and `categoryHeld` comes back true beside the word.")
...
q(1, "A summary layer was added to this module. What did it add to the pin table and to the held list?",
 "Nothing to either. The held list still stands at 11 items and the pin table at 30 constants.",
 ...
 "The held list still carries 11 items and the pin table still carries 30 constants. The reporting margin was already a property of the controlling word before any summary existed, so it is not something the summary brought with it.")
q(2, "The shipped default screen carries a rate, a word, a partial pressure, an effective protection figure and a remaining life. Which of those can a capstone in this course grade?",
 ...
 ["The rate and the remaining life, because both are computed by the engine and both are reported to six decimals, which is the precision a graded field is checked at.",
  "The word and the remaining life, because the band edges are pinned and the life is a single division, so both are traceable to numbers the digest measures.",
...
q(0, "What do the pre-repair screen and the modern one have in common, and where do they part?",
 "Both print seven independent numbers. The modern one adds a summary naming which of its own limits governs, derived from what it had already computed.",
 ["Both print a binding constraint. The modern one derives it from the numbers rather than from the largest of them, which is what the pre-repair version ranked by.",
  "Both withhold a category in the sulphide regime. The modern one adds the withheld block, which the pre-repair version reported as an empty paragraph instead.",
  "Both refuse on a blank density. The modern one adds the message naming the box, which is what makes the refusal actionable rather than merely present."],
...
q(3, "This course grades no corrosion rate the correlation produced. Which single fact is the reason?",
 ...
 ["The golden is synthetic, so no rate in it can be compared against a published case and the tolerance it is checked at has no meaning attached to it.",
  ...
  "The studio's psig divisor is truncated, so a rate converted through the app differs from a rate converted through the engine's exact factor in the sixth figure."],
...
q(1, "Two fields on the same screening declare their own absence and one does not. Which is the one that does not?",
...
q(0, "A sentence lifted out of an engine source comment arrives with no frame around it. How many of this module's comment lines are written in a past tense about former behaviour?",
 "Five, in a module of 775 lines, and none of them carries a repair marker.",
 ["Eleven, one for each held item, and each one carries a repair marker naming the change that removed the behaviour it describes for a reader of the source.",
  "None, because the repair stripped every past-tense comment out of the module and moved the history into the digest section that carries the frame for it.",
  "Eight, matching the entries in the not-provided list, and each one records the behaviour that stood where the absence is now declared."],
 "Roughly one line in 155 of the module carries a past-tense sentence about former behaviour. Engine source comments are provenance, and so are the wave recon and findings reports and the repair record vendored beside the oracle.")
q(3, "One digest section describes behaviour this engine no longer has, and one describes a current absence that looks like history. How are the two told apart?",
 ...
 ["The history section is the only one that names the repair, and the withdrawal section is identified by the two flags it describes rather than by anything it says.",
  "The history section is the last in the file and the withdrawal section is the second, so their positions in the digest are what carry the frame for each of them.",
  "Neither is told apart in the digest. The frame lives in the module directory name in the course and in the lesson heading, which is where the gate looks for it."],
...
q(2, "A caller reads a screening and finds no severity region and no material guidance. What has the engine committed itself to?",
 ...
 "A held item is waiting on a source and a scope gap is outside what the module attempts. This is neither, which is why it appears on neither exported list and is reported through two flags of its own instead.")
