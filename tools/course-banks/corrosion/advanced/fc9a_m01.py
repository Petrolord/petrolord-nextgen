...
q(0, "The withdrawn sour-service severity region is not on `HELD_FOR_LITERATURE`, and `NOT_PROVIDED` names it marked withdrawn. Why is it not on the held list?",
 ...
 "Both fields are false today and will stay false, so the absence arrives as a field and a caller cannot read it as an unset property. The region function is not exported and neither standard name appears anywhere in the engine source.")
...
q(0, "A paired battery moved fifteen of seventeen constants in the engine and in the oracle together and left the suite green. What does that establish about a suite comparing only those two files?",
 ...
 "No tolerance saves a shared constant, because a tighter comparison makes it more invisible rather than less. Thirty constants are pinned instead, each measured out of the engine's behaviour and compared against a literal in the digest generator, which is a third file, with the golden's own block as a fourth.")

q(2, "A pinned constant agrees with the literal typed in the digest generator. What has that proved, and what does it leave open?",
 ...
 ["That the constant is the published value, since the generator's literals were transcribed out of the correlation's own papers.",
  "That the engine and the golden agree with each other, which is what a pin is for and what the generator records.",
 ...
 "A pin is a weaker instrument than a validation. Thirty constants are pinned that way, with the golden's own block as a fourth copy, and the digest says plainly that no source for any of them exists in this repository.")

q(3, "How does the digest reach the pH reference of 4.000000000000 and the friction branch switch at Reynolds 4000.000000?",
...
q(1, "The scale constant A measures 2399.999999999998 out of the engine against a literal of 2400.000000000000. Why does the digest print that rather than round it into agreement?",
 ...
 ["The engine genuinely holds 2399.999999999998, and the literal in the generator is the typing error that the pin table exists to find and to report on every rebuild of the digest.",
...
q(1, "Case 7 of the seventeen vendored cases differs from the golden rate at 8.901e-5 against a tolerance of 1e-3, while the other rate differences sit in the last bits of a double. How is that figure read?",
 "As a size, which is what makes the agreement a result. The digest prints the difference on every case rather than printing a pass.",
 ...
 "Agreement between two separate derivations is a result with a size and the digest prints the size. The combined rate on the same case agrees at 5.458e-16 against a tolerance of 1e-9, so the two columns are checked at two different tolerances on purpose.")
