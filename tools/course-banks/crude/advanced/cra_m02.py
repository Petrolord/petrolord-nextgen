...
q(1, "The Apapa AGO cargo of 6000 bbl costs 594720.9475 $. Which unit cost and binding pair does the engine print for it?",
 "99.1202 $/bbl, binding Cetane number and Density.",
 ["87.3377 $/bbl, binding Cetane number and Density.",
  "99.1202 $/bbl, binding Sulfur and Viscosity at 40 C.",
  "99.1202 $/bbl, binding Cetane number and Flash point."],
 "The AGO result prints Unit cost 99.1202 $/bbl and Binding: Cetane number and Density. 87.3377 $/bbl is the Apapa PMS cargo's unit cost, and sulfur, viscosity and flash point each carry giveaway on the AGO recipe.")
...
q(3, "A specification declares the mass basis. What are its two weights in the row sum((w_i - L d_i) v_i) <= 0?",
 ...
 "The basis table gives the mass basis w_i = SG x the property and d_i = SG. The index-on-mass basis is the one that puts the property's index in w_i.")
...
q(0, "At Apapa the template's Sulfur maximum is 50 ppm, and the components carry 4 ppm (Reformate), 110 ppm (FCC gasoline), 1 ppm (Isomerate) and 1 ppm (Butane). Which component carries a positive coefficient in the sulfur row?",
 "FCC gasoline, the one component whose sulfur sits above the 50 ppm limit.",
 ...
 "On the mass basis the coefficient is SG x the property less 50 x SG, so its sign is the sign of the component's sulfur against the limit. Only FCC gasoline's 110 ppm sits above 50.")

q(2, "The 9 psi RVP limit enters its row as the index 15.5885. Reformate carries 4.2799, FCC gasoline 9.7834, Isomerate 24.4477 and Butane 142.3287. Which components carry a negative coefficient in the RVP row?",
 "Reformate and FCC gasoline, whose indices sit below 15.5885.",
 ["Isomerate and Butane, whose indices sit above the 15.5885 limit.",
  "Butane alone, the component taken up to its availability.",
  "None of them, since an index row has no negative weights."],
 "On the index basis w_i is the component's index and d_i is 1, so a coefficient is the component's index less 15.5885, rvpIndex of the 9 psi limit. Reformate at 4.2799 and FCC gasoline at 9.7834 sit below it.")

q(1, "RVP_INDEX_EXPONENT is 1.25. What part does it play in rvpIndex?",
 "It is n in RVPI = RVP^n, the index that RVP blends through.",
 ["It is the RVP limit's index at 9 psi, the figure the row takes.",
  "It is the slope of the index at the 9 psi limit.",
  "It is set per template, fixed by the 50 ppm gasoline template."],
 "RVP blends through an index: RVPI = RVP^n, blended on volume and inverted, and RVP_INDEX_EXPONENT is 1.25. The index of the 9 psi limit is 15.5885, and the slope of the index there is 2.1651.")
...
q(3, "propertyOfBlend recomputes each Apapa property from the finished recipe, and every difference against optimiseBlend is 0.0000, including Sulfur at 50.0000 and RVP at 9.0000. What does that agreement show?",
 "It is a check on the recipe, since a route apart from the LP rows gives the same figure.",
 ...
 "The two-route table prints a difference of 0.0000 on every row. The second route recomputes each property by its own rule and does not read the LP rows.")
...
q(0, "The AGO template treats cetane and flash point linearly on volume. How does the template itself describe that treatment?",
 "As a screening approximation, for both cetane and flash point.",
 ["As the exact rule, which is why neither has an index.",
  "As a mass basis in effect, since cetane and flash point follow the specific gravity.",
  "As a held limit, so no recipe may bind on cetane or flash point."],
 "The template notes that treating cetane and flash point linearly on volume is a screening approximation. The AGO recipe binds on Cetane number and Density.")
...
q(0, "The Automotive gasoil, 50 ppm sulfur template carries a Density specification. What range does it set, and on which basis does density blend there?",
 "0.82 to 0.845 kg/l, on volume.",
 ["0.72 to 0.775 kg/l, on volume.",
  "0.82 to 0.845 kg/l, on mass.",
  "No minimum and 0.991 kg/l, on volume."],
 "The gasoil template carries Density on volume from 0.82 to 0.845 kg/l. 0.72 to 0.775 kg/l is the range of both gasoline templates, and 0.991 kg/l with no minimum is the fuel oil template's.")
