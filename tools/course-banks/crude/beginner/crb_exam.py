...
# crude Associate tier exam, 42 questions over the whole tier (digest.txt
# SECTIONS 1 to 12). It is not the module banks concatenated: many questions
...
q(3, "For three crudes at 50, 30 and 20, the course prints the figure 29.6100 in two places. What does it stand for in each?",
 ...
 ["The engine's API for the blend typed by volume, and the mass-weighted mean of the API numbers for the same blend.",
  ...
  "The volume-weighted mean of the API numbers, printed a second time as a check on the engine."],
...
q(1, "Egbema Medium arrives with neither API nor specific gravity, and no crude has SARA. Compare the blend engine and the stand-alone stability screen.",
 ...
 ["Both refuse, in the same words, naming the crude that has no gravity.",
  "blendCrudes names the crude missing and blends the rest; the screen returns api-contrast.",
...
q(2, "The engine puts a half-and-half mix of Egbema Medium with Asarama Heavy at 91.4520 cSt. Which printed gap measures how far a plain linear cSt average overshoots that?",
 "232.9041 cSt.",
 ["4.1448 cSt, the gap between the two index bases.",
  "87.3072 cSt.",
  "330.3100 cSt, the gap for the heavy and condensate pair."],
 "The straight average on mass is 324.3561, and linear average minus the engine prints 232.9041. 4.1448 is mass basis minus volume basis for the index, 87.3072 is the index on volume, and 330.3100 belongs to Asarama Heavy with Ubie Condensate."),

q(1, "Ubie Condensate is the thinnest crude in the library, at 1.1 cSt. Where does it sit against the Refutas index domain?",
 "Inside it, with an index of 4.5307; the edge is at 0.2000 cSt.",
 ["Outside it, since the index is undefined below 1 cSt and a condensate is thinner.",
  "At the edge itself, where the index reads -122.8888.",
  "Inside it, with an index of 3.2518 read at 1 cSt."],
 "Every crude in the library sits inside the domain. The index is undefined only where ln(nu + 0.8) is zero or less, and the lowest viscosity it reaches is 0.2000. 3.2518 is the index of 1 cSt."),

q(2, "At 60 F, one library curve reads 0.0000 and another reads unknown: Obigbo Light and the Ebocha partial assay. What decides which answer each gets?",
 "Only Obigbo Light's curve begins at 0 percent, which says nothing has distilled below 85 F.",
 ["Obigbo Light is the lighter crude, so less of it has distilled by 60 F.",
  "The engine needs seven measured points to read below a curve's first one.",
  "The engine clamps a full curve to its first reading and leaves a partial one blank, as unknown."],
 "A curve answers outside its range only where it says so itself. Ebocha's curve starts at 4 at 110, so below 110 F the value is unknown."),

q(0, "Obigbo Light is 95 percent distilled at which temperature?",
 "1220.0000 F, between 90 at 1060 and 100 at 1380.",
 ["unknown, since 95 percent lies past its 90 percent point.",
  "1380.0000 F, the curve's end point.",
  "1060 F, the last point below 95 percent, held flat."],
 "Obigbo Light's curve has a point at 100 percent, so 95 percent lies between measured points and is read on the straight line: 1220.0000 F. Ebocha, which stops at 88, returns unknown at 95 percent."),

q(1, "Which library crude brings the most Diesel / Gasoil, the 500 to 650 F cut, per barrel?",
 "Egbema Medium, at 17.8571.",
 ["Obigbo Light, at 16.5914, the light crude of the library.",
  "Ubie Condensate, at 14.8897.",
  "Asarama Heavy, at 35.5556, the heaviest of the four."],
 "The Diesel / Gasoil column, 500 to 650 F, prints 16.5914, 17.8571, 14.0097 and 14.8897 for the four full crudes, and Ebocha's is 17.2861. 35.5556 is Asarama Heavy's Vacuum gasoil, 650 to 1000 F."),

q(3, "Ubie Condensate's Vacuum residue yield is 0.0000. Is that zero a blank?",
 "No: its curve ends 100 at 760, so everything has distilled by 1000 F.",
 ["Yes: its curve stops short of 1000 F, so the cut is unknown and prints zero.",
  "Yes: a condensate carries no residue analysis, so the cut defaults to zero.",
...
q(0, "What are the two thresholds of the gravity-contrast rule, as bisection on the engine finds them?",
 ...
 ["0.7 and 0.9, the two edges held in CII_BANDS and read against the index.",
  "A contrast of 15.0100 and a lighter crude of 35.01 API.",
  "A contrast of 37.4000 and a contrast of 10.9000."],
 "The two thresholds are not exported, so the engine is probed. A flag needs a contrast above 15.0000 and a lighter crude above 35.0000, both at once. 0.7 and 0.9 are the CII band edges."),

q(2, "Asarama Heavy with Egbema Medium at 70 and 30 has a CII of 0.7161. What does the engine's message tell the terminal to do?",
 "Spot test to ASTM D7112 or D7157 before commingling.",
 ["Do not commingle without a lab test.",
  "Supply SARA for a colloidal instability index.",
...
q(0, "The export blend's CII is 0.9163. Which band edge decides its answer?",
 ...
 ["0.7: past it the answer is uncertain.",
  "0.9: the answer stays uncertain until the index passes it.",
  "No edge: the gravity screen decides for the export blend."],
...
q(1, "In the three-crude blend at 50, 30 and 20 by volume, sulfur on mass is 0.6138 wt%. What is 0.5840?",
 ...
 ["The engine's sulfur for the blend typed by volume, once the mass fractions from resolveFractions are applied.",
  "Sulfur on volume for the export blend of Obigbo Light and Egbema Medium, printed beside this one for comparison.",
...
q(3, "Where does the 0.8408 that Obigbo Light carries in the mass-share table come from?",
 ...
 ["A density measured in a laboratory and typed in.",
  "The blend's 0.8611 scaled by the crude's share.",
  "apiFromSg, run on the crude's blend share."],
 "Specific gravity is the one library column the engine computes, from API through sgFromApi. The mass-share conversion then uses it: volume share times specific gravity, over the sum of those products."),

q(1, "Egbema Medium with Asarama Heavy, half and half by volume: which printed figure is the viscosity with the index averaged on volume fractions?",
 "87.3072 cSt.",
 ["91.4520 cSt, the figure the engine reports.",
  "4.1448 cSt, the gap between the two bases.",
  "324.3561 cSt, a straight cSt average on mass."],
 "This pair prints 91.4520 with the index on mass, which is the engine's, and 87.3072 on volume. The course holds the choice of basis open, and the two are read side by side."),

q(0, "What does viscosityBlendIndex return just inside the domain edge, at 0.2001 cSt?",
 "-122.8888.",
...
q(1, "Why do Ebocha's Kerosene / Jet and Diesel / Gasoil cuts have yields when its other four default cuts do not?",
 ...
 ["They are the cuts the partial assay was run for, and the rest were skipped.",
  "The engine extends the curve for the middle cuts and for no others.",
  "Their yields are the 33.6742 total, shared between the two by span."],
 "A cut has a yield only when the curve can answer at both bounds. Ebocha's other four cuts reach below 110 F or above 920 F, where its curve is unknown. The two known cuts are 16.3881 and 17.2861."),

q(3, "Ebocha's inner cut set totals 84.0000 with no cut unknown; its default set totals 33.6742 with four unknown. Which closing condition does each fail?",
 "The inner set fails on its total alone; the default set fails on both.",
 ["The inner set fails on an unknown cut; the default set fails on its total alone.",
  "Both fail on the total alone, since unknown cuts do not count against closing.",
  "The inner set closes; only the default set fails."],
 "A set closes only when every cut has a yield and the total is within tolerance of 100 percent. The inner set has every yield and misses the total; the default set misses both."),

q(2, "Ebocha's curve is asked for the temperature where 2 percent has come over. What comes back?",
 "unknown.",
...
q(0, "Which basis does the export blend's screen carry when its SARA is removed?",
 ...
 ["cii, band unstable, at 0.9163.",
  "none, with the contrast not formed.",
  "api-contrast, at a contrast of 37.4000 that flags."],
 "With no SARA and both gravities present, the screen falls back to the API contrast, 10.9000, and gives no verdict. 37.4000 is Asarama Heavy with Ubie Condensate, which flags."),

q(0, "Which of these is the export blend's nickel as the engine reports it?",
 "6.5980 ppm, on mass.",
 ["6.4550, on volume.",
  "0.1430 ppm, mass minus volume.",
...
q(3, "Where does the 0.8000 inside the Refutas index come from, in the engine's own reading?",
 ...
 ["viscosityBlendIndex(e - 0.8), which gives the constant B.",
  "The lowest viscosity the index reaches, read from the inverse.",
  "A minus B, read from the two constants the engine gives."],
 "viscosityFromBlendIndex of a very negative index is 0.2000, the lowest viscosity the index reaches, and 1 minus that gives the offset 0.8000. viscosityBlendIndex(e - 0.8) gives B, 10.9750."),

q(1, "How much of Obigbo Light has distilled by 600 F, as the engine reads it?",
 "55.3608, linear between 50 at 548 and 70 at 742.",
 ["50.0000, held at the point below.",
  "unknown, since 600 F is not a measured point.",
  "20.2703, the reading between 10 and 30 percent."],
 "Between measured points the engine reads a straight line. 20.2703 is the reading at 300 F, between 205 and 390 F."),

q(3, "Which probe raises the gravity rule's flag?",
 "40 and 24.99 API, a contrast of 15.0100.",
 ["40 and 25 API, a contrast of 15.0000.",
  "35 and 19 API, a contrast of 16.0000.",
...
q(3, "Asarama Heavy's curve reads 70 at 1000. What is its Vacuum residue yield on the default cuts?",
 "30.0000.",
 ["35.5556, its Vacuum gasoil.",
  "unknown, with no upper bound.",
  "70 percent, the curve at 1000 F."],
 "The residue cut has no upper bound, so it takes everything not yet distilled at 1000 F: 30.0000. 35.5556 is Asarama Heavy's Vacuum gasoil, 650 to 1000 F."),

emit(Q, '/root/wt-md-crude-nextgen/tools/course-banks/crude/beginner/crb_exam.json', label='crb_exam', expect_n=42)
finish()
