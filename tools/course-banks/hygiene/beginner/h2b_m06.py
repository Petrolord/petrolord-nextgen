...
q(3, "The capstone TWAs are graded on which coefficient, and why?",
 ...
 "The TWAs are graded on the coefficient each source prints: 16.61 on OSHA, as the mandatory Appendix A text writes it, and 10.0 on NIOSH. A TWA worked with the exact coefficient will disagree, and the course measures how far: eight hours at a constant 100 dBA on the NIOSH preset gives 100.051500 dBA where the exact coefficient gives 100.000000. That is judgement J1, the same rule the engine's presets follow.")
...
q(0, "Which of these is never graded in this course, in any tier?",
 ...
 "The course lists any verdict word, exceeds, passes or at or above an action value, among the things never graded, because a verdict means nothing without its criterion and the same record gives different verdicts on different criteria. The other three name quantities the course counts among the eighteen graded fields of this course: noise doses and TWAs, LEX values, a protector estimate, chemical averages and indices, reduction-factor arithmetic and two one-hour averages.")
...
q(2, "Four subjects sit beside this course and are owned elsewhere. What does this course do with them?",
 ...
 "The course says to cite and to stop. A radiant flux in kW/m2, an emission from a glycol unit, a rate per hours worked and a matrix of likelihood and consequence are each a different quantity from a noise dose, measured a different way, so this course names the owner beside the figure and computes nothing. No tier of it teaches them, and the engine offers no conversion between them.")

q(1, "Rule 1 of the course's naming rules keeps one word from standing on its own here. Which word, and who else claims it?",
 ...
 "The course's naming rule 1: this course writes \"noise dose\" on first use in every lesson and every bank question and never the shorter word on its own in a prompt, an option or a heading, because that word already belongs to inhibitor and methanol work in the Flow Assurance and Gas Processing courses. The reading, the acronym and the decibel exchange rate are rules 3, 4 and the two misreadings that close the rules, each a different collision.")
...
q(0, "A question asks for an octave-band protector estimate or an ISO 9612 uncertainty budget. What is the honest answer from this engine?",
 ...
 "The course lists what the engine does not provide and says so by having no door: spectral (octave-band) protector methods, ISO 9612 uncertainty budgets, ISO 7243 adjustments and any licensed limit table. `hearingProtectorEstimate` takes a named method from a fixed list, and none of them is spectral.")
...
q(3, "A reading taken on the C-weighting is typed into a report. How should it be written?",
 ...
 "The course's unit rule: levels are in dBA unless a row says dBC, and a level read in dBC is a different weighting that must never be written as dBA. Adding a fixed step to turn one weighting into the other is no conversion this tier teaches, and a noise dose is built from A-weighted levels.")
