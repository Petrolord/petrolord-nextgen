...
q(1, "A download runs two shifts together and its periods total 25 hours. Which field does the engine name, and what does it report?",
 "`periods`, the record as a whole, and it reports no noise dose at all.",
 ["`periods[0].durationH`, the first period, since that is where the total is checked.",
  "`periods`, and it still reports the noise dose of all 25 hours beside a warning.",
  "No field at all: it reports the first 24 hours and warns that the rest was trimmed."],
 "Judgement J4 refuses noise, LEX and chemical periods totalling over 24 hours, and the refused field is `periods`, the record taken as a whole. A refusal carries `error` and `field` in place of a result, so no noise dose is reported and no warning stands in for it. `periods[0].durationH` is the field for a negative first duration, which names one period rather than the record.")
