...
q(0, "Why does the engine return a refusal for bad input instead of the nearest number it could compute?",
 "A refusal names the field to go back to, while a wrong number would carry no signal into a report.",
 ["A refusal is a verdict that the record exceeds its limit noise dose, so no number needs to be reported.",
  "Refusals are logged and resent to the dosimeter, which reads the period again automatically.",
  "A refusal means the criterion has no reference duration at that level, so the answer is zero."],
 "Every function returns a finite result or an object carrying `error` and `field`, the name of the input it refused, so the refusal is information about the input. A number built from a bad row would travel into a report unnoticed. A refusal is no verdict on the worker or the workplace, and the engine does nothing to the instrument.")
