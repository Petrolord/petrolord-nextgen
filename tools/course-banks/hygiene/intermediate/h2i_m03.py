...
q(0, "A C-weighted level of 103.200000 dBC is worn under the same NRR 27.000000 dB protector. How does Appendix B treat it?",
 ...
 "The engine's Appendix B rule is C-weighted minus the NRR, so on C-weighted data the whole label is credited: 103.200000 less 27.000000 is 76.200000 dBA. Taking the 7 off is the A-weighted rule applied to the wrong weighting. The refusal of C-weighted data belongs to the field derating, and halving the label is neither method.")

q(2, "On the teaching case the same NRR 27.000000 dB protector gives an attenuation of 20.000000 dB against the A-weighted level and 27.000000 dB against the C-weighted one. What accounts for the difference of 7.000000 dB?",
 "Appendix B credits (NRR - 7) on A-weighted data and the whole NRR on C-weighted data.",
...
q(3, "Under the OSHA dual-protection rule, what does the engine do with the two protectors' labels?",
 "It works from the higher of the two NRRs and never from both.",
 ["It adds the two labels together and credits the sum against the level.",
  "It averages the two labels and credits the mean against the level.",
  "It credits each label in turn and returns one estimate for each."],
 "The engine's dual-protection method reads the higher NRR, subtracts 7 if the level is A-weighted and adds 5, so only one of the two labels enters the arithmetic. Nothing adds or averages the labels, and the door returns one estimate for the method it is given. The 5 dB this method adds is ORACLE ONLY and is never graded.")
