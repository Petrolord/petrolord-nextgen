...
q(0, "What is the evidence class of the weekly LEX on a week of four or six days?",
 ...
 "Figure 26 reproduces the energy averaging the week shares with the day, and it says nothing about the divisor. The evidence table carries `lexWeeklyDbA` as PUBLISHED, REPRODUCED on the strength of one published case, five equal days at 85 dBA, and its note adds that the divisor of 5 is the statutory formula with no printed case of other than five days known, so on such a week the result is ORACLE ONLY. Transcription only is the class of constants two files copied from one page, and arithmetic by definition belongs to averages whose only constant is their own window.")
