...
q(2, "Why does the course give the teaching crew no single combined figure across noise, chemical and heat stress?",
 ...
 ["The engine has no door that combines hazards, although the three figures are ratios that could be added by hand",
 ...
 "The crew's record is three readings, each against its own line, each with its own evidence, and a report lists them side by side. The three are not three ratios: the heat stress row is a one-hour WBGT in degrees C, set against an equation in degrees C, while the noise row is a percentage of an allowance and the chemical row a dimensionless index. The risk matrix ranks hazards and never adds them, and the noise and chemical rows are already in different units against different criteria.")
