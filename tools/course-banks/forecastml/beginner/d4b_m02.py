...
q(1, "A `beta` is passed with the method 'ses'. What happens?",
 ...
 ["It is ignored, since ses has no trend, and the fit comes back as usual.",
  "Refused: \"phi applies to 'damped' only: 'ses' has no trend to damp\".",
  "Accepted, and the fit is switched to holt with that beta, noted in the basis."],
 "Simple smoothing has no trend, so the engine refuses anything that would set one, naming `beta` with the message in the key. The message about phi is the refusal for a phi passed to ses, which names `phi`. The engine does not silently ignore a parameter or change the method the caller asked for."),
