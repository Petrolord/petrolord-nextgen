...
q(1, "What do simple smoothing, Holt's linear trend and the damped trend have in common in this engine?",
 "All three are additive: any trend adds to the level, and an error to the forecast.",
...
q(0, "When does simple smoothing's flat forecast earn its place on a well?",
 ...
 ["On any steady decline, where the naive forecast follows the fall best of all the methods offered.",
  "On a short series only, since the engine refuses holt and damped on any series under 12 months.",
  "Whenever its fitted alpha lands on the bound 1, since that proves the level is the right model."],
 "A flat forecast says the method sees no direction, which is right on a plateau or on noise about a steady rate and wrong on a clear decline, where every step inherits the error. Holt fits on as few as 3 months, so no rule of twelve exists. alpha on its bound 1 says the fit wants more weight on the newest month than the box allows, which on a decline is a sign the series asks for a trend."),
...
q(3, "Fitted on EKENE-P1, holt reaches SSE 23041.767516 and damped SSE 22288.217610. What happens to their order once each SSE becomes an MSE?",
 "It holds: both divide by 46 scored errors, so damped's MSE is the lower one too.",
 ["It reverses, since damped carries one parameter more and its MSE is penalised for it.",
  "No order survives, because holt divides by 47 errors and damped by 46 on the same months.",
  "Nothing can be said until both SSEs are divided by the 48 months of the series."],
 "The course compares methods by MSE because ses scores 47 errors and holt and damped 46. Holt and damped both start at l_1 = y_1, b_1 = y_2 - y_1 and score from index 2, so on EKENE-P1's 48 months both SSEs are divided by 46 and their order is kept. The MSE adds no penalty for parameters, holt scores 46 errors just as damped does, and the divisor is the scored errors, never the months."),

q(0, "On EKENE-P4, Holt's fit reaches SSE 278219.131326 and ses's SSE 215501.852140, each over its own scored errors. Which MSEs does the engine report?",
 "6048.241985 for holt over 46 errors, and 4585.145790 for ses over 47.",
 ["4585.145790 for holt and 6048.241985 for ses, since ses scores the extra month.",
  "One MSE for both, because the two methods score the same months of EKENE-P4.",
  "Holt's is the lower of the two, as it divides its SSE by one error more than ses."],
 "The MSE is the SSE over the scored errors. Holt scores from index 2, so 278219.131326 over 46 is 6048.241985; ses scores from index 1, so 215501.852140 over 47 is 4585.145790. Swapping the two figures swaps the methods, the two methods score different numbers of months, and holt divides by one error fewer than ses, which leaves its larger SSE with the higher MSE."),
