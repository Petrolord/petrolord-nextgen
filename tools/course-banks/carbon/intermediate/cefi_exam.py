...
q(2, "In the SECTION 11 demand table, what unweighted c + h/4 + s - o/2 term does the C3H8 row of the invented Isiokpo fuel carry?",
 "5",
 ["3.5",
  "2",
  "0"],
...
q(2, "SECTION 11 says the air's nitrogen is the rest of the stoichiometric air once its oxygen is met. Which figure does it print for it, in kmol per kmol fuel?",
 ...
 ["9.975652",
  "8.999152",
  "0.015000"],
 "SECTION 11: the air's nitrogen is the rest of that air, 7.886152 (computed here; products.airN2PerKmolFuel 7.886152). 9.975652 is the stoichiometric air itself, 8.999152 the dry flue gas at 0 percent oxygen in SECTION 12, and 0.015000 the fuel's own nitrogen.")

q(3, "Which heating values does SECTION 11 print for the invented Isiokpo fuel gas as analysed, inerts included?",
 ...
 "SECTION 11 prints lhvMJPerKmolFuel 840.9925 and hhvMJPerKmolFuel 930.6273. 862.5564 is the LHV with the CO2 taken out and the rest renormalised; 802.6 and 890.8 are FUEL_REFERENCE's typical methane pair, held as H2.")

q(1, "At which stack oxygen does SECTION 11 strike its flue gas mass balance, and what does each side weigh?",
 ...
 "SECTION 11 strikes THE MASS BALANCE at 3 percent stack oxygen, per kmol of fuel: 351.0222 in, 351.0222 out, out less in 0.000001 (computed here). Air's argon leaves inside the atmospheric nitrogen.")
...
q(2, "Which rows of energyEfficiency.FUEL_REFERENCE carry the flag inert yes?",
 ...
 ["N2 and H2",
  "CO2 alone",
  "CH4 and H2"],
 "SECTION 1 prints inert yes on the Carbon dioxide (inert) and Nitrogen (inert) rows, each with 0 typical LHV and HHV. H2, Hydrogen, prints no with 241.8 LHV and 285.8 HHV MJ/kmol.")
...
q(2, "Held item H2 sets two computed-here figures beside the engine's typical methane pair. Which?",
 ...
 ["A difference of 87.985, and 88.2 MJ for two moles of water condensed at the engine's latent heat",
  "A corrected methane HHV that replaces 890.8 in FUEL_REFERENCE from this digest onward, graded",
  "A difference of 88.2 that the course grades, and an ISO 6976 value of 87.985 to be filed instead"],
 "SECTION 25 prints both as computed here: 88.2 between the typical LHV and HHV, and 87.985 MJ from 2 x 18.015 kg at 2442 kJ/kg. H2 is a stated limit; nothing in it is graded or corrected.")

q(1, "In the SECTION 14 sweep for the invented Isiokpo heater, how much fuel a year does a 4.5 percent target save on LHV?",
 "2782.362 GJ",
 ["5214.422 GJ",
  "6742.370 GJ",
  "8337.934 GJ"],
...
q(3, "What does the second sentence of the engine's tuning method say subtracting the efficiency percentages does to the saving?",
 ...
 "The second sentence ends on the word understates, and names the cause: a divisor of a hundred where the target efficiency belongs. On the Isiokpo figures the digest's shortcut lands 819.363 GJ a year short.")
...
q(3, "SECTION 15 states the critical pressure ratio at or below which a trap's flow is choked. Which expression is it?",
 "(2/(k+1))^(k/(k-1))",
 ["(2/(k+1))^((k-1)/k)",
  "(2/(k-1))^(k/(k+1))",
  "(1/(k+1))^(k/(k-1))"],
 "SECTION 15: the flow is choked while the downstream pressure over the upstream is at or below the critical ratio (2/(k+1))^(k/(k-1)). The digest prints it as 0.577430 at k = 1.135 and 0.5457 at 1.3.")
...
q(0, "The trap's discharge coefficient is not given. Which answer does the engine give?",
 ...
 "SECTION 15 prints the blank discharge coefficient refused with that sentence. 0.72 is the Isiokpo trap's typed coefficient (invented). The orifice refusal answers a blank orifice.")

q(2, "At the stated exponent of 1.135 and an invented 22 USD a tonne, what annual cost does SECTION 15 print for the trap's lost steam?",
 "7826.65 USD",
 ["8216.58 USD",
  "22176.00 USD",
  "54432.00 USD"],
 "The 1.135 row prints 355.757 tonnes a year and 7826.65 USD. 8216.58 is the 1.3 row, and 22176.00 and 54432.00 are SECTION 16's raw water and treatment lines.")

q(1, "SECTION 15 prints the trap's carbon chain as relations. How is its 63.721 tCO2e a year formed?",
 ...
 "SECTION 15: annual tCO2e = GJ x kg per GJ / 1000 = 63.721, on the annual fuel 1135.851 GJ and the SYNTHETIC 56.1 kg CO2e per GJ. The boiler efficiency 0.83 enters one step earlier, in the fuel. 1192.439 GJ is the fuel of the 1.3 row.")

q(3, "Switch the trap's steam to superheated, exponent 1.3. What carbon a year does SECTION 15 then print, in tCO2e?",
 "66.896",
 ["63.721",
  "56.1",
  "742.220"],
...
q(2, "What does SECTION 15 compute from the engine's figures for the gap between the 1.3 and 1.135 exponent rows of the trap?",
 ...
 "SECTION 15: at the superheated exponent the same trap loses 17.724 tonnes a year more, a ratio of 1.049821 to the saturated figure (computed here). 373.481 against 355.757 tonnes a year.")
...
q(1, "Someone types 9000 into the trap's hours box. Which answer comes back?",
 ...
 "9000 is above the 8784 bound the refusal states, and the digest prints it refused in the blank's words. Only hours left out of the call entirely reach the 8760 default.")

q(3, "With the treatment cost priced on the invented Isiokpo condensate case, which line does SECTION 16 print as 54432.00 USD?",
...
q(0, "A return fraction of 1.2 is typed as the condensate target. Which answer comes back?",
 ...
 "SECTION 16 prints the target of 1.2 refused with \"Return fractions must lie between 0 and 1.\" The target-below-current refusal answers a target of 0.25 against 0.35, and the boiler efficiency refusal a blank boiler efficiency.")

q(2, "pinchTargets is handed streams whose supply and target temperatures are all equal. What does SECTION 17 print?",
 ...
 "SECTION 17 prints that refusal for a set of streams where nothing changes temperature. The minimum approach refusal answers a blank approach.")

q(0, "Which pair of condensate figures does SECTION 16 print the same with the treatment cost priced and with it left blank?",
...
q(1, "Four invented Isiokpo streams are printed in SECTION 17. Which one is cold, by the digest's rule?",
...
q(2, "A stream with a heat capacity flowrate of -2 is passed to pinchTargets. What does SECTION 17 print?",
 ...
 ["No refusal: a CP of -2 marks the stream as cold, and it is cascaded at 2 kW/K on the cold side of the table.",
  "REFUSED: A minimum approach temperature is required and must not be negative.",
  "REFUSED: No stream changes temperature, so there is nothing to target."],
 "SECTION 17 prints the CP of -2 refused with that sentence, and SECTION 25 lists it among the rules in force. The approach refusal answers a blank minimum approach, and the last refusal a set of streams that never change temperature.")

q(1, "SECTION 17's threshold problem runs one hot and one cold stream at a 10 C minimum approach. What cold utility does it print?",
 "1470.000 kW",
 ["1480.000 kW",
  "1300.000 kW",
  "240.150 kW"],
...
q(3, "How does SECTION 17 say the streams are shifted before the problem table is cascaded?",
 ...
 "SECTION 17: hot streams are shifted down and cold streams up by half the minimum approach; the cascade below already carries the hot utility at the top. At 15 C the heat flow is zero at shifted 110.500 C, reported as 118.000 C hot and 103.000 C cold.")
