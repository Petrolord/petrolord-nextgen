...
q(0, "Which of these does the course hold as a stated limit under H1?",
 ...
 ["The engine ships the AR6 set as its default when no GWP is supplied.",
  ...
  "The AR5 values are retired from the course and no longer printed beside it."],
...
q(2, "The course names two oracles in the engines repository. Where are their goldens asserted?",
 "By the engine test suites",
 ["By the course's capstone grader",
  "By the course's computed-here lines",
  "By the Carbon Studio each time it runs"],
 "In the course, two independent oracles in tools/validation/downstream recompute these modules by other routes, and their goldens are asserted by the engine test suites.")
...
q(1, "What does held item H3 state?",
 ...
 "Held item H3: every escaped carbon atom is counted as methane (the inventory lesson). It is a held item, taught as a stated limit and never graded.")

q(3, "The course lists each rule in force with the table that prints it. Which rule does it place in the target checks?",
 ...
 "The course places the over-claim rule, and the rule that a verdict is none while a claim acts on a source with no computed emission passed, in the target checks. The refusedMeasures rule prints in the refusal examples, the over-abatement refusal on the path and the basis refusal in the saving example.")
...
q(2, "The curve prints additive false for the two heater measures. How does the path count them once they are live?",
 ...
 "On the path, each measure counts in full from its start year. Tune the fired heaters counts from 2027 and the Heat integration project from 2029, and by 2031 the abated column reads 15460.000 t, the curve's total.")
...
q(0, "By which route does oracle_carbonabatement.py recompute a measure's cost per tonne?",
 ...
 "The oracle's route: the cost per tonne LEVELISED from a year-by-year present value ledger where the engine uses a capital recovery factor; the curve by explicit rank. The species ledger is oracle_energyefficiency.py's combustion route.")
...
q(3, "Solar for purchased power is order 5 on the Agbor curve. In which year does it go live on the path?",
 ...
 "Start years are inputs. The path gives Solar for purchased power 2028 and the Heat integration project, order 3 on the curve, 2029; the path goes by those years.")
...
q(1, "How does oracle_energyefficiency.py model the steam trap?",
 ...
 "The oracle's route: the steam trap as an ISENTROPIC NOZZLE with its throat at the larger of the downstream and critical pressures, where the engine uses the choked and subsonic flux formulas. The duty ledger is the tuning saving's route and bisection is the excess air route.")
...
q(2, "What are H1 to H4 in this course?",
 ...
 "The course marks them HELD, taught as stated limits and never graded. The rules in force are printed in tables of their own, each as the engine answers now with the section that prints it, and H1 to H4 are not among them.")
...
q(1, "Which figure is both the Agbor curve's totalAbatementTonnes and the path's abated tonnes in 2031?",
 ...
 ["13610.000 t",
  "12060.000 t",
  "7410.000 t"],
 "The curve prints totalAbatementTonnes 15460.000, and the path's 2031 row prints 15460.000 t abated with all six measures live. 13610.000 t is the 2030 row, 12060.000 t the curve with one measure refused, and 7410.000 t the 2029 row.")

q(1, "Two outputs of the Agbor curve are on the course's list of what neither oracle recomputes. Which two?",
 ...
 "Both curve outputs sit on the course's closing list beside carbonIntensity, compositeCurve and the simple payback: taught from the engine and never graded. Rank order has an oracle route of its own.")
...
q(2, "The engine finds the pinch from a cascade. Which route does the efficiency oracle take to it?",
 ...
 "The oracle's route: the pinch by the LARGEST HEAT DEFICIT with no cascade. The loss ledger is the efficiency route and bisection the excess air route; the course lists the engine's rule that only an interior zero of the cascade is a pinch.")

q(3, "The path prints the full Agbor baseline less the partial one as 10988.000 tCO2e. What does it say that figure is?",
 "The inventory's purchased electricity line",
 ["The heaters' emission passed to the curve",
  "The vented and fugitive methane line",
  "The gap between the two end-year targets"],
 "On the path, the full baseline less the partial is 10988.000 tCO2e (computed here), the inventory's purchased electricity line: the partial inventory is the full one without it. The heaters are passed as 34927.743 tCO2e and the vented line is 2622.400 tCO2e.")
