...
q(2, "Which function does energyEfficiency.priceSaving call to cost a tonne of carbon saved?",
 ...
 "The course: the Energy & Utilities Efficiency Studio's cost per tonne is carbonAbatement.abatementCost called from inside energyEfficiency.priceSaving. The course lists carbonIntensity and abatementCurve as other carbonAbatement functions, and excessAirSaving as an energyEfficiency function.")
...
q(0, "What does the constant carbonAbatement.SCOPE hold?",
 ...
 "The engine returns carbonAbatement.SCOPE: ONE 1, TWO 2. The inventory labels its totals Scope 1 (direct) and Scope 2 (purchased energy), and a line on scope 3 is blocked because it is not Scope 1 or Scope 2.")
...
q(3, "A Business travel line on scope 3 is added to the complete Igbogene inventory. What does buildInventory do with it?",
 ...
 "In the lab, the scope 3 line is blocked with the reason \"scope 3 is not Scope 1 or Scope 2, which is all this inventory totals\". The totals stay the Scope 1 and Scope 2 figures of the complete inventory, total tCO2e 42945.777, and the inventory stops being reportable.")
...
q(1, "A flare call reaches combustionCo2FromCarbon with the destruction efficiency null. What does the engine say?",
 ...
 "The engine returns the same refusal for a blank and for a null destruction efficiency. The interval refusal is for 0 or for 98 typed as a percentage, and the other two are the fuel refusals, one for a missing input and one for a negative fuel.")
...
q(0, "The course says who the default of complete combustion is for. What does it say?",
 ...
 "The engine states: \"A burner is the case that default is for. A flare is asked for its efficiency every time.\" A blank box is refused, and an argument left out takes the stated default, so the argument does have one.")
...
q(3, "carbonIntensity is called with no boundary named. What does the engine say?",
 ...
 "The engine returns the boundary refusal for this call. carbonIntensity gives the denominator refusal for a denominator of 0 or a blank one; the factor refusal belongs to emissionLine and the GWP refusal to makeGwpSet.")
...
q(1, "A named boundary and a denominator of 0 reach carbonIntensity. What comes back?",
 ...
 "The engine returns \"REFUSED: A positive denominator is required.\" for a denominator of 0 and for a blank denominator. The boundary refusal is for a call with no boundary named, and no row reads a 0 or a blank as some other figure.")
...
q(2, "makeGwpSet is given a methane GWP of 0. What comes back?",
 ...
 "The engine: a methane GWP of 0, and one of -5, return \"REFUSED: A global warming potential must be positive. Refused for CH4, so the set is not declared.\" The set then reads declared false.")
...
q(0, "What does makeGwpSet({}) return?",
 ...
 "The course prints the row for makeGwpSet({}): label none, gases 0, declared false. Neither module exports a global warming potential, and the engine declares a set only with a label and at least one value.")
...
q(1, "A factor is typed with a value of 2.5 tCO2/t and nothing else. What does its record read?",
 ...
 "The course prints the row for \"A factor typed with no source\": value 2.5, unit tCO2/t, gas CO2, hasValue true, provenanceComplete false, missingProvenance source, version. A value with nothing else is still a record, and the record says what it lacks.")
...
q(3, "Which of these does neither engine module export?",
 ...
 "The course: \"Neither module exports an emission factor or a global warming potential.\" carbonAbatement exports MW_C, MW_CO2 and MW_CH4, and energyEfficiency exports FUEL_REFERENCE with typical heating values and HEATING_VALUE_BASIS: LHV, HHV.")
...
q(2, "What does FUEL_REFERENCE_NOTE say about the heating values in energyEfficiency's fuel table?",
 ...
 "The course's note on the fuel table labels the heating values typical, says the fuel analysis governs, and asks for a measured value in their place. The definitional label belongs to the atom counts.")
...
q(1, "Which Igbogene line is the one line on Scope 2?",
 ...
 "The lab prints Purchased electricity on scope 2 at 12915.000 tCO2e, and Scope 2 is the sum of its 1 line. The heaters, the flare's two lines and the vented methane are the 4 lines of Scope 1.")
...
q(3, "PROPERTY_REFERENCE lists a latent heat of vaporisation of water of 2442 kJ/kg. How does the table label it?",
 ...
 "The course heads the table \"typical values a stack loss needs, each labelled as typical\", and the waterLatentHeatKJkg row reads 2442, at 25 C reference.")
