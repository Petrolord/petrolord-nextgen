...
q(0,
 ...
 "Beside the simplified 0.000362528185 sits 2758.406219, one over it. 2706.319458 answers the full form channel with its detected rate and its 8 hour restoration times. 3377.890772 is the simplified pair and 3801.573360 the simplified three channel arrangement, both of which sit lower on PFDavg and so higher here.")
...
q(1,
 "Hold the EKULAMA pair at an MTTR of 8 hours and set the repair time after a test to 24 hours. Which group equivalent down time is reported?",
 "888.800000",
...
q(1,
 "Besides the two equivalent down times, which other term of a redundant call does the repair time after a proof test enter?",
 "The undetected common cause term, b lambdaDU (T1/2 + MRT), with b the beta factor.",
 ["The detected common cause term, betaD times lambdaDD times the MTTR, since both restoration times are charged to the same detected population.",
  "The group coefficient in front of the squared independent rate, which moves from 2 towards 6 as the restoration time grows longer.",
  "None, since the MRT is charged to the single channel alone."],
 "The repair time section says the engine adds MRT to every undetected down time and that MRT enters the common cause DU term too, as b lambdaDU (T1/2 + MRT). The detected common cause term is carried on the MTTR, which is a different restoration time. The coefficients 2 and 6 count orderings of failures and never move with a restoration time. And MRT reaches the group figure as well as the channel figure.")
...
q(3,
 "At a beta factor of 0.02 with betaD at half of it, which term does the two out of three report as dominant?",
 "The independent term, 0.000109292223 against a common cause contribution of 0.000105536000.",
 ["The common cause contribution, 0.000105536000 against an independent term of 0.000036430741, the same turn the pair makes on that row.",
  "The common cause contribution, since a voted arrangement is always common cause dominated once any beta factor has been typed.",
  "Neither, since the two terms tie on that row."],
 "The sweep reports the 2oo3 as independent dominated at 0.02, with an independent term of 0.000109292223 just above the shared common cause contribution of 0.000105536000. 0.000036430741 is the pair's independent term on the same row, which is why the pair turns there and the two out of three does not. The two terms are close and unequal, and a typed beta factor of zero leaves both arrangements independent dominated.")
...
q(0,
 "At a proof test coverage of 0.7 on the OBAGI valve, what share of the undetected failures is left for the ten year overhaul?",
 "30.00 percent, derived.",
 ["0.7 of them, the share the coverage names, since a partial test leaves behind the fraction it was credited with.",
  "3.685286 of the undetected failures, since that is the ratio the coverage section prints beside this row of the table.",
  "4.53 percent, the same addition the week long repair time makes."],
 "The coverage section says a coverage of 0.7 leaves, derived, 30.00 percent of the undetected failures to wait for the 10 year overhaul. The coverage is the share a test reveals, so the remainder is one minus it. 3.685286 is how many times the perfect test's PFDavg the valve reaches, a ratio of PFDavg values. 4.53 percent is the repair time sweep on the EKULAMA pair.")
...
q(2,
 "In the full form table for the EKULAMA channel, which two architectures print 0.000000000000 in both common cause columns?",
 "1oo1 and 2oo2.",
 ["1oo2 and 2oo3, the two arrangements that share one group equivalent down time of 884.000000 hours on this channel.",
  "Only 1oo1, since a two out of two carries a common cause term whenever its call has detected failures on it.",
  "2oo2 and 1oo3."],
 "The table prints zero common cause terms for the single channel and for the two out of two, because the engine adds common cause only to the redundant arrangements and the 2oo2 form carries no beta factor term. The 1oo2, the 2oo3 and the 1oo3 all carry 0.000263280000 and 0.000000448000.")

q(0,
 "Built from identical channels, what does voting two of three give a design that a redundant pair cannot?",
...
q(3,
 "Where do the failure rates of the published worked function come from, and what does this course take them for?",
 "The slide's own example values, cited there to SINTEF PDS and vendor certificates, and taken for nothing more than an example.",
 ["The engine's own teaching table of failure rates, which the course carries so that any verification can be run without a data source of its own.",
  "A licensed IEC table, reproduced in the engine so that every published row can be checked against the standard it came from.",
  "Field data from the plant the source describes, offered as a recommended starting point."],
 "The reproduction section says the rates are the slide's own, cited there to SINTEF PDS and vendor certificates, and that they are its example values and this course offers them as nothing more. The engine carries no failure rate data at all, and no licensed IEC or ISA table is reproduced anywhere in it.")
...
q(0,
 ...
 "Printed at three figures, that row reads 2.36E-04, and the reproduction's 0.000235764375 rounds onto it. 3.44E-04 is the same subsystem in the second table, where the beta factor is raised by half for the voted arrangement. 1.05E-03 is the valve assembly and 5.34E-07 the processor.")
...
q(3,
 ...
 ["That every row shared one beta factor of 0.1, so the voted arrangements and the electronic cards carried one common cause fraction between them.",
 ...
 "The reproduction section says the published table reproduces only with MRT equal to the MTTR on every row. The one year interval holds on every row as the table states it. The rates are the source's own, cited there to SINTEF PDS and vendor certificates. And the beta factors differ row by row: 0.1 on the pressure transmitters and the valves, 0.02 on the three electronic cards.")
