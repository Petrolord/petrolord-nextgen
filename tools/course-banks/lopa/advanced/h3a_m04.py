...
q(0,
 ...
 "The digest states that three inputs had to be INFERRED. Stopping at the lifetime and the beta factor leaves out the restoration time after a proof test, which the valve row needs. Stopping at the lifetime alone leaves out two. The reproduction is the reason the inferences were needed at all, so none is not an option."),

q(1,
 "The published valve subsystem is computed with the restoration time after a proof test set equal to the detected restoration time of 120 hours. What comes back, and does it reproduce?",
 "0.001048767640, which is 1.05E-03 at three significant figures and matches the printed 1.05E-03.",
 ["0.001017167254, which is 1.02E-03 at three significant figures and matches the printed 1.05E-03 once the rounding is allowed for.",
  "0.000676060358, which is 6.76E-04 and reproduces a printed value in the coverage table.",
  "0.000473588152, which misses the printed value at the third figure."],
 "With the two restoration times equal the valve row returns 0.001048767640 and reproduces the printed 1.05E-03. 0.001017167254 is the same row with the restoration time after a test set to zero, and it rounds to 1.02E-03, which misses at the third figure. 0.000676060358 and 0.000473588152 both belong to the transmitter coverage row, computed at two different beta factors."),

q(3,
 "The same valve row is computed with the restoration time after a proof test set to zero. What does the engine return?",
 "0.001017167254",
...
q(1,
 ...
 "The valve row carries an MTTR and an MRT of 120 hours, which is long, and the digest computes the row both ways to show the effect. The published table uses a proof test interval of one year throughout, and several of its subsystems carry detected failures, so neither of those makes the valve special. A redundant architecture does not double charge a restoration time."),
...
q(0,
 ...
 "Only a lifetime of ten years reproduces all four printed coverage rows in the sweep, which is strong evidence and not proof, so the golden records the value as INFERRED.",
...
q(2,
 ...
 "A single figure can be hit by accident, which is why one row would prove little; four independent subsystems agreeing at one lifetime and failing elsewhere is what carries the argument. The rows are computed from their own inputs and not from one another, more chances to hit by accident would weaken the case, and the published table carries five subsystems in its main results."),
...
q(1,
 "The transmitter coverage row prints a beta factor of 10 percent. Computed at that printed value, what does the engine return?",
...
q(1,
 ...
 "The pressure transmitters without coverage at a beta factor of 0.15, which return 0.000344085754, reproducing their own printed 3.44E-04.",
 ...
 "The transmitters without coverage return 0.000344085754 at a beta factor of 0.15 and reproduce their printed 3.44E-04, so two independent rows reproduce at 15 percent and neither reproduces at 10 percent. The valve carries its own beta factor and its own inference. The analogue input card is the row the golden leaves out, and no total is offered as evidence here."),
...
q(0,
 ...
 "The digest's lesson for a verification note is that a reproduction which needs an inferred input says which input and why, and that a published table is evidence about the equations only for the rows it reproduces. The rates are the source's own example values, the inference is about the source and travels no further, and an inference that is declared still supports the rows that reproduce."),
