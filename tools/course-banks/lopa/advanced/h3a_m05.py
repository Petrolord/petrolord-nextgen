...
q(1,
 ...
 "The digest states the action plainly: check the architecture against the standard separately, because a PFDavg that meets the target does not by itself satisfy the architectural constraint. The engine returns no band, no warning and no output of any kind about the constraint, so nothing in its answer can settle it, and redundancy alone is not the test the standard applies."),
...
q(3,
 ...
 "The digest's row says the analyst uses a PFH method when the demand rate is high, and that a low demand PFDavg is the wrong quantity there. A wrong quantity is not repaired by shortening an interval, by a caveat beside it or by scaling the target, because the average probability is no longer what characterises the function."),
...
q(2,
 ...
 "The digest's row reads that every one is an input and the analyst justifies each against the organisation's criteria. The engine invents no number and derives none of these, it checks only the ranges its own refusals police, and a missing term is refused rather than worked around."),
...
q(1,
 ...
 "The rule quoted above is the engine's own. A missing auditable flag is not a false one, so a layer with no such flag can still be credited, which is what makes the rule exactly as narrow as it is. Credit is one per IPL whatever it protects, and a valid figure is a range check and not a credit rule."),
...
q(2,
 ...
 "The digest says `decadeOf` refuses nothing and returns null in all four of those cases. A value inside the snap is exactly the case it DOES answer, by returning the power of ten. Nothing is named as an offending input, because there is no error object here at all."),
