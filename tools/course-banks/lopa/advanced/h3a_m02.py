...
q(1,
 ...
 "They stay until the item is restored as new at the lifetime T2, so the uncovered share is averaged over the lifetime and not over the test interval.",
...
q(2,
 ...
 "At a coverage of one there is no uncovered share at all, so the floor of 0.000021600000 is only the MRT after each test. This channel has no detected failures, so no MTTR term is in the answer, and the floor is an exact limit of the form and not an artefact of the search."),
...
q(2,
 ...
 "The digest derives the floor as lambdaDU (1 - PTC)(T2/2 + MRT) + lambdaDU PTC MRT and states that it is the engine's floor to the digit. Nothing about T1 appears in it, which is exactly why no interval reaches below it. Putting T1 into the uncovered term, or charging every failure or the covered share against the lifetime, all misplace the split."),
...
q(3,
 "What longest interval does the same valve support against that target of 0.02 when the proof test is perfect?",
 "44396.444444 hours, which is 5.068087 years",
 ["46473.426980 hours, or 5.305186 years, which the engine returns for a different subsystem entirely",
  "42122.573099 hours, or 4.808513 years, which is the answer at a coverage of 0.95",
  "87600.000000 hours, the lifetime, because a perfect test has no uncovered share to cap it"],
 "A perfect test gives 44396.444444 hours, 5.068087 years. 46473.426980 hours is the IDU valve answer against a target of 0.01 and belongs to another subsystem. 42122.573099 hours is the coverage 0.95 row. The lifetime would be returned only with the state CAPPED_AT_LIFETIME, and this search found a crossing."),
...
q(0,
 ...
 "A target below the floor is UNACHIEVABLE whatever the interval, and the digest names the two things that move a floor: a better test or a shorter lifetime. No finite interval exists, so FOUND is impossible, and the lifetime cap is a different case where the target IS met. A refusal is a finding about the call, and this call is well formed."),
...
q(1,
 "A lifetime is typed that is shorter than the proof test interval. What does the engine say?",
 "It declines and names the field, in its own words: lifetimeHours: must be at least the proof test interval",
 ["It swaps the two, on the reading that the shorter of the pair was meant to be the proof test interval and the longer of the pair the lifetime.",
  "It accepts the pair and returns a result in which the uncovered share is charged over the shorter of the two times.",
  "It accepts the pair and warns that the item is restored as new more often than it is tested."],
 "The engine refuses the pair and names `lifetimeHours`. An item restored as new more often than it is proof tested is a description of something else, so the engine neither swaps the inputs, nor charges the uncovered share over the shorter time, nor answers with a warning. A refusal carries no number of its own."),

q(2,
 "A proof test coverage of zero is typed. What does the engine say?",
 "It declines and names the field, in its own words: proofTestCoverage: must lie in (0, 1]",
 ["It accepts it, since a coverage of zero simply means every undetected failure waits for the lifetime and the form still has an answer.",
  "It accepts it and warns that no proof test interval will make any difference to the answer on this subsystem.",
  "It clamps it to the smallest coverage the sweep uses and returns a result at that value."],
 "The engine's own message is quoted above and it names `proofTestCoverage`. A coverage has to lie above zero and no higher than one. Nothing is accepted, warned or clamped, because a refusal replaces the result and the engine invents no substitute value."),
