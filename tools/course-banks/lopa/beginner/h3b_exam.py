...
q(2,
  ...
  ["NO_SIF_REQUIRED, SIL and BELOW_SIL4_TABLE_FLOOR, which are the words the engine uses when it places an achieved figure on the low demand table.",
   "NOT_SIL_RATED, SIL1, SIL2 and SIL3, since a state has to say which band an achieved figure fell in as well as that it fell in one at all.",
...
q(3,
  ...
  ["Both are per hour and above zero, so a channel with no detected failures at all cannot be described to the engine by this pair of fields.",
...
q(1,
  ...
  ["T1 must be above 0 and T2 must be strictly greater than T1, because a lifetime equal to the interval would leave no uncovered failures to carry.",
...
q(0,
  ...
  ["ipls[0].name: every IPL needs a name, because the engine reads the object it was handed as a single unnamed layer before it checks anything else",
   ...
   "ipls[0].pfd: the layer given carries no figure the engine can read"],
...
q(1,
  ...
  ["a probability of 0 means the scenario cannot happen, which is not a LOPA scenario, so the figure cannot be carried into the product for the row",
   ...
   "a PFD of 0 sits below the bottom of the low demand table"],
...
q(3,
  ...
  ["sifPfdAvg: the proposed function has been clipped to 1 and the row has been computed with that value, which the engine reports as a warning",
   ...
   "tmelPerYr: a proposed function above one cannot meet any tolerance"],
...
q(1,
  ...
  ["Because the word means a different thing in the fluid course and in the metering course, and a reader arriving from either would misread it here.",
   "Because the engine returns a category of its own for every row, and two names for one returned quantity would leave a worksheet ambiguous.",
...
q(3,
  ...
  ["PFDavg for a layer's credited figure, and IPL PFD for a whole function, which is how the two are told apart on a worksheet that carries both.",
   "PFDavg everywhere, including for a credited layer, so that one word covers every probability of failing on demand that appears in this course.",
...
q(0,
  ...
  ["The condition by an argument about ignition or occupancy, the modifier by how much of the year the plant spends in the configuration concerned.",
   "Both by the same evidence, since the engine multiplies them at the same place and makes no distinction of any kind between the two lists.",
...
q(2,
  ...
  ["A consequence category compared with the bands of a corporate matrix, with the frequency carried alongside it for the reviewer to read.",
   "A frequency per year compared with the frequency the same row reported before any of its protection layers had been credited into it.",
...
q(0,
  ...
  ["The condition multiplied by the conditional modifiers, which on this row brings the key to 0.100000000000 before the layers are applied to it.",
   ...
   "0.045000000000, which is the condition read straight from the table."],
...
q(3,
  ...
  ["1.000000, against an unmitigated frequency of 0.000013500000 per year, which is the figure the full row reports once it has been completed.",
...
q(1,
  ...
  ["0.1, the same figure as each of the other three entries, so the four layers on this row differ only in their flags and never in their figures.",
   "0.01, against 0.1 on the alarm and 0.001 on each of the two layers the engine sets aside when it applies the credit rule to the list.",
...
q(3,
  ...
  ["It is credited, because the credit rule only removes credit from a layer whose independence has been actively denied by the analyst who typed it.",
   ...
   "It is credited at the figure entered, with a warning returned beside the result."],
...
q(2,
  ...
  ["From 0.013500000000 per year to 0.000013500000 per year, which is the step the two layers the engine does credit already make on this row.",
   "From 0.000013500000 per year to 0.000001000000 per year, which brings the row exactly to the tolerance the organisation recorded for it.",
...
q(1,
  ...
  ["An IPL is credited once for each scenario it protects, so that a layer appearing on several rows of a worksheet is given its credit on each.",
   "An IPL is credited once for each flag it carries, so a layer flagged both independent and auditable enters the product two times over.",
...
q(0,
  ...
  ["135000.000000, BEYOND_SIL3_REDESIGN and a required PFDavg of null, because a demand past the last band the table names returns no target.",
   "13500.000000, BEYOND_SIL3_REDESIGN and a required PFDavg of 0.000074074074, which is the figure printed for the tightest tolerance on the ladder.",
...
q(2,
  ...
  ["Because the datasheet quotes a risk reduction factor and the row demands a PFDavg, and the two are read in opposite directions from each other.",
   "Because the engine bands a required PFDavg on the low demand table while a datasheet bands an achieved figure on the high demand table instead.",
...
q(1,
  ...
  ["Its own band is 2, it leaves 0.000000270000 per year, and it misses, because the proposal sits inside the band the row requires and above the target.",
   "Its own band is 1, it leaves 0.000000121500 per year, and it misses, which is the line the table of four proposed functions prints for it.",
...
q(2,
  ...
  ["BEYOND_SIL3_REDESIGN, with a required PFDavg of 0.000100000000, since this demand has passed the last band the low demand table carries.",
   "SIL3, with a required PFDavg of 0.000200000000, which is the figure the ladder of demands prints against a required factor of this size.",
...
q(0,
  ...
  ["It says the required PFDavg is below the SIL 4 band and that no function can supply it, and the figure returned with that note is 0.000010000000.",
   "It says the demand has passed the last band on the table and no further claim is made, and the required PFDavg is returned as null for the row.",
   "It says the required PFDavg lies in the SIL 4 band, and the figure is 0.000002000000."],
...
q(2,
  ...
  ["A PFDavg from 1e-5 up to 1e-4, and a risk reduction factor above 10000 and up to 100000, which is the row below it on the same band table.",
   "A PFDavg from 1e-3 up to 1e-2, and a risk reduction factor above 100 and up to 1000, read across the two columns of the low demand table.",
...
q(2,
  ...
  "That is the engine's own note, returned with SIL 4 and the state BELOW_SIL4_TABLE_FLOOR. Nothing is raised to the floor and nothing is refused: the figure stays visible and the state says where it sits. The last option is the note a demand of 500000 returns on the determination side and belongs to a different call.")
...
q(0,
  ...
  ["100.00000000000001, and a plain comparison would band it SIL2, which is the pair of readings the digest prints for this product in its table.",
   "100.00000000000000, and a plain comparison would band it SIL2, because the product has reached the decade that divides those two bands.",
...
q(1,
  ...
  ["0.7 x 0.1 x 0.1 against a tolerable frequency of 7e-5, which is one of the products whose double sits strictly above the decade it belongs on.",
   "0.3 x 0.1 against a tolerable frequency of 3e-4, whose double the digest prints to seventeen significant digits beside its two outcomes.",
...
q(2,
  ...
  ["decadeOf null and the outcome RISK_REDUCTION_BELOW_SIL1, because the figure has not yet reached the decade at which the SIL 1 band begins.",
   "decadeOf null and the outcome SIL1, because the snap covers only figures that sit above a decade and never those that sit below one.",
...
q(0,
  ...
  ["10.000000 with the outcome SIL1, because an exact decade belongs to the band that has the decade as its own inclusive upper bound.",
   "10.000000 with the outcome NO_SIF_REQUIRED, since the golden marks this case as one whose tolerance is already met on the row.",
...
q(1,
  ...
  ["A required RRF of 100.000000 with the outcome SIL2, because a layer the engine declines to credit leaves the demand a whole band higher.",
   "A required RRF of 90.000000 with the outcome SIL1, which is the figure the golden table records against this particular named case.",
...
q(1,
  ...
  ["76 records, written by the engine itself and stored, so the two agree by construction on every case the golden covers for this course.",
   "13 records, written by the stdlib oracle in exact rational arithmetic, which is the count of LOPA scenarios the digest runs through it.",
...
q(0,
  "Two of the golden LOPA scenarios have a relative difference of 0 and two more carry 1.46e-16 and 1.58e-16. Which cases carry those last two?",
  "beyond-sil4-band carries 1.46e-16 and enabling-and-sif carries 1.58e-16.",
  ["enabling-and-sif carries 1.46e-16 and beyond-sil4-band carries 1.58e-16, which is the way round the golden table prints those two entries.",
   "rrf-exactly-1000 carries 1.46e-16 and rrf-exactly-10000 carries 1.58e-16, the two cases in that table sitting exactly on a decade.",
   "sif-just-short carries 1.46e-16 and ipl-sufficient carries 1.58e-16."],
  "The golden table gives beyond-sil4-band a relative difference of 1.46e-16 at a required RRF of 200000.000000, and enabling-and-sif 1.58e-16 at 90.000000. rrf-exactly-1000 carries 1.14e-16 and rrf-exactly-10000 carries 1.82e-16, and sif-just-short and ipl-sufficient both carry 0.")
