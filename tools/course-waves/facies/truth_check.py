#!/usr/bin/env python3
"""truth-facies.json MUST BE THE HARVEST OF THE digest.txt BESIDE IT.
...
KIT = os.environ.get('D3_KIT', '/root/dc-wavekit')
truth = os.path.join(HERE, 'truth-facies.json')
if not os.path.exists(truth):
    print('REFUSED: no truth-facies.json; run harvest_digest.py'); sys.exit(2)
tmp = tempfile.mkdtemp(prefix='d3-truth-')
...
same = filecmp.cmp(truth, os.path.join(tmp, 'truth-facies.json'), shallow=False)
n = len(__import__('json').load(open(truth)))
shutil.rmtree(tmp)
print(f'truth_check: truth-facies.json ({n} digest lines) {"IS" if same else "IS NOT"} the harvest of digest.txt')
if '--plant' in sys.argv:
