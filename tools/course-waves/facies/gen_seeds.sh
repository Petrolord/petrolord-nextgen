...
# precision.json, digest.txt, the capstone generator d3_capstone.mjs, the three
...
if [ -d "$HERE/../../../.git" ] || [ -f "$HERE/../../../.git" ]; then
  ...
  DEFAULT_REPO=/root/wt-dai-d3-nextgen
fi
...
for f in fields.json precision.json digest.txt d3_capstone.mjs hdr_beginner.txt hdr_intermediate.txt hdr_advanced.txt; do
...
echo "staged: $(ls "$STAGE/banks" | wc -l) banks, 3 manifests, 3 headers, fields.json, precision.json, digest.txt, d3_capstone.mjs, the engine closure and gradedTolerance.js"
echo
...
export D3_WAVE="$STAGE" D3_ENGINES="$STAGE/engines" D3_TOLERANCE="$STAGE/gradedTolerance.js"
D3_COURSE_OUT="$OUTDIR/${DATE}_${PREFIX}_${SLUG}_course.sql" D3_CASES_OUT="$OUTDIR/cases" python3 "$HERE/gen_course.py"
...
D3_COURSE_SQL="$OUTDIR/${DATE}_${PREFIX}_${SLUG}_course.sql" \
  D3_GOLIVE_OUT="$OUTDIR/${DATE}_${PREFIX}_${SLUG}_go_live.sql" \
