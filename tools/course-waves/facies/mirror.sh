#!/usr/bin/env bash
# MIRROR THE D3 WAVE INPUTS INTO THE NEXTGEN REPOSITORY, byte for byte. Every
# suite under src/components/course/panels/facies reads the committed copy by
...
DEST="$NG/tools/course-waves/facies"
mkdir -p "$DEST"
INPUTS=$(node -e "const w=require('$NG/tools/course-waves/waves.json');console.log(w.facies.inputs.join('\n'))")
n=0
for f in $INPUTS; do
