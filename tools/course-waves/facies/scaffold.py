#!/usr/bin/env python3
"""Lay out the D3 facies content tree from structure.py: the three tier
...
it left alone.
    ...
    python3 scaffold.py [--repo /root/wt-dai-d3-nextgen] [--check]
...
content = os.path.join(repo, 'src/content/courses/facies')

wrote = kept = manifests = drift = 0
for tier, modules in S.TIERS.items():
    manifest = {'app_slug': 'facies', 'tier': tier, 'content_version': 1, 'modules': []}
