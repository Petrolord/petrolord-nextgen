#!/usr/bin/env node
// THE D3 NUMSWEEP: the kit's numsweep.mjs, run per tier, with ONE exemption and
...
// whose header names a reason or the engine's message, and (D3) every line of
// the engine's PRINTED TREE, which the digest indents by four spaces, and every
// backtick span of eight characters or more. cartFit prints a threshold as the
// shortest decimal that reads back to the stored float, so a tree line carries
// `RHOB <= 2.3810000000000002`; a lesson may show that line only verbatim, in
// an indented block or in backticks, exactly as the digest prints it.
//
//   node numsweep_facies.mjs [--tier beginner|intermediate|advanced] [--content DIR]
//   node numsweep_facies.mjs --selftest     the negative controls
//
...
const KIT = process.env.D3_KIT || '/root/dc-wavekit';
...
const die = (m) => { console.log(`numsweep_facies REFUSES: ${m}`); process.exit(2); };
...
const run = (content, tiers, reasons) => {
  ...
  tiers.forEach((tier) => {
    ...
    const tmp = fs.mkdtempSync(path.join(os.tmpdir(), `d3ns-${tier}-`));
...
const selftest = () => {
  ...
  console.log(`numsweep_facies selftest ${pass ? 'OK' : 'FAILED'}`);
  return pass ? 0 : 1;
};

if (argv.includes('--selftest')) process.exit(selftest());
const content = opt('--content') || path.join(REPO, 'src/content/courses/facies');
const tier = opt('--tier');
...
console.log(`[numsweep_facies] ${reasons.size} engine reason and message strings read from digest.txt`);
