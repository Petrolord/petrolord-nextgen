...
// wave's RECON REPORT rather than from the digest, and both were wrong. In D3 the
...
// gate_claims.mjs, itself H1's and FC9's, through D2's.)
//
...
const ENG = process.env.D3_ENGINES || '/root/wt-dai-d3-nextgen/packages/engines';
...
const RECON = null;
const FINDINGS = (() => { try { return fs.readFileSync(`${ENG}/tools/validation/dataai/FINDINGS-cluster.md`, 'utf8'); } catch { return null; } })();
...
BRIEFS.forEach((f) => {
  ...
  if (PLANT && f === 'BRIEF.md') {
    ...
    text += '\n\nThe engine gate runs 316 tests.\n';
  }
  briefsRead += 1;
...
check('the six legislated words are all in the vocabulary section',
  ['cluster', 'facies', 'standard deviation', 'distance', 'accuracy', 'machine learning'].every((w) => (DIGEST.split(/^# SECTION 28:/m)[1] ?? '').includes(`| ${w} |`)),
  'digest section 28');
check('the BRIEF tier table matches the owners the digest prints: sections 1 to 13 Associate, 14 to 20 Professional, 21 to 27 Expert, 3 and 28 all',
  (() => {
