...
w();
w(`Raw, the four centres spread over ${f6(grSpanRaw)} gAPI of GR and ${f6(pefSpanRaw)} b/e of PEF (derived, largest centre less smallest): the raw clusters are cut along GR. The inertias are in different units and cannot be compared across the three: standard ${f6(KM.inertia)}, raw ${f6(KR.inertia)}, min-max ${f6(KMM.inertia)}.`);
w();
...
w(`A SPLIT NEEDS A DECREASE ABOVE ZERO. Golden \`cart-xor-no-split\`: four rows ${list(xor.args.X.map((r) => `(${r.join(', ')})`))} labelled ${list(xor.args.y.map(S))}, maxDepth ${xor.args.maxDepth}. Every single split leaves each side half one label and half the other, so no split lowers the Gini (root ${f6(TX.nodes[0].gini)}), and the tree stays a single leaf (${TX.nLeaves} node) predicting ${TX.nodes[0].prediction}. A deeper look ahead would find the pattern; CART judges one split at a time.`);
w();
...
w();
w(`kNN misses ${wrong8.length} rows of ${UN2}: ${wrongKinds.map(([k, v]) => `${v} ${k}`).join('; ')}. A sandstone read ${S(T.HOT_GR_ADD)} gAPI hot has the gamma ray of a shaly sand.`);
...
w();
w(`A RANGE CHECK SEES ONLY THE ROWS THAT LEAVE THE RANGE. Min-max flagged ${hot} rows of ${UN2}; kNN missed ${wrong8.length}, and ${KU8.predictions.filter((p, i) => p !== W8[i] && mm8.X[i][0] <= 1).length} of the misses sit inside the GR range. Every row of ${UN2} carries the same offset, flagged or not.`);
w();
