#!/usr/bin/env python3
"""GATE: the words digest section 28 legislates, over everything a learner reads.

  * no P label (P10, P50, P90 and the like) anywhere: this course has no
    distribution of outcomes, and the programme's P vocabulary belongs to one.
  * no claim of artificial intelligence: "AI", "AI-powered" or "artificial
    ...
    what it is: k-means, agglomerative clustering, k nearest neighbours or a
    CART tree (owner copy rule).
  * REPORTED FOR A HUMAN READ, never failed, because each is a judgement:
    ...
    "R-squared" (owned by D2), "standard deviation" (the rule is that its
    divisor is named nearby), "distance" (the rule is that raw is said when
    the logs are not scaled), "accuracy" (the rule is that its rows are named)
    and "self-organising map" (named only as not built).

SWEPT: the digest (EXCEPT section 28, whose table has to name the words it
...
HERE = os.environ.get('D3_WAVE_DIR', os.path.dirname(os.path.abspath(__file__)))
...
APP = [os.path.join(REPO, 'src/components/course/panels/facies', f) for f in
...
REPORT = [
    ...
    ('distance', re.compile(r'\bdistance', re.I)),
    ('accuracy', re.compile(r'\baccuracy', re.I)),
    ('self-organising map', re.compile(r'self-organi[sz]ing map', re.I)),
]
...
def main():
    ...
    if '--plant' in sys.argv:
        # Planted BEFORE the first section, because section 28 is the last
    ...
    # Section 28 names the words it legislates; it is not swept.
    ...
    if len(kept) != len(parts) - 1:
        ...
        return 2
    texts = [('digest.txt (section 28 excepted)', ''.join(kept))]
    for p in APP:
