#!/usr/bin/env python3
"""GATE: no number is TYPED into the D3 digest.

Adapted from D2's gate_typed_literals.py (itself D1's, H1's and FC9's). Every figure in digest.txt has to be
...
d3_dump.mjs OUTSIDE a ${...} substitution.
...
DUMP = os.path.join(os.environ.get('D3_WAVE_DIR', os.path.dirname(os.path.abspath(__file__))), 'd3_dump.mjs')
...
ALLOWED = {
 ...
 # ---------------------------------------------------------------------------
 '0':     'zero: cluster 0, rows counted from 0, maxDepth 0 (a single leaf), a decrease of zero, the lower end of [0, 1], a denominator of 0',
 '1':     'one: n - 1 and k - 1 in stated formulas, k 1, n = 1 refused, the upper end of [0, 1], an index of 1, row 1 of a stated golden',
 '2':     'two: n = 2, sqrt(2 x rise) in the Ward basis, D^2 and (share)^2 in stated formulas, C(n, 2), row 2 of a stated golden',
 '1e-12': 'the tie band quoted in the prose of a rule the engine states in its basis',
 '1936':  'the year of Fisher\'s iris paper, in its citation',
 '179':   'the first page of the Fisher citation',
 '188':   'the last page of the Fisher citation',
 '1985':  'the year of Hubert and Arabie\'s adjusted Rand index, in its citation',
}
