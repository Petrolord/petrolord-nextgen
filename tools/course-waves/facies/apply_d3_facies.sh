...
# sha256 below, pinned to the head of feat/d3-facies-course, or the run REFUSES
...
#                   ladder rolled back (dryrun_d3.sh), the ladder run twice for
...
#                   snapshotted before and after and compared (dryrun_d3.sh
...
#
# ENV
#   REF=origin/main (before the merge: REF=origin/feat/d3-facies-course)
...
rows() {
  d=$(date -u +%F)
  for f in $SEEDS; do
    echo "| $d | \`$f.sql\` | D3 Electrofacies (facies), seed | **APPLIED $d** by the owner via tools/course-waves/facies/apply_d3_facies.sh apply --prod from $REF (content pinned by sha256; scratch dry run and prod rolled-back dry run first) |"
  done
  echo "| $d | \`$GOLIVE.sql\` | D3 Electrofacies (facies), GO-LIVE: coming_soon to available | **APPLIED $d** via apply_d3_facies.sh apply --prod --go-live, after the deployed site served /dashboard/apps/facies |"
}

dryrun() {
  local ok=1
  REPO="$NG_REPO" bash "$HERE/scratch_db.sh" "$REF" | tail -2 || refuse "scratch_db.sh failed"
  REPO="$NG_REPO" bash "$HERE/dryrun_d3.sh" "$REF" | tail -3 | sed 's/^/  /'; [ "${PIPESTATUS[0]}" = 0 ] || ok=0
  REPO="$NG_REPO" bash "$HERE/dryrun_d3.sh" "$REF" --idempotent | grep -E '^IDEMPOTENT' | sed 's/^/  /'; [ "${PIPESTATUS[0]}" = 0 ] || ok=0
  for c in ledger route trap prompt leak; do
    REPO="$NG_REPO" bash "$HERE/dryrun_d3.sh" "$REF" --control "$c" | grep -E '^CONTROL' | sed 's/^/  /'; [ "${PIPESTATUS[0]}" = 0 ] || ok=0
  done
...
case "${1:-verify}" in
  ...
  prod-dryrun) fetch; say "target: PRODUCTION, the whole ladder rolled back"
               TARGET=linked LINKED="$NG_REPO" REPO="$NG_REPO" bash "$HERE/dryrun_d3.sh" "$SHA" || refuse "the production rolled-back dry run did not come back clean" ;;
  apply)
