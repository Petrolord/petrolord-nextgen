#!/usr/bin/env bash
# =============================================================================
# D2 SEEDS AND CASE FILES, GENERATED FROM THE COMMITTED TREE AND NEVER FROM A
# WORKING TREE. Adapted from tools/course-waves/dataqc/gen_seeds.sh (D1).
#
# This script stages the COMMITTED tree first: wave.json, fields.json,
# precision.json, digest.txt, the capstone generator d2_capstone.mjs, the three
# tier headers, every manifest.json, all 21 bank JSONs, the vendored engine
# closure (engines/dataai/ml.js and the two files it imports) and the one
...
if [ -d "$HERE/../../../.git" ] || [ -f "$HERE/../../../.git" ]; then
  ...
  DEFAULT_REPO=/root/wt-dai-d2-nextgen
fi
...
for f in fields.json precision.json digest.txt d2_capstone.mjs hdr_beginner.txt hdr_intermediate.txt hdr_advanced.txt; do
  git -C "$REPO" show "$REF:tools/course-waves/$SLUG/$f" > "$STAGE/$f"
done
for e in engines/dataai/ml.js lib/stats/stats.js lib/lp/simplex.js; do
...
echo "staged: $(ls "$STAGE/banks" | wc -l) banks, 3 manifests, 3 headers, fields.json, precision.json, digest.txt, d2_capstone.mjs, the engine closure and gradedTolerance.js"
echo
...
export D2_WAVE="$STAGE" D2_ENGINES="$STAGE/engines" D2_TOLERANCE="$STAGE/gradedTolerance.js"
D2_COURSE_OUT="$OUTDIR/${DATE}_${PREFIX}_${SLUG}_course.sql" D2_CASES_OUT="$OUTDIR/cases" python3 "$HERE/gen_course.py"
...
D2_COURSE_SQL="$OUTDIR/${DATE}_${PREFIX}_${SLUG}_course.sql" \
  D2_GOLIVE_OUT="$OUTDIR/${DATE}_${PREFIX}_${SLUG}_go_live.sql" \
