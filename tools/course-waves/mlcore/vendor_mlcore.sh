#!/usr/bin/env bash
# VENDOR THE D2 MLCORE CLOSURE (engines/dataai/ml.js) INTO NEXTGEN,
# SHA-IDENTICAL, AND PROVE IT. Adapted from D1's vendor_dataqc.sh.
#
...
# those two, the NIST StRD source files the oracle parses, the oracle, the pin
# writer, the timing script, FINDINGS and the negative control are NAMED with
# the reason they travel.
#
...
# 966bb9e would also move economics cashflow goldens, facilities, production,
...
# "d2-mlcore-course", pinned to its vendored blob. The entries clear by the
# guard's own STALE rule when the pin moves to 966bb9e or later.
set -euo pipefail
ENG=/root/petrolord-engines
NG=${NG:-/root/wt-dai-d2-nextgen}
...
echo
cd "$EXP"
SEED=__tests__/dataai.ml.test.js
...
for extra in \
  "test-data/dataai/goldens/ml_cases.json:read at runtime by the suite through read('test-data','dataai','goldens',...)" \
  "test-data/dataai/pins/ml_pins.json:read at runtime by the suite through read('test-data','dataai','pins',...)" \
  "test-data/dataai/nist-strd/Norris.dat:NIST StRD source the oracle parses for its certified anchors" \
  "test-data/dataai/nist-strd/Pontius.dat:NIST StRD source the oracle parses for its certified anchors" \
  "test-data/dataai/nist-strd/NoInt1.dat:NIST StRD source the oracle parses for its certified anchors" \
  "test-data/dataai/nist-strd/NoInt2.dat:NIST StRD source the oracle parses for its certified anchors" \
  "test-data/dataai/nist-strd/Filip.dat:NIST StRD source the oracle parses for its certified anchors" \
  "test-data/dataai/nist-strd/Longley.dat:NIST StRD source the oracle parses for its certified anchors" \
  "test-data/dataai/nist-strd/Wampler1.dat:NIST StRD source the oracle parses for its certified anchors" \
  "test-data/dataai/nist-strd/Wampler2.dat:NIST StRD source the oracle parses for its certified anchors" \
  "test-data/dataai/nist-strd/Wampler3.dat:NIST StRD source the oracle parses for its certified anchors" \
  "test-data/dataai/nist-strd/Wampler4.dat:NIST StRD source the oracle parses for its certified anchors" \
  "test-data/dataai/nist-strd/Wampler5.dat:NIST StRD source the oracle parses for its certified anchors" \
  "tools/validation/dataai/oracle_ml.py:writes the golden the suite reads" \
  "tools/validation/dataai/pin_ml.py:writes the library pins the suite reads" \
  "tools/validation/dataai/timing_ml.mjs:produced the timings table FINDINGS carries" \
  "tools/validation/dataai/FINDINGS-ml.md:the validation record the engine header cites" \
  "tools/validation/dataai/negcontrol_ml.sh:produced the negative-control table FINDINGS carries"; do
  p="${extra%%:*}"; why="${extra#*:}"
...
echo
DA_ALL=$(git -C "$ENG" ls-tree -r --name-only "$FULL" | grep -E '(^|/)dataai/|dataai\.' | grep -E '(^|[/._])ml([._]|$)|synthetic_wells|nist-strd/' | sort)
MISSED=$(comm -23 <(printf '%s\n' "$DA_ALL") <(printf '%s\n' "$PATHS") || true)
if [ -n "$MISSED" ]; then echo "REFUSES: dataai ml paths in ${FULL:0:7} outside the closure: $MISSED"; exit 1; fi
echo "  every dataai ml path at ${FULL:0:7} is in the closure"
echo
...
for p in paths:
    ...
    added.append({
        ...
        'reason': (f"D2 mlcore course: vendored sha-identical from petrolord-engines {full[:7]} "
                   f"(PR #252: engines/dataai/ml.js, its jest suite, golden, library pins, NIST StRD sources, synthetic wells, oracle, pin writer, "
                   f"timing script, FINDINGS, negative control) by the wave's vendor_mlcore.sh, 4 proofs per path. "
...
print(f'  VENDOR.json: pin {pin[:7]} unchanged; {same} closure path(s) already match the manifest; group {GROUP}: {len(added)} path(s) ledgered')
for e in added:
