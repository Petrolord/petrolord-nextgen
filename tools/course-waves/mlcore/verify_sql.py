#!/usr/bin/env python3
"""Compare the D2 mlcore seed ladder against its committed sources, FIELD BY FIELD:
...
REPO = os.environ.get(
    'REPO', UP if os.path.exists(os.path.join(UP, '.git')) else '/root/wt-dai-d2-nextgen')
SLUG, PREFIX, DATE = 'mlcore', 'd2', '20261101'
...
cap_rows = re.split(r"\n\(\n  'mlcore', ", course.split('insert into public.academy_capstones', 1)[1])[1:]
got_fields = []
for row in cap_rows:
    tier = row.split("'", 2)[1]
