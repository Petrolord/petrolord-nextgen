#!/usr/bin/env bash
# =============================================================================
# D2 DRY RUN: THE WHOLE LADDER, ROLLED BACK. Adapted from D1's dryrun_d1.sh.
#
...
#
# APPLYING IS NOT THIS SCRIPT'S JOB. The real apply is apply_d2_mlcore.sh
# and it is the owner's to run. The go-live is HELD and is dry-run here only.
#
# TARGETS.
#   TARGET=scratch  the local d2-scratch container (scratch_db.sh). The default,
...
#                          oboria_ridge_gr_coef_us_ft_per_gapi, the SMALLEST
#                          graded value, by 1.0000001: an absolute move of
#                          2.3e-8, a twenty-second of its 5e-7 tolerance, so
...
#   prompt                 the l2 edited in the shipped Expert prompt; the
#                          go-live must refuse the prompt.
#   leak                   the Professional F1 of pay, to six decimals,
...
#
# Usage: dryrun_d2.sh [ref|WORKTREE] [--control <name> [field] [mult] | --idempotent]   default HEAD
# =============================================================================
set -u
REF=${1:-HEAD}
CONTROL=${2:-}
...
RUN=$(mktemp -d /tmp/d2dry.XXXXXX)
SLUG=mlcore
GOLIVE=20261101_d2_mlcore_go_live
FILES="20261101_d2_mlcore_course
20261101_d2_mlcore_beginner_deep
20261101_d2_mlcore_intermediate_deep
20261101_d2_mlcore_advanced_deep
$GOLIVE"
KEYS="akpara_rhob_train_centre_g_cm3:beginner akpara_gr_train_scale_gapi:beginner akpara_ols_nphi_coef_us_ft_per_vv:beginner
akpara_ols_residual_se_us_ft:beginner akpara_test_rmse_us_ft:beginner akpara_test_r2:beginner
oboria_ridge_gr_coef_us_ft_per_gapi:intermediate oboria_fold2_test_rmse_us_ft:intermediate oboria_leak_optimism_rmse_us_ft:intermediate
oboria_logistic_rt_coef_per_ohmm:intermediate oboria_pay_f1:intermediate oboria_test_log_loss:intermediate
isuama_scaled_condition_attrs:advanced isuama_l2_phic_coef_per_vv:advanced isuama_nphi_coef_after_three_updates_per_vv:advanced
isuama_perm_nphi_mean_drop_us_ft:advanced isuama_lc_test_rmse_three_wells_us_ft:advanced isuama_pred_dt_first_row_us_ft:advanced"
...
union all select 'path_order 67 held', count(*)::text from public.academy_apps where path_order = 67
...
union all select 'mlcore apps',        count(*)::text from public.academy_apps where slug = 'mlcore'
union all select 'mlcore any',         (select count(*) from public.academy_course_structures where app_slug = 'mlcore')
                                     + (select count(*) from public.academy_quiz_questions where app_slug = 'mlcore')
                                     + (select count(*) from public.academy_capstones where app_slug = 'mlcore') || ''
...
case "$CONTROL:$CTL" in
  ...
  --control:ledger|--control:route)
    FIELD=${FIELD_ARG:-oboria_ridge_gr_coef_us_ft_per_gapi}
  ...
  --control:trap)
    ...
    # by the engine (fitStandardScaler with sd 'sample' on the stated split),
    # with the ledger and the second route cut out so ONLY the trap can object.
    FIELD=akpara_gr_train_scale_gapi; TIER=beginner
    WRONG=$(D2_ENGINES="$REPO/packages/engines" node "$HERE/d2_capstone.mjs" --inputs 2>/dev/null \
      | ENG="$REPO/packages/engines/engines/dataai/ml.js" node --input-type=module -e "
          ...
          const A = JSON.parse(s).AKPARA; const rows = A.field.rows;
          const sp = ML.groupSplit({ groups: rows.map((r) => r.well), testFraction: A.stated.testFraction, seed: A.stated.seed });
          const sc = ML.fitStandardScaler({ X: rows.map((r) => [r.GR]), trainIndices: sp.trainIndices, sd: 'sample' });
 ...
 where c.app_slug = '$SLUG' and c.tier = '$TIER';"
    ...
    ;;
  --control:prompt)
    PRE_GOLIVE="update public.academy_capstones set prompt = replace(prompt, 'with l2 0.5,', 'with l2 0.6,')
 where app_slug = '$SLUG' and tier = 'advanced';"
    WANT="the advanced prompt is not the prompt gen_course.py rendered"
    ;;
  --control:leak)
    PLANT=$(python3 -c "import json; f=[x for x in json.load(open('$HERE/fields.json')) if x[1]=='oboria_pay_f1'][0]; print(f'{f[2]:.6f}')")
    PRE_GOLIVE="update public.academy_capstones c
   set fields = (select jsonb_agg(case when f->>'key' = 'akpara_test_rmse_us_ft'
                                       then jsonb_set(f, '{label}', to_jsonb((f->>'label') || ', against a benchmark of $PLANT'))
                                       else f end)
                   from jsonb_array_elements(c.fields) f)
 where c.app_slug = '$SLUG' and c.tier = 'beginner';"
    WANT="intermediate/oboria_pay_f1 in the beginner capstone text"
    ;;
  --control:sweep)
    fails=0
    for kt in $KEYS; do
...

{
  ...
  if [ "$pass" = 2 ]; then
    echo "create temp table d2_pass1 as select * from ($(tr '\n' ' ' < "$RUN/snap.sql" | sed 's/;[[:space:]]*$//')) s;"
  fi
  for f in $FILES; do
    echo "-- ================= $f"
    if [ "$f" = "$GOLIVE" ]; then
  ...
  if [ "$pass" = 2 ]; then
    echo "do \$\$ declare v_n int; begin select count(*) into v_n from ((select * from d2_pass1 except select * from ($(tr '\n' ' ' < "$RUN/snap.sql" | sed 's/;[[:space:]]*$//')) s) union all (select * from ($(tr '\n' ' ' < "$RUN/snap.sql" | sed 's/;[[:space:]]*$//')) s except select * from d2_pass1)) d; if v_n <> 0 then raise exception 'D2 go-live refused: IDEMPOTENCE, the second pass of the ladder moved % snapshot row(s)', v_n; end if; raise notice 'IDEMPOTENT: the second pass left every count and digest where the first left it'; end \$\$;"
  fi
  done
  cat <<'SQL'
select 'DRY RUN' as what,
       'mlcore ' || a.status
       || ' | ' || (select count(*) from public.academy_course_structures where app_slug='mlcore' and active)::text || ' tiers'
       || ' | ' || (select count(*) from public.academy_course_structures s,
                           lateral jsonb_array_elements(s.structure->'modules') m,
                           lateral jsonb_array_elements_text(m->'lesson_keys') lk
                     where s.app_slug='mlcore' and s.active)::text || ' lessons'
       || ' | ' || (select count(*) from public.academy_quiz_questions where app_slug='mlcore')::text || ' questions ('
       || (select string_agg(n::text, '/' order by ord) from (
             select case tier when 'beginner' then 1 when 'intermediate' then 2 else 3 end ord, count(*) n
               from public.academy_quiz_questions where app_slug='mlcore' group by tier) t) || ')'
       || ' | ' || (select count(*) from public.academy_capstones where app_slug='mlcore')::text || ' capstones'
       || ' | ' || (select count(*) from public.academy_capstones c, lateral jsonb_array_elements(c.fields) f
                     where c.app_slug='mlcore')::text || ' graded'
       ...
       || ' coming_soon' as v
  from public.academy_apps a where a.slug = 'mlcore';
rollback;
SQL
...
if [ -n "$CONTROL" ] && [ "$CONTROL" != --idempotent ]; then
  ...
  if [ $FAILED = 1 ] && grep -qE "D2 go-live refused.*$WANT" "$RUN/out.txt"; then
    echo "CONTROL $CTL FIRED:"
    grep -oE "D2 go-live refused[^\"]*" "$RUN/out.txt" | head -2 | cut -c1-400 | sed 's/^/    /'
    exit 0
  fi
...
grep -q 'mlcore available | 3 tiers | 78 lessons | 396 questions' "$RUN/out.txt" \
