// GATE: nothing the D2 capstone grades, no input it is set on and no field it
...
// banks.
//
// Adapted from D1's gate_capstone_leak.mjs. The D2 capstone inputs are three
// FIELDS of wells, each well a run of samples with continuous log channels
// (GR, RHOB, NPHI, RT, CALI, DT, PHIC) and well-level attributes, plus a
// handful of stated scalars (fractions, seeds, lambdas). Direction 1 builds one
// SERIES per well and channel, in depth order, and one per attribute across
// the wells, and looks for RUNS OF THREE CONSECUTIVE capstone values, written
// as the course prints them (six decimals) and as the generator writes them,
// and for every distinctive stated scalar. The PAY label is 0 and 1 and a run
// of three labels matches any example array, so it is left out and printed as
// left out.
//
// DIRECTIONS, every one counted and printed:
//   1. INPUTS. No run of three consecutive capstone values and no distinctive
//      capstone scalar in digest.txt, d2_dump.mjs, d2_fields.mjs or the lab's
...
//   4. NAMES. No capstone field name (AKPARA, OBORIA, ISUAMA) in the digest, the
...
//      <repo>/src/content/courses/mlcore), and THE BRIEFS (BRIEF.md,
...
const ENG = process.env.D2_ENGINES || path.join(REPO, 'packages/engines');
const WAVES = process.env.D2_WAVES || path.join(REPO, 'tools/course-waves');
...
const DUMP = read(path.join(HERE, 'd2_dump.mjs'));
const STREAMS = read(path.join(HERE, 'd2_fields.mjs'));
const CAPSTONE = read(path.join(HERE, 'd2_capstone.mjs'));
const FIELDS = JSON.parse(read(path.join(HERE, 'fields.json')));
if (FIELDS.length !== 18) die(`fields.json carries ${FIELDS.length} fields`);
const INPUTS = JSON.parse(execFileSync('node', [path.join(HERE, 'd2_capstone.mjs'), '--inputs'],
  { encoding: 'utf8', stdio: ['ignore', 'pipe', 'ignore'], maxBuffer: 1e8, env: { ...process.env, D2_ENGINES: ENG } }));
const NAMES = Object.keys(INPUTS);
if (NAMES.join() !== 'AKPARA,OBORIA,ISUAMA') die(`unexpected capstone datasets ${NAMES.join()}`);
...
const CHANNELS = ['GR', 'RHOB', 'NPHI', 'RT', 'CALI', 'DT', 'PHIC'];
const ATTR = ['easting', 'northing', 'kb', 'mudWeight'];
const series = [];
const scalars = [];
Object.entries(INPUTS).forEach(([name, o]) => {
  ...
  ATTR.forEach((a) => series.push({ name, k: `wells.${a}`, v: F.wells.map((w) => w[a]) }));
...
console.log('  PAY is left out of the runs: a run of three 0 and 1 labels matches any example array');

/* ---- the negative controls, planted in memory ---- */
if (has('--plant-value')) DIGEST += `\n| planted | ${FIELDS[3][2].toFixed(6)} |\n`;
if (has('--plant-input')) DIGEST += `\nA planted run: ${series.find((x) => x.k === 'OBORIA-2.RHOB').v.slice(4, 7).map((v) => v.toFixed(6)).join(', ')}.\n`;
if (has('--plant-name')) DIGEST += '\nA planted line naming Isuama.\n';
...
// 1. INPUTS
const LAB_DATA = read(path.join(REPO, 'src/components/course/panels/mlcore/ekeneWells.json'));
[['digest.txt', DIGEST], ['d2_dump.mjs', DUMP], ['d2_fields.mjs', STREAMS], ['ekeneWells.json', LAB_DATA]].forEach(([label, text]) => {
...
const GOLD = JSON.parse(read(path.join(ENG, 'test-data/dataai/goldens/ml_cases.json')));
const goldNums = [];
const walk = (n) => {
...
// 4. NAMES
const nameRe = /\b(akpara|oboria|isuama)\b/i;
[['digest.txt', DIGEST], ['d2_dump.mjs', DUMP], ['d2_fields.mjs', STREAMS], ['ekeneWells.json', LAB_DATA]].forEach(([label, text]) => {
...
// 5. NEITHER FILE READS THE OTHER
[['d2_dump.mjs', DUMP, /d2_capstone|fields\.json/], ['d2_fields.mjs', STREAMS, /import[^\n]*d2_capstone|fields\.json/],
  ['d2_capstone.mjs', CAPSTONE, /d2_dump|d2_fields|digest\.txt/]].forEach(([label, text, re]) => {
...
if (fs.existsSync(WAVES)) {
  fs.readdirSync(WAVES).filter((w) => w !== 'mlcore').forEach((w) => {
...
// 8. THE APP
const APP = ['mlcoreLab.js', 'FitExplorer.jsx', 'ValidateExplorer.jsx', 'DiagnoseExplorer.jsx', 'panelBits.jsx', 'ekeneWells.json']
  .map((f) => path.join(REPO, 'src/components/course/panels/mlcore', f))
  .concat([path.join(REPO, 'src/pages/apps/MlCoreLearningPage.jsx')]);
APP.forEach((p) => {
  const text = read(p);
  count('8 app sources');
  const m = text.match(nameRe);
...
const lessonsDir = opt('--lessons') || path.join(REPO, 'src/content/courses/mlcore');
const lessonFiles = [];
let lessonLits = 0;
let plantedIn = null;
