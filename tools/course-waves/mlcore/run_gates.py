#!/usr/bin/env python3
"""RUN EVERY D2 FOUNDATION GATE, WITH ITS NEGATIVE CONTROL, AND PIN WHAT RAN.
...
count the digest does not have.
    ...
    D2_STAGE=lessons python3 run_gates.py   the same, once lesson prose exists
...
any other reason still fails the run. D2_STAGE=lessons makes both expect 0.
...
STAGE = os.environ.get('D2_STAGE', 'foundation')
FOUNDATION = STAGE == 'foundation'
# At the foundation, the stub reason each lesson gate must print to count as met.
STUB_REASON = {
    'numsweep_mlcore.mjs': r'held NOT ONE literal of seven or more significant figures',
    'lengths.py': r'out of band: 78  not written yet: 0',
}
PY_ORACLE = 'python3'  # the D2 oracle is standard library only
...
RUNS = [
    ...
    ('d2_capstone.mjs', f'{HERE}/d2_capstone.mjs', ['node', f'{HERE}/d2_capstone.mjs'], 0),
    ...
    ('numsweep_mlcore.mjs', f'{HERE}/numsweep_mlcore.mjs', ['node', f'{HERE}/numsweep_mlcore.mjs'], 1 if FOUNDATION else 0),
    ('numsweep_mlcore.mjs --selftest', f'{HERE}/numsweep_mlcore.mjs', ['node', f'{HERE}/numsweep_mlcore.mjs', '--selftest'], 0),
...
def main():
    ...
    for name, gate, cmd, want in RUNS:
        ...
        if FOUNDATION and name in STUB_REASON:
            import re
            met = met and re.search(STUB_REASON[name], p.stdout + p.stderr) is not None
        ok = ok and met
        results[name] = {
