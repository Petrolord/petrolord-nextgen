#!/usr/bin/env bash
# MIRROR THE D2 WAVE INPUTS INTO THE NEXTGEN REPOSITORY, byte for byte. Every
# suite under src/components/course/panels/mlcore reads the committed copy by
...
DEST="$NG/tools/course-waves/mlcore"
mkdir -p "$DEST"
INPUTS=$(node -e "const w=require('$NG/tools/course-waves/waves.json');console.log(w.mlcore.inputs.join('\n'))")
n=0
for f in $INPUTS; do
