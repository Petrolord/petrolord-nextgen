#!/usr/bin/env bash
# =============================================================================
# D2 mlcore "Machine Learning on Well Data" SEED LADDER, content-addressed. The owner's
# script. Adapted from apply_d1_dataqc.sh (D1): content pins read out of the git
...
# sha256 below, pinned to the head of feat/d2-mlcore-course, or the run REFUSES
# and names the file. Re-pin only with `pin <ref>`, never by hand.
#
# THE FIVE FILES.
#   20261101_d2_mlcore_course             catalogue row (coming_soon, module
#                                         data_ai, path_order 67) + 3 capstones,
#                                         18 graded fields
#   20261101_d2_mlcore_{beginner,intermediate,advanced}_deep
#                                         3 structures, 78 lesson keys, 396 questions
#   20261101_d2_mlcore_go_live            HELD. coming_soon -> available, behind
...
# site serves /dashboard/apps/mlcore: serve the built DashboardPage chunk and see
# `apps/mlcore` in it. `apply --prod --go-live` will not run until you type it.
# ============================================================================
#
# MODES
#   verify          content pins only. Touches no database.
#   dryrun          LOCAL SCRATCH ONLY: scratch_db.sh at $REF, then the whole
#                   ladder rolled back (dryrun_d2.sh), the ladder run twice for
...
#                   mlcore (enrolments, lesson progress, quiz and capstone
...
#                   snapshotted before and after and compared (dryrun_d2.sh
...
#                   deployed site serves /dashboard/apps/mlcore.
#   rows            the MIGRATIONS.md rows to paste over the NOT YET APPLIED rows.
#   pin <ref>       reprint the digest table at a ref.
#
# ENV
#   REF=origin/main (before the merge: REF=origin/feat/d2-mlcore-course)
...
RUN=$(mktemp -d /tmp/d2apply.XXXXXX)
SLUG=mlcore

SEEDS="20261101_d2_mlcore_course
20261101_d2_mlcore_beginner_deep
20261101_d2_mlcore_intermediate_deep
20261101_d2_mlcore_advanced_deep"
GOLIVE=20261101_d2_mlcore_go_live
FILES="$SEEDS
$GOLIVE"
digest_for() {
  case $1 in
    20261101_d2_mlcore_course           ) echo d9c99334301fe4929bc8b0561c2aec4480d73203288c65ffcd884fa37f636ab1 ;;
    20261101_d2_mlcore_beginner_deep    ) echo 5efdda8e4ac8a35db2fe8a4eef6f2d8183912fc6476b42294c2104ba81af7082 ;;
    20261101_d2_mlcore_intermediate_deep) echo c5db05a0dbed337f364174fecc279acd7e9f00f596e827671c9d21367019907a ;;
    20261101_d2_mlcore_advanced_deep    ) echo ea29fe676cc491f9dc295272715ce6b8a8053a3de7bc3aa4bc891803b10a9bf3 ;;
    20261101_d2_mlcore_go_live          ) echo 2a59bdbbcd49f57972acbc38a0059476a07c41056e753a0257f8a6e915c1b500 ;;
    *) echo UNPINNED ;;
  esac
}
...
# The hash of every mlcore row the seeds write, so a seed inside BEGIN ... ROLLBACK
# can say whether it would change anything.
HASH_Q="md5(coalesce((select string_agg(md5(a::text), ',') from (select slug, name, module, path_order, prereq_slug from public.academy_apps where slug = 'mlcore') a), '')
 || coalesce((select string_agg(md5(s::text), ',' order by s.tier) from (select tier, structure, content_version, active from public.academy_course_structures where app_slug = 'mlcore') s), '')
 || coalesce((select string_agg(md5(q::text), ',' order by q.tier, q.scope, q.module_key, q.ord) from (select tier, scope, module_key, ord, prompt, options, answer_index, explanation, active from public.academy_quiz_questions where app_slug = 'mlcore') q), '')
 || coalesce((select string_agg(md5(c::text), ',' order by c.tier) from (select tier, cert_tier, dataset, title, prompt, fields from public.academy_capstones where app_slug = 'mlcore') c), ''))"

state_of() {  # file -> PENDING | ALREADY-APPLIED | REFUSED:<reason>
  { echo "begin;"; echo "create temp table d2_before on commit drop as select $HASH_Q as h;"
    cat "$RUN/$1.sql"; echo
    echo "select case when (select h from d2_before) = $HASH_Q then 'ALREADY-APPLIED' else 'PENDING' end as d2_state;"
...
golive_state() {
  echo "select 'D2|' || coalesce((select status from public.academy_apps where slug = 'mlcore'), 'absent') as d2_golive;" > "$RUN/golive.sql"
  out=$(prod_q "$RUN/golive.sql")
  case "$(grep -oE 'D2\|[a-z_]+' <<<"$out" | head -1)" in
    'D2|available') echo APPLIED ;;
    'D2|coming_soon') echo HELD ;;
    'D2|absent') echo "HELD (the course row is not seeded yet)" ;;
    *) echo UNKNOWN ;;
  esac
}
...
select 'ATT|' || string_agg(t || ' ' || n, ' | ' order by t) as d2_attempts from (
  select 'enrollments' t, count(*) n from public.academy_enrollments where app_slug = 'mlcore'
  union all select 'lesson_progress', count(*) from public.academy_lesson_progress where app_slug = 'mlcore'
  union all select 'quiz_attempts', count(*) from public.academy_quiz_attempts where app_slug = 'mlcore'
  union all select 'capstone_attempts', count(*) from public.academy_capstone_attempts where app_slug = 'mlcore'
  union all select 'certifications', count(*) from public.academy_certifications where app_slug = 'mlcore') x;
SQL
  out=$(prod_q "$RUN/attempts.sql")
  line=$(grep -oE 'ATT\|[^"]+' <<<"$out" | head -1)
  [ -n "$line" ] || { echo "$out" | tail -5; refuse "could not read the attempt counts"; }
  say "  ${line#ATT|}"
  say "  A new course must show 0 everywhere. A non-zero count means mlcore rows already exist in production: stop and read them before applying."
}

rows() {
  d=$(date -u +%F)
  for f in $SEEDS; do
    echo "| $d | \`$f.sql\` | D2 Machine Learning on Well Data (mlcore), seed | **APPLIED $d** by the owner via tools/course-waves/mlcore/apply_d2_mlcore.sh apply --prod from $REF (content pinned by sha256; scratch dry run and prod rolled-back dry run first) |"
  done
  echo "| $d | \`$GOLIVE.sql\` | D2 Machine Learning on Well Data (mlcore), GO-LIVE: coming_soon to available | **APPLIED $d** via apply_d2_mlcore.sh apply --prod --go-live, after the deployed site served /dashboard/apps/mlcore |"
}

dryrun() {
  local ok=1
  REPO="$NG_REPO" bash "$HERE/scratch_db.sh" "$REF" | tail -2 || refuse "scratch_db.sh failed"
  REPO="$NG_REPO" bash "$HERE/dryrun_d2.sh" "$REF" | tail -3 | sed 's/^/  /'; [ "${PIPESTATUS[0]}" = 0 ] || ok=0
  REPO="$NG_REPO" bash "$HERE/dryrun_d2.sh" "$REF" --idempotent | grep -E '^IDEMPOTENT' | sed 's/^/  /'; [ "${PIPESTATUS[0]}" = 0 ] || ok=0
  for c in ledger route trap prompt leak; do
    REPO="$NG_REPO" bash "$HERE/dryrun_d2.sh" "$REF" --control "$c" | grep -E '^CONTROL' | sed 's/^/  /'; [ "${PIPESTATUS[0]}" = 0 ] || ok=0
  done
...
case "${1:-verify}" in
  ...
  prod-dryrun) fetch; say "target: PRODUCTION, the whole ladder rolled back"
               TARGET=linked LINKED="$NG_REPO" REPO="$NG_REPO" bash "$HERE/dryrun_d2.sh" "$SHA" || refuse "the production rolled-back dry run did not come back clean" ;;
  apply)
    ...
    if [ "${3:-}" = "--go-live" ]; then
      ...
      say "THE GO-LIVE. Confirm you have SERVED the built DashboardPage chunk and SEEN 'apps/mlcore' in it."
      printf "Type exactly  apps/mlcore  to proceed: "; read -r answer
      [ "$answer" = "apps/mlcore" ] || refuse "upload gate not confirmed"
    ...
    say; say "SEEDED. mlcore is coming_soon and no learner can reach it."
    say "Now run: $0 rows, and paste the four seed rows over their NOT YET APPLIED rows in MIGRATIONS.md."
    say "The go-live stays HELD until the deployed site serves /dashboard/apps/mlcore." ;;
  rows)        rows ;;
