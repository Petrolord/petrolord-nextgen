#!/usr/bin/env python3
"""GATE: no number is TYPED into the D2 digest.

Adapted from D1's gate_typed_literals.py (itself H1's and FC9's). Every figure in digest.txt has to be
...
d2_dump.mjs OUTSIDE a ${...} substitution.
...
DUMP = os.path.join(os.environ.get('D2_WAVE_DIR', os.path.dirname(os.path.abspath(__file__))), 'd2_dump.mjs')

ALLOWED = {
 # ---------------------------------------------------------------------------
 # WHAT IS ALLOWED, AND WHY. A figure in the digest is an engine return, a
 # golden, a stated input named in code and substituted, or derived
 # arithmetic on printed values. Every entry below is a NAME, a LABEL or
 # ALGEBRA rather than a measurement. A DEAD ENTRY FAILS THE GATE.
 # ---------------------------------------------------------------------------
 '0':     'zero: a label (PAY = 0, class 0), the lower end of [0, 1], beta = 0 where Newton starts, l2 = 0, S\'w = 0 in the stated theorems, a denominator of 0, rows counted from 0',
 '1':     'one: a label (PAY = 1, class 1), n - 1 and 1 - eps in stated formulas, the upper end of [0, 1], 1/Pa and 1/GPa as units, w >= 1 in the stated theorem',
 '2':     'two: 2TP in the stated F1 formula, kappa^2 and d^2 in stated formulas',
 '0.5':   'the engine\'s class threshold, stated in its predict basis: above 0.5 is class 1, exactly 0.5 is class 0',
 '16':    'UTF-16, the name of the code unit the engine sorts by; and the LRE cap of 16 digits in the stated definition',
 '1e-12': 'the step-halving floor 1e-12 x (1 + |l|), the algebra of the rule the engine states in its basis',
 '0.01':  'a unit conversion stated in prose: per 0.01 v/v and per 0.01 g/cm3',
 '100':   'the arithmetic of that conversion: the coefficient over 100',
 '14':    'the worked example in the stated LRE definition: an LRE of 14 means fourteen digits agree',
 '1967':  'the year of Longley\'s paper, in its citation',
 '62':    'the volume of the Journal of the American Statistical Association in the Longley citation',
 '819':   'the first page of the Longley citation',
 '841':   'the last page of the Longley citation',
 '1.9':   'the scikit-learn version whose log loss clip the choices table names',
 '44000': 'part of the golden case id ols-longley-limit-44000, which the digest names as the id it runs',
}
...
def main():
    ...
    body_src = strip_calls(body_src, ['must', 'refusal', 'success', 'agree', 'planted', 'golden'])
    ...
    for c in calls:
        ...
        body = re.sub(r'\bEKN?E?NE-\d+\b', ' WELLNAME ', body)
        # A NIST/SEMATECH SECTION NUMBER (6.3.2.4, 1.3.5.17.1) is an address.
        body = re.sub(r'\b\d+\.\d+\.\d+(?:\.\d+)*\b', ' NISTREF ', body)
