#!/usr/bin/env bash
# =============================================================================
# FC8 "Metering, Control Valves & Storage" SEED LADDER, content-addressed.
#
...
# THE TWO STEPS.
#
#   seed      Step 1. Catalogue row goes in as `coming_soon` at path_order 46,
...
#
# =========================== THE UPLOAD GATE ================================
# DO NOT RUN `go-live` UNTIL A PRODUCTION UPLOAD CARRIES /dashboard/apps/metering.
#
# The 78 lessons, the teaching lab (meteringLab.js) and its four explorer
...
# DashboardPage chunk and confirm it carries `apps/metering`. `go-live`
...
#
# USAGE
#   ./apply_fc8_metering.sh verify        content check only, no database
#   ./apply_fc8_metering.sh seed          step 1
#   ./apply_fc8_metering.sh go-live       step 2, ONLY after the upload
#   ./apply_fc8_metering.sh pin <ref>     reprint the digest table
#
...
SEEDS="20260925_fc8_metering_course
20260925_fc8_metering_beginner_deep
20260925_fc8_metering_intermediate_deep
20260925_fc8_metering_advanced_deep"
GOLIVE=20260925_fc8_metering_go_live

RUN=$(mktemp -d /tmp/fc8apply.XXXXXX)
...
resolve() {
  ...
  Either $REF does not yet carry the FC8 course PR, or a later change moved a seed.
...
select 'course' as what, status as v from public.academy_apps where slug = 'metering'
union all select 'tiers', count(*)::text from public.academy_course_structures where app_slug = 'metering' and active
...
union all select 'questions', count(*)::text from public.academy_quiz_questions where app_slug = 'metering'
union all select 'capstones', count(*)::text from public.academy_capstones where app_slug = 'metering'
...
case "${1:-verify}" in
  ...
  seed)
    resolve
    echo
    echo "STEP 1: seeding FC8 HIDDEN as coming_soon."
    ...
    echo "DO NOT run go-live until an upload carries /dashboard/apps/metering."
    ;;
  ...
  go-live)
    ...
    echo "'apps/metering' in it. Checking a hash is not this check."
    printf "Type exactly  apps/metering  to proceed: "
    read -r answer
    [ "$answer" = "apps/metering" ] || refuse "upload gate not confirmed"
    run_file "$GOLIVE"
    echo
    state
    echo
    echo "FC8 is available in the Facilities catalogue at path_order 46."
