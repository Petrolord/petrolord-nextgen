#!/usr/bin/env bash
# =============================================================================
# FC8 SEEDS, GENERATED FROM THE COMMITTED TREE AND NEVER FROM A WORKING TREE.
#
...
# that matches nothing a reviewer can read. FC8 ran three tier audits that made
...
if [ -d "$HERE/../../../.git" ] || [ -f "$HERE/../../../.git" ]; then
  ...
  DEFAULT_REPO=/root/wt-fc8-nextgen
fi
...
STAGE=$(mktemp -d /tmp/fc8seed.XXXXXX)
OUTDIR="$STAGE/out"
...
FC8_WAVE="$STAGE" FC8_COURSE_OUT="$OUTDIR/${DATE}_${PREFIX}_${SLUG}_course.sql" \
...
FC8_WAVE="$STAGE" \
  FC8_COURSE_SQL="$OUTDIR/${DATE}_${PREFIX}_${SLUG}_course.sql" \
  FC8_GOLIVE_OUT="$OUTDIR/${DATE}_${PREFIX}_${SLUG}_go_live.sql" \
