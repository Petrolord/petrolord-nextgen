#!/usr/bin/env python3
"""Compare the supply seed ladder against its committed sources, FIELD BY FIELD:
...
canary run must print DISAGREE and exit 2:
  ...
  --canary-field  moves one graded expected value by one whole unit (a hundred
                  tolerances or more on every field of this wave);
  --canary-lesson renames one lesson key in one manifest;
  --canary-prompt changes the cargo tonnage in the advanced capstone prompt.
...
REPO = os.environ.get(
    'REPO', UP if os.path.exists(os.path.join(UP, '.git')) else '/root/wt-md-supply-nextgen')
SLUG, PREFIX, DATE = 'supply', 'tds', '20261012'
...
cap_rows = re.split(r"\n\(\n  'supply', ", course.split('insert into public.academy_capstones', 1)[1])[1:]
got_fields = []
for row in cap_rows:
    tier = row.split("'", 2)[1]
...
if CANARY_PROMPT:
    capjson['tiers']['advanced']['prompt'] = capjson['tiers']['advanced']['prompt'].replace('28500 tonnes', '28600 tonnes', 1)
prompts_compared = 0
for row in cap_rows:
    tier = row.split("'", 2)[1]
...
for flag, what in ((CANARY, 'one character flipped in one intermediate explanation'),
                   ...
                   (CANARY_PROMPT, 'the cargo tonnage changed in the advanced prompt')):
    if flag:
