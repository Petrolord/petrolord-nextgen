#!/usr/bin/env bash
# =============================================================================
# supply SEEDS, GENERATED FROM THE COMMITTED TREE AND NEVER FROM A WORKING TREE.
#
...
# with the geometry helpers they import, the fake clock, the oracle cross-check
# and the discriminate sweep (the go-live writes the oracle's answers and the
# closest wrong routes in by value from these two), every manifest.json, all
# 21 bank JSONs AND the vendored engines/downstream family with the downstream
# Python oracles are pulled out of the git object store at REF into a
# throwaway directory, and all three generators are pointed at that. Nothing
# they read can have been edited after the commit it is named for, and every
# engine and oracle run gen_course.py and gen_golive.py make is a run of the
# COMMITTED engines and oracles.
#
...
if [ -d "$HERE/../../../.git" ] || [ -f "$HERE/../../../.git" ]; then
  ...
  DEFAULT_REPO=/root/wt-md-supply-nextgen
fi
...
STAGE=$(mktemp -d /tmp/tdsseed.XXXXXX)
OUTDIR="$STAGE/out"
...
for f in fields.json precision.json capstone.json digest.txt \
         supply_capstone.mjs supply_fields_capstone.mjs supply_fields.mjs fakeclock.mjs \
...
# The vendored engines the capstone generator runs through, and the downstream
# Python oracles oracle_check.py imports, from the object store.
...
TDS_WAVE="$STAGE" TDS_ENGINES="$STAGE/repo/packages/engines" \
  TDS_COURSE_OUT="$OUTDIR/${DATE}_${PREFIX}_${SLUG}_course.sql" \
...
TDS_WAVE="$STAGE" TDS_ENGINES="$STAGE/repo/packages/engines" MD_ENGINES="$STAGE/repo/packages/engines" \
  TDS_COURSE_SQL="$OUTDIR/${DATE}_${PREFIX}_${SLUG}_course.sql" \
  TDS_GOLIVE_OUT="$OUTDIR/${DATE}_${PREFIX}_${SLUG}_go_live.sql" \
