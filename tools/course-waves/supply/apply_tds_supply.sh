#!/usr/bin/env bash
# =============================================================================
# supply "Terminals, Depots & Fuel Supply" SEED LADDER, content-addressed.
#
...
# THE TWO STEPS.
#
#   seed      Step 1. Catalogue row goes in as `coming_soon` at path_order 50,
#             module `supply_chain`, so no learner can reach any of it. Three
...
#             precision.json tolerances, the prompts, the leak, pairwise and
#             digest sweeps, the engine, a second route in SQL, the oracle and
#             eighteen traps) and TWO typed confirmations, one per upload. It
#             does not need crude (48) or refinery (49) seeded or live first.
#
# =========================== THE UPLOAD GATE ================================
# DO NOT RUN `go-live` UNTIL BOTH OF THESE ARE LIVE:
#
#   1. A NEXTGEN PRODUCTION UPLOAD THAT CARRIES /dashboard/apps/supply.
...
#      and a live Supply Chain & Logistics tile points at a route that does not
#      exist. Verify the upload by CHUNK CONTENT and not by hash: serve the built
#      DashboardPage chunk and confirm it carries `apps/supply`.
#
#   2. THE SUITE PRODUCTION UPLOAD CARRYING SUITE MAIN 1a71d9c90 (Suite #540,
#      fix/md3-0-supply-apps, merged and NOT yet uploaded when this ladder was
#      cut). The course teaches the Suite's Terminal & Depot page as #540
#      repaired it: an opening stock input, so a day's reconciliation can show a
#      gap; days of cover on liftings; only a loss counted to air; and the
#      engines at 13f0936 that page and the Fuel Pricing page run on. Until that
#      upload is live the page a learner opens beside the course still takes the
#      opening stock from today's own dip, so every day balances, which is the
#      exact trap the course is built on. Verify it by serving the Suite
#      production build and confirming it is at or after 1a71d9c90 (the build
#      stamp, or the Terminal & Depot chunk carrying the opening stock input).
#
...
#
# USAGE
#   ./apply_tds_supply.sh verify        content check only, no database
#   ./apply_tds_supply.sh seed          step 1
#   ./apply_tds_supply.sh go-live       step 2, ONLY after BOTH uploads
#   ./apply_tds_supply.sh pin <ref>     reprint the digest table
#
...
digest_for() {
  case $1 in
    20261012_tds_supply_course                  ) echo 879cf7e79e9792e77683cedc19e789a49e306e3a0729dced7a1616c57fe4f23b ;;
    20261012_tds_supply_beginner_deep           ) echo 3618a3a97f3db17f070a3c8fa49aff2660b87fa5e7e33294383a3571ef27b76b ;;
    20261012_tds_supply_intermediate_deep       ) echo 26cfbeb653401c6e18721741025b77de10dcbe55d5871988b0080cd8488da67f ;;
    20261012_tds_supply_advanced_deep           ) echo dd14a416a99d4ecb5e56a8bea43443947b9609c5912b875d71959f8a544cc80e ;;
    20261012_tds_supply_go_live                 ) echo 7cde52365b71417edd695b87e6df108b15d8c8068609687e4f4dad1961f3d098 ;;
    *) echo UNPINNED ;;
  esac
}
SEEDS="20261012_tds_supply_course
20261012_tds_supply_beginner_deep
20261012_tds_supply_intermediate_deep
20261012_tds_supply_advanced_deep"
GOLIVE=20261012_tds_supply_go_live

RUN=$(mktemp -d /tmp/tdsapply.XXXXXX)
...
resolve() {
  ...
  Either $REF does not yet carry the supply course PR, or a later change moved a seed.
...
select 'course' as what, status as v from public.academy_apps where slug = 'supply'
union all select 'tiers', count(*)::text from public.academy_course_structures where app_slug = 'supply' and active
...
union all select 'questions', count(*)::text from public.academy_quiz_questions where app_slug = 'supply'
union all select 'capstones', count(*)::text from public.academy_capstones where app_slug = 'supply'
...
case "${1:-verify}" in
  ...
  seed)
    resolve
    echo
    echo "STEP 1: seeding supply HIDDEN as coming_soon."
    ...
    echo "DO NOT run go-live until BOTH are live: a NextGen upload carrying /dashboard/apps/supply,"
    echo "and the Suite production upload carrying Suite main 1a71d9c90 (Suite #540)."
    ;;
  ...
  go-live)
    ...
    echo "and SEEN 'apps/supply' in it. Checking a hash is not this check."
    printf "Type exactly  apps/supply  to proceed: "
    read -r answer
    [ "$answer" = "apps/supply" ] || refuse "NextGen upload gate not confirmed"
    echo
    echo "UPLOAD 2 of 2. Confirm the Suite production upload carrying Suite main"
    echo "1a71d9c90 (Suite #540, the Terminal & Depot page repairs this course"
    echo "teaches) is LIVE: serve the Suite production build and see it at or after"
    echo "1a71d9c90. Merged is not uploaded."
    printf "Type exactly  1a71d9c90  to proceed: "
    read -r answer
    [ "$answer" = "1a71d9c90" ] || refuse "Suite upload gate (1a71d9c90, Suite #540) not confirmed"
    run_file "$GOLIVE"
    echo
    state
    echo
    echo "supply is available in the Supply Chain & Logistics module."
