...
three draft prompts in capstone.json this refuses:
  ...
  1. ANY GRADED VALUE OF ANY TIER as a number token in the prompt: a token
     ...
     says "0.64" has handed over a probability of waiting of 0.6367 to two
     places, whatever the tolerance).
  2. ANY ENGINE-DERIVED INTERMEDIATE on the way to a graded field (the water
     volumes, the closing stock, the offered load, the cycle, the trips a day,
     C&F, the outturn, the dollar cost a litre), read from the engine through
     supply_capstone.mjs --json rather than typed, in any rounding of three or
     more significant figures. A prompt that prints the cycle has turned the
     truck count into a division.
...
def main():
    ...
    if '--plant' in sys.argv:
        prompts['beginner'] += f' For reference: {GRADED[13][2]:.2f} and {DERIVED["ogwashi_cycle_hours"]}.'
    bad, tokens = [], 0
