#!/usr/bin/env bash
# =============================================================================
# supply SCRATCH DATABASE. A local Postgres 16 the ladder is iterated on
...
# own ladders seed them, crude at 48 and refinery at 49, Commercial & Trading,
# coming_soon, so the go-live's path_order and catalogue checks read something
...
#
# Usage: scratch_db.sh          (re)creates container tds-scratch
#        TDS_NO_SIBLING=1 scratch_db.sh   the same, without the crude and refinery rows
# =============================================================================
set -eu
C=tds-scratch
...
P <<SQL
set tds.no_sibling = '${TDS_NO_SIBLING:-0}';
...
-- The siblings, as their own ladders seed them: crude at 48 and refinery at 49,
-- Commercial & Trading, coming_soon. Any of the three may land first, so the
-- dry run is also taken without them (TDS_NO_SIBLING=1).
insert into public.academy_apps (slug, name, module, path_order, status)
select s, n, 'commercial_trading', o, 'coming_soon'
  from (values ('crude', 'Crude Assay & Blending', 48), ('refinery', 'Refinery Feasibility & Planning', 49)) v(s, n, o)
 where coalesce(current_setting('tds.no_sibling', true), '') <> '1';
SQL
echo "tds-scratch ready: 4 academy tables, $(P -Atc "select count(*) from public.academy_apps") catalogue rows, $(P -Atc "select count(*) from public.academy_apps where slug in ('crude', 'refinery')") of them the siblings crude and refinery"
