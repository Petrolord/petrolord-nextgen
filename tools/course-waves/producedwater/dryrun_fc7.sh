#!/usr/bin/env bash
# =============================================================================
# FC7 DRY RUN: THE WHOLE LADDER, ROLLED BACK, AGAINST A SCRATCH DATABASE.
#
...
# (fc7-scratch: the four academy tables from the repository's own DDL, plus
...
# NAME. The default field is the SMALLEST graded value on the wave, the OGULAGHA
# rise velocity at 7.8e-5 m/s, where one part in 1e7 is an absolute move of about
# 7.8e-12 m/s: under a twelfth of the field's own 1e-10 tolerance, so it would
# pass a grader, and it must still be refused here. Every assertion in this
# go-live is RELATIVE, at 1e-12 or 1e-10, so a move of that size is caught on
# any of the eighteen and not only on the large ones.
#
# Usage: dryrun_fc7.sh [ref|WORKTREE] [--control [<tier>] [<field key>] [<multiplier>]]
# =============================================================================
set -u
...
CONTROL=${2:-}
TIER=${3:-beginner}
FIELD=${4:-ogulagha_droplet_rise_ms}
MULT=${5:-1.0000001}
HERE=$(cd "$(dirname "$0")" && pwd)
if up=$(cd "$HERE/../../.." 2>/dev/null && pwd) && { [ -d "$up/.git" ] || [ -f "$up/.git" ]; }; then
  REPO=${REPO:-$up}
else
  REPO=${REPO:-/root/wt-fc7-nextgen}
fi
C=${SCRATCH:-fc7-scratch}
...
RUN=$(mktemp -d /tmp/fc7dry.XXXXXX)
Q() { docker exec -i "$C" psql -U postgres -At -F ' | ' "$@"; }

FILES="20260925_fc7_producedwater_course
20260925_fc7_producedwater_beginner_deep
20260925_fc7_producedwater_intermediate_deep
20260925_fc7_producedwater_advanced_deep
20260925_fc7_producedwater_go_live"
...
union all select 'path_order 45 held', count(*)::text from public.academy_apps where path_order = 45
...
union all select 'producedwater any',       count(*)::text from public.academy_apps where slug = 'producedwater'
...

{
  ...
  for f in $FILES; do
    ...
    echo
    if [ "$CONTROL" = --control ] && [ "$f" = 20260925_fc7_producedwater_advanced_deep ]; then
      cat <<CTL
...
CTL
  ...
  cat <<'SQL'
select 'DRY RUN',
       'producedwater ' || a.status
       || ' | ' || (select count(*) from public.academy_course_structures where app_slug='producedwater' and active)::text || ' tiers'
       || ' | ' || (select count(*) from public.academy_course_structures s,
                           lateral jsonb_array_elements(s.structure->'modules') m,
                           lateral jsonb_array_elements_text(m->'lesson_keys') lk
                     where s.app_slug='producedwater' and s.active)::text || ' lessons'
       || ' | ' || (select count(*) from public.academy_quiz_questions where app_slug='producedwater')::text || ' questions ('
       || (select string_agg(n::text, '/' order by ord) from (
             select case tier when 'beginner' then 1 when 'intermediate' then 2 else 3 end ord, count(*) n
               from public.academy_quiz_questions where app_slug='producedwater' group by tier) t) || ')'
       || ' | ' || (select count(*) from public.academy_capstones where app_slug='producedwater')::text || ' capstones'
       || ' | ' || (select count(*) from public.academy_capstones c, lateral jsonb_array_elements(c.fields) f
                     where c.app_slug='producedwater')::text || ' graded'
       || ' | path_order ' || a.path_order::text || ' in ' || a.module
  from public.academy_apps a where a.slug = 'producedwater';
rollback;
SQL
...
if [ "$CONTROL" = --control ]; then
  if [ $FAILED = 1 ] && grep -q "$FIELD" "$RUN/out.txt"; then
    ...
    grep -oE "FC7 go-live refused[^\"]*" "$RUN/out.txt" | head -2 | sed 's/^/    /'
    exit 0
  fi
