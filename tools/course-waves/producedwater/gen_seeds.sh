#!/usr/bin/env bash
# =============================================================================
# FC7 SEEDS, GENERATED FROM THE COMMITTED TREE AND NEVER FROM A WORKING TREE.
#
...
# that matches nothing a reviewer can read. FC7 ran three tier audits that made
...
if [ -d "$HERE/../../../.git" ] || [ -f "$HERE/../../../.git" ]; then
  ...
  DEFAULT_REPO=/root/wt-fc7-nextgen
fi
...
STAGE=$(mktemp -d /tmp/fc7seed.XXXXXX)
OUTDIR="$STAGE/out"
...
for f in fields.json precision.json digest.txt hdr_beginner.txt hdr_intermediate.txt hdr_advanced.txt \
         fc7_fields_capstone.mjs; do
...
echo "staged: $(ls "$STAGE/banks" | wc -l) banks, 3 manifests, 3 headers, fields.json, precision.json, digest.txt, the capstone constants and the engine"
echo
...
FC7_WAVE="$STAGE" FC7_REPO="$REPO" FC7_COURSE_OUT="$OUTDIR/${DATE}_${PREFIX}_${SLUG}_course.sql" \
