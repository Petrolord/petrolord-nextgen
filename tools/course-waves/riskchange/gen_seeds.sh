#!/usr/bin/env bash
# =============================================================================
# riskchange SEEDS, GENERATED FROM THE COMMITTED TREE AND NEVER FROM A WORKING TREE.
#
...
if [ -d "$HERE/../../../.git" ] || [ -f "$HERE/../../../.git" ]; then
  ...
  DEFAULT_REPO=/root/wt-as-riskchange-nextgen
fi
...
STAGE=$(mktemp -d /tmp/asrcseed.XXXXXX)
OUTDIR="$STAGE/out"
...
for f in fields.json precision.json digest.txt capstone_calls.json riskchange_fields_capstone.mjs \
...
echo "staged: $(ls "$STAGE/banks" | wc -l) banks, 3 manifests, 3 headers, fields.json, precision.json, digest.txt, capstone_calls.json, riskchange_fields_capstone.mjs"
echo
...
RC_WAVE="$STAGE" RC_COURSE_OUT="$OUTDIR/${DATE}_${PREFIX}_${SLUG}_course.sql" \
...
RC_WAVE="$STAGE" \
  RC_COURSE_SQL="$OUTDIR/${DATE}_${PREFIX}_${SLUG}_course.sql" \
  RC_GOLIVE_OUT="$OUTDIR/${DATE}_${PREFIX}_${SLUG}_go_live.sql" \
