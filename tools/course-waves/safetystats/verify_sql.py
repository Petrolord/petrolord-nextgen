#!/usr/bin/env python3
"""Compare the H1 safetystats seed ladder against its committed sources, FIELD BY FIELD:
...
REPO = os.environ.get(
    'REPO', UP if os.path.exists(os.path.join(UP, '.git')) else '/root/wt-h1-nextgen')
SLUG, PREFIX, DATE = 'safetystats', 'h1', '20261003'
...
cap_rows = re.split(r"\n\(\n  'safetystats', ", course.split('insert into public.academy_capstones', 1)[1])[1:]
got_fields = []
for row in cap_rows:
    tier = row.split("'", 2)[1]
