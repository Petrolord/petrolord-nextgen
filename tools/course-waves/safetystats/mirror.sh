#!/usr/bin/env bash
# MIRROR THE H1 WAVE INPUTS INTO THE NEXTGEN REPOSITORY, byte for byte, and
# re-pin them. Every suite under src/components/course/panels/safetystats reads
...
DEST="$NG/tools/course-waves/safetystats"
mkdir -p "$DEST"
INPUTS=$(node -e "const w=require('$NG/tools/course-waves/waves.json');console.log(w.safetystats.inputs.join('\n'))")
n=0
for f in $INPUTS; do
