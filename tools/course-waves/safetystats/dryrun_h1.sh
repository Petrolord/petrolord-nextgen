#!/usr/bin/env bash
# =============================================================================
# H1 DRY RUN: THE WHOLE LADDER, ROLLED BACK.
#
...
#
# APPLYING IS NOT THIS SCRIPT'S JOB. The real apply is apply_h1_safetystats.sh
# and it is the owner's to run. The go-live is HELD and is dry-run here only.
#
# TARGETS.
#   TARGET=scratch  the local h1-scratch container (scratch_db.sh). The default,
...
#                          forcados_before_after_p_value, the SMALLEST graded
#                          value, by 1.0000001: an absolute move of 1.2e-9, a
#                          four-hundredth of its 5e-7 tolerance, so a grader
#                          would pass it); the engine-ledger check must refuse
#                          it by name.
...
#                          Associate recordable rate set to the MEAN OF THE TWO
#                          WORKFORCE RATES; the trap must refuse.
#   prompt                 one month's hours edited in the shipped Expert
#                          prompt; the go-live must refuse the prompt.
#   leak                   the Professional p-value, to six decimals, planted in
...
#
# Usage: dryrun_h1.sh [ref|WORKTREE] [--control <name> [field] [mult]]   default HEAD
# =============================================================================
set -u
REF=${1:-HEAD}
CONTROL=${2:-}
...
RUN=$(mktemp -d /tmp/h1dry.XXXXXX)
SLUG=safetystats
GOLIVE=20261003_h1_safetystats_go_live
FILES="20261003_h1_safetystats_course
20261003_h1_safetystats_beginner_deep
20261003_h1_safetystats_intermediate_deep
20261003_h1_safetystats_advanced_deep
$GOLIVE"
KEYS="okrika_combined_trir_per_200k:beginner okrika_combined_ltir_per_1m:beginner okrika_far_per_100m:beginner
okrika_severity_rate_per_200k:beginner okrika_tier1_pse_rate_per_200k:beginner okrika_rolling12_trir_month14_per_200k:beginner
bonny_alpha_trir_lower95_per_200k:intermediate bonny_alpha_trir_upper95_per_200k:intermediate
bonny_crew_zero_event_upper95_per_200k:intermediate bonny_rate_ratio_lower95:intermediate
bonny_rate_ratio_upper95:intermediate bonny_compare_p_value:intermediate
forcados_centre_per_200k:advanced forcados_ucl_month09_per_200k:advanced forcados_lcl_month07_per_200k:advanced
forcados_revised_centre_per_200k:advanced forcados_before_after_p_value:advanced
forcados_before_after_p_value_without_month06:advanced"
...
union all select 'path_order 61 held', count(*)::text from public.academy_apps where path_order = 61
...
union all select 'safetystats apps',   count(*)::text from public.academy_apps where slug = 'safetystats'
union all select 'safetystats any',    (select count(*) from public.academy_course_structures where app_slug = 'safetystats')
                                     + (select count(*) from public.academy_quiz_questions where app_slug = 'safetystats')
                                     + (select count(*) from public.academy_capstones where app_slug = 'safetystats') || ''
...
case "$CONTROL:$CTL" in
  ...
  --control:ledger|--control:route)
    FIELD=${FIELD_ARG:-forcados_before_after_p_value}
    ...
    ;;
  --control:trap)
    # The Associate recordable rate set to the mean of the two workforce rates,
    # (5 x 200000 / 1846220 + 11 x 200000 / 3127655) / 2, the first wrong method
    # discriminate.mjs aims at it, with the ledger and the second route cut out
    # so ONLY the trap can object.
    FIELD=okrika_combined_trir_per_200k; TIER=beginner
    WRONG=$(python3 -c "print(repr((5*200000.0/1846220.0 + 11*200000.0/3127655.0)/2.0))")
 ...
 where c.app_slug = '$SLUG' and c.tier = '$TIER';"
    ...
    WANT="the mean of the two workforce rates gives .*so the field does not discriminate the trap"
    ;;
  --control:prompt)
    PRE_GOLIVE="update public.academy_capstones set prompt = replace(prompt, '; month 9, 2 in 118460;', '; month 9, 2 in 118470;')
 where app_slug = '$SLUG' and tier = 'advanced';"
    WANT="the advanced prompt is not the prompt gen_course.py rendered"
    ;;
  --control:leak)
    PRE_GOLIVE="update public.academy_capstones c
   set fields = (select jsonb_agg(case when f->>'key' = 'okrika_far_per_100m'
                                       then jsonb_set(f, '{label}', to_jsonb((f->>'label') || ', against a benchmark of 0.055087'))
                                       else f end)
                   from jsonb_array_elements(c.fields) f)
 where c.app_slug = '$SLUG' and c.tier = 'beginner';"
    WANT="intermediate/bonny_compare_p_value in the beginner capstone text"
    ;;
  --control:sweep)
    fails=0
    for kt in $KEYS; do
...
select 'DRY RUN' as what,
       'safetystats ' || a.status
       || ' | ' || (select count(*) from public.academy_course_structures where app_slug='safetystats' and active)::text || ' tiers'
       || ' | ' || (select count(*) from public.academy_course_structures s,
                           lateral jsonb_array_elements(s.structure->'modules') m,
                           lateral jsonb_array_elements_text(m->'lesson_keys') lk
                     where s.app_slug='safetystats' and s.active)::text || ' lessons'
       || ' | ' || (select count(*) from public.academy_quiz_questions where app_slug='safetystats')::text || ' questions ('
       || (select string_agg(n::text, '/' order by ord) from (
             select case tier when 'beginner' then 1 when 'intermediate' then 2 else 3 end ord, count(*) n
               from public.academy_quiz_questions where app_slug='safetystats' group by tier) t) || ')'
       || ' | ' || (select count(*) from public.academy_capstones where app_slug='safetystats')::text || ' capstones'
       || ' | ' || (select count(*) from public.academy_capstones c, lateral jsonb_array_elements(c.fields) f
                     where c.app_slug='safetystats')::text || ' graded'
       ...
       || ' coming_soon' as v
  from public.academy_apps a where a.slug = 'safetystats';
rollback;
SQL
...
if [ -n "$CONTROL" ]; then
  ...
  if [ $FAILED = 1 ] && grep -qE "H1 go-live refused.*$WANT" "$RUN/out.txt"; then
    echo "CONTROL $CTL FIRED:"
    grep -oE "H1 go-live refused[^\"]*" "$RUN/out.txt" | head -2 | cut -c1-400 | sed 's/^/    /'
    exit 0
  fi
...
grep -q 'safetystats available | 3 tiers | 78 lessons | 396 questions' "$RUN/out.txt" \
