#!/usr/bin/env bash
# =============================================================================
# H1 SEEDS, GENERATED FROM THE COMMITTED TREE AND NEVER FROM A WORKING TREE.
#
...
# seed that matches nothing a reviewer can read. H1's three key-truth audits
# repaired banks and lessons in both places, so the same holds here.
#
...
# engines/hse/safetyStats.js and the one tolerance module gradedTolerance.js are
# pulled out of the git object store at REF with `git show` into a throwaway
# directory, and all three generators are pointed at that. The ENGINE the
# course generator runs is the committed one, so the answer key the migrations
# carry is what the committed engine returns.
#
...
if [ -d "$HERE/../../../.git" ] || [ -f "$HERE/../../../.git" ]; then
  ...
  DEFAULT_REPO=/root/wt-h1-nextgen
fi
...
STAGE=$(mktemp -d /tmp/h1seed.XXXXXX)
OUTDIR="$STAGE/out"
...
for f in fields.json precision.json digest.txt h1_capstone.mjs hdr_beginner.txt hdr_intermediate.txt hdr_advanced.txt; do
...
echo "staged: $(ls "$STAGE/banks" | wc -l) banks, 3 manifests, 3 headers, fields.json, precision.json, digest.txt, h1_capstone.mjs, the engine and gradedTolerance.js"
echo
...
export H1_WAVE="$STAGE" H1_ENGINES="$STAGE/engines" H1_TOLERANCE="$STAGE/gradedTolerance.js"
H1_COURSE_OUT="$OUTDIR/${DATE}_${PREFIX}_${SLUG}_course.sql" python3 "$HERE/gen_course.py"
...
H1_COURSE_SQL="$OUTDIR/${DATE}_${PREFIX}_${SLUG}_course.sql" \
  H1_GOLIVE_OUT="$OUTDIR/${DATE}_${PREFIX}_${SLUG}_go_live.sql" \
