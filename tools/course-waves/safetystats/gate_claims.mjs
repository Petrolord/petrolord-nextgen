...
// nothing in the wave checked a brief against anything. In H1 the engine's
// FINDINGS record, the oracle and the engine's source comments are PROVENANCE,
// and none of them is teaching truth. (Adapted from FC9's gate_claims.mjs.)
//
...
const ENG = process.env.H1_ENGINES || '/root/wt-h1-nextgen/packages/engines';
...
const RECON = null;
const FINDINGS = (() => { try { return fs.readFileSync(`${ENG}/tools/validation/hse/FINDINGS-safetystats.md`, 'utf8'); } catch { return null; } })();
...
BRIEFS.forEach((f) => {
  ...
  if (PLANT && f === 'BRIEF.md') {
    ...
    text += '\n\nThe engine gates 353 tests and the negative control turned 110 of them red.\n';
  }
  briefsRead += 1;
...
check('the four legislated words are all in the vocabulary section',
  ['Poisson', 'severity', 'confidence interval', 'FAR'].every((w) => (DIGEST.split(/^# SECTION 28:/m)[1] ?? '').includes(w)),
  'digest section 28');
check('the BRIEF tier table matches the owners the digest prints: sections 1 to 12 Associate, 13 to 20 Professional, 21 to 27 Expert',
  (() => {
    ...
    return heads.every((h) => {
      ...
      if (n <= 12) return /Associate/.test(own) && (n <= 3 || !/^Professional/.test(own));
      if (n <= 20) return /^Professional/.test(own);
      if (n <= 27) return /^Expert/.test(own);
      return true;
    }) && heads.length === 28;
