#!/usr/bin/env python3
"""RUN EVERY D5 FOUNDATION GATE, WITH ITS NEGATIVE CONTROL, AND PIN WHAT RAN.
...
count the digest does not have.
    ...
    D5_STAGE=lessons python3 run_gates.py   the same, once lesson prose exists
    D5_STAGE=banks python3 run_gates.py     and every bank gate over the 21 banks
...
any other reason still fails the run. D5_STAGE=lessons makes both expect 0.
...
STAGE = os.environ.get('D5_STAGE', 'foundation')
FOUNDATION = STAGE == 'foundation'
# At the foundation, the stub reason each lesson gate must print to count as met.
STUB_REASON = {
    'numsweep_appliedai.mjs': r'held NOT ONE literal of seven or more significant figures',
...
PY_ORACLE = 'python3'  # the D5 oracle is standard library only
...
RUNS = [
    ...
    ('d5_capstone.mjs', f'{HERE}/d5_capstone.mjs', ['node', f'{HERE}/d5_capstone.mjs'], 0),
    ...
    ('numsweep_appliedai.mjs', f'{HERE}/numsweep_appliedai.mjs', ['node', f'{HERE}/numsweep_appliedai.mjs'], 1 if FOUNDATION else 0),
    ('numsweep_appliedai.mjs --selftest', f'{HERE}/numsweep_appliedai.mjs', ['node', f'{HERE}/numsweep_appliedai.mjs', '--selftest'], 0),
...
if BANKS:
    BK = f'{HERE}/banks'
    TIERS = [("d5b", "beginner"), ("d5i", "intermediate"), ("d5a", "advanced")]
    ...
    RUNS += [
        ...
        ('numsweep_appliedai.mjs --banks', f'{HERE}/numsweep_appliedai.mjs', ['node', f'{HERE}/numsweep_appliedai.mjs', '--banks', BK], 0),
