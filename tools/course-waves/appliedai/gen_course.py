...
1. THE ENGINE IS RUN HERE. `node d5_capstone.mjs --json` is executed through the
   ...
   D5_ENGINES) and the one tolerance module (D5_TOLERANCE), and this file
...
2. THE DATA TRAVELS AS CASE FILES. A D4 capstone is set on a field of two
   ...
   readably. So `d5_capstone.mjs --inputs` is rendered into one CSV case file
   per field under src/content/capstone-cases/appliedai/ (the dataqc, mlcore
...
DATE = '20261103'
OUT = os.environ.get('D5_COURSE_OUT', f'{REPO}/migrations/{DATE}_d4_appliedai_course.sql')
CASES_OUT = os.environ.get('D5_CASES_OUT', f'{REPO}/src/content/capstone-cases/appliedai')

SLUG, MODULE, PATH_ORDER = 'appliedai', 'data_ai', 69
...
P50_STEP = 6  # the step of the graded P50, read by d5_capstone.mjs as P50[5]
if not re.search(r'P50\[5\]', open(f'{W}/d5_capstone.mjs', encoding='utf-8').read()):
    bad.append('d5_capstone.mjs no longer grades the P50 at step 6, which the prompt states')
...
-- OWN RUN (d5_capstone.mjs through the vendored engines/dataai/forecast.js and
...
-- /dashboard/apps/appliedai, because the 78 lessons, the teaching lab
