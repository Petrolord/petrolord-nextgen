...
#   20261104_d5_appliedai_go_live            HELD. coming_soon -> available, behind
...
# `apps/appliedai` in it. `apply --prod --go-live` will not run until you type it.
# ============================================================================
#
# MODES
#   verify          content pins only. Touches no database.
#   dryrun          LOCAL SCRATCH ONLY: scratch_db.sh at $REF, then the whole
#                   ladder rolled back (dryrun_d5.sh), the ladder run twice for
...
#                   appliedai (enrolments, lesson progress, quiz and capstone
...
#                   snapshotted before and after and compared (dryrun_d5.sh
...
#                   deployed site serves /dashboard/apps/appliedai.
#   rows            the MIGRATIONS.md rows to paste over the NOT YET APPLIED rows.
#   pin <ref>       reprint the digest table at a ref.
#
# ENV
#   REF=origin/main (before the merge: REF=origin/feat/d5-appliedai-course)
...
# The hash of every appliedai row the seeds write, so a seed inside BEGIN ... ROLLBACK
# can say whether it would change anything.
HASH_Q="md5(coalesce((select string_agg(md5(a::text), ',') from (select slug, name, module, path_order, prereq_slug from public.academy_apps where slug = 'appliedai') a), '')
 || coalesce((select string_agg(md5(s::text), ',' order by s.tier) from (select tier, structure, content_version, active from public.academy_course_structures where app_slug = 'appliedai') s), '')
...
select 'ATT|' || string_agg(t || ' ' || n, ' | ' order by t) as d5_attempts from (
  select 'enrollments' t, count(*) n from public.academy_enrollments where app_slug = 'appliedai'
  union all select 'lesson_progress', count(*) from public.academy_lesson_progress where app_slug = 'appliedai'
  union all select 'quiz_attempts', count(*) from public.academy_quiz_attempts where app_slug = 'appliedai'
  union all select 'capstone_attempts', count(*) from public.academy_capstone_attempts where app_slug = 'appliedai'
  union all select 'certifications', count(*) from public.academy_certifications where app_slug = 'appliedai') x;
SQL
  out=$(prod_q "$RUN/attempts.sql")
  line=$(grep -oE 'ATT\|[^"]+' <<<"$out" | head -1)
  [ -n "$line" ] || { echo "$out" | tail -5; refuse "could not read the attempt counts"; }
  say "  ${line#ATT|}"
  say "  A new course must show 0 everywhere. A non-zero count means appliedai rows already exist in production: stop and read them before applying."
}
...
dryrun() {
  ...
  REPO="$NG_REPO" bash "$HERE/dryrun_d5.sh" "$REF" | tail -3 | sed 's/^/  /'; [ "${PIPESTATUS[0]}" = 0 ] || ok=0
  REPO="$NG_REPO" bash "$HERE/dryrun_d5.sh" "$REF" --idempotent | grep -E '^IDEMPOTENT' | sed 's/^/  /'; [ "${PIPESTATUS[0]}" = 0 ] || ok=0
  for c in ledger route trap prompt leak; do
    REPO="$NG_REPO" bash "$HERE/dryrun_d5.sh" "$REF" --control "$c" | grep -E '^CONTROL' | sed 's/^/  /'; [ "${PIPESTATUS[0]}" = 0 ] || ok=0
  done
...
case "${1:-verify}" in
  ...
  prod-dryrun) fetch; say "target: PRODUCTION, the whole ladder rolled back"
               TARGET=linked LINKED="$NG_REPO" REPO="$NG_REPO" bash "$HERE/dryrun_d5.sh" "$SHA" || refuse "the production rolled-back dry run did not come back clean" ;;
  apply)
    ...
    if [ "${3:-}" = "--go-live" ]; then
      ...
      say "THE GO-LIVE. Confirm you have SERVED the built DashboardPage chunk and SEEN 'apps/appliedai' in it."
      printf "Type exactly  apps/appliedai  to proceed: "; read -r answer
      [ "$answer" = "apps/appliedai" ] || refuse "upload gate not confirmed"
    ...
    say; say "SEEDED. appliedai is coming_soon and no learner can reach it."
    say "Now run: $0 rows, and paste the four seed rows over their NOT YET APPLIED rows in MIGRATIONS.md."
    say "The go-live stays HELD until the deployed site serves /dashboard/apps/appliedai." ;;
  rows)        rows ;;
