#!/usr/bin/env bash
# PROVE THE D5 VENDORING LEAVES D1, D2, D3 AND D4 UNCHANGED. The D5 closure
# moves no shared path (evaluate.js, its suite, golden, pins, fixtures and
# tools are new; lib/stats, percentile.js, ml.js and the ekene-dynamic files
# already match). For each prior Data & AI course this rebuilds, from the COMMITTED
...
[ $fail = 0 ] && echo "PRIOR COURSES UNCHANGED: D1, D2, D3, D4 digests, fields and precision rebuild byte-identical against $ENG" || { echo "PRIOR COURSES: A DIFFERENCE"; exit 1; }
