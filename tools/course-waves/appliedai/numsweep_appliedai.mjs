#!/usr/bin/env node
// THE D5 NUMSWEEP: the kit's numsweep.mjs, run per tier, with ONE exemption and
...
const KIT = process.env.D5_KIT || '/root/dc-wavekit';
...
const die = (m) => { console.log(`numsweep_appliedai REFUSES: ${m}`); process.exit(2); };
...
const run = (content, tiers, reasons) => {
  ...
  tiers.forEach((tier) => {
    ...
    const tmp = fs.mkdtempSync(path.join(os.tmpdir(), `d5ns-${tier}-`));
...
const selftest = () => {
  ...
  console.log(`numsweep_appliedai selftest ${pass ? 'OK' : 'FAILED'}`);
  return pass ? 0 : 1;
};

if (argv.includes('--selftest')) process.exit(selftest());
const content = opt('--content') || path.join(REPO, 'src/content/courses/appliedai');
const tier = opt('--tier');
