#!/usr/bin/env python3
"""truth-appliedai.json MUST BE THE HARVEST OF THE digest.txt BESIDE IT.

numsweep resolves lesson literals against truth-*.json, which the wave's
harvest_appliedai.py (the kit's harvest_digest.py plus comma-grouped figures)
...
truth = os.path.join(HERE, 'truth-appliedai.json')
if not os.path.exists(truth):
    print('REFUSED: no truth-appliedai.json; run harvest_appliedai.py'); sys.exit(2)
tmp = tempfile.mkdtemp(prefix='d5-truth-')
...
subprocess.run(['python3', os.path.join(HERE, 'harvest_appliedai.py'), tmp], check=True, capture_output=True)
same = filecmp.cmp(truth, os.path.join(tmp, 'truth-appliedai.json'), shallow=False)
n = len(__import__('json').load(open(truth)))
shutil.rmtree(tmp)
print(f'truth_check: truth-appliedai.json ({n} digest lines) {"IS" if same else "IS NOT"} the harvest of digest.txt')
if '--plant' in sys.argv:
