#!/usr/bin/env python3
"""GATE: no number is TYPED into the D5 digest.

Adapted from D4's gate_typed_literals.py (itself D3's, D2's, D1's, H1's and FC9's). Every figure in digest.txt has to be
...
d5_dump.mjs OUTSIDE a ${...} substitution.
...
DUMP = os.path.join(os.environ.get('D5_WAVE_DIR', os.path.dirname(os.path.abspath(__file__))), 'd5_dump.mjs')
...
ALLOWED = {
 ...
 # substituted, or derived arithmetic on printed values. Every entry below is a
 # GRADE on the stated 0 to 3 scale, a FORMULA CONSTANT the engine's basis
 # states, a VERSION or a CITATION rather than a measurement. A DEAD ENTRY
 # FAILS THE GATE.
 # ---------------------------------------------------------------------------
 '0':    'zero: grade 0, a score above 0, b = 0, k1 = 0, the lower end of [0, 1], hit 0, exact 0',
 '1':    'one: grade 1, the + 1 of the idf formulas, rank 1, hit 1, the upper end of [0, 1], i - 1 and M - 1 in stated bin rules',
 '2':    'two: grade 2, rank 2, 2 decimals of the calibration probabilities, the ^2 of stated formulas',
 '3':    'three: grade 3, the top of the stated 0 to 3 grading scale, and the 3 of BSD-3-Clause',
 '0.5':  'the 0.5 of the Lucene BM25 idf formula ln(1 + (N - df + 0.5) / (df + 0.5)), as the engine basis states it',
 '8':    'Lucene version 8, a product version named in the conventions table',
 '2017': 'the year of Guo et al., in its citation',
}
...
def main():
    ...
    body_src = strip_decls(body_src, ['ORDER', 'HAND', 'TOKX', 'NORMX', 'SAT', 'NT', 'REFUSALS', 'PLANTED', 'PROBE', 'oily', 'JG', 'LEN_IDS', 'XF', 'XL', 'XT', 'XT_TOL', 'CW', 'GRADE_BAD'])
    ...
    for c in calls:
        ...
        body = re.sub(r'\b(?:EKD-\d{3}|Q\d{2})\b', ' IDREF ', body)
        ...
        # A GOLDEN CASE ID (nist-6434-holt-fit, cmp-constant-ties) is an address.
        body = re.sub(r'\b[a-z][a-z0-9]*(?:-[a-z0-9.]+){2,}\b', ' GOLDENREF ', body)
