...
// wave's RECON REPORT rather than from the digest, and both were wrong. In D5 the
...
// truth. (Adapted from D4's gate_claims.mjs, itself D3's, D2's, D1's, H1's and
// FC9's.)
//
...
const ENG = process.env.D5_ENGINES || '/root/wt-dai-d5-nextgen/packages/engines';
...
const RECON = null;
const FINDINGS = (() => { try { return fs.readFileSync(`${ENG}/tools/validation/dataai/FINDINGS-evaluate.md`, 'utf8'); } catch { return null; } })();
...
const ALLOWED = {
  70: 'the path order of the course in the data_ai module, settled by wave.json pathOrder and the plan of record',
...
check('the six legislated words are all in the vocabulary section',
  ['relevant', 'hallucination', 'grounded', 'accuracy', 'score', 'AI'].every((w) => (DIGEST.split(/^# SECTION 24:/m)[1] ?? '').includes(`| ${w} |`)),
  'digest section 24');
check('the BRIEF tier table matches the owners the digest prints: sections 1 to 10 Associate, 11 to 16 Professional, 17 to 23 Expert, 3 and 24 all',
  (() => {
    ...
    return heads.every((h) => {
      ...
      if (n <= 23) return /^Expert/.test(own);
      return /Associate/.test(own) && /Professional/.test(own) && /Expert/.test(own);
    }) && heads.length === 24;
  })(), 'section owner clauses');
check('the planted-defect count the BRIEF states is the digest\'s',
  (read('BRIEF.md') || '').replace(/\s+/g, ' ').includes(`the ${(DIGEST.match(/^(\d+) items are planted\.$/m) || [])[1]} planted defects`), 'BRIEF.md against digest section 2');
check('the refusal count the BRIEF states is the digest\'s',
  (read('BRIEF.md') || '').replace(/\s+/g, ' ').includes((DIGEST.match(/^(\d+ refusals across \d+ functions)\.$/m) || [])[1] || 'NONE'), 'BRIEF.md against digest section 3');
