#!/usr/bin/env bash
# MIRROR THE D5 WAVE INPUTS INTO THE NEXTGEN REPOSITORY, byte for byte. Every
# suite under src/components/course/panels/appliedai reads the committed copy by
...
DEST="$NG/tools/course-waves/appliedai"
mkdir -p "$DEST"
INPUTS=$(node -e "const w=require('$NG/tools/course-waves/waves.json');console.log(w.appliedai.inputs.join('\n'))")
n=0
for f in $INPUTS; do
