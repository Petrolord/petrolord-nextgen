...
if [ -d "$HERE/../../../.git" ] || [ -f "$HERE/../../../.git" ]; then
  ...
  DEFAULT_REPO=/root/wt-dai-d5-nextgen
fi
...
echo "staged: $(ls "$STAGE/banks" | wc -l) banks, 3 manifests, 3 headers, fields.json, precision.json, digest.txt, d5_capstone.mjs, the engine closure and gradedTolerance.js"
echo
...
export D5_WAVE="$STAGE" D5_ENGINES="$STAGE/engines" D5_TOLERANCE="$STAGE/gradedTolerance.js"
D5_COURSE_OUT="$OUTDIR/${DATE}_${PREFIX}_${SLUG}_course.sql" D5_CASES_OUT="$OUTDIR/cases" python3 "$HERE/gen_course.py"
...
D5_COURSE_SQL="$OUTDIR/${DATE}_${PREFIX}_${SLUG}_course.sql" \
  D5_GOLIVE_OUT="$OUTDIR/${DATE}_${PREFIX}_${SLUG}_go_live.sql" \
