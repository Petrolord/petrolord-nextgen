#!/usr/bin/env python3
"""Lay out the SC2 procurement content tree from structure.py: the three tier
...
it left alone.
    ...
    python3 scaffold.py [--repo /root/wt-sc2-nextgen] [--check]
...
content = os.path.join(repo, 'src/content/courses/procurement')

wrote = kept = manifests = drift = 0
for tier, modules in S.TIERS.items():
    manifest = {'app_slug': 'procurement', 'tier': tier, 'content_version': 1, 'modules': []}
