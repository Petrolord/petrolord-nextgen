#!/usr/bin/env python3
"""RUN EVERY SC2 FOUNDATION GATE, WITH ITS NEGATIVE CONTROL, AND PIN WHAT RAN.
...
count the digest does not have.
    ...
    SC2_STAGE=lessons python3 run_gates.py   the same, once lesson prose exists
    SC2_STAGE=banks python3 run_gates.py     and every bank gate over the 21 banks
...
any other reason still fails the run. SC2_STAGE=lessons makes both expect 0.
...
STAGE = os.environ.get('SC2_STAGE', 'foundation')
FOUNDATION = STAGE == 'foundation'
# At the foundation, the stub reason each lesson gate must print to count as met.
STUB_REASON = {
    'numsweep_procurement.mjs': r'held NOT ONE literal of seven or more significant figures',
...
PY_ORACLE = 'python3'  # the SC2 oracle is standard library only
...
RUNS = [
    ...
    ('sc2_capstone.mjs', f'{HERE}/sc2_capstone.mjs', ['node', f'{HERE}/sc2_capstone.mjs'], 0),
    ...
    ('numsweep_procurement.mjs', f'{HERE}/numsweep_procurement.mjs', ['node', f'{HERE}/numsweep_procurement.mjs'], 1 if FOUNDATION else 0),
    ('numsweep_procurement.mjs --selftest', f'{HERE}/numsweep_procurement.mjs', ['node', f'{HERE}/numsweep_procurement.mjs', '--selftest'], 0),
...
# THE BANK STAGE. D5_STAGE=banks (or full) runs everything above AND every bank
...
if BANKS:
    BK = f'{HERE}/banks'
    TIERS = [("sc2b", "beginner"), ("sc2i", "intermediate"), ("sc2a", "advanced")]
    ...
    RUNS += [
        ...
        ('numsweep_procurement.mjs --banks', f'{HERE}/numsweep_procurement.mjs', ['node', f'{HERE}/numsweep_procurement.mjs', '--banks', BK], 0),
