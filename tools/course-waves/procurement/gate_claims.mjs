...
// wave's RECON REPORT rather than from the digest, and both were wrong. In SC2 the
// engine's FINDINGS record, the oracle and its golden, the fixture README and
// the engine's source comments are PROVENANCE, and none of them is teaching
// truth. (Adapted from D5's gate_claims.mjs, itself D4's, D3's, D2's, D1's,
// H1's and FC9's.)
//
...
const RECON = null;
const FINDINGS = (() => { try { return fs.readFileSync(`${ENG}/tools/validation/supplychain/FINDINGS-tender.md`, 'utf8'); } catch { return null; } })();
...
const ALLOWED = {
  71: 'the path order of the course in the supply_chain module, settled by wave.json pathOrder and the plan of record',
...
check('the six legislated words are all in the vocabulary section',
  ['responsive', 'lowest evaluated cost', 'most advantageous', 'content', 'P90', 'should-cost'].every((w) => (DIGEST.split(/^# SECTION 25:/m)[1] ?? '').includes(`| ${w} |`)),
  'digest section 25');
check('the BRIEF tier table matches the owners the digest prints: sections 1 to 10 Associate, 11 to 16 Professional, 17 to 24 Expert, 4 and 25 all',
  (() => {
    ...
    return heads.every((h) => {
      ...
      if (n === 4 || n === 25) return /Associate/.test(own) && /Professional/.test(own) && /Expert/.test(own);
      if (n <= 10) return /^Associate/.test(own);
      if (n <= 16) return /^Professional/.test(own);
      return /^Expert/.test(own);
    }) && heads.length === 25;
  })(), 'section owner clauses');
check('the planted-situation count the BRIEF states is the digest\'s',
  (read('BRIEF.md') || '').replace(/\s+/g, ' ').includes(`the ${(DIGEST.match(/^All (\d+) planted situations are found/m) || [])[1]} planted situations`), 'BRIEF.md against digest section 3');
check('the refusal count the BRIEF states is the digest\'s',
  (read('BRIEF.md') || '').replace(/\s+/g, ' ').includes((DIGEST.match(/^(\d+ refusals across \d+ functions):/m) || [])[1] || 'NONE'), 'BRIEF.md against digest section 4');
