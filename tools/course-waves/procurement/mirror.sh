#!/usr/bin/env bash
# MIRROR THE SC2 WAVE INPUTS INTO THE NEXTGEN REPOSITORY, byte for byte. Every
# suite under src/components/course/panels/procurement reads the committed copy by
...
DEST="$NG/tools/course-waves/procurement"
mkdir -p "$DEST"
INPUTS=$(node -e "const w=require('$NG/tools/course-waves/waves.json');console.log(w.procurement.inputs.join('\n'))")
n=0
for f in $INPUTS; do
