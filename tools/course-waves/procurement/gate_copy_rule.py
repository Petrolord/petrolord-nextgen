...
<wave>/banks, SC2_BANKS to override). Headings included.
...
BANKS = os.environ.get('SC2_BANKS', os.path.join(HERE, 'banks'))
APP_TEXT = [os.path.join(REPO, 'src/components/course/panels/procurement', f) for f in
...
def main():
    ...
    if os.path.isdir(BANKS):
        names = sorted(n for n in os.listdir(BANKS) if re.match(r'^sc2[bia]_.*\.json$', n))
        planted_only = False
        if '--plant-bank' in sys.argv and not names:
            ...
            names, planted_only = ['sc2b_planted.json'], True
        for n in names:
