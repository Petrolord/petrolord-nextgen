#!/usr/bin/env bash
# =============================================================================
# SC2 procurement "Procurement, Tendering & Contracting" SEED LADDER, content-addressed.
# The owner's script. Adapted from apply_d5_appliedai.sh (D5): content pins read out of the git
...
# sha256 below, pinned to the head of feat/sc2-procurement-course, or the run REFUSES
...
#
# THE FIVE FILES.
#   20261106_sc2_procurement_course      catalogue row (coming_soon, module
#                                         supply_chain, path_order 71, no
#                                         prerequisite) + 3 capstones, 18
#                                         graded fields
#   20261106_sc2_procurement_{beginner,intermediate,advanced}_deep
#                                         3 structures, 78 lesson keys, 396 questions
#   20261106_sc2_procurement_go_live            HELD. coming_soon -> available, behind
#                                         the shape, grader, prompt, engine-ledger,
#                                         second-route, oracle and trap assertions.
#
# =========================== THE UPLOAD GATE ================================
# The 78 lessons, the teaching lab (tenderLab.js), the three calculator panels
...
# site serves /dashboard/apps/procurement: serve the built DashboardPage chunk and see
# `apps/procurement` in it. `apply --prod --go-live` will not run until you type it.
# ============================================================================
#
# MODES
#   verify          content pins only. Touches no database.
#   dryrun          LOCAL SCRATCH ONLY: scratch_db.sh at $REF, then the whole
#                   ladder rolled back (dryrun_sc2.sh), the ladder run twice for
#                   idempotence, the six negative controls (ledger, route,
#                   oracle, trap, prompt, leak), and verify_sql.py with its
#                   canaries. The scratch container sc2-scratch is removed
#                   afterwards.
#   attempts        PRODUCTION, READ-ONLY: every learner row that already names
#                   procurement (enrolments, lesson progress, quiz and capstone
...
#                   snapshotted before and after and compared (dryrun_sc2.sh
...
#                   deployed site serves /dashboard/apps/procurement.
#   rows            the MIGRATIONS.md rows to paste over the NOT YET APPLIED rows.
#   pin <ref>       reprint the digest table at a ref.
#
# ENV
#   REF=origin/main (before the merge: REF=origin/feat/sc2-procurement-course)
...
RUN=$(mktemp -d /tmp/sc2apply.XXXXXX)
SLUG=procurement

SEEDS="20261106_sc2_procurement_course
...
# The hash of every procurement row the seeds write, so a seed inside BEGIN ... ROLLBACK
# can say whether it would change anything.
HASH_Q="md5(coalesce((select string_agg(md5(a::text), ',') from (select slug, name, module, path_order, prereq_slug from public.academy_apps where slug = 'procurement') a), '')
 || coalesce((select string_agg(md5(s::text), ',' order by s.tier) from (select tier, structure, content_version, active from public.academy_course_structures where app_slug = 'procurement') s), '')
 || coalesce((select string_agg(md5(q::text), ',' order by q.tier, q.scope, q.module_key, q.ord) from (select tier, scope, module_key, ord, prompt, options, answer_index, explanation, active from public.academy_quiz_questions where app_slug = 'procurement') q), '')
 || coalesce((select string_agg(md5(c::text), ',' order by c.tier) from (select tier, cert_tier, dataset, title, prompt, fields from public.academy_capstones where app_slug = 'procurement') c), ''))"

COURSE_SEED=20261106_sc2_procurement_course
state_of() {  # file -> PENDING | ALREADY-APPLIED | REFUSED:<reason>
  { echo "begin;"
    ...
    echo "create temp table sc2_before on commit drop as select $HASH_Q as h;"
    cat "$RUN/$1.sql"; echo
    echo "select case when (select h from sc2_before) = $HASH_Q then 'ALREADY-APPLIED' else 'PENDING' end as sc2_state;"
...
golive_state() {
  echo "select 'SC2|' || coalesce((select status from public.academy_apps where slug = 'procurement'), 'absent') as sc2_golive;" > "$RUN/golive.sql"
  out=$(prod_q "$RUN/golive.sql")
  case "$(grep -oE 'SC2\|[a-z_]+' <<<"$out" | head -1)" in
    'SC2|available') echo APPLIED ;;
    'SC2|coming_soon') echo HELD ;;
    'SC2|absent') echo "HELD (the course row is not seeded yet)" ;;
    *) echo UNKNOWN ;;
  esac
}
...
select 'ATT|' || string_agg(t || ' ' || n, ' | ' order by t) as sc2_attempts from (
  select 'enrollments' t, count(*) n from public.academy_enrollments where app_slug = 'procurement'
  union all select 'lesson_progress', count(*) from public.academy_lesson_progress where app_slug = 'procurement'
  union all select 'quiz_attempts', count(*) from public.academy_quiz_attempts where app_slug = 'procurement'
  union all select 'capstone_attempts', count(*) from public.academy_capstone_attempts where app_slug = 'procurement'
  union all select 'certifications', count(*) from public.academy_certifications where app_slug = 'procurement') x;
SQL
  out=$(prod_q "$RUN/attempts.sql")
  line=$(grep -oE 'ATT\|[^"]+' <<<"$out" | head -1)
  [ -n "$line" ] || { echo "$out" | tail -5; refuse "could not read the attempt counts"; }
  say "  ${line#ATT|}"
  say "  A new course must show 0 everywhere. A non-zero count means procurement rows already exist in production: stop and read them before applying."
}

rows() {
  d=$(date -u +%F)
  for f in $SEEDS; do
    echo "| $d | \`$f.sql\` | SC2 Procurement, Tendering & Contracting (procurement), seed | **APPLIED $d** by the owner via tools/course-waves/procurement/apply_sc2_procurement.sh apply --prod from $REF (content pinned by sha256; scratch dry run and prod rolled-back dry run first) |"
  done
  echo "| $d | \`$GOLIVE.sql\` | SC2 Procurement, Tendering & Contracting (procurement), GO-LIVE: coming_soon to available | **APPLIED $d** via apply_sc2_procurement.sh apply --prod --go-live, after the deployed site served /dashboard/apps/procurement |"
}

dryrun() {
  local ok=1
  REPO="$NG_REPO" bash "$HERE/scratch_db.sh" "$REF" | tail -2 || refuse "scratch_db.sh failed"
  REPO="$NG_REPO" bash "$HERE/dryrun_sc2.sh" "$REF" | tail -3 | sed 's/^/  /'; [ "${PIPESTATUS[0]}" = 0 ] || ok=0
  REPO="$NG_REPO" bash "$HERE/dryrun_sc2.sh" "$REF" --idempotent | grep -E '^IDEMPOTENT' | sed 's/^/  /'; [ "${PIPESTATUS[0]}" = 0 ] || ok=0
  for c in ledger route oracle trap prompt leak; do
    REPO="$NG_REPO" bash "$HERE/dryrun_sc2.sh" "$REF" --control "$c" | grep -E '^CONTROL' | sed 's/^/  /'; [ "${PIPESTATUS[0]}" = 0 ] || ok=0
  done
  ...
  docker rm -f sc2-scratch >/dev/null 2>&1 && say "  scratch container sc2-scratch removed"
...
case "${1:-verify}" in
  ...
  prod-dryrun) fetch; say "target: PRODUCTION, the whole ladder rolled back"
               TARGET=linked LINKED="$NG_REPO" REPO="$NG_REPO" bash "$HERE/dryrun_sc2.sh" "$SHA" || refuse "the production rolled-back dry run did not come back clean" ;;
  apply)
    ...
    if [ "${3:-}" = "--go-live" ]; then
      ...
      say "THE GO-LIVE. Confirm you have SERVED the built DashboardPage chunk and SEEN 'apps/procurement' in it."
      printf "Type exactly  apps/procurement  to proceed: "; read -r answer
      [ "$answer" = "apps/procurement" ] || refuse "upload gate not confirmed"
    ...
    say; say "SEEDED. procurement is coming_soon and no learner can reach it."
    say "Now run: $0 rows, and paste the four seed rows over their NOT YET APPLIED rows in MIGRATIONS.md."
    say "The go-live stays HELD until the deployed site serves /dashboard/apps/procurement." ;;
  rows)        rows ;;
