#!/usr/bin/env bash
# =============================================================================
# SC2 SCRATCH DATABASE. A local Postgres 16 the ladder is iterated on BEFORE any
...
# production).
#
# Adapted from tools/course-waves/appliedai/scratch_db.sh (D5). The four
...
# and 53 to 58, and the TWENTY SIX FC, Commercial & Trading, Energy Transition,
...
# 65, D1 dataqc at 66, D2 mlcore at 67, D3 facies at 68, D4 forecastml at 69,
# D5 appliedai at 70), so the path_order, catalogue and prompt checks meet real
# neighbours and real capstone rows, supply among them, the first course of the
# supply_chain module SC2 joins. Every path_order from 1 to 70 is asserted
...
# which is why 'supply_chain' needs no schema change; the scratch table mirrors that.
#
# What this cannot prove, said plainly: that production's rows match. That is
# what `dryrun_sc2.sh` with TARGET=linked is for (rolled back, snapshotted
...
#
# Usage: scratch_db.sh [ref]     (re)creates container sc2-scratch; default HEAD
#        (remove it afterwards with: docker rm -f sc2-scratch)
# =============================================================================
set -eu
REF=${1:-HEAD}
...
for f in $(git -C "$REPO" ls-tree --name-only "$REF" migrations/ \
           | grep -E '_(fc[0-9]+_[a-z]+|cr_crude|rf_refinery|tds_supply|gv_gasvalue|cef_carbon|asrc_riskchange|cq_compliance|h[1-5]_[a-z]+|d1_dataqc|d2_mlcore|d3_facies|d4_forecastml|d5_appliedai)_course\.sql$'); do
  git -C "$REPO" show "$REF:$f" | P
  n=$((n + 1))
done
[ "$n" = 26 ] || { echo "REFUSED: expected the 26 seeded course migrations at $REF, found $n"; exit 2; }
# No hole: every path_order 1..70 must hold a row, stand-in or real, so a
# neighbour course the pattern above misses refuses instead of passing short.
holes=$(P -Atc "select coalesce(string_agg(g::text, ',' order by g), '') from generate_series(1, 70) g where not exists (select 1 from public.academy_apps a where a.path_order = g)")
