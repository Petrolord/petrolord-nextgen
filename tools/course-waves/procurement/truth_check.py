#!/usr/bin/env python3
"""truth-procurement.json MUST BE THE HARVEST OF THE digest.txt BESIDE IT.

numsweep resolves lesson literals against truth-*.json, which the wave's
harvest_procurement.py (the kit's harvest_digest.py plus comma-grouped figures)
builds FROM the digest. A rebuilt digest with a stale truth
...
KIT = os.environ.get('SC2_KIT', '/root/dc-wavekit')
truth = os.path.join(HERE, 'truth-procurement.json')
if not os.path.exists(truth):
    print('REFUSED: no truth-procurement.json; run harvest_procurement.py'); sys.exit(2)
tmp = tempfile.mkdtemp(prefix='sc2-truth-')
...
subprocess.run(['python3', os.path.join(HERE, 'harvest_procurement.py'), tmp], check=True, capture_output=True)
same = filecmp.cmp(truth, os.path.join(tmp, 'truth-procurement.json'), shallow=False)
n = len(__import__('json').load(open(truth)))
shutil.rmtree(tmp)
print(f'truth_check: truth-procurement.json ({n} digest lines) {"IS" if same else "IS NOT"} the harvest of digest.txt')
if '--plant' in sys.argv:
