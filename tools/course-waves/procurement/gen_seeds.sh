#!/usr/bin/env bash
# =============================================================================
# SC2 SEEDS AND CASE FILES, GENERATED FROM THE COMMITTED TREE AND NEVER FROM A
# WORKING TREE. Adapted from tools/course-waves/appliedai/gen_seeds.sh (D5).
#
# This script exports the COMMITTED tree at REF with `git archive` into a
# throwaway directory: the wave mirror tools/course-waves/procurement (the
...
if [ -d "$HERE/../../../.git" ] || [ -f "$HERE/../../../.git" ]; then
  ...
  DEFAULT_REPO=/root/wt-sc2-nextgen
fi
...
STAGE=$(mktemp -d /tmp/sc2seed.XXXXXX)
OUTDIR="$STAGE/out"
...
export SC2_WAVE="$W" SC2_REPO="$STAGE/repo" SC2_ENGINES="$STAGE/repo/packages/engines" \
       SC2_TOLERANCE="$STAGE/repo/src/components/course/panels/$SLUG/gradedTolerance.js"
( cd "$W" && SC2_COURSE_OUT="$OUTDIR/${DATE}_${PREFIX}_${SLUG}_course.sql" SC2_CASES_OUT="$OUTDIR/cases" \
...
( cd "$W" && SC2_COURSE_SQL="$OUTDIR/${DATE}_${PREFIX}_${SLUG}_course.sql" \
    SC2_GOLIVE_OUT="$OUTDIR/${DATE}_${PREFIX}_${SLUG}_go_live.sql" \
