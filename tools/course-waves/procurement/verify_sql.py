#!/usr/bin/env python3
"""Compare the SC2 procurement seed ladder against its committed sources, FIELD BY FIELD:
...
REPO = os.environ.get(
    'REPO', UP if os.path.exists(os.path.join(UP, '.git')) else '/root/wt-sc2-nextgen')
SLUG, PREFIX, DATE = 'procurement', 'sc2', '20261106'
...
cap_rows = re.split(r"\n\(\n  'procurement', ", course.split('insert into public.academy_capstones', 1)[1])[1:]
got_fields = []
for row in cap_rows:
    tier = row.split("'", 2)[1]
