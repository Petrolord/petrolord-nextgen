#!/usr/bin/env bash
# MIRROR THE EC8 WAVE INPUTS INTO THE NEXTGEN REPOSITORY, byte for byte. Every
# suite under src/components/course/panels/gsa reads the committed copy by
...
DEST="$NG/tools/course-waves/gsa"
mkdir -p "$DEST"
INPUTS=$(node -e "const w=require('$NG/tools/course-waves/waves.json');console.log(w.gsa.inputs.join('\n'))")
n=0
for f in $INPUTS; do
