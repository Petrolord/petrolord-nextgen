#!/usr/bin/env python3
"""truth-gsa.json MUST BE THE HARVEST OF THE digest.txt BESIDE IT.

numsweep resolves lesson literals against truth-*.json, which the wave's
harvest_gsa.py (the kit's harvest_digest.py plus comma-grouped figures)
...
KIT = os.environ.get('EC8_KIT', '/root/dc-wavekit')
truth = os.path.join(HERE, 'truth-gsa.json')
if not os.path.exists(truth):
    print('REFUSED: no truth-gsa.json; run harvest_gsa.py'); sys.exit(2)
tmp = tempfile.mkdtemp(prefix='ec8-truth-')
...
subprocess.run(['python3', os.path.join(HERE, 'harvest_gsa.py'), tmp], check=True, capture_output=True)
same = filecmp.cmp(truth, os.path.join(tmp, 'truth-gsa.json'), shallow=False)
n = len(__import__('json').load(open(truth)))
shutil.rmtree(tmp)
print(f'truth_check: truth-gsa.json ({n} digest lines) {"IS" if same else "IS NOT"} the harvest of digest.txt')
if '--plant' in sys.argv:
