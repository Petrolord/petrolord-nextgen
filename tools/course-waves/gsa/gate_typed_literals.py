#!/usr/bin/env python3
"""GATE: no number is TYPED into the EC8 digest.
...
gsa_dump.mjs OUTSIDE a ${...} substitution.
...
DUMP = os.path.join(os.environ.get('EC8_WAVE_DIR', os.path.dirname(os.path.abspath(__file__))), 'gsa_dump.mjs')

ALLOWED = {
 # ---------------------------------------------------------------------------
 # WHAT IS ALLOWED, AND WHY. Every figure the engine returns, every golden
 # input, every published figure (read from the golden file's `published`
 # block) and every stated probe (the PROBES declaration) reaches the digest
 # as a substitution. What is left is below. A DEAD ENTRY FAILS THE GATE.
 # ---------------------------------------------------------------------------
 '0.3048': 'the international foot in metres, the definition the cubic foot is the cube of (NIST SP 811), named beside the engine constant',
 '2100': 'a calendar year named as a year (the not-a-leap-year rule); the engine returns its day count, substituted',
 '1': 'the algebra of the Fourth Schedule formula as the Act prints it, CP = NRP x (1 + EPF)',
 '90': 'the 90-day investigation of the DGDO Regulations 2022 r.6(3), named as a rule the engine does not compute and quoted in the provisions section',
}
...
def main():
    ...
    body_src = strip_decls(body_src, ['ORDER', 'SOURCES', 'EXPORTS', 'DSRC', 'GROUPS', 'PLANTED', 'ENERGY', 'CQ', 'DAYS', 'ONE', 'ORD', 'EXP', 'CFC', 'PR', 'DP', 'DG', 'PAR', 'CAPS', 'PROBES'])
    ...
    # EC8: every rule figure in this digest is SUBSTITUTED from the engine's
