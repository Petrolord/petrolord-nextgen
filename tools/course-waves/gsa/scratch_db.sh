#!/usr/bin/env bash
# =============================================================================
# EC8 SCRATCH DATABASE. A local Postgres 16 the ladder is iterated on BEFORE any
...
# production).
#
# Adapted from tools/course-waves/pia/scratch_db.sh (EC7). The four academy
# tables the ladder writes, from the NextGen repository's own DDL, then
# production's catalogue shape: stand-ins for the courses at path_order 1 to 38
# and 53 to 58 (the economics courses among them), and the TWENTY EIGHT FC,
# Commercial & Trading, Energy Transition, Assurance, HSE, Data & AI, SC2 and
# EC7 courses read out of the COMMITTED course migrations at REF (39 to 52, 59
# to 72, SC2 procurement at 71, EC7 pia at 72), so the path_order, catalogue
# and prompt checks meet real neighbours and real capstone rows. Every
# path_order from 1 to 72 is asserted occupied, so a hole refuses rather than
# passing short. EC8 gsa joins at 73.
#
# academy_apps.module is free text in production (no check constraint, no enum);
# the scratch table mirrors that.
#
# What this cannot prove, said plainly: that production's rows match. That is
# what `dryrun_ec8.sh` with TARGET=linked is for (rolled back, snapshotted
...
#
# Usage: scratch_db.sh [ref]     (re)creates container ec8-scratch; default HEAD
#        (remove it afterwards with: docker rm -f ec8-scratch)
# =============================================================================
set -eu
REF=${1:-HEAD}
...
for f in $(git -C "$REPO" ls-tree --name-only "$REF" migrations/ \
           | grep -E '_(fc[0-9]+_[a-z]+|cr_crude|rf_refinery|tds_supply|gv_gasvalue|cef_carbon|asrc_riskchange|cq_compliance|h[1-5]_[a-z]+|d1_dataqc|d2_mlcore|d3_facies|d4_forecastml|d5_appliedai|sc2_procurement|ec7_pia)_course\.sql$'); do
  git -C "$REPO" show "$REF:$f" | P
  n=$((n + 1))
done
[ "$n" = 28 ] || { echo "REFUSED: expected the 28 seeded course migrations at $REF, found $n"; exit 2; }
# No hole: every path_order 1..72 must hold a row, stand-in or real, so a
# neighbour course the pattern above misses refuses instead of passing short.
holes=$(P -Atc "select coalesce(string_agg(g::text, ',' order by g), '') from generate_series(1, 72) g where not exists (select 1 from public.academy_apps a where a.path_order = g)")
