// GATE: nothing the EC8 capstone grades, no input it is set on and no field it
...
//
// Adapted from SC2's gate_capstone_leak.mjs. The EC8 capstone inputs are
// SYNTHETIC AGREEMENTS OF THEIR OWN (OZUBU, IFEYI, NWAKA): their own terms,
// production rows, capex and opex. What is distinctive, and swept: every
// capstone label, every run of three consecutive production figures of a
// capstone lease (oil, gas and condensate columns) as the course might print
...
//      digest.txt, gsa_dump.mjs or the golden inputs the lab imports; no
...
//   4. NAMES. No capstone name (OZUBU, IFEYI, NWAKA) in the digest, the
...
//      <repo>/src/content/courses/gsa), and THE BRIEFS (BRIEF.md,
...
const REPO = process.env.EC8_REPO || '/root/wt-ec8-nextgen';
const ENG = process.env.EC8_ENGINES || path.join(REPO, 'packages/engines');
const WAVES = process.env.EC8_WAVES || path.join(REPO, 'tools/course-waves');
...
const DUMP = read(path.join(HERE, 'gsa_dump.mjs'));
const CAPSTONE = read(path.join(HERE, 'gsa_capstone.mjs'));
const FIELDS = JSON.parse(read(path.join(HERE, 'fields.json')));
if (FIELDS.length !== 18) die(`fields.json carries ${FIELDS.length} fields`);
const INPUTS = JSON.parse(execFileSync('node', [path.join(HERE, 'gsa_capstone.mjs'), '--inputs'],
  { encoding: 'utf8', stdio: ['ignore', 'pipe', 'ignore'], maxBuffer: 1e8, env: { ...process.env, EC8_ENGINES: ENG } }));
const NAMES = Object.keys(INPUTS);
if (NAMES.join() !== 'OZUBU,IFEYI,NWAKA') die(`unexpected capstone datasets ${NAMES.join()}`);
...
const GOLDPATH = path.join(ENG, 'test-data/economics/goldens/gascontract_cases.json');
...
Object.entries(INPUTS).forEach(([name, o]) => {
  ...
  if (o.fortnight) ['nominated', 'available', 'taken'].forEach((col) => series.push({ name, k: `fortnight ${col}`, v: o.fortnight.days.map((d) => d[col]) }));
  if (o.contract) series.push({ name, k: 'taken by year', v: o.contract.years.map((y) => y.taken) });
  if (o.price) series.push({ name, k: 'index', v: o.price.months.map((m) => m.values.oil) });
  if (o.year) series.push({ name, k: 'year terms', v: Object.values(o.year.years[0]).filter((x) => typeof x === 'number') });
});
...
if (has('--plant-input')) DIGEST += `\nA planted label: ${INPUTS.IFEYI.label}.\n`;
if (has('--plant-name')) DIGEST += '\nA planted line naming Nwaka.\n';
...
[['digest.txt', DIGEST], ['gsa_dump.mjs', DUMP], ['the golden inputs', LAB_DATA]].forEach(([label, text]) => {
...
// 4. NAMES
const nameRe = /\b(ozubu|ifeyi|nwaka)\b/i;
[['digest.txt', DIGEST], ['gsa_dump.mjs', DUMP], ['the golden inputs', LAB_DATA]].forEach(([label, text]) => {
...
// 5. NEITHER FILE READS THE OTHER
[['gsa_dump.mjs', DUMP, /gsa_capstone|fields\.json/],
  ['gsa_capstone.mjs', CAPSTONE, /gsa_dump|digest\.txt/]].forEach(([label, text, re]) => {
...
if (fs.existsSync(WAVES)) {
  fs.readdirSync(WAVES).filter((w) => w !== 'gsa').forEach((w) => {
...
// 8. THE APP
const APP = ['gsaLab.js', 'QuantityCalculator.jsx', 'LedgerCalculator.jsx', 'ContractCalculator.jsx', 'panelBits.jsx']
  .map((f) => path.join(REPO, 'src/components/course/panels/gsa', f))
  .concat([path.join(REPO, 'src/pages/apps/GsaLearningPage.jsx')]);
APP.forEach((p) => {
  const text = read(p);
  count('8 app sources');
  const m = text.match(nameRe);
...
const lessonsDir = opt('--lessons') || path.join(REPO, 'src/content/courses/gsa');
const lessonFiles = [];
let lessonLits = 0;
let plantedIn = null;
