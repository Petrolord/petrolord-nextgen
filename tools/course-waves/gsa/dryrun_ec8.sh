#!/usr/bin/env bash
# =============================================================================
# EC8 DRY RUN: THE WHOLE LADDER, ROLLED BACK. Adapted from SC2's dryrun_sc2.sh.
#
...
#
# APPLYING IS NOT THIS SCRIPT'S JOB. The real apply is apply_ec8_gsa.sh
# and it is the owner's to run. The go-live is HELD and is dry-run here only.
#
# TARGETS.
#   TARGET=scratch  the local ec8-scratch container (scratch_db.sh). The default,
...
#                   The owner's to run (apply_ec8_gsa.sh prod-dryrun).
#
...
#                          ozubu_effective_swing, the SMALLEST graded
...
#                          name the field.
#   oracle                 ifeyi_2031_make_up_expired moved by ten tolerances
...
#
# Usage: dryrun_ec8.sh [ref|WORKTREE] [--control <name> [field] [mult] | --idempotent]   default HEAD
# =============================================================================
set -u
REF=${1:-HEAD}
CONTROL=${2:-}
...
if up=$(cd "$HERE/../../.." 2>/dev/null && pwd) && { [ -d "$up/.git" ] || [ -f "$up/.git" ]; }; then
  ...
  REPO=${REPO:-/root/wt-ec8-nextgen}
fi
LINKED=${LINKED:-/opt/petrolord-studio/workspaces/dev1/projects/petrolord-nextgen}
C=${SCRATCH:-ec8-scratch}
RUN=$(mktemp -d /tmp/ec8dry.XXXXXX)
SLUG=gsa
GOLIVE=20261109_ec8_gsa_go_live
FILES="20261109_ec8_gsa_course
20261109_ec8_gsa_beginner_deep
20261109_ec8_gsa_intermediate_deep
20261109_ec8_gsa_advanced_deep
$GOLIVE"
KEYS="ozubu_march_2028_mmbtu:beginner ozubu_2028_acq:beginner ozubu_effective_swing:beginner ozubu_fortnight_buyer_shortfall:beginner ozubu_fortnight_seller_shortfall:beginner ozubu_2029_deficiency_payment:beginner ifeyi_2030_average_price:intermediate ifeyi_2028_deficiency_payment:intermediate ifeyi_2031_make_up_taken:intermediate ifeyi_2031_make_up_expired:intermediate ifeyi_total_net_to_seller:intermediate ifeyi_2031_dgdo_penalty:intermediate nwaka_july_2029_price:advanced nwaka_2034_average_price:advanced nwaka_2030_carry_forward_credit:advanced nwaka_2035_refund:advanced nwaka_npv_seller_revenue:advanced nwaka_2032_royalty:advanced"
...
union all select 'path_order 73 held', count(*)::text from public.academy_apps where path_order = 73
...
union all select 'gsa apps',        count(*)::text from public.academy_apps where slug = 'gsa'
union all select 'gsa any',         (select count(*) from public.academy_course_structures where app_slug = 'gsa')
                                     + (select count(*) from public.academy_quiz_questions where app_slug = 'gsa')
                                     + (select count(*) from public.academy_capstones where app_slug = 'gsa') || ''
...
case "$CONTROL:$CTL" in
  ...
  --control:ledger|--control:route)
    FIELD=${FIELD_ARG:-ozubu_effective_swing}
  ...
  --control:oracle)
    ...
    FIELD=ifeyi_2031_make_up_expired; TIER=intermediate
 ...
 where c.app_slug = '$SLUG' and c.tier = '$TIER';"
    ...
    ;;
  --control:trap)
    # The Professional make-up drawn newest first, read from the engine's own
    # wrong-method sweep,
    # with the ledger, the second route and the oracle cut out so ONLY a trap
    # can object.
    FIELD=ifeyi_2031_make_up_expired; TIER=intermediate
    WRONG=$(EC8_WAVE_DIR=$HERE node "$HERE/discriminate.mjs" --json 2>/dev/null | python3 -c "import json,sys; print(repr(json.load(sys.stdin)['$FIELD']['wrong']['makeup_lifo']))")
 ...
 where c.app_slug = '$SLUG' and c.tier = '$TIER';"
    ...
    WANT="the trap \\(makeup lifo\\) reads .*so the field does not discriminate the trap \\[graded field: $TIER/$FIELD\\]"
    ;;
  --control:prompt)
    PRE_GOLIVE="update public.academy_capstones set prompt = replace(prompt, '(refund)', '(forfeit)')
 where app_slug = '$SLUG' and tier = 'advanced';"
    WANT="the advanced prompt is not the prompt gen_course.py rendered"
    ;;
  --control:leak)
    PLANT=$(python3 -c "import json; f=[x for x in json.load(open('$HERE/fields.json')) if x[1]=='ifeyi_2031_make_up_expired'][0]; print(f'{f[2]:.6f}')")
    PRE_GOLIVE="update public.academy_capstones c
   set fields = (select jsonb_agg(case when f->>'key' = 'ozubu_2029_deficiency_payment'
                                       then jsonb_set(f, '{label}', to_jsonb((f->>'label') || ', against a benchmark of $PLANT'))
                                       else f end)
                   from jsonb_array_elements(c.fields) f)
 where c.app_slug = '$SLUG' and c.tier = 'beginner';"
    WANT="intermediate/ifeyi_2031_make_up_expired in the beginner capstone text"
    ;;
  --control:sweep)
    fails=0
    for kt in $KEYS; do
...

{
  ...
  if [ "$pass" = 2 ]; then
    echo "create temp table ec8_pass1 as select * from ($(tr '\n' ' ' < "$RUN/snap.sql" | sed 's/;[[:space:]]*$//')) s;"
  fi
  for f in $FILES; do
    echo "-- ================= $f"
    if [ "$f" = "$GOLIVE" ]; then
  ...
  if [ "$pass" = 2 ]; then
    echo "do \$\$ declare v_n int; begin select count(*) into v_n from ((select * from ec8_pass1 except select * from ($(tr '\n' ' ' < "$RUN/snap.sql" | sed 's/;[[:space:]]*$//')) s) union all (select * from ($(tr '\n' ' ' < "$RUN/snap.sql" | sed 's/;[[:space:]]*$//')) s except select * from ec8_pass1)) d; if v_n <> 0 then raise exception 'EC8 go-live refused: IDEMPOTENCE, the second pass of the ladder moved % snapshot row(s)', v_n; end if; raise notice 'IDEMPOTENT: the second pass left every count and digest where the first left it'; end \$\$;"
  fi
  done
  cat <<'SQL'
select 'DRY RUN' as what,
       'gsa ' || a.status
       || ' | ' || (select count(*) from public.academy_course_structures where app_slug='gsa' and active)::text || ' tiers'
       || ' | ' || (select count(*) from public.academy_course_structures s,
                           lateral jsonb_array_elements(s.structure->'modules') m,
                           lateral jsonb_array_elements_text(m->'lesson_keys') lk
                     where s.app_slug='gsa' and s.active)::text || ' lessons'
       || ' | ' || (select count(*) from public.academy_quiz_questions where app_slug='gsa')::text || ' questions ('
       || (select string_agg(n::text, '/' order by ord) from (
             select case tier when 'beginner' then 1 when 'intermediate' then 2 else 3 end ord, count(*) n
               from public.academy_quiz_questions where app_slug='gsa' group by tier) t) || ')'
       || ' | ' || (select count(*) from public.academy_capstones where app_slug='gsa')::text || ' capstones'
       || ' | ' || (select count(*) from public.academy_capstones c, lateral jsonb_array_elements(c.fields) f
                     where c.app_slug='gsa')::text || ' graded'
       ...
       || ' coming_soon' as v
  from public.academy_apps a where a.slug = 'gsa';
rollback;
SQL
...
if [ -n "$CONTROL" ] && [ "$CONTROL" != --idempotent ]; then
  ...
  if [ $FAILED = 1 ] && grep -qE "EC8 go-live refused.*$WANT" "$RUN/out.txt"; then
    echo "CONTROL $CTL FIRED:"
    grep -oE "EC8 go-live refused[^\"]*" "$RUN/out.txt" | head -2 | cut -c1-400 | sed 's/^/    /'
    exit 0
  fi
...
grep -q 'gsa available | 3 tiers | 78 lessons | 396 questions' "$RUN/out.txt" \
