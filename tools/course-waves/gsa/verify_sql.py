#!/usr/bin/env python3
"""Compare the EC8 gsa seed ladder against its committed sources, FIELD BY FIELD:
...
REPO = os.environ.get(
    'REPO', UP if os.path.exists(os.path.join(UP, '.git')) else '/root/wt-ec8-nextgen')
SLUG, PREFIX, DATE = 'gsa', 'ec8', '20261109'
...
cap_rows = re.split(r"\n\(\n  'gsa', ", course.split('insert into public.academy_capstones', 1)[1])[1:]
got_fields = []
for row in cap_rows:
    tier = row.split("'", 2)[1]
