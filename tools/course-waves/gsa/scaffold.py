#!/usr/bin/env python3
"""Lay out the EC8 gsa content tree from structure.py: the three tier
...
it left alone.
    ...
    python3 scaffold.py [--repo /root/wt-ec8-nextgen] [--check]
...
content = os.path.join(repo, 'src/content/courses/gsa')

wrote = kept = manifests = drift = 0
for tier, modules in S.TIERS.items():
    manifest = {'app_slug': 'gsa', 'tier': tier, 'content_version': 1, 'modules': []}
