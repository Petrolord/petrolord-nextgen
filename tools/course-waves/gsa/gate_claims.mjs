...
// wave's RECON REPORT rather than from the digest, and both were wrong. In EC8 the
// engine's FINDINGS record, the oracle and its golden and the
// engine's source comments are PROVENANCE, and none of them is teaching
// truth. (Adapted from EC7's and SC2's gate_claims.mjs, itself D5's, D4's, D3's, D2's,
// D1's, H1's and FC9's.)
//
...
const RECON = (() => { try { return fs.readFileSync(`${ENG}/test-data/economics/ekene-gsa/README.md`, 'utf8'); } catch { return null; } })();
const FINDINGS = (() => { try { return fs.readFileSync(`${ENG}/tools/validation/economics/FINDINGS-gasContract.md`, 'utf8'); } catch { return null; } })();
...
const ALLOWED = {
  73: 'the path order of the course in the economics module, settled by wave.json pathOrder and the plan of record',
...
BRIEFS.forEach((f) => {
  ...
  if (PLANT && f === 'BRIEF.md') {
    ...
    text += '\n\nThe fixture plants a deficiency of 688,800 in the year of the outage, at 2.18 a unit, and 1,501,584 in all.\n';
  }
  briefsRead += 1;
...
check('the legislated words are all in the vocabulary section',
  ['shortfall', 'deficiency', 'make-up', 'carry-forward', 'Adjusted ACQ', 'domestic base price'].every((w) => VOCAB.includes(`| ${w} |`)),
...
check('the BRIEF tier table matches the owners the digest prints',
  (() => {
    ...
    return heads.length === 24 && Object.entries(own).every(([t, a]) => brief.includes(`| ${t} |`) && brief.includes(`| ${phrase(a)} |`));
  })(), 'section owner clauses against the BRIEF table');
check('the refusal count the BRIEF states is the digest\'s',
  (() => { const m = DIGEST.match(/^(\d+) refusals across (\d+) functions:/m); return m && (read('BRIEF.md') || '').replace(/\s+/g, ' ').includes(`tables ${m[1]} refusals across ${m[2]} functions`); })(), 'BRIEF.md against the refusal section');
check('the BRIEF quotes the four stated readings verbatim, as the digest prints them',
  (() => {
    const b = (read('BRIEF.md') || '').replace(/\s+/g, ' ');
    const rs = [...DIGEST.matchAll(/^\| \d\. ([^|]+) \| (?:dailyBalance|takeOrPay|gsaCashFlows) basis/gm)].map((m) => m[1].trim());
    return rs.length === 4 && rs.every((r) => b.includes(`"${r}"`)) && Object.keys(WAVE.openReadings || {}).length === 5;
  })(), 'BRIEF.md against the readings section and wave.json openReadings');
check('the BRIEF states the domestic base price rule and the flare rule',
  /quoted only as reported/.test(read('BRIEF.md') || '') && /Flaring is concept only/.test(read('BRIEF.md') || ''), 'BRIEF.md');
