...
any lesson body or manifest title, and in the three EC8 calculator panels, their shared bits and the lab
...
<wave>/banks, EC8_BANKS to override). Headings included.
...
BANKS = os.environ.get('EC8_BANKS', os.path.join(HERE, 'banks'))
APP_TEXT = [os.path.join(REPO, 'src/components/course/panels/gsa', f) for f in
...
ENGINE_TEXT = {
    # A VERBATIM QUOTATION OF THE GAZETTED ACT, checked against the text by
    # quote_check.py: PIA s.110(14)(a) prints "under subsection (8), not be
    # entitled". It is the Act's own sentence and is quoted exactly.
    'in addition to the penalties provided under subsection (8), not be entitled to supply natural gas to any new midstream gas export operations,': 'PIA s.110(14)(a), verbatim',
}
...
def main():
    ...
    if os.path.isdir(BANKS):
        names = sorted(n for n in os.listdir(BANKS) if re.match(r'^ec8[bia]_.*\.json$', n))
        planted_only = False
        if '--plant-bank' in sys.argv and not names:
            # THE FOUNDATION HAS NO BANK JSON YET. The control still has to prove
            # the bank sweep fires, so it sweeps one in-memory bank of one string.
            names, planted_only = ['ec8b_planted.json'], True
        for n in names:
            data = {'questions': [{'explanation': 'A planted bank string.'}]} if planted_only else json.load(open(os.path.join(BANKS, n), encoding='utf-8'))
            strs = []
            ...
            files += 1
        if names and bank_strings == 0 and not planted_only:
    ...
    if '--plant-bank' in sys.argv:
        caught = len([f for f in bad if f[0].startswith("ec8")])
