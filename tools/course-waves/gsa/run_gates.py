#!/usr/bin/env python3
"""RUN EVERY EC8 FOUNDATION GATE, WITH ITS NEGATIVE CONTROL, AND PIN WHAT RAN.
...
count the digest does not have.
    ...
    EC8_STAGE=lessons python3 run_gates.py   the same, once lesson prose exists
    EC8_STAGE=banks python3 run_gates.py     and every bank gate over the 21 banks
...
any other reason still fails the run. EC8_STAGE=lessons makes both expect 0.
...
STAGE = os.environ.get('EC8_STAGE', 'foundation')
FOUNDATION = STAGE == 'foundation'
# At the foundation, the stub reason each lesson gate must print to count as met.
STUB_REASON = {
    'numsweep_gsa.mjs': r'held NOT ONE literal of seven or more significant figures',
...
PY_ORACLE = 'python3'  # the EC8 oracle is standard library only
...
RUNS = [
    ...
    ('gsa_capstone.mjs', f'{HERE}/gsa_capstone.mjs', ['node', f'{HERE}/gsa_capstone.mjs'], 0),
    ...
    ('digestpromise_gsa.py', f'{HERE}/digestpromise_gsa.py', ['python3', f'{HERE}/digestpromise_gsa.py'], 0),
    ('digestpromise_gsa.py NEGATIVE CONTROL', f'{HERE}/digestpromise_gsa.py', ['python3', f'{HERE}/digestpromise_gsa.py', '--plant'], 1),
    ...
    ('gate_vocabulary.py NEGATIVE CONTROL price', f'{HERE}/gate_vocabulary.py', ['python3', f'{HERE}/gate_vocabulary.py', '--plant-price'], 1),
    ...
    ('numsweep_gsa.mjs', f'{HERE}/numsweep_gsa.mjs', ['node', f'{HERE}/numsweep_gsa.mjs'], 1 if FOUNDATION else 0),
    ('numsweep_gsa.mjs --selftest', f'{HERE}/numsweep_gsa.mjs', ['node', f'{HERE}/numsweep_gsa.mjs', '--selftest'], 0),
...
if STAGE in ('lessons', 'banks', 'full', 'final'):
    LESSONS = f'{REPO}/src/content/courses/gsa'
...
# THE BANK STAGE. EC8_STAGE=banks (or full) runs everything above AND every bank
...
if BANKS:
    BK = f'{HERE}/banks'
    TIERS = [("ec8b", "beginner"), ("ec8i", "intermediate"), ("ec8a", "advanced")]
    ...
    RUNS += [
        ...
        ('numsweep_gsa.mjs --banks', f'{HERE}/numsweep_gsa.mjs', ['node', f'{HERE}/numsweep_gsa.mjs', '--banks', BK], 0),
...
# THE FINAL (SHIP) STAGE. EC8_STAGE=final runs everything above AND the seed
...
if STAGE == 'final':
    MIG = f'{REPO}/migrations/20261109_ec8_gsa_course.sql'
    RUNS += [
        ...
        ('promptleak.py --sql', f'{KIT}/promptleak.py', ['python3', f'{KIT}/promptleak.py', '--sql', MIG, '--course', 'gsa'], 0),
    ]
