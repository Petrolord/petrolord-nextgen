...
const SOURCES = [
  ...
  ['NMDPRA domestic base price 2026 (and 2025), as reported', 'circular effective 1 April 2026, not retrieved; reported by BusinessDay (31 March 2026) and by Advocaat Law Practice through Legal 500 (7 April 2026)', 'a required input of domesticPrice; the reported figures appear only as reported (' + 'the next paragraph' + ')', 'BusinessDay, 31 March 2026'],
...
w();
w(`WHAT IS NEVER GRADED. No graded figure depends on a reading the engine states (${ref('readings')}): every capstone field is the same number under either side of each reading. No graded figure uses the domestic base price, which is quoted only as reported (${ref('sources')}).`);
w();
...
w();
w('A DAY COUNT IS A STATED TERM. The engine takes a stated number of days, a calendar year (365 days, or 366 in a leap year; 2100 is not a leap year and 2000 is), or a period whose end date is excluded, as a contract year that finishes on the following 1 January. A call that states more than one day count, or none, is refused.');
w();
const hm = runG('cq-hmrc-ot05402-effective-swing');
const hmA = argsOf('cq-hmrc-ot05402-effective-swing');
w(`EFFECTIVE SWING (golden input ${hmA.maxDcqPct} and ${hmA.topPct}): MaxDCQ percent / take-or-pay percent = ${f6(hm.effectiveSwing)} (engine). HMRC's manual prints the same division as 1.66 and the swing above the take-or-pay level as a factor of 6.6 (text: OT05402); the manual's 1.66 is the engine's figure cut after two decimals (derived: ${f6(Math.floor(hm.effectiveSwing * 100) / 100)}), and its 6.6 is (1.66 - 1) x 10 (derived). The course quotes the engine's figure and names the manual's printed one as the manual's.`);
must('the manual truncates: floor(100 x swing) / 100 = 1.66', Math.floor(hm.effectiveSwing * 100) / 100 === 1.66 && hm.effectiveSwing !== 1.66, hm.effectiveSwing);
w();
const cqz = runG('cq-top-zero-without-swing');
w(`A take-or-pay percentage of 0 is accepted when no MaxDCQ is stated (golden input cq-top-zero-without-swing: take-or-pay quantity ${f6(cqz.topQuantity)}, effective swing ${f6(cqz.effectiveSwing)}); with a MaxDCQ stated the effective swing would divide by it, so that call is refused (${ref('refusals')}).`);
w();
...
w();
w(`THE READING THIS SECTION RESTS ON. The engine measures a seller shortfall against the quantity the seller made AVAILABLE, so on daily-available-not-taken (100 made available, 60 taken) the seller shortfall is ${f6(ant.sellerShortfall)} and the buyer shortfall ${f6(ant.buyerShortfall)}. ${refCap('readings')} quotes the reading and the model formula it departs from.`);
...
must('the four-decimal rule: 11.23459 -> 11.2346, 11.23449 -> 11.2345, 100.00005 -> 100.0001, 11.234346 -> 11.2343', ['2026-03', '2026-04', '2026-05', '2026-07'].map((m) => rd.months.find((x) => x.month === m).price).join() === '11.2346,11.2345,100.0001,11.2343', rd.months.map((m) => m.price).join());
w('THE FOUR-DECIMAL RULE on price-round-model-4dp: a fifth decimal of 5 or more rounds the fourth up (11.23459 prints 11.234600, 100.00005 prints 100.000100), 4 or less leaves it (11.23449 prints 11.234500), and the price is computed in full before the rule reads its fifth decimal (11.234346 prints 11.234300). The prices above are the engine\'s.');
w();
...
w(`Every domesticPrice call states the domestic base price; ${ref('sources')} says where the reported figures come from and why none is graded. The rows below state 2.18 and 2.13 as the figures reported for 2026 and 2025, and other figures as stated probes.`);
w();
...
const dbpLabel = (v) => (v === 2.18 ? `${f6(v)} (reported 2026)` : v === 2.13 ? `${f6(v)} (reported 2025)` : `${f6(v)} (stated)`);
...
must('power 2026 = 2.18 and commercial 2026 = 2.68 on the reported base price', runG('dp-power-2026').price === 2.18 && Math.abs(runG('dp-commercial-2026').price - 2.68) < 1e-12, 'dp');
w();
w(`THE FOURTH SCHEDULE ARITHMETIC (dp-gbi-urea-inside): EPF = (CMPP - PRP) / PRP and CP = NRP x (1 + EPF), then held at or below the domestic base price and at or above US$0.90 per MMBtu. The ceiling applies before the floor. ${refCap('provisions')} quotes the Schedule.`);
...
w();
w(`THE PRINTED SLOPE AND THE EXACT ONE. The Energy Charter Secretariat prints the heat-equivalence slope as 0.172 (text). On 5.8 MMBtu per barrel the engine returns ${f6(p58.slope)}; 1 / 0.172 = ${f6(1 / 0.172)} MMBtu per barrel (derived), so the printed slope is a rounded figure. On the EIA's 5,689,000 Btu per barrel (text) the slope is ${f6(runG('parity-eia-2026').slope)}. A contract slope below the parity slope prices gas below oil on heat content.`);
must('0.172 is not the engine slope at 5.8', p58.slope !== 0.172, p58.slope);
w();
...
w();
w(`The plateau levels follow from the printed parameters: ${f6(ecs.months[0].price)} at and below the lower kink and ${f6(ecs.months[4].price)} above the upper kink, inside the figure's printed axis of 2.50 to 5.50 (text).`);
w();
...
w();
w('Both NPVs above are the canonical npv of engines/economics/cashflow.ts on the seller revenue rows, year-end flows (checked when this digest is built). The royalty rate is the canonical deriveGasRoyaltyRate of the same file: 5 percent, and 2.5 percent on gas utilised in-country (PIA Seventh Schedule para 10(6)); the pia course teaches the fiscal system that rate sits in.');
const cfz = runG('cf-small-zero-rate');
w();
w(`A ZERO DISCOUNT RATE (golden input cf-small-zero-rate, deep offshore, ${S(argsOf('cf-small-zero-rate').royalty.inCountrySharePct)} percent in-country): royalty rate ${f6(cfz.royaltyRate)}; NPV of the seller revenue ${f6(cfz.npvSellerRevenue)}, which at a rate of 0 is the plain sum of the rows (${f6(sum(cfz.years.map((y) => y.sellerRevenue)))}, derived).`);
...
w(`READING 2 ACTS in a year that draws a carry-forward credit. On top-carry-forward-capped, ${S(capY.year)}: deficiency ${f6(capY.deficiency)}, carry-forward credit ${f6(capY.carryForwardApplied)}, deficiency paid ${f6(capY.deficiencyPaid)}; in ${S(capNext.year)} the make-up available is ${f6(capNext.makeUpAvailable)}, the deficiency paid (${ref('carryforward')}). The model agreement's Make-Up Aggregate sums the deficiency quantities before any credit.`);
w();
const eot = runG('top-end-of-term-refund');
const eotLast = eot.years[eot.years.length - 1];
w(`READING 3 ACTS in a last contract year with a deficiency. On top-end-of-term-refund, ${S(eotLast.year)}: the reasons read, verbatim:`);
...
w(`READING 4 ACTS in a year with a deficiency payment. On cf-power, 2028: deficiency payment ${f6(cfp28.lines.deficiencyPayment)}, delivered value ${f6(cfp28.deliveredValue)}, royalty ${f6(cfp28.royalty)}; the royalty is ${f6(cfp28.royaltyRate)} x the delivered value (the gas pays its royalty when it is made up).`);
...
w();
w(`PRINTED FIGURES AND EXACT FIGURES. HMRC prints 150 / 90 = 1.66 (the engine: ${f6(hm.effectiveSwing)}, ${ref('quantities')}); the Energy Charter Secretariat prints the parity slope 0.172 (the engine on 5.8 MMBtu per barrel: ${f6(p58.slope)}, ${ref('parity')}). A reason the engine prints carries the double it holds: the export refund reads, verbatim,`);
...
w();
w('THE BTU AND THE CUBIC FOOT. The engine uses the International Table Btu, 1055.05585262 J, exact by definition (NIST SP 811 prints it as 1.055 056 E+03 J). The model agreement defines its Btu at 59 F to 60 F, about 1054.80 J, about 0.02 percent smaller (FINDINGS, read from the text); a contract on that Btu should state its heating value in those units. The cubic foot is (0.3048 m) cubed, 0.028316846592 cubic metres, exact.');
must('FINDINGS records the 59 F Btu figure', /about 1054\.80 J, 0\.02% smaller/.test(FINDINGS), '59F');
w();
w('THE FOURTH SCHEDULE\'S TABLE HEADER. The gazetted table heads its NRP column "US $/mmbtu" and, beneath it, "Net of transport Tariff US $/Kcf" (text); the formula above the table states NRP as US$1 per MMBtu, and the engine holds NRP in US$ per MMBtu.');
...
table(['rule', 'at the boundary (probed)', 'engine result'], [
  ...
  ['make-up expiry', 'the last year of the period, y + N (top-makeup-on-last-day-of-period)', 'usable in that year; the rest expires at its end; none in y + N + 1 (top-makeup-one-year-late)'],
  ['make-up period 0', 'a deficiency (top-makeup-period-zero)', `paid (${f6(pz.years[0].deficiencyPayment)}), no make-up right`],
  ...
  ['seller shortfall on a day', `(PNQ - tolerance) - available EQUAL to 0 (daily-tolerance-covers-the-gap)`, `no shortfall (${f6(tolCover.sellerShortfall)}); one unit more (daily-tolerance-one-short) gives ${f6(tolShort.sellerShortfall)}`],
  ...
  ['zero nomination', `a nomination of 0 with an adjusted DCQ above 0 (January 2027, the 5th)`, `the whole adjusted DCQ is a buyer shortfall (${f6(zeroNom.buyerShortfall)})`],
  ...
  ['four-decimal rounding', 'a fifth decimal of 5', 'rounds up (11.23459 to 11.2346)'],
  ...
  ['gas based industries', `a formula price EQUAL to the base price, or to 0.90 (dp-gbi-exactly-dbp, dp-gbi-exactly-floor)`, `not held (held at: ${gbiDbp.heldAt ?? 'none'}, ${gbiFl.heldAt ?? 'none'}); the ceiling applies before the floor`],
  ...
  ['DGDO agreement rate', `an agreement rate EQUAL to 3.50 (dgdo-agreement-equal)`, `the agreement's rate (${f6(agrEq.rate)})`],
  ['DGDO excuses', 'excuses stated above the undelivered quantity (dgdo-all-excused)', 'applied only up to it, in the order (a) to (d)'],
  ['period day count', 'the end date of a period', 'excluded (a contract year that finishes on the following 1 January)'],
  ['maxDcqPct', 'EQUAL to 100', 'allowed'],
]);
...
const CAP_PROBE = { years: [{ year: 2027, acq: 1000, taken: 1100, contractPrice: 3, topPrice: 3, makeUpPrice: 0 }, { year: 2028, acq: 1000, taken: 700, contractPrice: 3, topPrice: 3, makeUpPrice: 0 }], topPct: 80, makeUp: { periodYears: 0, order: 'after-adjusted-acq', endOfTerm: 'forfeit' }, carryForward: { periodYears: 1, base: 'top-quantity', capPct: 50 } };
const capProbe = success('takeOrPay at the carry-forward cap (stated probe)', G.takeOrPay(clone(CAP_PROBE)));
must('a credit equal to the cap is applied in full', capProbe.years[1].carryForwardApplied === 50 && capProbe.years[1].deficiency === 100 && capProbe.years[1].carryForwardAvailable > 50, JSON.stringify(capProbe.years[1]).slice(0, 200));
const FLOOR_PROBE = { months: [{ month: '2027-01', values: { oil: 30 } }, { month: '2027-02', values: { oil: 29 } }], formula: { type: 'oil-indexed', index: 'oil', slope: 0.1, constant: 1, floor: 4, ceiling: 9 }, from: '2027-01', to: '2027-02' };
const floorProbe = success('priceSeries with a raw price equal to the floor (stated probe)', G.priceSeries(clone(FLOOR_PROBE)));
must('a raw price equal to the floor is not labelled clamped; below it is', floorProbe.months[0].price === 4 && floorProbe.months[0].clamped === null && floorProbe.months[1].clamped === 'floor', JSON.stringify(floorProbe.months.map((m) => [m.price, m.clamped])));
...
const cq100 = G.contractQuantities({ dcq: 100, days: 365, maxDcqPct: 100, topPct: 80 });
must('maxDcqPct = 100 is allowed (stated probe)', !cq100.error && cq100.maxDcq === 100, JSON.stringify(cq100).slice(0, 80));
const nomMax = G.dailyBalance({ dcq: 100, maxDcqPct: 120, days: [{ date: '2027-03-01', nominated: 120, available: 120, taken: 120 }] });
must('a nomination equal to MaxDCQ is properly nominated in full (stated probe)', !nomMax.error && nomMax.days[0].properlyNominated === 120 && nomMax.days[0].reasons.every((r) => !/not properly nominated/.test(r)), JSON.stringify(nomMax.days[0]));
w();
w(`Four probes above are stated in this file. contractQuantities with dcq 100, 365 days, maxDcqPct 100 and topPct 80 returns MaxDCQ ${f6(cq100.maxDcq)}. dailyBalance with dcq 100, maxDcqPct 120 and a nomination of 120 made available and taken returns a properly nominated quantity of ${f6(nomMax.days[0].properlyNominated)}. takeOrPay on two years of ACQ 1000 at 80 percent, taken 1100 then 700, with carry-forward of one year above the take-or-pay quantity capped at 50 percent and a make-up period of 0, applies a credit of ${f6(capProbe.years[1].carryForwardApplied)} against a deficiency of ${f6(capProbe.years[1].deficiency)}. priceSeries on 1 + 0.1 x oil with a floor of 4 and a ceiling of 9 prices oil 30 at ${f6(floorProbe.months[0].price)}, clamped ${floorProbe.months[0].clamped ?? 'none'}, and oil 29 at ${f6(floorProbe.months[1].price)}, clamped ${floorProbe.months[1].clamped}.`);
...
w();
w('FIGURES QUOTED ONLY AS REPORTED. The domestic base price for 2026 (US$2.18 per MMBtu for power, US$2.68 for the commercial sector, effective 1 April 2026) and for 2025 (2.13 and 2.63) are reported figures; the regulator\'s circular was not read. The engine\'s own sentence is in ' + ref('sources') + '. A lesson quotes them with that attribution and never as a value the course grades.');
...
w();
w('A panel stays well inside these caps. FINDINGS records that a call at every cap stays well under 100 ms on the build host.');
...
table(['convention', 'the engine\'s choice', 'where it comes from'], [
  ['the Btu', 'the International Table Btu, 1055.05585262 J', 'NIST SP 811 (2008) Appendix B; the model agreement\'s 59 F Btu is about 0.02 percent smaller'],
  ...
  ['the index window', 'the mean of averagingMonths months ending lagMonths months before the delivery month; lag 0 ends it at the delivery month', 'engine convention; the model agreement ends it one month before the review month'],
  ['the reset', 'every month of a block carries the price of the block\'s first month, counted from `from`', 'engine convention'],
  ['the four-decimal rule', 'the double normalised to 12 significant digits before the fifth decimal decides', 'Commonwealth model GSA Article 15.4, with the normalisation stated'],
...
table(['term', 'what it can mean elsewhere', 'the rule here'], [
  ...
  ['deficiency', 'any shortage', 'the take-or-pay quantity less the quantity counted, when above 0; the money is the deficiency payment'],
