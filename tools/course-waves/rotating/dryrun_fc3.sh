#!/usr/bin/env bash
# =============================================================================
# FC3 DRY RUN: THE WHOLE LADDER, ROLLED BACK.
#
...
# evaluate, and a held-for-literature gate refusing the very field that proves
# the hold. FC2's dry run caught the last two.
#
...
# apply_fc3_rotating.sh and it is the owner's to run.
#
# THE NEGATIVE CONTROL. With --control, one graded field is moved by ONE PART
# IN 1e7 inside the same rolled-back transaction, and the go-live must REFUSE
# it BY NAME. A dry run that passes proves nothing unless the same run can be
# made to fail.
#
# Usage: dryrun_fc3.sh [ref] [--control [<field key>]]      default HEAD
# =============================================================================
set -u

REF=${1:-HEAD}
CONTROL=${2:-}
FIELD=${3:-bonny_stage1_gas_hp}
# The size of the move. One part in 1e7 by default. ONE FIELD IS NOT ASSERTED
# TO THAT PRECISION and says so: the driver fuel carries the whole train, whose
# later stages hold a compressibility no closed form reaches, so it is asserted
# to a BAND. Passing a larger multiplier measures what that band does catch,
# which is the honest way to state a weaker assertion.
MULT=${4:-1.0000001}
REPO=/root/wt-fc3-nextgen
LINKED=/opt/petrolord-studio/workspaces/dev1/projects/petrolord-nextgen
RUN=$(mktemp -d /tmp/fc3dry.XXXXXX)

FILES="20260923_fc3_rotating_course
...
union all select 'rotating any',     count(*)::text from public.academy_apps where slug = 'rotating'
...
# ------------------------------------------------- the ladder, then rollback
{
  ...
  for f in $FILES; do
    ...
    echo
    if [ "$CONTROL" = --control ] && [ "$f" = 20260923_fc3_rotating_advanced_deep ]; then
      # THE NEGATIVE CONTROL, injected AFTER the seeds and BEFORE the go-live,
      # so the go-live reads a capstone that has been moved by a factor of $MULT
      # and by nothing else. A factor rather than a round number: the point is
      # that a tiny move is caught, not a visible one.
      cat <<CTL
-- ================= NEGATIVE CONTROL: move $FIELD by one part in 1e7
...
select 'DRY RUN' as what,
       'rotating ' || a.status
       || ' | ' || (select count(*) from public.academy_course_structures where app_slug='rotating' and active)::text || ' tiers'
       || ' | ' || (select count(*) from public.academy_course_structures s,
                           lateral jsonb_array_elements(s.structure->'modules') m,
                           lateral jsonb_array_elements_text(m->'lesson_keys') lk
                     where s.app_slug='rotating' and s.active)::text || ' lessons'
       || ' | ' || (select count(*) from public.academy_quiz_questions where app_slug='rotating')::text || ' questions ('
       || (select string_agg(n::text, '/' order by ord) from (
             select case tier when 'beginner' then 1 when 'intermediate' then 2 else 3 end ord, count(*) n
               from public.academy_quiz_questions where app_slug='rotating' group by tier) t) || ')'
       || ' | ' || (select count(*) from public.academy_capstones where app_slug='rotating')::text || ' capstones'
       || ' | ' || (select count(*) from public.academy_capstones c, lateral jsonb_array_elements(c.fields) f
                     where c.app_slug='rotating')::text || ' graded'
       ...
       || ' coming_soon' as v
  from public.academy_apps a where a.slug = 'rotating';
rollback;
SQL
...
[ "$CONTROL" = --control ] && echo "### WITH THE NEGATIVE CONTROL ON $FIELD, moved by a factor of $MULT"
...
if [ "$CONTROL" = --control ]; then
  if [ $FAILED = 1 ] && grep -q "$FIELD" "$RUN/out.txt"; then
    echo "CONTROL FIRED: the go-live refused, and named $FIELD."
    exit 0
  fi
  echo "CONTROL DID NOT FIRE. A move of $MULT on $FIELD passed the go-live, so the go-live does not catch a move that size on that field."
  exit 2
fi
if [ $FAILED = 1 ]; then
