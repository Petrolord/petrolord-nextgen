...
every field of every question, verbatim, with the committed bank JSON: the
slug, the tier, the scope, the module key, the ordinal, the prompt, all four
options IN ORDER, the answer index and the explanation. Twelve comparisons per
question, 1,584 per tier, 4,752 across the course, plus the structure row
rebuilt from the tier's manifest and the 36 expected values and tolerances of
the capstone rows.
...
moment a writer uses the phrase, and reports a clean result for a corpus it
never read.
...
for tier in TIERS:
    ...
    srow = parse_rows(sql, STRUCTURE_MARKER[:-len('values ')] + 'values ', 4)[0]
    structure = srow[2]
...
i = 0
while True:
    j = course.find("jsonb_build_object('key','", i)
    if j < 0:
        break
    # The key VALUE, not the literal 'key' that names the column: the quote to
    # start at is the one after "jsonb_build_object('key',".
    key, k = sql_string(course, j + len("jsonb_build_object('key',"))
    k = course.index("'expected',", k) + len("'expected',")
    end = course.index(", 'tol',", k)
    exp = float(course[k:end])
    k = end + len(", 'tol',")
    tol = float(course[k:course.index(')', k)])
    sqlf.append((key, exp, tol))
    i = j + 1
...
CATALOGUE = ("values ('rotating', 'Rotating Equipment', 'facilities', 41, "
             "'coming_soon', null)")
compared += 1
if CATALOGUE not in course:
    disagree.append('DISAGREE the catalogue row is not rotating/facilities/41/coming_soon with no prerequisite')
