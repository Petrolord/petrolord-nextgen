...
# ---- the walk ------------------------------------------------------------
SEED=__tests__/hse.exposure.test.js
...
for extra in \
  "tools/validation/hse/oracle_exposure.py:writes the golden the suite reads" \
  "tools/validation/hse/FINDINGS-exposure.md:the record of sources, errata and judgement calls J1-J10 the suite's header cites" \
  "tools/validation/hse/negcontrol_exposure.sh:the negative controls FINDINGS section 6 reports (mode 755 upstream, kept)"; do
  p="${extra%%:*}"; why="${extra#*:}"
