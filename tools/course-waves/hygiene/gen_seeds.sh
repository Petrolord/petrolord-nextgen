#!/usr/bin/env bash
# =============================================================================
# H2 SEEDS, GENERATED FROM THE COMMITTED TREE AND NEVER FROM A WORKING TREE.
#
...
# seed that matches nothing a reviewer can read. H2's three key-truth audits run
...
# engines/hse/exposure.js and the one tolerance module gradedTolerance.js are
...
# generators are mirrored into tools/course-waves/hygiene and not yet committed,
...
if [ -d "$HERE/../../../.git" ] || [ -f "$HERE/../../../.git" ]; then
  ...
  DEFAULT_REPO=/root/wt-h2-nextgen
fi
...
STAGE=$(mktemp -d /tmp/h2seed.XXXXXX)
OUTDIR="$STAGE/out"
...
for f in fields.json precision.json digest.txt h2_capstone.mjs hdr_beginner.txt hdr_intermediate.txt hdr_advanced.txt; do
  show "tools/course-waves/$SLUG/$f" > "$STAGE/$f"
done
show "packages/engines/engines/hse/exposure.js" > "$STAGE/engines/engines/hse/exposure.js"
...
echo "staged: $(ls "$STAGE/banks" | wc -l) banks, 3 manifests, 3 headers, fields.json, precision.json, digest.txt, h2_capstone.mjs, the engine and gradedTolerance.js"
echo
...
export H2_WAVE="$STAGE" H2_ENGINES="$STAGE/engines" H2_TOLERANCE="$STAGE/gradedTolerance.js"
H2_COURSE_OUT="$OUTDIR/${DATE}_${PREFIX}_${SLUG}_course.sql" python3 "$HERE/gen_course.py"
...
H2_COURSE_SQL="$OUTDIR/${DATE}_${PREFIX}_${SLUG}_course.sql" \
  H2_GOLIVE_OUT="$OUTDIR/${DATE}_${PREFIX}_${SLUG}_go_live.sql" \
