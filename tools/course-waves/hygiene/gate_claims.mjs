...
// settles it.
//
// Adapted for H2 from FC9's gate_claims.mjs. H2 has no RECON or FINDINGS report
// in its wave directory; its provenance record is FINDINGS-exposure.md, vendored
// beside the oracle in packages/engines, and THAT is what the negative control
// plants from.
//
...
const RECON = null;
const ENGINES = process.env.H2_ENGINES || '/root/wt-h2-nextgen/packages/engines';
let FINDINGS = null;
try { FINDINGS = fs.readFileSync(`${ENGINES}/tools/validation/hse/FINDINGS-exposure.md`, 'utf8'); } catch { FINDINGS = null; }
if (FINDINGS === null) { console.log('REFUSED: the vendored FINDINGS-exposure.md could not be read, so a provenance figure could not be named'); process.exit(2); }
 ...
 */
const ALLOWED = {
  62: 'the path_order of this course, settled by wave.json and NextGen-Remaining-Courses-PLAN.md section 14',
};
...
BRIEFS.forEach((f) => {
  ...
  if (PLANT && f === 'BRIEF.md') {
    ...
    text += '\n\nThe engine checks 518 tests and the golden carries 441 cases.\n';
  }
  briefsRead += 1;
...
check('the five legislated vocabulary collisions are all in digest section 24',
  ['"dose"', '"exposure"', '"noise"', '"exchange rate"', '"heat"'].every((w) => (DIGEST.split('# SECTION 24')[1] ?? '').split('# SECTION 25')[0].includes(w)),
  'digest section 24');
check('the heat decision is in digest section 23 and in BRIEF.md', /THE HEAT DECISION/.test(DIGEST) && /THE HEAT DECISION/.test(read('BRIEF.md') || ''), 'section 23');
...
check('every brief that exists names PROVENANCE or the digest as the only truth',
