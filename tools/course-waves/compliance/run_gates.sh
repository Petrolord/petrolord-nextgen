...
run() { # name expect(0|nonzero) cmd...
  ...
  echo "$name|$rc|$(echo "$out" | tail -1)" >> $W/.gates.tsv
}
: > $W/.gates.tsv
run structure 0 python3 structure.py
run vendor-closure 0 python3 vendor/closure.py
run digest-repro 0 ./gate_repro.sh
run kit-digestrepro 0 /root/dc-wavekit/digestrepro.sh $W /root/wt-as-compliance-nextgen
...
run capstone-generator 0 node compliance_capstone.mjs
...
import hashlib, json
W = '/root/as-wip-compliance'
files = {'structure': 'structure.py', 'vendor-closure': 'vendor/closure.py', 'digest-repro': 'gate_repro.sh',
