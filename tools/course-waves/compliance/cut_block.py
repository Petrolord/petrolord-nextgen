#!/usr/bin/env python3
"""Cut named check blocks out of the go-live on stdin, for dryrun_cq.sh's
negative controls only. `engine` is the block that compares the seeded values
with the engine run, `route` the second route in SQL. A block runs from its
section banner to the next banner, and the script refuses if a named banner is
absent, so a control can never pass because the cut silently removed nothing."""
import sys
