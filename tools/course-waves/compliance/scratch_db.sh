#!/usr/bin/env bash
# =============================================================================
# compliance SCRATCH DATABASE. A local Postgres 16 the ladder is iterated on
...
# neighbours: the six Economics catalogue rows at path_order 53 to 58, the
# forty-four available courses in total and the sibling riskchange at 59, so the go-live's path_order and
...
# what dryrun_cq.sh is for, and the owner's apply script re-runs every
# assertion against production inside the transaction that flips the course.
#
# Usage: scratch_db.sh          (re)creates container cq-scratch
#        CQ_NO_SIBLING=1 scratch_db.sh   the same, without the riskchange row
# =============================================================================
set -eu
C=cq-scratch
...
P <<SQL
set cq.no_sibling = '${CQ_NO_SIBLING:-0}';
...
-- The sibling, as its own ladder seeds it: riskchange at 59, Assurance,
-- coming_soon. Either course may land first, so the dry run is also taken
-- without it (CQ_NO_SIBLING=1).
insert into public.academy_apps (slug, name, module, path_order, status)
select 'riskchange', 'Risk, Change & Learning', 'assurance', 59, 'coming_soon'
 where coalesce(current_setting('cq.no_sibling', true), '') <> '1';
SQL
echo "cq-scratch ready: 4 academy tables, $(P -Atc "select count(*) from public.academy_apps") catalogue rows, $(P -Atc "select count(*) from public.academy_apps where slug = 'riskchange'") of them the sibling riskchange"
