#!/usr/bin/env bash
# =============================================================================
# compliance "Compliance, Audit & Quality" SEED LADDER, content-addressed.
#
...
# THE TWO STEPS.
#
#   seed      Step 1. Catalogue row goes in as `coming_soon` at path_order 60,
...
#             assertions (shape, a numeric grader, the prompts, the digest and
#             pairwise sweeps, the engine, a second route in SQL and eighteen
#             traps) and a typed confirmation of the upload. It does not need
#             riskchange (path_order 59) to be seeded or live first.
#
# =========================== THE UPLOAD GATE ================================
# DO NOT RUN `go-live` UNTIL A PRODUCTION UPLOAD CARRIES /dashboard/apps/compliance.
#
# The 78 lessons, the teaching lab (complianceLab.js) and its three explorer
...
# DashboardPage chunk and confirm it carries `apps/compliance`. `go-live`
...
#
# USAGE
#   ./apply_cq_compliance.sh verify        content check only, no database
#   ./apply_cq_compliance.sh seed          step 1
#   ./apply_cq_compliance.sh go-live       step 2, ONLY after the upload
#   ./apply_cq_compliance.sh pin <ref>     reprint the digest table
#
...
digest_for() {
  case $1 in
    20261002_cq_compliance_course           ) echo c1557c8c9d935f13e0ea18fd7129d02756435d31ad03f56a48e88848e7b617cf ;;
    20261002_cq_compliance_beginner_deep    ) echo 8ab2bf1176853314b9ff99a0c923953df4cd9a43b4fcdd8a156303d2fd479022 ;;
    20261002_cq_compliance_intermediate_deep) echo 1579f88b834561ddc0a9c04a7d5a1133268ff4abbc31ebbed93c717f191cb13c ;;
    20261002_cq_compliance_advanced_deep    ) echo a90a6bf9b35a6e6203a0a0b743a29b1117930ca4834dbcb19b4957b339076207 ;;
    20261002_cq_compliance_go_live          ) echo 3ae95c9c096beb3e8bdfe3127c4882013c36237054b2c8c4d6979e3a685d43d0 ;;
    *) echo UNPINNED ;;
  esac
}
SEEDS="20261002_cq_compliance_course
20261002_cq_compliance_beginner_deep
20261002_cq_compliance_intermediate_deep
20261002_cq_compliance_advanced_deep"
GOLIVE=20261002_cq_compliance_go_live

RUN=$(mktemp -d /tmp/cqapply.XXXXXX)
...
resolve() {
  ...
  Either $REF does not yet carry the compliance course PR, or a later change moved a seed.
...
select 'course' as what, status as v from public.academy_apps where slug = 'compliance'
union all select 'tiers', count(*)::text from public.academy_course_structures where app_slug = 'compliance' and active
...
union all select 'questions', count(*)::text from public.academy_quiz_questions where app_slug = 'compliance'
union all select 'capstones', count(*)::text from public.academy_capstones where app_slug = 'compliance'
...
case "${1:-verify}" in
  ...
  seed)
    resolve
    echo
    echo "STEP 1: seeding compliance HIDDEN as coming_soon."
    ...
    echo "DO NOT run go-live until an upload carries /dashboard/apps/compliance."
    ;;
  ...
  go-live)
    ...
    echo "'apps/compliance' in it. Checking a hash is not this check."
    printf "Type exactly  apps/compliance  to proceed: "
    read -r answer
    [ "$answer" = "apps/compliance" ] || refuse "upload gate not confirmed"
    run_file "$GOLIVE"
    echo
    state
    echo
    echo "compliance is available in the Assurance module."
