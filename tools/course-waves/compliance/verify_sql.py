#!/usr/bin/env python3
"""Compare the compliance seed ladder against its committed sources, FIELD BY FIELD:
...
canary run must print DISAGREE and exit 2:
  ...
  --canary-field  moves one graded expected value by one whole unit;
  --canary-lesson renames one lesson key in one manifest;
  --canary-prompt changes one clause reference in one capstone prompt.
...
REPO = os.environ.get(
    'REPO', UP if os.path.exists(os.path.join(UP, '.git')) else '/root/wt-as-compliance-nextgen')
SLUG, PREFIX, DATE = 'compliance', 'cq', '20261002'
...
cap_rows = re.split(r"\n\(\n  'compliance', ", course.split('insert into public.academy_capstones', 1)[1])[1:]
got_fields = []
for row in cap_rows:
    tier = row.split("'", 2)[1]
...
if CANARY_PROMPT:
    capjson['tiers']['advanced']['prompt'] = capjson['tiers']['advanced']['prompt'].replace('9.3', '9.2', 1)
prompts_compared = 0
for row in cap_rows:
    tier = row.split("'", 2)[1]
...
for flag, what in ((CANARY, 'one character flipped in one intermediate explanation'),
                   ...
                   (CANARY_PROMPT, 'one clause reference changed in the advanced prompt')):
    if flag:
