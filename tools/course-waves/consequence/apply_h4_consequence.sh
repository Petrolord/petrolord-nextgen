#!/usr/bin/env bash
# =============================================================================
# H4 consequence "Consequence Modelling" SEED LADDER,
# content-addressed.
#
...
#
# H4 HAS ONE EXTRA REASON TO BE CONTENT-ADDRESSED. Three key-truth audits ran
...
# THE TWO STEPS.
#
#   seed      Step 1. Catalogue row goes in as `coming_soon` at path_order 64,
...
#
# =========================== THE UPLOAD GATE ================================
# DO NOT RUN `go-live` UNTIL A PRODUCTION UPLOAD CARRIES /dashboard/apps/consequence.
#
# The 78 lessons, the teaching lab (consequenceLab.js) and its three explorer
...
# DashboardPage chunk and confirm it carries `apps/consequence`. `go-live`
...
#
# USAGE
#   ./apply_h4_consequence.sh verify          content check only, no database
#   ./apply_h4_consequence.sh seed            step 1
#   ./apply_h4_consequence.sh go-live        step 2, ONLY after the upload
#   ./apply_h4_consequence.sh pin <ref>       reprint the digest table
#
...
SEEDS="20261006_h4_consequence_course
20261006_h4_consequence_beginner_deep
20261006_h4_consequence_intermediate_deep
20261006_h4_consequence_advanced_deep"
GOLIVE=20261006_h4_consequence_go_live

RUN=$(mktemp -d /tmp/h4apply.XXXXXX)
...
resolve() {
  ...
  Either $REF does not yet carry the H4 consequence course PR, or a later change moved a seed.
...
select 'course' as what, status as v from public.academy_apps where slug = 'consequence'
union all select 'tiers', count(*)::text from public.academy_course_structures where app_slug = 'consequence' and active
union all select 'lessons', count(*)::text from public.academy_course_structures s,
       lateral jsonb_array_elements(s.structure->'modules') m,
       lateral jsonb_array_elements_text(m->'lesson_keys') lk
 where s.app_slug = 'consequence' and s.active
union all select 'questions', count(*)::text from public.academy_quiz_questions where app_slug = 'consequence'
union all select 'capstones', count(*)::text from public.academy_capstones where app_slug = 'consequence'
union all select 'graded', count(*)::text from public.academy_capstones c, lateral jsonb_array_elements(c.fields) f
 where c.app_slug = 'consequence'
...
case "${1:-verify}" in
  ...
  seed)
    resolve
    echo
    echo "STEP 1: seeding consequence HIDDEN as coming_soon."
    ...
    echo "DO NOT run go-live until an upload carries /dashboard/apps/consequence."
    ;;
  ...
  go-live)
    ...
    echo "'apps/consequence' in it. Checking a hash is not this check."
    printf "Type exactly  apps/consequence  to proceed: "
    read -r answer
    [ "$answer" = "apps/consequence" ] || refuse "upload gate not confirmed"
    run_file "$GOLIVE"
    echo
    state
    echo
    echo "consequence is available. HSE now carries its fourth course."
