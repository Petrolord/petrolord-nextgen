...
// nothing in the wave checked a brief against anything. In H4 the engine's
...
const ENG = process.env.H4_ENGINES || '/root/wt-h4-nextgen/packages/engines';
...
const RECON = null;
const FINDINGS = (() => { try { return fs.readFileSync(`${ENG}/tools/validation/hse/FINDINGS-consequence.md`, 'utf8'); } catch { return null; } })();
...
BRIEFS.forEach((f) => {
  ...
  if (PLANT && f === 'BRIEF.md') {
    ...
    text += '\n\nThe engine gates 206 tests and its baseline passed 206 before the plants.\n';
  }
  briefsRead += 1;
...
check('the legislated words are all in the vocabulary section',
  ['flux', 'beta', 'dose', 'severity', 'likelihood', 'risk', 'radiation'].every((w) => (DIGEST.split(/^# SECTION 34:/m)[1] ?? '').includes(w)),
  'digest section 34');
check('the BRIEF tier table matches the owners the digest prints: sections 1 to 13 Associate, 14 to 22 Professional, 23 to 33 Expert',
  (() => {
    ...
    return heads.every((h) => {
      ...
      if (n <= 22) return /^Professional/.test(own);
      if (n <= 33) return /^Expert/.test(own);
      return true;
    }) && heads.length === 34;
  })(), 'section owner clauses');
check('the BRIEF says 37 refusals across 24 functions and the digest agrees',
  /37 refusals are tabled above, across 24 functions/.test(DIGEST) && /37 refusals across 24 functions/.test(read('BRIEF.md') || ''), 'section 3');
