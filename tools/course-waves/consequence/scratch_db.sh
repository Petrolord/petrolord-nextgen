#!/usr/bin/env bash
# =============================================================================
# H4 SCRATCH DATABASE. A local Postgres 16 the ladder is iterated on, and THE
...
# runs supabase with any flag: the production dry run (dryrun_h4.sh with
...
# DDL (copied verbatim from tools/course-waves/lopa/scratch_db.sh (H3), which copies hygiene (H2), which copies safetystats, which
...
# safetystats (path_order 61) among them. H2 hygiene (62) and H3 lopa (63) are
# NOT, because both are unmerged, so path_order 64 is free here exactly as it is
# on main. The count is ASSERTED against what the ref actually carries, and the
# assertion prints the number it found when it fails.
#
...
# the course.
#
# Usage: scratch_db.sh [ref]     (re)creates container h4-scratch; default
...
if [ "$(P -Atc "select count(*) from public.academy_apps where path_order = 64")" != 0 ]; then
  echo "REFUSED: something at $REF already holds path_order 64, which is this course's slot"; exit 2
fi
echo "$C ready: 4 academy tables, $n committed course migrations loaded at $REF, path_order 64 free"
