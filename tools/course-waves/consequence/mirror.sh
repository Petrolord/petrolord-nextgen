#!/usr/bin/env bash
# MIRROR THE H4 WAVE INPUTS INTO THE NEXTGEN REPOSITORY, byte for byte, and
# re-pin them. Every suite under src/components/course/panels/consequence reads
...
NG=${NG:-/root/wt-h4-nextgen}
DEST="$NG/tools/course-waves/consequence"
mkdir -p "$DEST"
INPUTS=$(node -e "const w=require('$NG/tools/course-waves/waves.json');console.log(w.consequence.inputs.join('\n'))")
n=0
for f in $INPUTS; do
