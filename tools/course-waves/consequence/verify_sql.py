#!/usr/bin/env python3
"""Compare the H4 consequence seed ladder against its committed sources, FIELD BY FIELD:
...
Usage: verify_sql.py [ref|WORKTREE] [--canary]
...
REPO = os.environ.get(
    'REPO', UP if os.path.exists(os.path.join(UP, '.git')) else '/root/wt-h4-nextgen')
SLUG, PREFIX, DATE = 'consequence', 'h4', '20261006'
...
def show(path):
    if REF == 'WORKTREE':
        # The one window before the ladder is committed. Both sides then come
        # off the working tree, and the run says so.
...
cap_rows = re.split(r"\n\(\n  'consequence', ", course.split('insert into public.academy_capstones', 1)[1])[1:]
got_fields = []
for row in cap_rows:
    tier = row.split("'", 2)[1]
