#!/usr/bin/env bash
# =============================================================================
# H3 SCRATCH DATABASE. A local Postgres 16 the ladder is iterated on, and THE
...
# runs supabase with any flag: the production dry run (dryrun_h3.sh with
...
# safetystats (path_order 61) among them since nextgen #173. H2 hygiene is NOT,
# because it is unmerged, so path_order 62 and 63 are both free here exactly as
# they are on main. The count is ASSERTED against what the ref actually carries
...
# the course.
#
# Usage: scratch_db.sh [ref]     (re)creates container h3-scratch; default
...
if [ "$(P -Atc "select count(*) from public.academy_apps where path_order = 63")" != 0 ]; then
  echo "REFUSED: something at $REF already holds path_order 63, which is this course's slot"; exit 2
fi
echo "$C ready: 4 academy tables, $n committed course migrations loaded at $REF, path_order 63 free"
