...
//      h3_dump.mjs and h3_fields.mjs (as its JS spelling, its exponent
...
//      h3_capstone.mjs or fields.json; the capstone never names h3_dump.mjs,
//      h3_fields.mjs or digest.txt.
...
//      <repo>/src/content/courses/lopa, recursive), front matter included: no
...
const REPO = process.env.H3_REPO || '/root/wt-h3-nextgen';
const ENG = process.env.H3_ENGINES || path.join(REPO, 'packages/engines');
const WAVES = process.env.H3_WAVES || path.join(REPO, 'tools/course-waves');
const MIGRATIONS = process.env.H3_MIGRATIONS || path.join(REPO, 'migrations');
const LIVE_SIBLINGS = (process.env.H3_LIVE_SIBLINGS
  || 'safetystats=/root/hse-wip-safetystats/fields.json,hygiene=/root/hse-wip-hygiene/fields.json')
...
const DUMP = read(path.join(HERE, 'h3_dump.mjs'));
const STREAMS = read(path.join(HERE, 'h3_fields.mjs'));
const CAPSTONE = read(path.join(HERE, 'h3_capstone.mjs'));
...
const INPUTS = JSON.parse(execFileSync('node', [path.join(HERE, 'h3_capstone.mjs'), '--inputs'],
  { encoding: 'utf8', stdio: ['ignore', 'pipe', 'ignore'] }));
const NAMES = Object.keys(INPUTS);
if (NAMES.join() !== 'AKPO,USAN,YOHO') die(`unexpected capstone scenarios ${NAMES.join()}`);
...
const GENERIC = new Map([
  [8760, 'one year in hours, the engine export HOURS_PER_YEAR'],
  [1e-4, 'a decade TMEL'], [1e-5, 'a decade TMEL'], [1e-6, 'a decade TMEL'],
]);
const sigFigs = (v) => {
...
if (has('--plant-input')) DIGEST += `\nThe planted transmitter fails at ${INPUTS.USAN.transmitters.lambdaDuPerHour} per hour.\n`;
if (has('--plant-name')) DIGEST += '\nA planted line naming Yoho.\n';
...
// 1. INPUTS
[['digest.txt', DIGEST], ['h3_dump.mjs', DUMP], ['h3_fields.mjs', STREAMS]].forEach(([label, text]) => {
...
const GOLD = JSON.parse(read(path.join(ENG, 'test-data/hse/goldens/lopa_cases.json')));
const goldNums = [];
const walk = (n) => {
...
walk(GOLD);
// The lopa golden carries 649 numbers at 6703c00; a floor of 500 refuses a truncated or wrong file.
if (goldNums.length < 500) die(`only ${goldNums.length} numbers walked out of the golden`);
DISTINCT.forEach(({ name, at, v }) => {
...
const collect = (node, at) => {
  if (node && typeof node === 'object' && !Array.isArray(node)) {
    if ('architecture' in node || 'initiatingEventFrequencyPerYr' in node) records.push({ at, rec: node });
...
const collectG = (node) => {
  if (Array.isArray(node)) node.forEach(collectG);
    ...
    if ('architecture' in node || 'initiatingEventFrequencyPerYr' in node) goldRecords.push(node);
...
// 4. NAMES
const nameRe = /\b(akpo|usan|yoho)\b/i;
[['digest.txt', DIGEST], ['h3_dump.mjs', DUMP], ['h3_fields.mjs', STREAMS]].forEach(([label, text]) => {
...
[['h3_dump.mjs', DUMP, /h3_capstone|fields\.json/], ['h3_fields.mjs', STREAMS, /import[^\n]*h3_capstone|fields\.json/],
  ['h3_capstone.mjs', CAPSTONE, /h3_dump|h3_fields|digest\.txt/]].forEach(([label, text, re]) => {
...
if (fs.existsSync(WAVES)) {
  fs.readdirSync(WAVES).filter((w) => w !== 'lopa').forEach((w) => {
...
LIVE_SIBLINGS.forEach(([w, f]) => {
  if (!fs.existsSync(f)) die(`the live sibling answer key ${w} at ${f} is missing; set H3_LIVE_SIBLINGS to name it or drop it`);
...
// 8. THE APP
const PANEL_DIR = path.join(REPO, 'src/components/course/panels/lopa');
const APP = ['lopaLab.js', 'WorksheetExplorer.jsx', 'SifExplorer.jsx', 'ProofTestExplorer.jsx', 'panelBits.jsx']
  .map((f) => path.join(PANEL_DIR, f))
  .concat([path.join(REPO, 'src/pages/apps/LopaLearningPage.jsx')]);
...
// 10. THE LESSONS
const lessonsDir = opt('--lessons') || path.join(REPO, 'src/content/courses/lopa');
const lessonFiles = [];
let lessonLits = 0;
let plantedIn = null;
let synthetic = false;
