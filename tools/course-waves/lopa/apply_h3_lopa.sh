#!/usr/bin/env bash
# =============================================================================
# H3 lopa "Process Safety: LOPA & SIL Determination" SEED LADDER,
# content-addressed.
#
...
#
# H3 HAS ONE EXTRA REASON TO BE CONTENT-ADDRESSED. Three key-truth audits ran
...
# THE TWO STEPS.
#
#   seed      Step 1. Catalogue row goes in as `coming_soon` at path_order 63,
...
#
# =========================== THE UPLOAD GATE ================================
# DO NOT RUN `go-live` UNTIL A PRODUCTION UPLOAD CARRIES /dashboard/apps/lopa.
#
# The 78 lessons, the teaching lab (lopaLab.js) and its three explorer
...
# DashboardPage chunk and confirm it carries `apps/lopa`. `go-live`
...
#
# USAGE
#   ./apply_h3_lopa.sh verify          content check only, no database
#   ./apply_h3_lopa.sh seed            step 1
#   ./apply_h3_lopa.sh go-live        step 2, ONLY after the upload
#   ./apply_h3_lopa.sh pin <ref>       reprint the digest table
#
...
digest_for() {
  case $1 in
    ...
    20261005_h3_lopa_go_live          ) echo fc962a75d8b9e4d41ea2dacfd7c0ef695da591acb9ea028c3ac22a074608b1b7 ;;
    *) echo UNPINNED ;;
  esac
}
SEEDS="20261005_h3_lopa_course
20261005_h3_lopa_beginner_deep
20261005_h3_lopa_intermediate_deep
20261005_h3_lopa_advanced_deep"
GOLIVE=20261005_h3_lopa_go_live

RUN=$(mktemp -d /tmp/h3apply.XXXXXX)
...
resolve() {
  ...
  Either $REF does not yet carry the H3 lopa course PR, or a later change moved a seed.
...
select 'course' as what, status as v from public.academy_apps where slug = 'lopa'
union all select 'tiers', count(*)::text from public.academy_course_structures where app_slug = 'lopa' and active
...
union all select 'questions', count(*)::text from public.academy_quiz_questions where app_slug = 'lopa'
union all select 'capstones', count(*)::text from public.academy_capstones where app_slug = 'lopa'
...
case "${1:-verify}" in
  ...
  seed)
    resolve
    echo
    echo "STEP 1: seeding lopa HIDDEN as coming_soon."
    ...
    echo "DO NOT run go-live until an upload carries /dashboard/apps/lopa."
    ;;
  ...
  go-live)
    ...
    echo "'apps/lopa' in it. Checking a hash is not this check."
    printf "Type exactly  apps/lopa  to proceed: "
    read -r answer
    [ "$answer" = "apps/lopa" ] || refuse "upload gate not confirmed"
    run_file "$GOLIVE"
    echo
    state
    echo
    echo "lopa is available. HSE now carries its third course."
