...
// nothing in the wave checked a brief against anything. In H3 the engine's
...
const ENG = process.env.H3_ENGINES || '/root/wt-h3-nextgen/packages/engines';
...
const RECON = null;
const FINDINGS = (() => { try { return fs.readFileSync(`${ENG}/tools/validation/hse/FINDINGS-lopa.md`, 'utf8'); } catch { return null; } })();
...
BRIEFS.forEach((f) => {
  ...
  if (PLANT && f === 'BRIEF.md') {
    ...
    text += '\n\nThe engine gates 91 tests and one negative control turned 37 of them red.\n';
  }
  briefsRead += 1;
...
check('the five legislated words are all in the vocabulary section',
  ['beta', 'PFD', 'severity', 'likelihood', 'RRF'].every((w) => (DIGEST.split(/^# SECTION 32:/m)[1] ?? '').includes(w)),
  'digest section 32');
check('the BRIEF tier table matches the owners the digest prints: sections 1 to 13 Associate, 14 to 24 Professional, 25 to 31 Expert',
  (() => {
    ...
    return heads.every((h) => {
      ...
      if (n <= 13) return /^Associate/.test(own);
      if (n <= 24) return /^Professional/.test(own);
      if (n <= 31) return /^Expert/.test(own);
      return true;
    }) && heads.length === 32;
  })(), 'section owner clauses');
check('the BRIEF says 29 refusals across 7 functions and the digest agrees',
  /29 refusals are tabled above, across 7 functions/.test(DIGEST) && /29 refusals across 7 functions/.test(read('BRIEF.md') || ''), 'section 3');
