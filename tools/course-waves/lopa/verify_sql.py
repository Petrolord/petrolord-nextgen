#!/usr/bin/env python3
"""Compare the H3 lopa seed ladder against its committed sources, FIELD BY FIELD:
...
REPO = os.environ.get(
    'REPO', UP if os.path.exists(os.path.join(UP, '.git')) else '/root/wt-h3-nextgen')
SLUG, PREFIX, DATE = 'lopa', 'h3', '20261005'
...
cap_rows = re.split(r"\n\(\n  'lopa', ", course.split('insert into public.academy_capstones', 1)[1])[1:]
got_fields = []
for row in cap_rows:
    tier = row.split("'", 2)[1]
