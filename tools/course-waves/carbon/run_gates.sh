...
W=/root/et-wip-carbon
REPO=/root/wt-et-carbon-nextgen
cd $W
export TZ=UTC
LOG=$W/gates.log; : > $LOG
fail=0
...
run engine-suites 0 sh -c "cd $REPO/packages/engines && npx jest __tests__/downstream.carbon.golden.test.js __tests__/downstream.carbonAbatement.test.js __tests__/downstream.energyEfficiency.test.js > $W/scratch/jest.log 2>&1; rc=\$?; grep -E '^Tests:' $W/scratch/jest.log; exit \$rc"
...
run capstone-generator 0 node carbon_capstone.mjs
...
run kit-leakage-lessons 0 node /root/dc-wavekit/leakage.mjs $W --integers --content-root $REPO/src/content/courses/carbon
...
run kit-sourceprose 0 node /root/dc-wavekit/sourceprose.mjs $W/carbon_dump.mjs $W/carbon_fields.mjs $W/carbon_capstone.mjs $W/carbon_fields_capstone.mjs --rules $W
python3 - <<'PY'
import hashlib, json
W = '/root/et-wip-carbon'
files = {'structure': 'structure.py', 'engine-suites': 'run_gates.sh', 'digest-repro': 'gate_repro.sh',
 ...
 'kit-digestfigures': '/root/dc-wavekit/digestfigures.py', 'capstone-generator': 'carbon_capstone.mjs', 'oracle-check': 'oracle_check.py',
