...
#  1. THE SOURCES. carbonAbatement.js and energyEfficiency.js, the only engine modules
...
E=${MD_ENGINES:-/root/wt-et-carbon-nextgen/packages/engines}/engines/downstream
fail=0
for f in carbonAbatement.js energyEfficiency.js; do
...
for spec in "-400 0.1" "900 0.9"; do
  set -- $spec
  FAKE_CLOCK_DAYS=$1 FAKE_RANDOM=$2 TZ=UTC node --import $W/fakeclock.mjs $W/carbon_dump.mjs 2>/dev/null > $T/digest.txt
  FAKE_CLOCK_DAYS=$1 FAKE_RANDOM=$2 MD_FIELDS_OUT=$T/fields.json MD_PRECISION_OUT=$T/precision.json MD_CAPSTONE_OUT=$T/capstone.json node --import $W/fakeclock.mjs $W/carbon_capstone.mjs > /dev/null
