...
def texts():
    ...
    yield 'structure.py titles', '\n'.join([m[1] for mods in TIERS.values() for m in mods]
                                           + [l[1] for mods in TIERS.values() for m in mods for l in m[2]])
...
def main():
    ...
    if '--plant' in sys.argv:
        items.append(('planted', 'A flare is measured — never assumed.\nThe gap is the point, not a nuisance.'))
