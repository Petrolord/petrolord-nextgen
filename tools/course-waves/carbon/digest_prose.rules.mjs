...
// f0aef14 (MD5-0). SECTION 25 names the rules MD5-0 put in force as rules in
...
export default {
  ...
  pinned: [
    ...
    { frag: "A floor, not the value:", src: 'engines/downstream/energyEfficiency.js' },
    ...
    { frag: "A measure that adds emissions is not an abatement, and its cost per tonne would change sign.", src: 'engines/downstream/carbonAbatement.js' },
    ...
    { frag: "An inventory that computed nothing is not a baseline of zero.", src: 'engines/downstream/carbonAbatement.js' },
    ...
    { frag: "Carbon that escaped combustion is counted as methane, which is the usual and conservative assumption, and not as CO2.", src: 'engines/downstream/carbonAbatement.js' },
    { frag: "Comparable only with an intensity on the same boundary (", src: 'engines/downstream/carbonAbatement.js' },
    { frag: "Comparing a one-off capital cost against a recurring saving makes every measure look expensive.", src: 'engines/downstream/carbonAbatement.js' },
    ...
    { frag: "It is NOT the Solomon Energy Intensity Index, which is a proprietary benchmark with its own standard-energy methodology.", src: 'engines/downstream/energyEfficiency.js' },
    { frag: "It is deliberately not drawn as a wedge of future measures, because a wedge with nothing behind it is not a plan.", src: 'engines/downstream/carbonAbatement.js' },
    ...
    { frag: "Measures listed here act on the same source, so their abatements are NOT additive and the cumulative curve is an upper bound.", src: 'engines/downstream/carbonAbatement.js' },
    { frag: "No emission factor supplied, so the carbon figure is absent rather than zero.", src: 'engines/downstream/energyEfficiency.js' },
    ...
    { frag: "Obligations, evidence and deadlines belong in the compliance register, and keeping a second copy of them here would create two records that could disagree.", src: 'engines/downstream/carbonAbatement.js' },
    ...
    { frag: "Resolving the overlap needs an engineering judgement about sequencing, which is why it is surfaced rather than solved.", src: 'engines/downstream/carbonAbatement.js' },
    { frag: "Return fractions must lie between 0 and 1.", src: 'engines/downstream/energyEfficiency.js' },
    { frag: "Stack oxygen must lie between 0 and ", src: 'engines/downstream/energyEfficiency.js' },
    { frag: "Subtracting the efficiency percentages divides by a hundred instead of by the target efficiency, and understates the saving.", src: 'engines/downstream/energyEfficiency.js' },
    { frag: "That is what makes the pinch the constraint rather than a curiosity.", src: 'engines/downstream/energyEfficiency.js' },
    { frag: "The atom balance here counts carbon that escapes a burner or a flare as methane and NOT as CO2, so the oxidation CO2 is counted nowhere else and the fossil value is the consistent one for vented, fugitive and unburned fossil methane alike.", src: 'engines/downstream/carbonAbatement.js' },
    ...
    { frag: "This is a quantitative inventory, not a regulatory compliance register.", src: 'engines/downstream/carbonAbatement.js' },
    { frag: "This is conservation of mass, not an empirical factor, so it needs no source document.", src: 'engines/downstream/carbonAbatement.js' },
    ...
    { frag: "Without them it is absent rather than zero.", src: 'engines/downstream/energyEfficiency.js' },
    ...
    { frag: "the loss depends on the upstream pressure and not on what is downstream, so a trap blowing into a condensate header loses much the same steam as one blowing to atmosphere.", src: 'engines/downstream/energyEfficiency.js' },
  ],
};
