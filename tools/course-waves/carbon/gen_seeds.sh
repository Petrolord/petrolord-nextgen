#!/usr/bin/env bash
# =============================================================================
# carbon SEEDS, GENERATED FROM THE COMMITTED TREE AND NEVER FROM A WORKING TREE.
#
...
# they import, the fake clock, the oracle cross-check and the discriminate
# sweep (the go-live writes the oracle's answers and the closest wrong routes
# in by value from these two), every manifest.json, all 21 bank JSONs AND the
# vendored engines/downstream family with the downstream Python oracles
# (oracle_carbonabatement.py, oracle_energyefficiency.py) are pulled out of
# the git object store at REF into a
...
if [ -d "$HERE/../../../.git" ] || [ -f "$HERE/../../../.git" ]; then
  ...
  DEFAULT_REPO=/root/wt-et-carbon-nextgen
fi
...
STAGE=$(mktemp -d /tmp/cefseed.XXXXXX)
OUTDIR="$STAGE/out"
...
for f in fields.json precision.json capstone.json digest.txt \
         carbon_capstone.mjs carbon_fields_capstone.mjs carbon_fields.mjs fakeclock.mjs \
...
CEF_WAVE="$STAGE" CEF_ENGINES="$STAGE/repo/packages/engines" MD_ENGINES="$STAGE/repo/packages/engines" \
  CEF_COURSE_OUT="$OUTDIR/${DATE}_${PREFIX}_${SLUG}_course.sql" \
...
CEF_WAVE="$STAGE" CEF_ENGINES="$STAGE/repo/packages/engines" MD_ENGINES="$STAGE/repo/packages/engines" \
  CEF_COURSE_SQL="$OUTDIR/${DATE}_${PREFIX}_${SLUG}_course.sql" \
  CEF_GOLIVE_OUT="$OUTDIR/${DATE}_${PREFIX}_${SLUG}_go_live.sql" \
