#!/usr/bin/env python3
"""Compare the carbon seed ladder against its committed sources, FIELD BY FIELD:
...
tolerance, 18 in all) and capstone.json (every prompt, verbatim), and its
catalogue row against wave.json.
...
canary run must print DISAGREE and exit 2:
  ...
  --canary-field  moves one graded expected value by one whole unit (a hundred
                  tolerances or more on every field of this wave but the tuning
                  saving, where it is one tolerance and still a disagreement);
  --canary-lesson renames one lesson key in one manifest;
  --canary-prompt changes the discount rate in the advanced capstone prompt.

THE GRADED FIELDS ARE FLOATS. Each expected value and tolerance is parsed out of
the course migration with json.loads and compared with fields.json by value AND
by type, so 7772 against 7771.999995, or 0.01 against 0.010000000000000002, is a
disagreement: the seed must carry the engine's number as fields.json carries it.
The one tolerance that is an integer (1 GJ, the tuning saving) must be the
integer 1 on both sides for the same reason.
...
CANARY_PROMPT = '--canary-prompt' in sys.argv
HERE = os.path.dirname(os.path.abspath(__file__))
UP = os.path.abspath(os.path.join(HERE, '../../..'))
REPO = os.environ.get(
    'REPO', UP if os.path.exists(os.path.join(UP, '.git')) else '/root/wt-et-carbon-nextgen')
SLUG, PREFIX, DATE = 'carbon', 'cef', '20261014'
...
if CANARY_FIELD:
    fields[9] = [fields[9][0], fields[9][1], fields[9][2] + 1, fields[9][3]]
FIELD = re.compile(r"jsonb_build_object\('key','([a-z0-9_]+)', 'label','((?:[^']|'')*)', 'unit','((?:[^']|'')*)',"
                   r" 'expected',(-?\d+(?:\.\d+)?(?:e-?\d+)?), 'tol',(\d+(?:\.\d+)?(?:e-?\d+)?)\)")
cap_rows = re.split(r"\n\(\n  'carbon', ", course.split('insert into public.academy_capstones', 1)[1])[1:]
got_fields = []
for row in cap_rows:
    tier = row.split("'", 2)[1]
...
# ----------------------------------- the capstone prompts against capstone.json
# Each prompt is compared verbatim, unescaped out of its SQL literal, with the
# prompt capstone.json carries at the same ref. Every capstone gate on this wave
# ran on capstone.json's text, so a shipped prompt that differs from it by one
# character is a prompt no gate has read.
capjson = json.loads(show(f'{WAVE}/capstone.json'))
if CANARY_PROMPT:
    capjson['tiers']['advanced']['prompt'] = capjson['tiers']['advanced']['prompt'].replace('discount rate of 0.09', 'discount rate of 0.08', 1)
prompts_compared = 0
for row in cap_rows:
    tier = row.split("'", 2)[1]
    lits = re.findall(r"'((?:[^']|'')*)'", row.split('jsonb_build_array', 1)[0])
    # tier, cert_tier, dataset, title, prompt
    compared += 1
    prompts_compared += 1
    if len(lits) != 5 or lits[4].replace("''", "'") != capjson['tiers'][tier]['prompt']:
        disagree.append(f'capstone prompt {tier}: the seed is not capstone.json\'s prompt')
if prompts_compared != 3:
    disagree.append(f'{prompts_compared} capstone prompts compared, expected 3')
...
for flag, what in ((CANARY, 'one character flipped in one intermediate explanation'),
                   ...
                   (CANARY_FIELD, 'one graded expected value moved by one'),
                   (CANARY_LESSON, 'one beginner lesson key renamed'),
                   (CANARY_PROMPT, 'the discount rate changed in the advanced prompt')):
    if flag:
...
print(f'AGREE: all 396 questions (prompt, options, answer key, explanation), {lesson_keys} lesson keys in 18 modules, '
      f'{len(got_fields)} graded fields (key, expected, tolerance), {prompts_compared} capstone prompts and the catalogue row '
      'are byte for byte what the committed banks, manifests, fields.json, capstone.json and wave.json carry.')
