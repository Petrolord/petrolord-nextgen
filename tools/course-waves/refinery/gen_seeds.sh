#!/usr/bin/env bash
# =============================================================================
# refinery SEEDS, GENERATED FROM THE COMMITTED TREE AND NEVER FROM A WORKING TREE.
#
...
# audits that made their repairs IN THE REPOSITORY, so the working tree is
# exactly the copy that can be behind. (FC1's first apply read a stale worktree
# and would have seeded the unrepaired questions; this is that hole, closed.)
#
...
# with their clock guard, the teaching records oracle_check.py also loads,
# oracle_check.py and discriminate.mjs (the go-live's oracle and trap routes),
# every manifest.json, all 21 bank JSONs, AND the vendored engines the capstone
# generator runs through with the engines repo's Python oracles beside them
# (packages/engines/tools/validation/downstream) are pulled out of the git
# object store at REF into a throwaway directory, and all three generators are
# pointed at that. Nothing they read can have been edited after the commit it
# is named for, and the engine and oracle runs gen_course.py and gen_golive.py
# make are runs of the COMMITTED code.
#
...
if [ -d "$HERE/../../../.git" ] || [ -f "$HERE/../../../.git" ]; then
  ...
  DEFAULT_REPO=/root/wt-md-refinery-nextgen
fi
...
STAGE=$(mktemp -d /tmp/rfseed.XXXXXX)
OUTDIR="$STAGE/out"
...
for f in fields.json precision.json capstone.json digest.txt \
         refinery_capstone.mjs refinery_fields_capstone.mjs refinery_fields.mjs clockguard.mjs \
         oracle_check.py discriminate.mjs \
         hdr_beginner.txt hdr_intermediate.txt hdr_advanced.txt; do
  git -C "$REPO" show "$REF:tools/course-waves/$SLUG/$f" > "$STAGE/$f"
done

# The vendored engines the capstone generator runs through (the walked import
# closure of the five modules clockguard.mjs loads) and the Python oracles, from
# the object store.
git -C "$REPO" archive "$REF" packages/engines/package.json \
    packages/engines/engines/downstream/refineryPlanning.js packages/engines/engines/downstream/streamModel.js \
    packages/engines/engines/downstream/modularRefinery.js packages/engines/engines/economics/screening.js \
    packages/engines/lib/lp/simplex.js packages/engines/lib/stats/stats.js \
    packages/engines/tools/validation/downstream/oracle_refineryplanning.py \
    packages/engines/tools/validation/downstream/oracle_modularrefinery.py \
    packages/engines/tools/validation/downstream/exact_simplex.py \
  | tar -x -C "$STAGE/repo"
...
echo "staged: $(ls "$STAGE/banks" | wc -l) banks, 3 manifests, 3 headers, fields.json, precision.json, capstone.json, digest.txt, the capstone generator and records, the oracle check and the discriminate sweep, $(cd "$STAGE/repo/packages/engines" && find . -type f | wc -l) packages/engines paths"
echo
...
RF_WAVE="$STAGE" RF_ENGINES="$STAGE/repo/packages/engines" \
  RF_COURSE_OUT="$OUTDIR/${DATE}_${PREFIX}_${SLUG}_course.sql" \
...
RF_WAVE="$STAGE" RF_ENGINES="$STAGE/repo/packages/engines" \
  RF_COURSE_SQL="$OUTDIR/${DATE}_${PREFIX}_${SLUG}_course.sql" \
  RF_GOLIVE_OUT="$OUTDIR/${DATE}_${PREFIX}_${SLUG}_go_live.sql" \
