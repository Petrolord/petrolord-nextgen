#!/usr/bin/env python3
"""Compare the refinery seed ladder against its committed sources, FIELD BY FIELD:
...
canary run must print DISAGREE and exit 2:
  ...
  --canary-field  moves one graded expected value by one (a whole unit or more
                  than its tolerance, whichever field it lands on);
  --canary-lesson renames one lesson key in one manifest;
  --canary-prompt changes one condition (the cargo size) in the Expert prompt.
...
REPO = os.environ.get(
    'REPO', UP if os.path.exists(os.path.join(UP, '.git')) else '/root/wt-md-refinery-nextgen')
SLUG, PREFIX, DATE = 'refinery', 'rf', '20261011'
...
# The graded fields are FLOATS on this wave, so the expected value may carry a
# sign and decimals and the tolerance is a decimal; neither is ever written with
# an exponent (gen_course.py's num_lit), and json.loads reads both back as the
# very numbers fields.json carries.
FIELD = re.compile(r"jsonb_build_object\('key','([a-z0-9_]+)', 'label','((?:[^']|'')*)', 'unit','((?:[^']|'')*)',"
                   r" 'expected',(-?\d+(?:\.\d+)?), 'tol',(\d+(?:\.\d+)?)\)")
cap_rows = re.split(r"\n\(\n  'refinery', ", course.split('insert into public.academy_capstones', 1)[1])[1:]
got_fields = []
for row in cap_rows:
    tier = row.split("'", 2)[1]
...
if CANARY_PROMPT:
    capjson['tiers']['advanced']['prompt'] = capjson['tiers']['advanced']['prompt'].replace('cargo size of 450000', 'cargo size of 460000', 1)
prompts_compared = 0
for row in cap_rows:
    tier = row.split("'", 2)[1]
