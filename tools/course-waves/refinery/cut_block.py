#!/usr/bin/env python3
"""Cut named check blocks out of the go-live on stdin, for dryrun_rf.sh's
...
exact run. A block runs from its section banner to the next banner, and the
script refuses if a named banner is absent, so a control can never pass because
the cut silently removed nothing."""
import sys
BANNERS = {'engine': '-- ---------------------------------------------------- 1. against the engine',
           'route': '-- ----------------------------------------------- 2. the second route in SQL',
           'oracle': "-- ------------------------------------------------ 3. the oracle's exact run"}
sql = sys.stdin.read()
