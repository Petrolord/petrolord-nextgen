...
#  1. THE SOURCES. The three engine files in scope and the wave's generators
...
W=/root/md-wip-crude
E=${MD_ENGINES:-/root/wt-md-crude-nextgen/packages/engines}
T=$W/scratch/clock.$$
fail=0
hits=$(grep -nE '\bDate\b|performance\.now|Math\.random' $E/engines/downstream/crudeAssay.js $E/engines/downstream/productBlending.js $E/lib/lp/simplex.js $W/crude_dump.mjs $W/crude_fields.mjs $W/crude_fields_capstone.mjs $W/crude_capstone.mjs | grep -v '^\s*//' | grep -vE ':[0-9]+:\s*(//|\*)' )
if [ -z "$hits" ]; then echo "  sources: no Date, performance.now or Math.random in the 3 engine files and 4 generators"; else echo "  sources: CLOCK OR RANDOM READ FOUND:"; echo "$hits"; fail=1; fi
...
node $W/crude_capstone.mjs --json > $T.f0
for d in -400 900; do
  FAKE_CLOCK_DAYS=$d TZ=UTC node --import $W/fakeclock.mjs $W/crude_dump.mjs 2>/dev/null > $T
  ...
  FAKE_CLOCK_DAYS=$d node --import $W/fakeclock.mjs $W/crude_capstone.mjs --json > $T.f1
