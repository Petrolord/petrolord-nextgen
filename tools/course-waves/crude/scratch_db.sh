#!/usr/bin/env bash
# =============================================================================
# crude SCRATCH DATABASE. A local Postgres 16 the ladder is iterated on
...
# DDL (copied verbatim from tools/course-waves/compliance/scratch_db.sh, and
# through it from producedwater's, which cites the migrations each statement
# comes from), then production's neighbours as the merged ladders leave them:
# fifty-five available courses at path_order 1 to 47 and 53 to 60 (the two
# Assurance courses at 59 and 60 included), and the two wave siblings as their
# own ladders seed them, refinery at 49 (Commercial & Trading) and supply at 50
# (Supply Chain & Logistics), coming_soon, so the go-live's path_order and
# catalogue checks read something
# like the table they will meet.
#
# What this cannot prove, said plainly: that production's rows match. That is
# what dryrun_cr.sh is for, and the owner's apply script re-runs every
# assertion against production inside the transaction that flips the course.
#
# Usage: scratch_db.sh          (re)creates container cr-scratch
#        CR_NO_SIBLING=1 scratch_db.sh    the same, without the refinery and supply rows
# =============================================================================
set -eu
C=cr-scratch
...
P <<SQL
set cr.no_sibling = '${CR_NO_SIBLING:-0}';
...
-- The siblings, as their own ladders seed them: refinery at 49 in
-- commercial_trading and supply at 50 in supply_chain, coming_soon. Any of the
-- three may land first, so the dry run is also taken without them
-- (CR_NO_SIBLING=1).
insert into public.academy_apps (slug, name, module, path_order, status)
select s, n, m, o, 'coming_soon'
  from (values ('refinery', 'Refinery Feasibility & Planning', 'commercial_trading', 49),
               ('supply', 'Terminals, Depots & Fuel Supply', 'supply_chain', 50)) v(s, n, m, o)
 where coalesce(current_setting('cr.no_sibling', true), '') <> '1';
SQL
echo "cr-scratch ready: 4 academy tables, $(P -Atc "select count(*) from public.academy_apps") catalogue rows, $(P -Atc "select count(*) from public.academy_apps where slug in ('refinery', 'supply')") of them the siblings refinery and supply"
