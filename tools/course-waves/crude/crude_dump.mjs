...
for (const sg of [0.75, 0.8, 0.85, 0.9, 0.95, 1.0]) {
  ...
  row(inp(sg), fx(a), prevApi === null ? 'first row' : fx(a - prevApi));
  prevApi = a;
}
...
const apiRow = (label, comps) => {
  ...
  row(label, fx(b.properties.sg), fx(b.properties.api), fx(naive), fx(b.properties.api - naive), fx(onMass), fx(b.properties.api - onMass));
  return b;
};
...
out('A cargo is measured in barrels and half the properties blend on mass, so blendCrudes converts once (resolveFractions): each crude\'s mass share is its volume share times its specific gravity, over the sum of those products. The heavier crude carries more of the mass than of the volume.');
out('');
...
for (const f of obBlend.fractions) {
  const c = lib[f.id];
  row('Obigbo export blend', c.name, inp(obShares[c.id]), fx(f.volumeFraction), fx(C.sgFromApi(c.api)), fx(f.massFraction), fx(f.massFraction - f.volumeFraction));
}
...
for (const f of three.fractions) {
  const c = lib[f.id];
  row('three crudes, 50, 30 and 20 by volume', c.name, inp(threeComps.find((x) => x.id === c.id).volumeFraction), fx(f.volumeFraction), fx(C.sgFromApi(c.api)), fx(f.massFraction), fx(f.massFraction - f.volumeFraction));
}
out('');
...
const massTable = (label, comps, b) => {
  for (const [k, name] of massKeys) {
    ...
    row(label, name, fx(b.properties[k]), fx(onVol), fx(b.properties[k] - onVol), b.bases[k]);
  }
};
...
const viscRow = (label, comps) => {
  ...
  row(label, fx(b.properties.viscosityCSt), fx(onVolIndex), fx(b.properties.viscosityCSt - onVolIndex), fx(linear));
  return b;
};
...
out('cutYields returns each cut\'s yield in volume percent of the whole crude: the curve at the cut\'s upper bound minus the curve at its lower bound. A first cut with no lower bound starts at nothing distilled and a last cut with no upper bound runs to the end of the curve. The total is reported as it computes, never normalised, and the set closes only when every cut has a yield and the total is within the engine\'s closing tolerance of 100 percent. The cut set here is the studio\'s default:');
out('');
head('cut', 'from F', 'to F');
for (const c of F.STUDIO_CUTS) row(c.name, c.fromF === null ? 'start of the curve' : inp(c.fromF), c.toF === null ? 'end of the curve' : inp(c.toF));
out('');
...
row('a crude with no API and no SG beside one of 30 API', blind.basis, 'not formed', verdictWord(blind.stable), blind.message);
...
out('');
head('cut', 'from F', 'to F');
for (const c of F.KWALE_CUTS) row(c.name, c.fromF === null ? 'start of the curve' : inp(c.fromF), c.toF === null ? 'end of the curve' : inp(c.toF));
out('');
...
kwY.cuts.forEach((r, i) => {
  ...
  row(r.name, fx(each[0]), fx(each[1]), fx(r.yieldVolPercent), fx(v), fx(r.yieldVolPercent - v), fx(m));
});
...
kwY.cuts.forEach((r, i) => row(r.name, fx(r.yieldVolPercent), fx(kwY2.cuts[i].yieldVolPercent), fx(kwY2.cuts[i].yieldVolPercent - r.yieldVolPercent)));
row('total', fx(kwY.totalVolPercent), fx(kwY2.totalVolPercent), fx(kwY2.totalVolPercent - kwY.totalVolPercent));
out('');
...
out('');
out('A cut the curve cannot answer. Kwale Light with the Ebocha partial assay, 50 and 50, on Kwale\'s cuts: the blend\'s curve starts at 110 F and stops at 920 F, so the cuts whose bounds lie outside it have no yield:');
const kpY = C.cutYields({ curve: kpCurve, cuts: F.KWALE_CUTS });
head('cut', 'yield volume percent');
for (const r of kpY.cuts) row(r.name, fxOr(r.yieldVolPercent, 'unknown'));
out(`unknownCuts: ${kpY.unknownCuts.join(', ')}. Closes: ${kpY.closes}.`);
...
row('losses taken off the netback after the costs', fx(lossAfter), fx(lossAfter - kwNet.netback));
row('losses left out', fx(kwNet.grossValue - kv.processingCostPerBbl - kv.freightPerBbl), fx(kwNet.grossValue - kv.processingCostPerBbl - kv.freightPerBbl - kwNet.netback));
out('');
...
out('');
out(`The volume-weighted mean of the two crudes' own netbacks is ${fx(volNet)} $/bbl; the blend's netback minus that mean is ${fx(kwNet.netback - volNet)}. Yields add on volume and every other term is per barrel, so the blend is worth what its barrels are worth.`);
out('');
...
for (const k of [0, 1]) {
  ...
  row(`row ${k + 1}, rhs ${b2[k]}`, fx(r2.objective), fx(r2.objective - tbR.objective));
}
out('');
...
for (const t of Object.values(P.SPEC_TEMPLATES)) {
  for (const s of t.specs) row(t.name, s.name, s.basis + (s.indexOnMass ? ', on mass' : ''), s.min === undefined ? 'no minimum' : inp(s.min), s.max === undefined ? 'no maximum' : inp(s.max), s.unit || 'no unit');
}
out('');
...
for (const a of pms.achieved) {
  ...
  row(a.name, fx(a.value), fx(again), fx(again - a.value));
}
...
for (const [id, key, lim, sp] of [['sulfurPpm', 'max', 51, sulfurSp], ['sulfurPpm', 'max', 49, sulfurSp], ['rvp', 'max', 10, rvpSp], ['rvp', 'max', 8, rvpSp]]) {
  ...
  row(`${sp.name.replace(' maximum', '')} limit ${inp(lim)}`, fx(r.totalCost), fx(pms.totalCost - r.totalCost), fx(sp.price));
}
out('');
...
row('marginal minus average', fx(pmsSp('Total volume').price - pms.unitCost));
row(`re-solved at ${inp(F.APAPA_PMS_TARGET + 1)} bbl, cost minus the optimum`, fx(plus.totalCost - pms.totalCost));
out('');
...
row('C12', `the Refutas index blends on mass fraction ("${obBlend.bases.viscosityCSt}")`, 'ASTM D7152 blends on volume; the two disagree (SECTION 8 prints both), and the basis is a course and owner decision');
