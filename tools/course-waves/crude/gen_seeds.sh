#!/usr/bin/env bash
# =============================================================================
# crude SEEDS, GENERATED FROM THE COMMITTED TREE AND NEVER FROM A WORKING TREE.
#
...
# with the teaching records the oracle check also reads, the fake clock, the
# oracle cross-check, the discriminate sweep and the promptleak gate (the
# go-live writes the oracle's answers, the closest wrong routes and the
# engine-derived intermediates in by value from these three), gen_course.py and
# gen_golive.py THEMSELVES (the staged copies are the ones run, so a generator
# edited and not committed cannot write a seed), every manifest.json, all 21
# bank JSONs AND the vendored engines/downstream family, the LP kernel
# productBlending imports (lib/lp) and the downstream Python oracles are pulled
# out of the git object store at REF into a throwaway directory, and all three
# generators are pointed at that. Nothing they read can have been edited after
# the commit it is named for, and every engine and oracle run gen_course.py and
# gen_golive.py make is a run of the COMMITTED engines and oracles.
#
...
if [ -d "$HERE/../../../.git" ] || [ -f "$HERE/../../../.git" ]; then
  ...
  DEFAULT_REPO=/root/wt-md-crude-nextgen
fi
...
STAGE=$(mktemp -d /tmp/crseed.XXXXXX)
OUTDIR="$STAGE/out"
...
for f in fields.json precision.json capstone.json digest.txt \
         crude_capstone.mjs crude_fields_capstone.mjs crude_fields.mjs fakeclock.mjs \
         oracle_check.py discriminate.mjs gate_promptleak.py \
         gen_course.py gen_golive.py \
         hdr_beginner.txt hdr_intermediate.txt hdr_advanced.txt; do
  git -C "$REPO" show "$REF:tools/course-waves/$SLUG/$f" > "$STAGE/$f"
done

# The vendored engines the capstone generator runs through (the downstream
# family and the LP kernel productBlending imports), and the downstream Python
# oracles oracle_check.py imports, from the object store.
git -C "$REPO" archive "$REF" packages/engines/package.json packages/engines/engines/downstream \
    packages/engines/lib/lp packages/engines/tools/validation/downstream \
  | tar -x -C "$STAGE/repo"
...
echo "staged: $(ls "$STAGE/banks" | wc -l) banks, 3 manifests, 3 headers, fields.json, precision.json, capstone.json, digest.txt, the capstone generator and records, the oracle check, the discriminate sweep, the promptleak gate and the two generators, $(ls "$STAGE/repo/packages/engines/engines/downstream" | wc -l) engines/downstream paths, $(ls "$STAGE/repo/packages/engines/tools/validation/downstream" | grep -c '^oracle_') downstream oracles"
echo
...
CR_WAVE="$STAGE" CR_ENGINES="$STAGE/repo/packages/engines" \
  CR_COURSE_OUT="$OUTDIR/${DATE}_${PREFIX}_${SLUG}_course.sql" \
...
CR_WAVE="$STAGE" CR_ENGINES="$STAGE/repo/packages/engines" \
  CR_COURSE_SQL="$OUTDIR/${DATE}_${PREFIX}_${SLUG}_course.sql" \
  CR_GOLIVE_OUT="$OUTDIR/${DATE}_${PREFIX}_${SLUG}_go_live.sql" \
