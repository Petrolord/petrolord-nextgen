// GATE: nothing the D1 capstone grades, no input it is set on and no field it
...
// banks.
//
// Adapted from H1's gate_capstone_leak.mjs. The D1 capstone inputs are SERIES
// (a log, a SCADA index, a production sheet, core plugs, a density interval, a
// density-neutron cloud, two phases of a pressure) and a handful of stated
// scalars, so direction 1 looks for RUNS OF THREE CONSECUTIVE capstone values,
// written as the course prints them (six decimals) and as the generator writes
// them, and for every distinctive stated scalar.
//
// DIRECTIONS, every one counted and printed:
//   1. INPUTS. No run of three consecutive capstone values and no distinctive
//      capstone scalar in digest.txt, d1_dump.mjs, d1_fields.mjs or the lab's
...
//   4. NAMES. No capstone field name (ODUDU, IKORO, AMASIRI) in the digest, the
...
//      <repo>/src/content/courses/dataqc), and THE BRIEFS (BRIEF.md,
...
const REPO = process.env.D1_REPO || '/root/wt-dai-d1-nextgen';
const ENG = process.env.D1_ENGINES || path.join(REPO, 'packages/engines');
const WAVES = process.env.D1_WAVES || path.join(REPO, 'tools/course-waves');
...
const DUMP = read(path.join(HERE, 'd1_dump.mjs'));
const STREAMS = read(path.join(HERE, 'd1_fields.mjs'));
const CAPSTONE = read(path.join(HERE, 'd1_capstone.mjs'));
const FIELDS = JSON.parse(read(path.join(HERE, 'fields.json')));
if (FIELDS.length !== 18) die(`fields.json carries ${FIELDS.length} fields`);
const INPUTS = JSON.parse(execFileSync('node', [path.join(HERE, 'd1_capstone.mjs'), '--inputs'],
  { encoding: 'utf8', stdio: ['ignore', 'pipe', 'ignore'], maxBuffer: 1e8, env: { ...process.env, D1_ENGINES: ENG } }));
const NAMES = Object.keys(INPUTS);
if (NAMES.join() !== 'ODUDU,IKORO,AMASIRI') die(`unexpected capstone datasets ${NAMES.join()}`);

/* ---- the negative controls, planted in memory ---- */
if (has('--plant-value')) DIGEST += `\n| planted | ${FIELDS[3][2].toFixed(6)} |\n`;
if (has('--plant-input')) DIGEST += `\nA planted run: ${INPUTS.IKORO.core.slice(4, 7).map((v) => v.toFixed(6)).join(', ')}.\n`;
if (has('--plant-name')) DIGEST += '\nA planted line naming Amasiri.\n';

const findings = [];
const counts = {};
const count = (k, n = 1) => { counts[k] = (counts[k] || 0) + n; };

/* ---- the capstone inputs: every numeric series, flattened pairs included ---- */
const series = [];
const scalars = [];
const walkInputs = (name, o, at) => {
  if (Array.isArray(o)) {
    if (o.length && Array.isArray(o[0])) { o[0].forEach((_, c) => series.push({ name, k: `${at}[*][${c}]`, v: o.map((r) => r[c]) })); return; }
    if (o.some((x) => typeof x === 'number')) { series.push({ name, k: at, v: o }); return; }
    o.forEach((x, i) => walkInputs(name, x, `${at}[${i}]`));
    return;
  }
  if (o && typeof o === 'object') { Object.entries(o).forEach(([k, v]) => walkInputs(name, v, at ? `${at}.${k}` : k)); return; }
  if (typeof o === 'number' && Number.isInteger(o) && Math.abs(o) >= 100) scalars.push({ name, k: at, v: o });
};
Object.entries(INPUTS).forEach(([name, o]) => walkInputs(name, o, ''));
...
// 1. INPUTS
const LAB_DATA = read(path.join(REPO, 'src/components/course/panels/dataqc/ekeneDataset.json'));
[['digest.txt', DIGEST], ['d1_dump.mjs', DUMP], ['d1_fields.mjs', STREAMS], ['ekeneDataset.json', LAB_DATA]].forEach(([label, text]) => {
...
const GOLD = JSON.parse(read(path.join(ENG, 'test-data/dataai/goldens/quality_cases.json')));
const goldNums = [];
const walk = (n) => {
...
// 4. NAMES
const nameRe = /\b(odudu|ikoro|amasiri)\b/i;
[['digest.txt', DIGEST], ['d1_dump.mjs', DUMP], ['d1_fields.mjs', STREAMS], ['ekeneDataset.json', LAB_DATA]].forEach(([label, text]) => {
...
// 5. NEITHER FILE READS THE OTHER
[['d1_dump.mjs', DUMP, /d1_capstone|fields\.json/], ['d1_fields.mjs', STREAMS, /import[^\n]*d1_capstone|fields\.json/],
  ['d1_capstone.mjs', CAPSTONE, /d1_dump|d1_fields|digest\.txt/]].forEach(([label, text, re]) => {
...
if (fs.existsSync(WAVES)) {
  fs.readdirSync(WAVES).filter((w) => w !== 'dataqc').forEach((w) => {
...
// 8. THE APP
const APP = ['dataqcLab.js', 'ChecksExplorer.jsx', 'OutliersExplorer.jsx', 'MonitorExplorer.jsx', 'panelBits.jsx', 'ekeneDataset.json']
  .map((f) => path.join(REPO, 'src/components/course/panels/dataqc', f))
  .concat([path.join(REPO, 'src/pages/apps/DataQcLearningPage.jsx')]);
APP.forEach((p) => {
  const text = read(p);
  count('8 app sources');
  const m = text.match(nameRe);
...
const lessonsDir = opt('--lessons') || path.join(REPO, 'src/content/courses/dataqc');
const lessonFiles = [];
let lessonLits = 0;
let plantedIn = null;
