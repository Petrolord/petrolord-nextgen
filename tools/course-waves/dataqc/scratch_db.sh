#!/usr/bin/env bash
# =============================================================================
# D1 SCRATCH DATABASE. A local Postgres 16 the ladder is iterated on BEFORE any
...
# production).
#
# Adapted from tools/course-waves/safetystats/scratch_db.sh (H1). The four
...
# and 53 to 58, and the TWENTY ONE FC, Commercial & Trading, Energy Transition,
# Assurance and HSE courses read out of the COMMITTED course migrations at REF
...
# 65), so the path_order, catalogue and prompt checks meet real neighbours and
# real capstone rows. Every path_order from 1 to 65 is asserted occupied, so a
# hole refuses rather than passing short.
#
...
# what `dryrun_d1.sh` with TARGET=linked is for (rolled back, snapshotted
...
#
# Usage: scratch_db.sh [ref]     (re)creates container d1-scratch; default HEAD
# =============================================================================
set -eu
REF=${1:-HEAD}
...
for f in $(git -C "$REPO" ls-tree --name-only "$REF" migrations/ \
           | grep -E '_(fc[0-9]+_[a-z]+|cr_crude|rf_refinery|tds_supply|gv_gasvalue|cef_carbon|asrc_riskchange|cq_compliance|h[1-5]_[a-z]+)_course\.sql$'); do
  git -C "$REPO" show "$REF:$f" | P
  n=$((n + 1))
done
[ "$n" = 21 ] || { echo "REFUSED: expected the 21 seeded course migrations at $REF, found $n"; exit 2; }
# No hole: every path_order 1..65 must hold a row, stand-in or real, so a
# neighbour course the pattern above misses refuses instead of passing short.
holes=$(P -Atc "select coalesce(string_agg(g::text, ',' order by g), '') from generate_series(1, 65) g where not exists (select 1 from public.academy_apps a where a.path_order = g)")
