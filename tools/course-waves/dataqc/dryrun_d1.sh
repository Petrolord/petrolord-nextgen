#!/usr/bin/env bash
# =============================================================================
# D1 DRY RUN: THE WHOLE LADDER, ROLLED BACK. Adapted from H1's dryrun_h1.sh.
#
...
#
# APPLYING IS NOT THIS SCRIPT'S JOB. The real apply is apply_d1_dataqc.sh
# and it is the owner's to run. The go-live is HELD and is dry-run here only.
#
# TARGETS.
#   TARGET=scratch  the local d1-scratch container (scratch_db.sh). The default,
...
#                          ikoro_rhob_hampel_threshold_entry57_g_cm3, the
#                          SMALLEST graded value, by 1.0000001: an absolute
#                          move of 1.4e-8, a thirty-fifth of its 5e-7
#                          tolerance, so a grader would pass it); the
...
#                          Associate density completeness set to the reading
#                          with the SENTINELS COUNTED AS MISSING; the trap must
#                          refuse.
#   prompt                 lambda edited in the shipped Expert prompt; the
#                          go-live must refuse the prompt.
#   leak                   the Professional Grubbs critical value, to six
...
#
# Usage: dryrun_d1.sh [ref|WORKTREE] [--control <name> [field] [mult] | --idempotent]   default HEAD
# =============================================================================
set -u
REF=${1:-HEAD}
CONTROL=${2:-}
...
if up=$(cd "$HERE/../../.." 2>/dev/null && pwd) && { [ -d "$up/.git" ] || [ -f "$up/.git" ]; }; then
  ...
  REPO=${REPO:-/root/wt-dai-d1-nextgen}
fi
LINKED=${LINKED:-/opt/petrolord-studio/workspaces/dev1/projects/petrolord-nextgen}
C=${SCRATCH:-d1-scratch}
RUN=$(mktemp -d /tmp/d1dry.XXXXXX)
SLUG=dataqc
GOLIVE=20261031_d1_dataqc_go_live
FILES="20261031_d1_dataqc_course
20261031_d1_dataqc_beginner_deep
20261031_d1_dataqc_intermediate_deep
20261031_d1_dataqc_advanced_deep
$GOLIVE"
KEYS="odudu_rhob_completeness:beginner odudu_nphi_coverage:beginner odudu_scada_expected_step_min:beginner
odudu_water_cut_day23:beginner odudu_cumulative_drop_bbl:beginner odudu_phase_sum_allowed_day44_bbl_d:beginner
ikoro_core_max_abs_z:intermediate ikoro_core_max_abs_modified_z:intermediate ikoro_rhob_upper_fence_g_cm3:intermediate
ikoro_rhob_hampel_threshold_entry57_g_cm3:intermediate ikoro_core_grubbs_critical:intermediate ikoro_max_mahalanobis_d2:intermediate
amasiri_phase1_individuals_ucl_psig:advanced amasiri_phase1_mr_ucl_psig:advanced amasiri_ewma_day14_psig:advanced
amasiri_ewma_exact_ucl_day2_psig:advanced amasiri_cusum_upper_day18_psi:advanced amasiri_scorecard_total:advanced"
...
union all select 'path_order 66 held', count(*)::text from public.academy_apps where path_order = 66
...
union all select 'dataqc apps',        count(*)::text from public.academy_apps where slug = 'dataqc'
union all select 'dataqc any',         (select count(*) from public.academy_course_structures where app_slug = 'dataqc')
                                     + (select count(*) from public.academy_quiz_questions where app_slug = 'dataqc')
                                     + (select count(*) from public.academy_capstones where app_slug = 'dataqc') || ''
...
case "$CONTROL:$CTL" in
  ...
  --control:ledger|--control:route)
    FIELD=${FIELD_ARG:-ikoro_rhob_hampel_threshold_entry57_g_cm3}
    ...
    ;;
  --control:trap)
    # The Associate density completeness set to the reading with the sentinels
    # counted as missing, the first wrong method discriminate.mjs aims at it,
    # with the ledger and the second route cut out so ONLY the trap can object.
    FIELD=odudu_rhob_completeness; TIER=beginner
    WRONG=$(D1_ENGINES="$REPO/packages/engines" node "$HERE/d1_capstone.mjs" --inputs 2>/dev/null \
      | python3 -c "import json,sys; r=json.load(sys.stdin)['ODUDU']['log']['rhob']; print(repr(sum(1 for x in r if x is not None and x != -999.25)/len(r)))")
 ...
 where c.app_slug = '$SLUG' and c.tier = '$TIER';"
    ...
    WANT="the sentinel counted as missing gives .*so the field does not discriminate the trap"
    ;;
  --control:prompt)
    PRE_GOLIVE="update public.academy_capstones set prompt = replace(prompt, 'lambda 0.2 and L 3', 'lambda 0.3 and L 3')
 where app_slug = '$SLUG' and tier = 'advanced';"
    WANT="the advanced prompt is not the prompt gen_course.py rendered"
    ;;
  --control:leak)
    GRUBBS=$(python3 -c "import json; f=[x for x in json.load(open('$HERE/fields.json')) if x[1]=='ikoro_core_grubbs_critical'][0]; print(f'{f[2]:.6f}')")
    PRE_GOLIVE="update public.academy_capstones c
   set fields = (select jsonb_agg(case when f->>'key' = 'odudu_water_cut_day23'
                                       then jsonb_set(f, '{label}', to_jsonb((f->>'label') || ', against a benchmark of $GRUBBS'))
                                       else f end)
                   from jsonb_array_elements(c.fields) f)
 where c.app_slug = '$SLUG' and c.tier = 'beginner';"
    WANT="intermediate/ikoro_core_grubbs_critical in the beginner capstone text"
    ;;
  --control:sweep)
    fails=0
    for kt in $KEYS; do
...

{
  ...
  if [ "$pass" = 2 ]; then
    echo "create temp table d1_pass1 as select * from ($(tr '\n' ' ' < "$RUN/snap.sql" | sed 's/;[[:space:]]*$//')) s;"
  fi
  for f in $FILES; do
    echo "-- ================= $f"
    if [ "$f" = "$GOLIVE" ]; then
  ...
  if [ "$pass" = 2 ]; then
    echo "do \$\$ declare v_n int; begin select count(*) into v_n from ((select * from d1_pass1 except select * from ($(tr '\n' ' ' < "$RUN/snap.sql" | sed 's/;[[:space:]]*$//')) s) union all (select * from ($(tr '\n' ' ' < "$RUN/snap.sql" | sed 's/;[[:space:]]*$//')) s except select * from d1_pass1)) d; if v_n <> 0 then raise exception 'D1 go-live refused: IDEMPOTENCE, the second pass of the ladder moved % snapshot row(s)', v_n; end if; raise notice 'IDEMPOTENT: the second pass left every count and digest where the first left it'; end \$\$;"
  fi
  done
  cat <<'SQL'
select 'DRY RUN' as what,
       'dataqc ' || a.status
       || ' | ' || (select count(*) from public.academy_course_structures where app_slug='dataqc' and active)::text || ' tiers'
       || ' | ' || (select count(*) from public.academy_course_structures s,
                           lateral jsonb_array_elements(s.structure->'modules') m,
                           lateral jsonb_array_elements_text(m->'lesson_keys') lk
                     where s.app_slug='dataqc' and s.active)::text || ' lessons'
       || ' | ' || (select count(*) from public.academy_quiz_questions where app_slug='dataqc')::text || ' questions ('
       || (select string_agg(n::text, '/' order by ord) from (
             select case tier when 'beginner' then 1 when 'intermediate' then 2 else 3 end ord, count(*) n
               from public.academy_quiz_questions where app_slug='dataqc' group by tier) t) || ')'
       || ' | ' || (select count(*) from public.academy_capstones where app_slug='dataqc')::text || ' capstones'
       || ' | ' || (select count(*) from public.academy_capstones c, lateral jsonb_array_elements(c.fields) f
                     where c.app_slug='dataqc')::text || ' graded'
       ...
       || ' coming_soon' as v
  from public.academy_apps a where a.slug = 'dataqc';
rollback;
SQL
...
if [ -n "$CONTROL" ] && [ "$CONTROL" != --idempotent ]; then
  ...
  if [ $FAILED = 1 ] && grep -qE "D1 go-live refused.*$WANT" "$RUN/out.txt"; then
    echo "CONTROL $CTL FIRED:"
    grep -oE "D1 go-live refused[^\"]*" "$RUN/out.txt" | head -2 | cut -c1-400 | sed 's/^/    /'
    exit 0
  fi
...
grep -q 'dataqc available | 3 tiers | 78 lessons | 396 questions' "$RUN/out.txt" \
