#!/usr/bin/env bash
# =============================================================================
# D1 SEEDS AND CASE FILES, GENERATED FROM THE COMMITTED TREE AND NEVER FROM A
# WORKING TREE. Adapted from tools/course-waves/safetystats/gen_seeds.sh (H1).
#
# This script stages the COMMITTED tree first: wave.json, fields.json,
# precision.json, digest.txt, the capstone generator d1_capstone.mjs, the three
# tier headers, every manifest.json, all 21 bank JSONs, the vendored engine
# closure (engines/dataai/quality.js and the four files it imports) and the one
...
#
# It then DIFFS what it generated (the five migrations AND the eight case files
...
if [ -d "$HERE/../../../.git" ] || [ -f "$HERE/../../../.git" ]; then
  ...
  DEFAULT_REPO=/root/wt-dai-d1-nextgen
fi
...
STAGE=$(mktemp -d /tmp/d1seed.XXXXXX)
OUTDIR="$STAGE/out"
trap 'rm -rf "$STAGE"' EXIT
mkdir -p "$STAGE/banks" "$OUTDIR" "$OUTDIR/cases" "$STAGE/repo/src/content/courses/$SLUG" "$STAGE/engines/engines/hse" "$STAGE/engines/engines/dataai" "$STAGE/engines/engines/petrophysics" "$STAGE/engines/lib/stats" "$STAGE/engines/lib/linalg"
...
for f in fields.json precision.json digest.txt d1_capstone.mjs hdr_beginner.txt hdr_intermediate.txt hdr_advanced.txt; do
  git -C "$REPO" show "$REF:tools/course-waves/$SLUG/$f" > "$STAGE/$f"
done
for e in engines/dataai/quality.js engines/hse/safetyStats.js engines/petrophysics/conditioning.js lib/stats/stats.js lib/linalg/solveDense.js; do
...
echo "staged: $(ls "$STAGE/banks" | wc -l) banks, 3 manifests, 3 headers, fields.json, precision.json, digest.txt, d1_capstone.mjs, the engine closure and gradedTolerance.js"
echo
...
export D1_WAVE="$STAGE" D1_ENGINES="$STAGE/engines" D1_TOLERANCE="$STAGE/gradedTolerance.js"
D1_COURSE_OUT="$OUTDIR/${DATE}_${PREFIX}_${SLUG}_course.sql" D1_CASES_OUT="$OUTDIR/cases" python3 "$HERE/gen_course.py"
...
D1_COURSE_SQL="$OUTDIR/${DATE}_${PREFIX}_${SLUG}_course.sql" \
  D1_GOLIVE_OUT="$OUTDIR/${DATE}_${PREFIX}_${SLUG}_go_live.sql" \
...
echo
if [ "$bad" = 0 ]; then
  echo "AGREE: all five migrations and the nine case-file outputs regenerate byte for byte from the committed inputs at $REF."
else
