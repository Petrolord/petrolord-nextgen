#!/usr/bin/env python3
"""GATE: no number is TYPED into the D1 digest.

Adapted from H1's gate_typed_literals.py (itself FC9's). Every figure in digest.txt has to be
...
DUMP = os.path.join(os.environ.get('D1_WAVE_DIR', os.path.dirname(os.path.abspath(__file__))), 'd1_dump.mjs')
...
ALLOWED = {
 ...
 # arithmetic on printed values. Every entry below is a NAME or ALGEBRA rather
 # than a measurement. A DEAD ENTRY FAILS THE GATE.
 # ---------------------------------------------------------------------------
 '0':    'zero: the start of an index, S(0) = 0 and max(0, .) in the stated CUSUM recursion, the lower end of [0, 1], entries counted from 0',
 '1':    'unity: n - 1 and 1 - alpha in stated formulas, the upper end of [0, 1], lambda 1 naming the individuals case, a value of 1 among zeros',
 '2':    'two: 2 x halfWindow + 1, 2N and N - 2 in the stated Grubbs formula, d^2, (2 - lambda) and 2t in the stated EWMA formula',
 '3':    'three: the 3 sigma of the individuals limits, |z| > 3 as the usual threshold named in a column, halfWindow 3 and nSigma 3 naming the Hampel defaults, 1/3 in the R8 rule, 3 for far out fences',
 '1993': 'the year of Iglewicz and Hoaglin, the source the CONSTANTS table names for the modified z-score',
 '9':    'nine: "0 to 9", the digit character class of the stated normalisation',
 '0.54': 'a figure PRINTED ON THE NIST 6.3.2.3 PAGE with the wrong sign, quoted as printed so section 31 can show the typo; it is not in the golden',
 '0.47': 'the second figure the NIST 6.3.2.3 page prints with the wrong sign, quoted as printed for the same reason',
}
