#!/usr/bin/env node
// THE D1 NUMSWEEP: the kit's numsweep.mjs, run per tier, with ONE exemption and
...
//
//   node numsweep_dataqc.mjs [--tier beginner|intermediate|advanced] [--content DIR]
//   node numsweep_dataqc.mjs --selftest     the negative controls
//
...
const KIT = process.env.D1_KIT || '/root/dc-wavekit';
...
const die = (m) => { console.log(`numsweep_dataqc REFUSES: ${m}`); process.exit(2); };
...
const run = (content, tiers, reasons) => {
  ...
  tiers.forEach((tier) => {
    ...
    const tmp = fs.mkdtempSync(path.join(os.tmpdir(), `d1ns-${tier}-`));
...
const selftest = () => {
  ...
  console.log(`numsweep_dataqc selftest ${pass ? 'OK' : 'FAILED'}`);
  return pass ? 0 : 1;
};

if (argv.includes('--selftest')) process.exit(selftest());
const content = opt('--content') || path.join(REPO, 'src/content/courses/dataqc');
const tier = opt('--tier');
...
console.log(`[numsweep_dataqc] ${reasons.size} engine reason and message strings read from digest.txt`);
