#!/usr/bin/env python3
"""Compare the D1 dataqc seed ladder against its committed sources, FIELD BY FIELD:
...
REPO = os.environ.get(
    'REPO', UP if os.path.exists(os.path.join(UP, '.git')) else '/root/wt-dai-d1-nextgen')
SLUG, PREFIX, DATE = 'dataqc', 'd1', '20261031'
...
cap_rows = re.split(r"\n\(\n  'dataqc', ", course.split('insert into public.academy_capstones', 1)[1])[1:]
got_fields = []
for row in cap_rows:
    tier = row.split("'", 2)[1]
