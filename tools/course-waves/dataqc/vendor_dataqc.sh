#!/usr/bin/env bash
# VENDOR THE D1 DATAQC CLOSURE (engines/dataai/quality.js) INTO NEXTGEN,
# SHA-IDENTICAL, AND PROVE IT. Adapted from H1's vendor_safetystats.sh.
#
...
# those two, the oracle, the pin writer, FINDINGS and the negative control are
# NAMED with the reason they travel.
#
# Four proofs per path: git blob hash, sha256, cmp, byte count.
#
# LEDGER MODE: THE PIN DOES NOT MOVE. VENDOR.json pins 54cf7c4; 54cf7c4..cc82bf3
# also moves economics cashflow goldens, facilities, production, seismolord and
# wellsite paths, so moving the pin would reopen live graded values in other
# courses. Every closure path either matches the pinned manifest (the four
# imported dependencies, byte-identical at both commits and already vendored),
# or is NEW since the pin and is ledgered as kind "extra", group
# "d1-dataqc-course", pinned to its vendored blob. The entries clear by the
# guard's own STALE rule when the pin moves to cc82bf3 or later.
set -euo pipefail
...
echo
cd "$EXP"
SEED=__tests__/dataai.quality.test.js
...
for extra in \
  "test-data/dataai/goldens/quality_cases.json:read at runtime by the suite through read('test-data','dataai','goldens',...)" \
  "test-data/dataai/pins/quality_pins.json:read at runtime by the suite through read('test-data','dataai','pins',...)" \
  "tools/validation/dataai/oracle_quality.py:writes the golden the suite reads" \
  "tools/validation/dataai/pin_quality.py:writes the library pins the suite reads" \
  "tools/validation/dataai/FINDINGS-quality.md:the validation record the engine header cites" \
  "tools/validation/dataai/negcontrol_quality.sh:produced the negative-control table FINDINGS carries"; do
  p="${extra%%:*}"; why="${extra#*:}"
...
echo
DA_ALL=$(git -C "$ENG" ls-tree -r --name-only "$FULL" | grep -E '(^|/)dataai/|dataai\.' | grep -iE 'quality' | sort)
MISSED=$(comm -23 <(printf '%s\n' "$DA_ALL") <(printf '%s\n' "$PATHS") || true)
if [ -n "$MISSED" ]; then echo "REFUSES: dataai quality paths in ${FULL:0:7} outside the closure: $MISSED"; exit 1; fi
echo "  every dataai quality path at ${FULL:0:7} is in the closure"
echo
...
for p in paths:
    ...
    added.append({
        ...
        'reason': (f"D1 dataqc course: vendored sha-identical from petrolord-engines {full[:7]} "
                   f"(PR #248: engines/dataai/quality.js, its jest suite, golden, library pins, oracle, pin writer, FINDINGS, "
