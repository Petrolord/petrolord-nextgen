#!/usr/bin/env python3
"""truth-dataqc.json MUST BE THE HARVEST OF THE digest.txt BESIDE IT.
...
KIT = os.environ.get('D1_KIT', '/root/dc-wavekit')
truth = os.path.join(HERE, 'truth-dataqc.json')
if not os.path.exists(truth):
    print('REFUSED: no truth-dataqc.json; run harvest_digest.py'); sys.exit(2)
tmp = tempfile.mkdtemp(prefix='d1-truth-')
...
same = filecmp.cmp(truth, os.path.join(tmp, 'truth-dataqc.json'), shallow=False)
n = len(__import__('json').load(open(truth)))
shutil.rmtree(tmp)
print(f'truth_check: truth-dataqc.json ({n} digest lines) {"IS" if same else "IS NOT"} the harvest of digest.txt')
if '--plant' in sys.argv:
