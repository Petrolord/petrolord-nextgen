...
// wave's RECON REPORT rather than from the digest, and both were wrong. In D1 the
// engine's FINDINGS record, the oracle, the library pins and the engine's source
// comments are PROVENANCE, and none of them is teaching truth. (Adapted from H1's
// gate_claims.mjs, itself FC9's.)
//
...
const ENG = process.env.D1_ENGINES || '/root/wt-dai-d1-nextgen/packages/engines';
...
const RECON = null;
const FINDINGS = (() => { try { return fs.readFileSync(`${ENG}/tools/validation/dataai/FINDINGS-quality.md`, 'utf8'); } catch { return null; } })();
...
const ALLOWED = {
  ...
  66: 'the academy path order of the course, settled by wave.json and the course migration',
};
...
check('the five legislated words are all in the vocabulary section',
  ['outlier', 'sigma', 'control limit', 'missing', 'percentile'].every((w) => (DIGEST.split(/^# SECTION 32:/m)[1] ?? '').includes(w)),
  'digest section 32');
check('the BRIEF tier table matches the owners the digest prints: sections 1 to 16 Associate, 17 to 24 Professional, 25 to 31 Expert, 32 all',
  (() => {
    ...
    return heads.every((h) => {
      ...
      if (n === 4) return /Associate/.test(own) && /Professional/.test(own) && /Expert/.test(own);
      if (n <= 16) return /^Associate/.test(own);
...
check('the planted-defect count the BRIEF states is the generator\'s',
  (read('BRIEF.md') || '').includes(`all ${(DIGEST.match(/^(\d+) defects are planted\.$/m) || [])[1]} planted defects`), 'BRIEF.md against digest section 2');
