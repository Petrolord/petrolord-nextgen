#!/usr/bin/env python3
"""Lay out the D1 dataqc content tree from structure.py: the three tier
...
it left alone.
    ...
    python3 scaffold.py [--repo /root/wt-dai-d1-nextgen] [--check]
...
content = os.path.join(repo, 'src/content/courses/dataqc')

wrote = kept = manifests = drift = 0
for tier, modules in S.TIERS.items():
    manifest = {'app_slug': 'dataqc', 'tier': tier, 'content_version': 1, 'modules': []}
