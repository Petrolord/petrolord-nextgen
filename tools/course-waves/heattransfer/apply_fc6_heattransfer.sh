#!/usr/bin/env bash
# =============================================================================
# FC6 "Heat Exchange & Cooling" SEED LADDER, content-addressed.
#
...
# THE TWO STEPS.
#
#   seed      Step 1. Catalogue row goes in as `coming_soon` at path_order 44,
...
#
# =========================== THE UPLOAD GATE ================================
# DO NOT RUN `go-live` UNTIL A PRODUCTION UPLOAD CARRIES /dashboard/apps/heattransfer.
#
# The 78 lessons, the teaching lab (heattransferLab.js) and its three explorer
...
# DashboardPage chunk and confirm it carries `apps/heattransfer`. `go-live`
...
#
# USAGE
#   ./apply_fc6_heattransfer.sh verify        content check only, no database
#   ./apply_fc6_heattransfer.sh seed          step 1
#   ./apply_fc6_heattransfer.sh go-live       step 2, ONLY after the upload
#   ./apply_fc6_heattransfer.sh pin <ref>     reprint the digest table
#
...
# which PR moved it.
FILES="
20260925_fc6_heattransfer_course
...
digest_for() {
  case $1 in
    ...
    20260925_fc6_heattransfer_go_live          ) echo 75f008b34e5ec542c991689bab2b315a876951cdfca5579366e8ba8c1f8e4223 ;;
    *) echo UNPINNED ;;
  esac
}
SEEDS="20260925_fc6_heattransfer_course
20260925_fc6_heattransfer_beginner_deep
20260925_fc6_heattransfer_intermediate_deep
20260925_fc6_heattransfer_advanced_deep"
GOLIVE=20260925_fc6_heattransfer_go_live

RUN=$(mktemp -d /tmp/fc6apply.XXXXXX)
...
resolve() {
  ...
  Either $REF does not yet carry the FC6 course PR, or a later change moved a seed.
...
select 'course' as what, status as v from public.academy_apps where slug = 'heattransfer'
union all select 'tiers', count(*)::text from public.academy_course_structures where app_slug = 'heattransfer' and active
...
union all select 'questions', count(*)::text from public.academy_quiz_questions where app_slug = 'heattransfer'
union all select 'capstones', count(*)::text from public.academy_capstones where app_slug = 'heattransfer'
...
case "${1:-verify}" in
  ...
  seed)
    resolve
    echo
    echo "STEP 1: seeding FC6 HIDDEN as coming_soon."
    ...
    echo "DO NOT run go-live until an upload carries /dashboard/apps/heattransfer."
    ;;
  ...
  go-live)
    ...
    echo "'apps/heattransfer' in it. Checking a hash is not this check."
    printf "Type exactly  apps/heattransfer  to proceed: "
    read -r answer
    [ "$answer" = "apps/heattransfer" ] || refuse "upload gate not confirmed"
    run_file "$GOLIVE"
    echo
    state
    echo
    echo "FC6 is available. Facilities now carries its sixth course."
