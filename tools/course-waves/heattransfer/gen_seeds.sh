#!/usr/bin/env bash
# =============================================================================
# FC6 SEEDS, GENERATED FROM THE COMMITTED TREE AND NEVER FROM A WORKING TREE.
#
...
# that matches nothing a reviewer can read. FC6 ran three tier audits that made
...
if [ -d "$HERE/../../../.git" ] || [ -f "$HERE/../../../.git" ]; then
  ...
  DEFAULT_REPO=/root/wt-fc6-nextgen
fi
...
STAGE=$(mktemp -d /tmp/fc6seed.XXXXXX)
OUTDIR="$STAGE/out"
...
FC6_WAVE="$STAGE" FC6_COURSE_OUT="$OUTDIR/${DATE}_${PREFIX}_${SLUG}_course.sql" \
...
FC6_WAVE="$STAGE" \
  FC6_COURSE_SQL="$OUTDIR/${DATE}_${PREFIX}_${SLUG}_course.sql" \
  FC6_GOLIVE_OUT="$OUTDIR/${DATE}_${PREFIX}_${SLUG}_go_live.sql" \
