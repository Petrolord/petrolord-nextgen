#!/usr/bin/env bash
# =============================================================================
# H5 SCRATCH DATABASE. A local Postgres 16 the ladder is iterated on, and THE
...
# `supabase db query --linked` points at it, so no step of this wave's writing
# runs supabase with any flag: the production dry run (dryrun_h5.sh with
# TARGET=linked) is handed to the OWNER in the pull request body and run by the
# owner.
#
# The four academy tables the ladder writes, from the NextGen repository's own
# DDL (copied verbatim from tools/course-waves/hygiene/scratch_db.sh (H2), which copies safetystats, which
...
# real ones. At origin/main that is every committed course migration, H1
# safetystats (61), H2 hygiene (62) and H3 lopa (63) among them. H4 consequence
# (64) is NOT, because it is unmerged, so path_order 65 is free here exactly as
# it is on main. The count is ASSERTED against what the ref actually carries
# rather than copied from a sibling wave, and the assertion prints the number it
# found when it fails.
#
...
# the course.
#
# Usage: scratch_db.sh [ref]     (re)creates container h5-scratch; default
...
if [ "$(P -Atc "select count(*) from public.academy_apps where path_order = 65")" != 0 ]; then
  echo "REFUSED: something at $REF already holds path_order 65, which is this course's slot"; exit 2
fi
echo "$C ready: 4 academy tables, $n committed course migrations loaded at $REF, path_order 65 free"
