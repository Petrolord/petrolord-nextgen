#!/usr/bin/env bash
# =============================================================================
# H5 DRY RUN: THE WHOLE LADDER, ROLLED BACK.
#
...
#
# APPLYING IS NOT THIS SCRIPT'S JOB. The real apply is apply_h5_qra.sh
# and it is the owner's to run. The go-live is HELD and is dry-run here only.
#
# TARGETS.
#   TARGET=scratch  the local h5-scratch container (scratch_db.sh). The default,
...
#                          ukpokiti_control_room_lsir_per_yr, the SMALLEST
#                          graded value, by 1.00000001: an absolute move of
#                          8.0e-14, under a sixth of its 5e-13 tolerance, so a
...
#                          Associate operator's IRPA set to what hours over
#                          8766 give, the value discriminate.mjs prints for
#                          that wrong method; the trap for that field must
#                          refuse.
#   prompt                 the blast wall's life edited in the shipped Expert
#                          prompt; the go-live must refuse the prompt.
#   leak                   the Professional crew FAR, to six decimals, planted
...
#
# Usage: dryrun_h5.sh [ref|WORKTREE] [--control <name> [field] [mult]]   default HEAD
# =============================================================================
set -u
REF=${1:-HEAD}
CONTROL=${2:-}
...
if up=$(cd "$HERE/../../.." 2>/dev/null && pwd) && { [ -d "$up/.git" ] || [ -f "$up/.git" ]; }; then
  ...
  REPO=${REPO:-/root/wt-h5-nextgen}
fi
LINKED=${LINKED:-/opt/petrolord-studio/workspaces/dev1/projects/petrolord-nextgen}
C=${SCRATCH:-h5-scratch}
RUN=$(mktemp -d /tmp/h5dry.XXXXXX)
SLUG=qra
GOLIVE=20261007_h5_qra_go_live
FILES="20261007_h5_qra_course
20261007_h5_qra_beginner_deep
20261007_h5_qra_intermediate_deep
20261007_h5_qra_advanced_deep
$GOLIVE"
KEYS="ukpokiti_compressor_explosion_frequency_per_yr:beginner ukpokiti_camp_exposure_frequency_per_yr:beginner ukpokiti_control_room_lsir_per_yr:beginner
ukpokiti_compressor_deck_lsir_per_yr:beginner ukpokiti_operator_irpa_per_yr:beginner ukpokiti_technician_irpa_per_yr:beginner
ogini_crew_pll_per_yr:intermediate ogini_crew_far:intermediate ogini_village_frequency_ten_or_more_per_yr:intermediate
ogini_village_vrom_max_ratio:intermediate ogini_village_exceedance_from_fatalities:intermediate ogini_village_r2p2_point_ratio:intermediate
ebughu_deluge_cost_to_benefit_ratio:advanced ebughu_deluge_icaf_gbp:advanced ebughu_deluge_maximum_reasonably_practicable_cost_gbp:advanced
ebughu_blast_wall_cost_to_benefit_ratio:advanced ebughu_blast_wall_icaf_gbp:advanced ebughu_gas_detection_cost_to_benefit_ratio:advanced"
...
union all select 'path_order 65 held', count(*)::text from public.academy_apps where path_order = 65
...
union all select 'qra apps',    count(*)::text from public.academy_apps where slug = 'qra'
union all select 'qra any',     (select count(*) from public.academy_course_structures where app_slug = 'qra')
                                     + (select count(*) from public.academy_quiz_questions where app_slug = 'qra')
                                     + (select count(*) from public.academy_capstones where app_slug = 'qra') || ''
...
case "$CONTROL:$CTL" in
  ...
  --control:ledger|--control:route)
    FIELD=${FIELD_ARG:-ukpokiti_control_room_lsir_per_yr}
    [ -n "$MULT" ] || { [ -z "$FIELD_ARG" ] && MULT=1.00000001 || MULT=1.0000001; }
    ...
    ;;
  --control:trap)
    # The operator's IRPA set to what hours over 8766 give, the value
    # discriminate.mjs prints for that wrong method through the engine, with
    # the ledger and the second route cut out so ONLY the trap can object.
    FIELD=ukpokiti_operator_irpa_per_yr; TIER=beginner
    WRONG=$(H5_ENGINES="$REPO/packages/engines" H5_REPO="$REPO" H5_WAVE_DIR="$HERE" node "$HERE/discriminate.mjs" --values 2>/dev/null \
      | sed -n 's/^  wrong ukpokiti_operator_irpa_per_yr hours_over_8766 = //p')
    [ -n "$WRONG" ] || { echo "REFUSED: discriminate.mjs printed no hours_over_8766 value for the operator"; exit 2; }
 ...
 where c.app_slug = '$SLUG' and c.tier = '$TIER';"
    ...
    WANT="the hours over 8766 gives .*so the field does not discriminate the trap"
    ;;
  --control:prompt)
    PRE_GOLIVE="update public.academy_capstones set prompt = replace(prompt, 'a life of 25 years', 'a life of 26 years')
 where app_slug = '$SLUG' and tier = 'advanced';"
    WANT="the advanced prompt is not the prompt gen_course.py rendered"
    ;;
  --control:leak)
    PRE_GOLIVE="update public.academy_capstones c
   set fields = (select jsonb_agg(case when f->>'key' = 'ukpokiti_operator_irpa_per_yr'
                                       then jsonb_set(f, '{label}', to_jsonb((f->>'label') || ', against a benchmark of 15.474882'))
                                       else f end)
                   from jsonb_array_elements(c.fields) f)
 where c.app_slug = '$SLUG' and c.tier = 'beginner';"
    WANT="intermediate/ogini_crew_far in the beginner capstone text"
    ;;
  --control:sweep)
    fails=0
    for kt in $KEYS; do
      for mode in ledger route; do
        OUT=$("$0" "$REF" --control "$mode" "${kt%%:*}" "${MULT:-1.0000001}" 2>&1)
    ...
    [ $fails = 0 ] && { echo "SWEEP: all 36 controls (18 fields x ledger and route, x${MULT:-1.0000001}) fired and named their field."; exit 0; }
...
select 'DRY RUN' as what,
       'qra ' || a.status
       || ' | ' || (select count(*) from public.academy_course_structures where app_slug='qra' and active)::text || ' tiers'
       || ' | ' || (select count(*) from public.academy_course_structures s,
                           lateral jsonb_array_elements(s.structure->'modules') m,
                           lateral jsonb_array_elements_text(m->'lesson_keys') lk
                     where s.app_slug='qra' and s.active)::text || ' lessons'
       || ' | ' || (select count(*) from public.academy_quiz_questions where app_slug='qra')::text || ' questions ('
       || (select string_agg(n::text, '/' order by ord) from (
             select case tier when 'beginner' then 1 when 'intermediate' then 2 else 3 end ord, count(*) n
               from public.academy_quiz_questions where app_slug='qra' group by tier) t) || ')'
       || ' | ' || (select count(*) from public.academy_capstones where app_slug='qra')::text || ' capstones'
       || ' | ' || (select count(*) from public.academy_capstones c, lateral jsonb_array_elements(c.fields) f
                     where c.app_slug='qra')::text || ' graded'
       ...
       || ' coming_soon' as v
  from public.academy_apps a where a.slug = 'qra';
rollback;
SQL
...
if [ -n "$CONTROL" ]; then
  ...
  if [ $FAILED = 1 ] && grep -qE "H5 go-live refused.*$WANT" "$RUN/out.txt"; then
    echo "CONTROL $CTL FIRED:"
    grep -oE "H5 go-live refused[^\"]*" "$RUN/out.txt" | head -2 | cut -c1-400 | sed 's/^/    /'
    exit 0
  fi
...
grep -q 'qra available | 3 tiers | 78 lessons | 396 questions' "$RUN/out.txt" \
