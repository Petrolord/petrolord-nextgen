#!/usr/bin/env bash
# =============================================================================
# H5 SEEDS, GENERATED FROM THE COMMITTED TREE AND NEVER FROM A WORKING TREE.
#
...
# seed that matches nothing a reviewer can read. H5's three key-truth audits run
...
# engines/hse/qra.js WITH EVERY FILE IT IMPORTS (the H4 consequence engine and
# its three helpers, the H1 safety statistics engine, the H3 lopa engine, the
# economics cashflow.ts and irrContract.js), the engine loader qra_engine.mjs
# with its TypeScript hook, and the one tolerance module gradedTolerance.js are
...
# generators are mirrored into tools/course-waves/qra and not yet committed,
...
if [ -d "$HERE/../../../.git" ] || [ -f "$HERE/../../../.git" ]; then
  ...
  DEFAULT_REPO=/root/wt-h5-nextgen
fi
...
STAGE=$(mktemp -d /tmp/h5seed.XXXXXX)
OUTDIR="$STAGE/out"
trap 'rm -rf "$STAGE"' EXIT
mkdir -p "$STAGE/banks" "$OUTDIR" "$STAGE/repo/src/content/courses/$SLUG" "$STAGE/engines/engines/hse" \
  "$STAGE/engines/engines/economics" "$STAGE/engines/engines/facilities" "$STAGE/engines/lib/stats"
...
for f in fields.json precision.json digest.txt h5_capstone.mjs qra_engine.mjs ts_loader.mjs discriminate.mjs hdr_beginner.txt hdr_intermediate.txt hdr_advanced.txt; do
  show "tools/course-waves/$SLUG/$f" > "$STAGE/$f"
done
for e in engines/hse/qra.js engines/hse/consequence.js engines/hse/safetyStats.js engines/hse/lopa.js \
         engines/economics/cashflow.ts engines/economics/irrContract.js engines/facilities/relief.js \
         engines/facilities/spacing.js lib/stats/stats.js; do
  show "packages/engines/$e" > "$STAGE/engines/$e"
done
...
echo "staged: $(ls "$STAGE/banks" | wc -l) banks, 3 manifests, 3 headers, fields.json, precision.json, digest.txt, h5_capstone.mjs, the engine loader, discriminate.mjs, the engine and its 8 imports, and gradedTolerance.js"
echo
...
# esbuild, which strips the types from cashflow.ts, is the only thing read from
# the repository itself: it changes no behaviour, and the stage has no node_modules.
export H5_WAVE="$STAGE" H5_ENGINES="$STAGE/engines" H5_TOLERANCE="$STAGE/gradedTolerance.js" H5_ESBUILD_REPO="$REPO"
H5_COURSE_OUT="$OUTDIR/${DATE}_${PREFIX}_${SLUG}_course.sql" python3 "$HERE/gen_course.py"
...
H5_COURSE_SQL="$OUTDIR/${DATE}_${PREFIX}_${SLUG}_course.sql" \
  H5_GOLIVE_OUT="$OUTDIR/${DATE}_${PREFIX}_${SLUG}_go_live.sql" \
