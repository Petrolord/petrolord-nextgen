#!/usr/bin/env python3
"""Compare the H5 qra seed ladder against its committed sources, FIELD BY FIELD:
...
REPO = os.environ.get(
    'REPO', UP if os.path.exists(os.path.join(UP, '.git')) else '/root/wt-h5-nextgen')
SLUG, PREFIX, DATE = 'qra', 'h5', '20261007'
...
cap_rows = re.split(r"\n\(\n  'qra', ", course.split('insert into public.academy_capstones', 1)[1])[1:]
got_fields = []
for row in cap_rows:
    tier = row.split("'", 2)[1]
