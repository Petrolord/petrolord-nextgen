...
// nothing in the wave checked a brief against anything. In H5 the engine's
// FINDINGS record, the oracle and the engine's source comments are PROVENANCE,
// and none of them is teaching truth. (Adapted from H1's gate_claims.mjs, which carried FC9's.)
//
...
// the five legislated naming collisions, each against the file that settles it.
//
...
const RECON = null;
const FINDINGS = (() => { try { return fs.readFileSync(`${ENG}/tools/validation/hse/FINDINGS-qra.md`, 'utf8'); } catch { return null; } })();
...
// number in the briefs (26, 18 and the rest) happens to be printed somewhere in
// the digest, so it resolves there; a DEAD entry fails this gate.
const ALLOWED = {
  78: 'the lessons in the wave, settled by structure.py (which the structural checks below confirm)',
...
BRIEFS.forEach((f) => {
  ...
  if (PLANT && f === 'BRIEF.md') {
    ...
    text += '\n\nThe engine gates 171 tests and one negative control turned 53 of them red.\n';
  }
  briefsRead += 1;
...
check('the legislated words are all in the vocabulary section',
  ['risk', 'severity', 'likelihood', 'FAR', 'PLL', 'ICAF', 'NPV', 'probit'].every((w) => (DIGEST.split(/^# SECTION 34:/m)[1] ?? '').includes(w)),
  'digest section 34');
check('the BRIEF tier table matches the owners the digest prints: sections 1 to 14 Associate, 15 to 24 Professional, 25 to 33 Expert',
  (() => {
    ...
    return heads.every((h) => {
      ...
      if (n <= 14) return /^Associate/.test(own);
      if (n <= 24) return /^Professional/.test(own);
      if (n <= 33) return /^Expert/.test(own);
      return true;
    }) && heads.length === 34;
  })(), 'section owner clauses');
check('the BRIEF says 39 refusals across 13 functions and the digest agrees',
  /39 refusals are tabled above, across 13 functions/.test(DIGEST) && /39 refusals across 13 functions/.test(read('BRIEF.md') || ''), 'section 3');
check('the BRIEF tier table ranges are the ones this gate checks',
  /\| 1 to 14 \|/.test(read('BRIEF.md') || '') && /\| 15 to 24 \|/.test(read('BRIEF.md') || '') && /\| 25 to 33 \|/.test(read('BRIEF.md') || ''), 'BRIEF.md');
check('wave.json prints no graded answer',
  (() => {
    const wj = read('wave.json');
    return FIELDS.every(([, , v]) => ![String(v), v.toFixed(12), v.toFixed(6)].some((r) => r.length >= 6 && wj.includes(r)));
  })(), 'wave.json against fields.json');
