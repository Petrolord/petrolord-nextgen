...
#
# NEGATIVE CONTROL: ET_PLANT_TZ=1 makes the generator print one line that reads
# the local time zone offset (the plant lives in gasvalue_dump.mjs and is never set
...
ET_PLANT_TZ=1 DIGEST_TZ=UTC $W/build_digest.sh 2>/dev/null > $T.utc
for z in Africa/Lagos Pacific/Kiritimati Pacific/Pago_Pago; do
  ET_PLANT_TZ=1 DIGEST_TZ=$z $W/build_digest.sh 2>/dev/null > $T
