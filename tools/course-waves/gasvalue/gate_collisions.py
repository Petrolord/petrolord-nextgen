...
than a calculation. Every graded field here carries its own tolerance (one unit in
the place the digest prints its class), so a collision is a digest token within
that field's tolerance of its graded value.
...
def main():
    ...
    if '--plant' in sys.argv:
        fields[0][2] = 1248.411  # EGBEMA's heating value, which the digest prints
    bad, near = [], []
    pub = sorted(published)
    for tier, key, val, tol in fields:
        d = min(pub, key=lambda p: abs(abs(val) - p))
        near.append(f'  {tier:<13} {key:<34} {val}: nearest digest token {d:g}, {abs(abs(val) - d) / tol:.3g} tolerances away')
