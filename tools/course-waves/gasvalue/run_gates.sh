...
run engine-jest 0 bash -c "cd $R/packages/engines && npx jest __tests__/downstream.flareToValue __tests__/downstream.lpgCng __tests__/downstream.gasvalue.golden 2>&1 | grep -E '^Tests:|^Test Suites:' ; exit \${PIPESTATUS[0]}"
...
run capstone-generator 0 node gasvalue_capstone.mjs
...
run kit-sourceprose 0 node /root/dc-wavekit/sourceprose.mjs $W/gasvalue_dump.mjs $W/gasvalue_fields.mjs $W/gasvalue_capstone.mjs $W/gasvalue_fields_capstone.mjs --rules $W
python3 - <<'PY'
import hashlib, json
W = '/root/et-wip-gasvalue'
files = {'structure': 'structure.py', 'vendor': 'gate_vendor.sh', 'engine-jest': 'gate_vendor.sh', 'digest-repro': 'gate_repro.sh',
 ...
 'kit-digestfigures': '/root/dc-wavekit/digestfigures.py', 'capstone-generator': 'gasvalue_capstone.mjs', 'oracle-check': 'oracle_check.py',
