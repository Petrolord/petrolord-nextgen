#!/usr/bin/env python3
"""Compare the gasvalue seed ladder against its committed sources, FIELD BY FIELD:
...
canary run must print DISAGREE and exit 2:
  ...
  --canary-field  moves one graded expected value by one whole unit (a hundred
                  tolerances on the tonnes, far more on every other field);
  --canary-lesson renames one lesson key in one manifest;
  --canary-prompt changes the storage bank's gauge reading in the advanced
                  capstone prompt.
...
by type, so 67 against 67.0, or 0.01 against 0.010000000000000002, is a
...
REPO = os.environ.get(
    'REPO', UP if os.path.exists(os.path.join(UP, '.git')) else '/root/wt-et-gasvalue-nextgen')
SLUG, PREFIX, DATE = 'gasvalue', 'gv', '20261013'
...
cap_rows = re.split(r"\n\(\n  'gasvalue', ", course.split('insert into public.academy_capstones', 1)[1])[1:]
got_fields = []
for row in cap_rows:
    tier = row.split("'", 2)[1]
...
if CANARY_PROMPT:
    capjson['tiers']['advanced']['prompt'] = capjson['tiers']['advanced']['prompt'].replace('reads 245 bar', 'reads 246 bar', 1)
prompts_compared = 0
for row in cap_rows:
    tier = row.split("'", 2)[1]
...
for flag, what in ((CANARY, 'one character flipped in one intermediate explanation'),
                   ...
                   (CANARY_PROMPT, 'the storage bank gauge reading changed in the advanced prompt')):
    if flag:
