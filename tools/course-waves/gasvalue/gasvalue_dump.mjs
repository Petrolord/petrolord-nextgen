...
out('# ENGINES: engines/downstream/flareToValue.js and engines/downstream/lpgCng.js at petrolord-engines e972ae7 (MD45-1 after MD4-0, with the engines #232 copy pass and queue vocabularies), with modularRefinery.js for the capital power law, vendored in NextGen under packages/engines.');
...
out('');
out('METHANE FROM THE METHANE. The methane that escapes is the methane in the gas times one less the destruction efficiency. Unburned ethane and heavier carry no GWP here. Beside the engine, the figure a flare gives if every unburned carbon atom is counted as methane, computed here as the lb-mol a year (scfPerYear over SCF_PER_LBMOL) times carbonPerMol (every carbon atom, the CO2\'s included) times one less the destruction efficiency, weighed at FLARE_MOLAR_MASS.CH4 and converted with LB_PER_KG:');
