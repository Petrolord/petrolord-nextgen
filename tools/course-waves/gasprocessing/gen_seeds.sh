#!/usr/bin/env bash
# =============================================================================
# FC4 SEEDS, GENERATED FROM THE COMMITTED TREE AND NEVER FROM A WORKING TREE.
#
...
# that matches nothing a reviewer can read. On THIS wave that was not a
# hypothetical: nine of the twenty-one bank sources in the wave directory were
# behind the committed tree when this script was written, because every audit
# repair had been made in the repository.
#
...
#
# It then DIFFS what it generated against the five files in the worktree and says
# so either way, because a generator that writes over its own evidence proves
# nothing.
#
...
# tools/course-waves/gasprocessing -> the repository root, when this script is
# running from the committed mirror. A wave directory falls back to the named
# worktree.
if [ -d "$HERE/../../../.git" ] || [ -f "$HERE/../../../.git" ]; then
  DEFAULT_REPO=$(cd "$HERE/../../.." && pwd)
else
  DEFAULT_REPO=/root/wt-fc4-nextgen
fi
...
STAGE=$(mktemp -d /tmp/fc4seed.XXXXXX)
OUTDIR="$STAGE/out"
...
echo "staged: $(ls "$STAGE/banks" | wc -l) banks, 3 manifests, 3 headers, fields.json, precision.json, digest.txt"
echo
...
FC4_WAVE="$STAGE" FC4_COURSE_OUT="$OUTDIR/${DATE}_${PREFIX}_${SLUG}_course.sql" \
...
FC4_WAVE="$STAGE" \
  FC4_COURSE_SQL="$OUTDIR/${DATE}_${PREFIX}_${SLUG}_course.sql" \
  FC4_GOLIVE_OUT="$OUTDIR/${DATE}_${PREFIX}_${SLUG}_go_live.sql" \
