#!/usr/bin/env node
// THE EC9 NUMSWEEP: the kit's numsweep.mjs, run per tier, with ONE exemption and
...
//
//   node numsweep_joa.mjs [--tier beginner|intermediate|advanced] [--content DIR]
//   node numsweep_joa.mjs --banks DIR   the 21 bank JSON files instead of the
...
//   node numsweep_joa.mjs --selftest     the negative controls
//
...
const KIT = process.env.EC9_KIT || '/root/dc-wavekit';
...
const die = (m) => { console.log(`numsweep_joa REFUSES: ${m}`); process.exit(2); };
...
const run = (content, tiers, reasons) => {
  ...
  tiers.forEach((tier) => {
    ...
    const tmp = fs.mkdtempSync(path.join(os.tmpdir(), `ec9ns-${tier}-`));
...
const runBanks = (dir, reasons) => {
  ...
  const files = fs.readdirSync(dir).filter((f) => /^ec9[bia]_.*\.json$/.test(f)).sort();
  if (files.length !== 21) die(`expected 21 ec9 bank JSON files in ${dir}, found ${files.length}`);
  const tmp = fs.mkdtempSync(path.join(os.tmpdir(), 'ec9ns-banks-'));
  let exempt = 0;
  const noise = [];
  files.forEach((f) => {
...
const selftest = () => {
  ...
  console.log(`numsweep_joa selftest ${pass ? 'OK' : 'FAILED'}`);
  return pass ? 0 : 1;
};

if (argv.includes('--selftest')) process.exit(selftest());
const content = opt('--content') || path.join(REPO, 'src/content/courses/joa');
const tier = opt('--tier');
...
console.log(`[numsweep_joa] ${reasons.size} engine reason and message strings read from digest.txt`);
