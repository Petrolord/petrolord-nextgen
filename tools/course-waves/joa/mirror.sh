#!/usr/bin/env bash
# MIRROR THE EC9 WAVE INPUTS INTO THE NEXTGEN REPOSITORY, byte for byte. Every
# suite under src/components/course/panels/joa reads the committed copy by
...
DEST="$NG/tools/course-waves/joa"
mkdir -p "$DEST"
INPUTS=$(node -e "const w=require('$NG/tools/course-waves/waves.json');console.log(w.joa.inputs.join('\n'))")
n=0
for f in $INPUTS; do
