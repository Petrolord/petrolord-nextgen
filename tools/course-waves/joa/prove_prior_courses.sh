#!/usr/bin/env bash
# PROVE THE EC9 FOUNDATION LEAVES EVERY EARLIER COURSE THAT SHARES ITS FILES
# UNCHANGED.
#
# PART 1, THE TREE. This course vendors the jointVenture.js closure at 3ae56e7
# (vendor_joa.sh) and moves nothing else: every packages/engines path on this
# branch that differs from origin/main must be one of the ten paths VENDOR.json
# ledgers as group ec9-joa-course, each byte-identical to petrolord-engines
# 3ae56e7, plus VENDOR.json itself, whose only change is those ten ledger
# entries; engines/economics/cashflow.ts and engines/economics/afe.js (the two
# shared files jointVenture.js imports, kept at NextGen's blobs) must be
# byte-identical to origin/main. Nothing under packages/engines may be left
# uncommitted.
#
...
# canonical npv or applyPSC: EC8 gsa, EC7 pia, SC2 procurement, H5 qra; and
# every Data & AI wave, D1 dataqc to D5 appliedai, and H1 safetystats, H3 lopa
# and H4 consequence, which share the kit and the vendored tree), this
# rebuilds, from the COMMITTED kit copied to a
...
if [ -n "${SCR:-}" ]; then
  ...
  SCR=$(mktemp -d /tmp/ec9-prior-XXXXXX)
  trap 'rm -rf "$SCR"' EXIT
fi
fail=0
...
REV=3ae56e7
LEDGERED=$(python3 -c "import json,sys;print('\n'.join(sorted(e['path'] for e in json.load(open(sys.argv[1]))['knownDeviations'] if e.get('group')=='ec9-joa-course')))" "$ENG/VENDOR.json")
nled=$(printf '%s\n' "$LEDGERED" | grep -c .)
[ "$nled" = 10 ] || { echo "  VENDOR.json ledgers $nled paths as ec9-joa-course, expected 10"; fail=1; }
...
# VENDOR.json: the only change against the base is the ec9-joa-course ledger entries.
...
strip = lambda v: {**v, 'knownDeviations': [e for e in v['knownDeviations'] if e.get('group') != 'ec9-joa-course']}
ok = strip(new) == strip(old) and new['canonical'] == old['canonical']
print(f"  VENDOR.json: {'only the ec9-joa-course ledger entries differ; the pin ' + new['canonical']['commit'][:7] + ' is unchanged' if ok else 'DIFFERS beyond the ec9-joa-course entries'}")
sys.exit(0 if ok else 1)
PY
for sh in engines/economics/cashflow.ts engines/economics/afe.js; do
...
specs="D1:dataqc D2:mlcore D3:facies D4:forecastml D5:appliedai H1:safetystats H3:lopa H4:consequence H5:qra SC2:procurement EC7:pia EC8:gsa"
...
[ $fail = 0 ] && echo "PRIOR COURSES UNCHANGED: packages/engines differs from $BASE only by the ten ec9-joa-course paths (sha-identical to 3ae56e7) and their ledger entries, with cashflow.ts and afe.js byte-identical; D1 to D5, H1, H3, H4, H5, SC2 procurement, EC7 pia and EC8 gsa digests, fields and precision rebuild byte-identical; cashflow and fiscal rebuild byte-identical against $ENG" || { echo "PRIOR COURSES: A DIFFERENCE"; exit 1; }
