#!/usr/bin/env python3
"""RUN EVERY EC9 FOUNDATION GATE, WITH ITS NEGATIVE CONTROL, AND PIN WHAT RAN.
...
count the digest does not have.
    ...
    EC9_STAGE=lessons python3 run_gates.py   the same, once lesson prose exists
    EC9_STAGE=banks python3 run_gates.py     and every bank gate over the 21 banks
...
any other reason still fails the run. EC9_STAGE=lessons makes both expect 0.
...
STAGE = os.environ.get('EC9_STAGE', 'foundation')
FOUNDATION = STAGE == 'foundation'
# At the foundation, the stub reason each lesson gate must print to count as met.
STUB_REASON = {
    'numsweep_joa.mjs': r'held NOT ONE literal of seven or more significant figures',
...
PY_ORACLE = 'python3'  # the EC9 oracle is standard library only
...
RUNS = [
    ...
    ('joa_capstone.mjs', f'{HERE}/joa_capstone.mjs', ['node', f'{HERE}/joa_capstone.mjs'], 0),
    ...
    ('make_concepts.py (every quotation found in its text)', f'{HERE}/make_concepts.py', ['python3', f'{HERE}/make_concepts.py'], 0),
    ...
    ('quote_check.py', f'{HERE}/quote_check.py', ['python3', f'{HERE}/quote_check.py'], 0),
    ('quote_check.py NEGATIVE CONTROL', f'{HERE}/quote_check.py', ['python3', f'{HERE}/quote_check.py', '--plant'], 1),
    ...
    ('digestpromise_joa.py', f'{HERE}/digestpromise_joa.py', ['python3', f'{HERE}/digestpromise_joa.py'], 0),
    ('digestpromise_joa.py NEGATIVE CONTROL', f'{HERE}/digestpromise_joa.py', ['python3', f'{HERE}/digestpromise_joa.py', '--plant'], 1),
    ...
    ('gate_vocabulary.py NEGATIVE CONTROL licensed', f'{HERE}/gate_vocabulary.py', ['python3', f'{HERE}/gate_vocabulary.py', '--plant-licensed'], 1),
    ...
    ('numsweep_joa.mjs', f'{HERE}/numsweep_joa.mjs', ['node', f'{HERE}/numsweep_joa.mjs'], 1 if FOUNDATION else 0),
    ('numsweep_joa.mjs --selftest', f'{HERE}/numsweep_joa.mjs', ['node', f'{HERE}/numsweep_joa.mjs', '--selftest'], 0),
...
if STAGE in ('lessons', 'banks', 'full', 'final'):
    LESSONS = f'{REPO}/src/content/courses/joa'
...
# THE BANK STAGE. EC9_STAGE=banks (or full) runs everything above AND every bank
...
if BANKS:
    BK = f'{HERE}/banks'
    TIERS = [("ec9b", "beginner"), ("ec9i", "intermediate"), ("ec9a", "advanced")]
    ...
    RUNS += [
        ...
        ('numsweep_joa.mjs --banks', f'{HERE}/numsweep_joa.mjs', ['node', f'{HERE}/numsweep_joa.mjs', '--banks', BK], 0),
...
# THE FINAL (SHIP) STAGE. EC9_STAGE=final runs everything above AND the seed
...
if STAGE == 'final':
    MIG = f'{REPO}/migrations/20261110_ec9_joa_course.sql'
    RUNS += [
        ...
        ('promptleak.py --sql', f'{KIT}/promptleak.py', ['python3', f'{KIT}/promptleak.py', '--sql', MIG, '--course', 'joa'], 0),
    ]
