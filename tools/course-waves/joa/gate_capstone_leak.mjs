// GATE: nothing the EC9 capstone grades, no input it is set on and no field it
// names reaches the digest, the digest generator, the vendored golden inputs
...
//
// Adapted from EC8's gate_capstone_leak.mjs. The EC9 capstone inputs are
// SYNTHETIC JOINT VENTURES OF THEIR OWN (IDUMU, OKWELLE, ABIAMA): their own
// parties, months, budgets, scales, costs and entitlements. What is
// distinctive, and swept: every capstone label, every run of three
// consecutive figures of a capstone series (monthly forecasts and actuals,
// yearly costs, entitlements, revenues, net values) as the course might print
// them, every capstone name, and every capstone number of 100 or more that is
// fractional, or whole, at least 10000 and not a multiple of 100. Text that
// is verbatim golden-input text is teaching material and is left out and
// COUNTED.
//
// DIRECTIONS, every one counted and printed:
//   1. INPUTS. No distinctive capstone string, no run of three consecutive
//      capstone production figures and no distinctive capstone scalar in
//      digest.txt, joa_dump.mjs or the golden inputs the lab imports; no
//      capstone scalar equal to a number in the golden.
...
//   4. NAMES. No capstone name (IDUMU, OKWELLE, ABIAMA) in the digest, the
//      generator or the golden inputs.
...
//      learning page carry no graded value, no capstone name, no distinctive
//      capstone string, run or scalar.
//   9. THE BANKS, under --banks (default <wave>/banks); --no-banks declares
//      the foundation phase.
//  10. THE LESSONS, every .md under --lessons (default
//      <repo>/src/content/courses/joa), and THE BRIEFS (BRIEF.md,
...
const REPO = process.env.EC9_REPO || '/root/wt-ec9-nextgen';
const ENG = process.env.EC9_ENGINES || path.join(REPO, 'packages/engines');
const WAVES = process.env.EC9_WAVES || path.join(REPO, 'tools/course-waves');
...
const DUMP = read(path.join(HERE, 'joa_dump.mjs'));
const CAPSTONE = read(path.join(HERE, 'joa_capstone.mjs'));
const FIELDS = JSON.parse(read(path.join(HERE, 'fields.json')));
if (FIELDS.length !== 18) die(`fields.json carries ${FIELDS.length} fields`);
const INPUTS = JSON.parse(execFileSync('node', [path.join(HERE, 'joa_capstone.mjs'), '--inputs'],
  { encoding: 'utf8', stdio: ['ignore', 'pipe', 'ignore'], maxBuffer: 1e8, env: { ...process.env, EC9_ENGINES: ENG } }));
const NAMES = Object.keys(INPUTS);
if (NAMES.join() !== 'IDUMU,OKWELLE,ABIAMA') die(`unexpected capstone datasets ${NAMES.join()}`);

const findings = [];
const counts = {};
const count = (k, n = 1) => { counts[k] = (counts[k] || 0) + n; };

/* ---- the capstone inputs: distinctive strings, production runs and scalars ---- */
const GOLDPATH = path.join(ENG, 'test-data/economics/goldens/jointventure_cases.json');
const GOLD = JSON.parse(read(GOLDPATH));
const GOLD_INPUTS = GOLD.cases.map((c) => ({ id: c.id, args: c.args }));
...
walkStrings(GOLD_INPUTS, (t) => FIXTEXT.add(t));
const strings = [];
const series = [];
const scalars = [];
...
const addString = (name, k, v) => {
  ...
  // An option value of the engine or the lab (a recovery order, a price basis, a rounding
  // rule) is a word of the API that every case uses; a distinctive string has a space in it.
  if (/^[a-z0-9-]+$/.test(v)) { optionLeftOut += 1; return; }
  if (strings.some((x) => x.v === v)) return;
  strings.push({ name, k, v, low: v.toLowerCase() });
};
// Every whole number below 10000 or a multiple of 100 is left out and counted:
// a year, a rate, a round price or a round cost is a figure any example may
// use. What stays is distinctive: a fractional price, an uneven volume.
...
Object.entries(INPUTS).forEach(([name, o]) => {
  ...
  if (o.cashCalls) ['forecast', 'actual'].forEach((col) => series.push({ name, k: `cash call ${col}`, v: o.cashCalls.months.map((m) => m[col]) }));
  if (o.budget) ['approved', 'actual'].forEach((col) => series.push({ name, k: `budget ${col}`, v: o.budget.items.map((it) => it[col]) }));
  if (o.carry) ['cost', 'entitlement'].forEach((col) => series.push({ name, k: `carry ${col}`, v: o.carry.years.map((y) => y[col]) }));
  if (o.backIn) series.push({ name, k: 'back-in entitlement', v: o.backIn.years.map((y) => y.entitlement) });
  if (o.psc) ['grossRevenue', 'capex', 'opex'].forEach((col) => series.push({ name, k: `psc ${col}`, v: o.psc.years.map((y) => y[col]) }));
  if (o.soleRisk) ['grossValue', 'deductions'].forEach((col) => series.push({ name, k: `sole risk ${col}`, v: o.soleRisk.years.map((y) => y[col]) }));
});
console.log('  the stated years, percentages, prices below 100 and rates are small numbers any example uses, so they are left out of the scalars and printed as left out');
console.log(`  capstone whole numbers below 10000 or multiples of 100, left out: ${roundLeftOut}`);
console.log(`  capstone strings that are verbatim golden-input text, left out: ${fixtureLeftOut}`);
console.log(`  capstone strings that are engine or lab option values (no space, lower case), left out: ${optionLeftOut}`);

/* ---- the negative controls, planted in memory ---- */
if (has('--plant-value')) DIGEST += `\n| planted | ${FIELDS[3][2].toFixed(6)} |\n`;
if (has('--plant-input')) DIGEST += `\nA planted label: ${INPUTS.OKWELLE.label}.\n`;
if (has('--plant-name')) DIGEST += '\nA planted line naming Abiama.\n';

if (strings.length < 3) die(`only ${strings.length} distinctive capstone strings parsed`);
if (series.length < 6) die(`only ${series.length} capstone series parsed`);
// Runs of three consecutive production figures of one lease, as the course prints
// them (six decimals), with two decimals, and as the generator writes them. A run
...
if (runs.length < 20) die(`only ${runs.length} capstone runs of three built`);
...
// 1. INPUTS
// The lab imports the vendored golden inputs; they are what a learner's panel
// starts from, so they are swept as the lab's dataset.
const LAB_DATA = JSON.stringify(GOLD_INPUTS);
[['digest.txt', DIGEST], ['joa_dump.mjs', DUMP], ['the golden inputs', LAB_DATA]].forEach(([label, text]) => {
...
// 4. NAMES
const nameRe = /\b(idumu|okwelle|abiama)\b/i;
[['digest.txt', DIGEST], ['joa_dump.mjs', DUMP], ['the golden inputs', LAB_DATA]].forEach(([label, text]) => {
...
// 5. NEITHER FILE READS THE OTHER
[['joa_dump.mjs', DUMP, /joa_capstone|fields\.json/],
  ['joa_capstone.mjs', CAPSTONE, /joa_dump|digest\.txt/]].forEach(([label, text, re]) => {
...
if (fs.existsSync(WAVES)) {
  fs.readdirSync(WAVES).filter((w) => w !== 'joa').forEach((w) => {
...
// 8. THE APP
const APP = ['joaLab.js', 'AccountCalculator.jsx', 'RecoveryCalculator.jsx', 'AgreementCalculator.jsx', 'panelBits.jsx']
  .map((f) => path.join(REPO, 'src/components/course/panels/joa', f))
  .concat([path.join(REPO, 'src/pages/apps/JoaLearningPage.jsx')]);
APP.forEach((p) => {
  const text = read(p);
  count('8 app sources');
  const m = text.match(nameRe);
...
const lessonsDir = opt('--lessons') || path.join(REPO, 'src/content/courses/joa');
const lessonFiles = [];
let lessonLits = 0;
let plantedIn = null;
...
console.log(`gate_capstone_leak: ${strings.length} distinctive capstone strings, ${series.length} capstone production series, ${runs.length} runs of three, ${scalars.length} distinctive scalars, `
