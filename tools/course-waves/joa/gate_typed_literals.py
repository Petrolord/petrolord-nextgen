#!/usr/bin/env python3
"""GATE: no number is TYPED into the EC9 digest.

Adapted from EC8's gate_typed_literals.py (itself SC2's, D5's, D4's, D3's, D2's, D1's, H1's and FC9's). Every figure in digest.txt has to be
...
joa_dump.mjs OUTSIDE a ${...} substitution.
...
DUMP = os.path.join(os.environ.get('EC9_WAVE_DIR', os.path.dirname(os.path.abspath(__file__))), 'joa_dump.mjs')

ALLOWED = {
 # ---------------------------------------------------------------------------
 # WHAT IS ALLOWED, AND WHY. Every figure the engine returns, every golden
 # input, every fixture term and every figure a text prints (the FT table,
 # typed from IMF FARI Tables 12 and 13 with its citation) reaches the digest
 # as a substitution. What is left is below. A DEAD ENTRY FAILS THE GATE.
 # ---------------------------------------------------------------------------
 '0': 'a zero named in prose as a count or a call (a call of 0, a forecast of 0, grace hours of 0 when a contract gives none); never a measurement',
 '100': 'the whole of an interest in per cent, in the algebra the engine states: (100 - target) / (100 - current), 100 - royalty',
}
...
def main():
    ...
    body_src = strip_decls(body_src, ['ORDER', 'SOURCES', 'EXPORTS', 'DSRC', 'GROUPS', 'PLANTED', 'INT', 'SMALL', 'BUD', 'OH', 'LEDG', 'CR', 'SMALLC', 'BI', 'DEF', 'TRIG', 'NC', 'WBP', 'FAP', 'FT', 'CAPS', 'OPENOIL', 'PRINTED_BAND', 'IDENTITY_TOL'])
    ...
    for c in calls:
        ...
        body = re.sub(r'\b\d{4}-\d{2}\b', ' MONTHREF ', body)
        ...
        # EC8 ADDRESSES: a model agreement Article or its Alternatives, a
        # figure or box of a report, a manual page, a publication number, a
        # golden case id with digits in it, a stated reading's heading word.
        body = re.sub(r'\bArticles? \d+(?:\.\d+)*(?:(?:, | and | to )\d+(?:\.\d+)*)*', ' CITEREF ', body)
        # EC9 ADDRESSES: an article of the Norwegian agreement or the Kenya
        # model (Art. 9.3, 2.2.3, Art. 18.12), a briefing note or a table of a
        # report by its number, a version, a month, a day of a named month.
        body = re.sub(r'\bArt\. \d+(?:\.\d+)*(?:(?:, | and | to )\d+(?:\.\d+)*)*', ' CITEREF ', body)
        body = re.sub(r'\b(?:Briefing Note|Note|NOTE|Tables?|TABLES?|FIGURE|version) \d+(?:(?: and | AND )\d+)?', ' CITEREF ', body)
        body = re.sub(r'\b\d{1,2} (?:January|February|March|April|May|June|July|August|September|October|November|December)\b', ' DATEREF ', body)
        body = re.sub(r'\bAlternatives? \d+(?:&\d+[AB]?)?(?:(?: and |, )\d+)*', ' CITEREF ', body)
        body = re.sub(r'\b(?:Figure|Box|OT0|SP|NG|Report) ?\d+(?:/\d+)?', ' CITEREF ', body)
        body = re.sub(r'\b[a-z]+(?:-[a-z0-9]+)*-\d+[a-z0-9-]*\b', ' CASEREF ', body)
    ...
    # EC9: every rule figure in this digest is SUBSTITUTED from the engine's
    # exported constants or the golden file's published block, so a clean
    # digest leaves very few numeric literals in its prose; the refusal is on
    # the number of string literals read, and --plant proves the sweep fires.
    if len(calls) < 200:
