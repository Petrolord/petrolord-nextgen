#!/usr/bin/env bash
# VENDOR THE EC9 JOA CLOSURE (engines/economics/jointVenture.js) INTO NEXTGEN,
...
# the ekene-jv fixture through a spread `read(...)` helper the walker
...
# reason they travel.
#
# jointVenture.js imports engines/economics/cashflow.ts (applyPSC, npv) and
# engines/economics/afe.js (calculatePartnerCosts); cashflow.ts walks on to
# engines/economics/irrContract.js and lib/dates/dates.js, afe.js to
# lib/dates/dates.js.
#
# SHARED PATHS STAY WITH THEIR OWNERS. cashflow.ts is owned by the economics
# courses (NextGen's copy is the 006ed85 blob, the same blob as 3ae56e7) and
# afe.js by the EC5 fdp course (NextGen's copy is an older blob). Neither is
# moved by this wave: the functions jointVenture.js calls (applyPSC and npv;
# calculatePartnerCosts) are cut out of both copies by their exported text and
# must be SOURCE-IDENTICAL, the whole-file blobs are compared and printed, and
# the jointVenture suite is then run against NextGen's own copies and must pass
# every test.
#
# LEDGER MODE: THE PIN DOES NOT MOVE. New paths are ledgered as kind "extra",
# group "ec9-joa-course", pinned to the vendored blob, and clear by the
# guard's own STALE rule when the pin moves to 3ae56e7 or later.
#
# Four proofs per path: git blob hash, sha256, cmp, byte count.
set -euo pipefail
ENG=/root/petrolord-engines
NG=${NG:-/root/wt-ec9-nextgen}
PIN=$(python3 -c "import json,sys;print(json.load(open(sys.argv[1]))['canonical']['commit'])" "$NG/packages/engines/VENDOR.json")
# Vendored at 3ae56e7 (engines PR #270, jointVenture.js and its validation kit).
REV=${REV:-3ae56e7}
...
# Shared paths kept at NextGen's blob, with the functions jointVenture.js calls in each.
declare -A SHARED=(
  [engines/economics/cashflow.ts]='export function applyPSC|export function npv'
...
echo
cd "$EXP"
SEED=__tests__/economics.jointVenture.test.js
...
for extra in \
  "test-data/economics/goldens/jointventure_cases.json:read at runtime by the suite through read('test-data','economics','goldens',...)" \
  "test-data/economics/ekene-jv/ekene-jv.json:read at runtime by the suite; the course dataset (the Ekene joint venture, synthetic)" \
  "test-data/economics/ekene-jv/README.md:the fixture record (synthetic, planted situations)" \
  "tools/validation/economics/make_jv_fixtures.py:writes the ekene-jv fixture" \
  "tools/validation/economics/oracle_jointventure.py:writes the golden the suite reads" \
  "tools/validation/economics/timing_jointventure.js:produced the timings table FINDINGS carries" \
  "tools/validation/economics/FINDINGS-jointVenture.md:the validation record the engine header cites (sources, editions, read dates)" \
  "tools/validation/economics/negcontrol_jointventure.sh:produced the negative-control table FINDINGS carries"; do
  p="${extra%%:*}"; why="${extra#*:}"
...
echo
SC_ALL=$(git -C "$ENG" ls-tree -r --name-only "$FULL" | grep -iE 'jointventure|ekene-jv|jv_fixtures' | sort)
MISSED=$(comm -23 <(printf '%s\n' "$SC_ALL") <(printf '%s\n' "$PATHS") || true)
if [ -n "$MISSED" ]; then echo "REFUSES: jointVenture paths in ${FULL:0:7} outside the closure: $MISSED"; exit 1; fi
echo "  every jointVenture path at ${FULL:0:7} is in the closure"
echo

echo "SHARED PATHS LEFT WITH THEIR OWNERS (each function jointVenture.js calls compared by its exported text):"
for p in "${!SHARED[@]}"; do
...
printf '%s\n' "$PATHS" | while read -r p; do
  ...
  if [ -f "$b" ]; then
    if [ "$(git hash-object "$b")" != "$(git -C "$ENG" rev-parse "$FULL:$p")" ]; then
      # A path this wave already ledgered (group ec9-joa-course) may move
      # on a re-vendor; any other differing path refuses.
      if python3 -c "import json,sys;v=json.load(open(sys.argv[1]));sys.exit(0 if any(e['path']==sys.argv[2] and e.get('group')=='ec9-joa-course' for e in v['knownDeviations']) else 1)" "$NG/packages/engines/VENDOR.json" "$p"; then
...
for p in paths:
    ...
    added.append({
        ...
        'reason': (f"EC9 joa course: vendored sha-identical from petrolord-engines {full[:7]} "
                   f"(PR #270: engines/economics/jointVenture.js, its jest suite, golden, the ekene-jv fixture and its writer, oracle, "
                   f"timing script, FINDINGS, negative control) by the wave's vendor_joa.sh, 4 proofs per path. "
...
cd "$NG/packages/engines" && npx jest __tests__/economics.jointVenture.test.js 2>&1 | grep -E "^Tests:|^Test Suites:"
