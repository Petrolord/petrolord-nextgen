#!/usr/bin/env python3
"""Lay out the EC9 joa content tree from structure.py: the three tier
...
it left alone.
    ...
    python3 scaffold.py [--repo /root/wt-ec9-nextgen] [--check]
...
content = os.path.join(repo, 'src/content/courses/joa')

wrote = kept = manifests = drift = 0
for tier, modules in S.TIERS.items():
    manifest = {'app_slug': 'joa', 'tier': tier, 'content_version': 1, 'modules': []}
