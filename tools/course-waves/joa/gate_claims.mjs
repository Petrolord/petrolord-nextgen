...
// wave's RECON REPORT rather than from the digest, and both were wrong. In EC9 the
...
// truth. (Adapted from EC8's gate_claims.mjs, itself EC7's, SC2's, D5's, D4's, D3's, D2's,
// D1's, H1's and FC9's.)
//
...
const RECON = (() => { try { return fs.readFileSync(`${ENG}/test-data/economics/ekene-jv/README.md`, 'utf8'); } catch { return null; } })();
const FINDINGS = (() => { try { return fs.readFileSync(`${ENG}/tools/validation/economics/FINDINGS-jointVenture.md`, 'utf8'); } catch { return null; } })();
...
const ALLOWED = {
  74: 'the path order of the course in the economics module, settled by wave.json pathOrder and the plan of record',
...
BRIEFS.forEach((f) => {
  ...
  if (PLANT && f === 'BRIEF.md') {
    ...
    text += '\n\nThe fixture carries an overrun of 5,950,000 against a budget tolerance of 3,800,000.\n';
  }
  briefsRead += 1;
...
check('the legislated words are all in the vocabulary section',
  ['interest', 'carry', 'recovery', 'premium', 'cash call', 'overhead'].every((w) => VOCAB.includes(`| ${w} |`)),
...
check('the BRIEF quotes the three stated readings verbatim, each a string the digest prints in its readings section',
  (() => {
    const b = (read('BRIEF.md') || '').replace(/\s+/g, ' ');
    const sec = (DIGEST.split(/^# SECTION \d+: The three readings/m)[1] || '').split(/^# SECTION/m)[0];
    const qs = [...b.matchAll(/\d\. The (?:PSC income tax|grace|cover): "([^"]+)"/g)].map((m) => m[1]);
    return qs.length === 3 && qs.every((q) => sec.includes(q)) && Object.keys(WAVE.openReadings || {}).length === 4;
  })(), 'BRIEF.md against the readings section and wave.json openReadings');
check('the BRIEF states the no-worked-schedule rule and the licensed-text rule',
  /No public text prints a worked schedule/.test(read('BRIEF.md') || '') && /AIPN model joint operating/.test(read('BRIEF.md') || ''), 'BRIEF.md');
