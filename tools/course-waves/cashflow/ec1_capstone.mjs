...
//   node ec1_capstone.mjs --check <served>   also compare the two tiers whose fields are unchanged
import fs from 'fs';
...
if (i > 0) {
  ...
  for (const tier of ['beginner', 'intermediate']) {
    ...
    // The EC7 copy-rule pass rewords one contrastive in the Professional prompt;
    // everything else, and every field, must stay byte-identical to the served row.
    const PROMPT_MAY_CHANGE = { intermediate: true };
    for (const k of ['cert_tier', 'title', 'dataset', 'prompt']) {
      if (s[k] === m[k]) continue;
      if (k === 'prompt' && PROMPT_MAY_CHANGE[tier]) { console.log(`${tier}: prompt reworded (copy rule), fields unchanged`); continue; }
      bad += 1; console.log(`DIFFERS ${tier}.${k}`);
    }
  ...
  console.log(bad === 0 ? 'Associate capstone byte-identical to the served row; Professional fields, title and dataset byte-identical' : `${bad} difference(s) in the unchanged tiers`);
