#!/usr/bin/env python3
"""FULL literal sweep: EVERY numeric literal in a bank must appear in digest.txt.

numsweep skips round literals with trailing zeros, so it cannot see most of
this tier's figures. This sweeps all of them, matching on FLOAT VALUE so that
880 resolves against the digest's 880.000000 and -30 against -30.000000.
"""
    ...
    if os.path.exists(cfg_path):
        for k in (json.load(io.open(cfg_path, encoding='utf-8')).get('constants') or {}):
            ...
            try:
                ...
                pass
    as_const = 0
    banks = sorted(f for f in os.listdir(os.path.join(wave, 'banks'))
