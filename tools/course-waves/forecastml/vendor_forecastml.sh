#!/usr/bin/env bash
# VENDOR THE D4 FORECASTML CLOSURE (engines/dataai/forecast.js) INTO NEXTGEN,
# SHA-IDENTICAL, AND PROVE IT. Adapted from D3's vendor_facies.sh.
#
...
# those two, the oracle, the pin writer, the timing script, FINDINGS and the
# negative control are NAMED with the reason they travel. forecast.js imports
# lib/stats (mulberry32, quantile), lib/conventions/percentile.js (the P90 /
# P10 labels) and engines/dca/arps.js (fitArpsModel, calculateArpsHyperbolic);
...
#   tools/validation/dataai/synthetic_wells.js  (ec89b6b: syntheticProduction
#       and an arps.js import appended; D2's syntheticWells and D3's
#       syntheticFacies unchanged)
# The move is proved harmless to D1, D2 and D3 by rebuilding their merged
# digests and fields.json against the new tree byte-identically and by running
# the vendored ml and cluster suites (wave.json enginesVendoredAt records it).
# Any OTHER closure path ledgered by another group refuses.
#
...
# ec89b6b would also move economics cashflow goldens, facilities, production,
# seismolord and wellsite paths, reopening live graded values in other
# courses. Every closure path either matches the pinned manifest (lib/stats,
# lib/conventions/percentile.js and engines/dca/arps.js, byte-identical and
# already vendored), or is NEW since the pin and is ledgered as kind "extra",
# group "d4-forecastml-course", pinned to its vendored blob. The entries clear
# by the guard's own STALE rule when the pin moves to ec89b6b or later.
set -euo pipefail
...
echo
cd "$EXP"
SEED=__tests__/dataai.forecast.test.js
...
for extra in \
  "test-data/dataai/goldens/forecast_cases.json:read at runtime by the suite through read('test-data','dataai','goldens',...)" \
  "test-data/dataai/pins/forecast_pins.json:read at runtime by the suite through read('test-data','dataai','pins',...)" \
  "tools/validation/dataai/oracle_forecast.py:writes the golden the suite reads" \
  "tools/validation/dataai/pin_forecast.py:writes the library pins the suite reads" \
  "tools/validation/dataai/timing_forecast.mjs:produced the timings table FINDINGS carries" \
  "tools/validation/dataai/FINDINGS-forecast.md:the validation record the engine header cites" \
  "tools/validation/dataai/negcontrol_forecast.sh:produced the negative-control table FINDINGS carries"; do
  p="${extra%%:*}"; why="${extra#*:}"
...
echo
DA_ALL=$(git -C "$ENG" ls-tree -r --name-only "$FULL" | grep -E '(^|/)dataai/|dataai\.' | grep -E '(^|[/._])forecast([._]|$)|synthetic_wells' | sort)
MISSED=$(comm -23 <(printf '%s\n' "$DA_ALL") <(printf '%s\n' "$PATHS") || true)
if [ -n "$MISSED" ]; then echo "REFUSES: dataai forecast paths in ${FULL:0:7} outside the closure: $MISSED"; exit 1; fi
echo "  every dataai forecast path at ${FULL:0:7} is in the closure"
echo
...
GROUP = 'd4-forecastml-course'
MOVABLE = {('tools/validation/dataai/synthetic_wells.js', 'd3-facies-course')}
canon = {}
...
print(f'  VENDOR.json: pin {pin[:7]} unchanged; {same} closure path(s) already match the manifest; group {GROUP}: {len(added)} path(s) ledgered; moved from D3: {moved}; left with their owner group at the same blob: {sorted(owned)}')
for e in added:
