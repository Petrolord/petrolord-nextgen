#!/usr/bin/env bash
# =============================================================================
# D3 SCRATCH DATABASE. A local Postgres 16 the ladder is iterated on BEFORE any
...
# production).
#
# Adapted from tools/course-waves/mlcore/scratch_db.sh (D2). The four
...
# and 53 to 58, and the TWENTY THREE FC, Commercial & Trading, Energy Transition,
...
# 65, D1 dataqc at 66, D2 mlcore at 67), so the path_order, catalogue and prompt
# checks meet real neighbours and real capstone rows. Every path_order from 1 to 67 is asserted
...
# what `dryrun_d4.sh` with TARGET=linked is for (rolled back, snapshotted
...
#
# Usage: scratch_db.sh [ref]     (re)creates container d3-scratch; default HEAD
# =============================================================================
set -eu
REF=${1:-HEAD}
...
for f in $(git -C "$REPO" ls-tree --name-only "$REF" migrations/ \
           | grep -E '_(fc[0-9]+_[a-z]+|cr_crude|rf_refinery|tds_supply|gv_gasvalue|cef_carbon|asrc_riskchange|cq_compliance|h[1-5]_[a-z]+|d1_dataqc|d2_mlcore)_course\.sql$'); do
  git -C "$REPO" show "$REF:$f" | P
  n=$((n + 1))
done
[ "$n" = 23 ] || { echo "REFUSED: expected the 23 seeded course migrations at $REF, found $n"; exit 2; }
# No hole: every path_order 1..67 must hold a row, stand-in or real, so a
# neighbour course the pattern above misses refuses instead of passing short.
holes=$(P -Atc "select coalesce(string_agg(g::text, ',' order by g), '') from generate_series(1, 67) g where not exists (select 1 from public.academy_apps a where a.path_order = g)")
