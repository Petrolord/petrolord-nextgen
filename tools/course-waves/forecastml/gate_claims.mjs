...
// wave's RECON REPORT rather than from the digest, and both were wrong. In D4 the
// engine's FINDINGS record, the oracle, the library pins and the engine's source
// comments are PROVENANCE, and none of them is teaching truth. (Adapted from D3's
// gate_claims.mjs, itself D1's, H1's and FC9's, through D2's.)
//
...
const ENG = process.env.D4_ENGINES || '/root/wt-dai-d4-nextgen/packages/engines';
...
const RECON = null;
const FINDINGS = (() => { try { return fs.readFileSync(`${ENG}/tools/validation/dataai/FINDINGS-forecast.md`, 'utf8'); } catch { return null; } })();
...
const ALLOWED = {
  69: 'the path order of the course in the data_ai module, settled by wave.json pathOrder and the plan of record',
...
BRIEFS.forEach((f) => {
  ...
  if (PLANT && f === 'BRIEF.md') {
    ...
    text += '\n\nThe engine gate runs 432 tests.\n';
  }
  briefsRead += 1;
  ...
  [...prose.matchAll(NUM)].forEach((m) => {
    ...
    numbersSwept += 1;
    // A DECLARED figure is checked FIRST: the path order 69 is settled by
...
check('the six legislated words are all in the vocabulary section',
  ['forecast', 'error', 'P90', 'accuracy', 'trend', 'machine learning'].every((w) => (DIGEST.split(/^# SECTION 25:/m)[1] ?? '').includes(`| ${w} |`)),
  'digest section 25');
check('the BRIEF tier table matches the owners the digest prints: sections 1 to 10 Associate, 11 to 17 Professional, 18 to 24 Expert, 3 and 25 all',
  (() => {
    ...
    return heads.every((h) => {
      ...
      if (n <= 17) return /^Professional/.test(own);
      if (n <= 24) return /^Expert/.test(own);
