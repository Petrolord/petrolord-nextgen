#!/usr/bin/env python3
"""GATE: no number is TYPED into the D4 digest.

Adapted from D3's gate_typed_literals.py (itself D2's, D1's, H1's and FC9's). Every figure in digest.txt has to be
...
d4_dump.mjs OUTSIDE a ${...} substitution.
...
DUMP = os.path.join(os.environ.get('D4_WAVE_DIR', os.path.dirname(os.path.abspath(__file__))), 'd4_dump.mjs')
...
ALLOWED = {
 ...
 # arithmetic on printed values. Every entry below is a NAME, a LABEL, a
 # CONVENTION the engine's basis states, or a CITATION rather than a
 # measurement. A DEAD ENTRY FAILS THE GATE.
 # ---------------------------------------------------------------------------
 '0':    'zero: month 0, index 0, the lower end of [0, 1], a rate of 0, a 0/0 term, h of 0',
 '1':    'one: n - 1 and t - 1 in stated recursions, the upper end of [0, 1], alpha at 1, m = 1, step 1',
 '2':    'two: the second month y_2, e^2 in stated formulas, 2 scored residuals the bootstrap needs, 2|e| in the sMAPE basis',
 '100':  'percent: the 0 to 100 alternative scale of sMAPE, a convention and never a measurement',
 '200':  'the top of the sMAPE scale, 0 to 200, as the engine basis states it',
 '2006': 'the year of Hyndman and Koehler, in its citation',
 '8.2':  'the FPP3 section number, in its citation',
}
...
def main():
    ...
    for c in calls:
        ...
        body = re.sub(r'\bEKENE-P\d+\b', ' WELLNAME ', body)
