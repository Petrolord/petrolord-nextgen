#!/usr/bin/env python3
"""Lay out the D4 forecastml content tree from structure.py: the three tier
...
it left alone.
    ...
    python3 scaffold.py [--repo /root/wt-dai-d4-nextgen] [--check]
...
content = os.path.join(repo, 'src/content/courses/forecastml')

wrote = kept = manifests = drift = 0
for tier, modules in S.TIERS.items():
    manifest = {'app_slug': 'forecastml', 'tier': tier, 'content_version': 1, 'modules': []}
