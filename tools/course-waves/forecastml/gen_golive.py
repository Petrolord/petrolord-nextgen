...
1. AGAINST THE ENGINE LEDGER. gen_course.py runs `node d4_capstone.mjs --json`
...
Usage: python3 gen_golive.py
   D4_WAVE, D4_REPO, D4_ENGINES, D4_TOLERANCE as gen_course.py
   D4_COURSE_SQL  the course migration this ladder emits, for the prompt checks
   D4_GOLIVE_OUT  where to write
"""
import hashlib
import os
import re
...
os.environ['D4_WAVE'] = W
import gen_course as GC  # noqa: E402  (runs the engine; writes nothing on import)

DATE = GC.DATE
COURSE = os.environ.get('D4_COURSE_SQL', f'{REPO}/migrations/{DATE}_d4_forecastml_course.sql')
OUT = os.environ.get('D4_GOLIVE_OUT', f'{REPO}/migrations/{DATE}_d4_forecastml_go_live.sql')
...
# AGULU
w(f"  v_p := pg_temp.d4_opt({A2}, 'ses');")
conv('agulu2_ses_alpha', 'v_p[5]')
route('agulu2_ses_alpha', ['v_s := v_p[1];'])
w(f"  v_r := pg_temp.d4_run({A1}, 'holt', {fl(AGS['alpha'])}, {fl(AGS['beta'])}, null);")
route('agulu1_holt_fixed_mse_bopd2', [f'v_s := v_r[1] / (array_length({A1}, 1) - 2)::double precision;'])
w(f"  v_p := pg_temp.d4_opt({A1}, 'holt');")
conv('agulu1_holt_beta', 'v_p[5]')
route('agulu1_holt_beta', ['v_s := v_p[2];'])
w(f"  v_r := pg_temp.d4_run({A1}, 'holt', v_p[1], v_p[2], null);")
route('agulu1_holt_forecast_h12_bopd', [f"v_s := (pg_temp.d4_fc('holt', v_r[2], v_r[3], null, {AH}))[{AH}];"])
w(f"  v_p := pg_temp.d4_opt({A1}, 'damped');")
conv('agulu1_damped_phi', 'v_p[5]')
route('agulu1_damped_phi', ['v_s := v_p[3];'])
w(f"  v_r := pg_temp.d4_run({A1}, 'damped', v_p[1], v_p[2], v_p[3]);")
route('agulu1_damped_forecast_h24_bopd', [f"v_s := (pg_temp.d4_fc('damped', v_r[2], v_r[3], v_p[3], {AD}))[{AD}];"])
...
NK_SETUP = [
    ...
    f"v_f := pg_temp.d4_fc('damped', v_r[2], v_r[3], v_p[3], {NH});",
...
w(f"  v_bt := pg_temp.d4_bt({N2}, 'holt', {NFO}, {NHZ}, {NST}, true, 1, null);")
w(f"  v_bh := pg_temp.d4_bt({N2}, 'holt', {NFO}, {NHZ}, {NST}, false, 1, null);")
...
# UMUNZE
UM_SETUP = [
    f"v_p := pg_temp.d4_opt({U1}, 'damped');",
    f"v_b := pg_temp.d4_boot({U1}, 'damped', v_p[1], v_p[2], v_p[3], {UH}, {USIM}, {USEED}, true);",
    f"v_r := pg_temp.d4_run({U1}, 'damped', v_p[1], v_p[2], v_p[3]);",
    f"v_f := pg_temp.d4_fc('damped', v_r[2], v_r[3], v_p[3], {UH});",
]
for l in UM_SETUP:
    w('  ' + l)
...
w(f"  v_ar := pg_temp.d4_arps({U1}, 'Auto-Select');")
...
for mt in METHODS:
    w(f"  v_bt := pg_temp.d4_bt({U2}, '{mt}', {UFO}, {UHZ}, {UST}, true, 1, 'Auto-Select');")
    if mt == 'arps':
...
# ------------------------------------------------------------ 3. the traps
w('  -- ------------------------------------------------------------ 3. the traps')
trap('agulu2_ses_alpha', 'the grid start quoted', [f"v_wrong := (pg_temp.d4_grid({A2}, 'ses'))[1];"])
trap('agulu2_ses_alpha', 'fitted on the first three years', [f"v_wrong := (pg_temp.d4_opt({A2}[1:36], 'ses'))[1];"])
trap('agulu2_ses_alpha', 'the other well', [f"v_wrong := (pg_temp.d4_opt({A1}, 'ses'))[1];"])
w(f"  v_r := pg_temp.d4_run({A1}, 'holt', {fl(AGS['alpha'])}, {fl(AGS['beta'])}, null);")
...
trap('agulu1_holt_beta', 'the grid start quoted', [f"v_wrong := (pg_temp.d4_grid({A1}, 'holt'))[2];"])
w(f"  v_r := pg_temp.d4_run({A1}, 'holt', v_p[1], v_p[2], null);")
w(f"  v_f := pg_temp.d4_fc('holt', v_r[2], v_r[3], null, {AH + 1});")
...
w(f"  v_r := pg_temp.d4_run({A1}, 'holt', {fl(AGS['alpha'])}, {fl(AGS['beta'])}, null);")
trap('agulu1_holt_forecast_h12_bopd', 'the stated parameters used', [f"v_wrong := (pg_temp.d4_fc('holt', v_r[2], v_r[3], null, {AH}))[{AH}];"])
w(f"  v_p := pg_temp.d4_opt({A1}, 'damped');")
trap('agulu1_damped_phi', 'the upper bound quoted', ['v_wrong := 0.98;'])
trap('agulu1_damped_phi', 'the alpha quoted', ['v_wrong := v_p[1];'])
trap('agulu1_damped_phi', 'the grid start quoted', [f"v_wrong := (pg_temp.d4_grid({A1}, 'damped'))[3];"])
w(f"  v_r := pg_temp.d4_run({A1}, 'damped', v_p[1], v_p[2], v_p[3]);")
w(f"  v_f := pg_temp.d4_fc('damped', v_r[2], v_r[3], v_p[3], {AD + 1});")
...
trap('nanka1_holdout_damped_mase', 'scaled by the whole series', [
    f'v_wrong := (select avg(abs(v_a[i] - v_f[i])) from generate_subscripts(v_a, 1) i) / pg_temp.d4_q({N1}, 1);'])
...
trap('nanka2_backtest_holt_rmse_bopd', 'a step of 1 between origins', [f"v_wrong := (pg_temp.d4_bt({N2}, 'holt', {NFO}, {NHZ}, 1, true, 1, null))[1];"])
trap('nanka2_backtest_holt_held_mase', 'the refitted run', ['v_wrong := v_bt[3];'])
trap('nanka2_backtest_holt_held_mase', 'a lag of 12', [f"v_wrong := (pg_temp.d4_bt({N2}, 'holt', {NFO}, {NHZ}, {NST}, false, 12, null))[3];"])
...
w(f"  v_b := pg_temp.d4_boot({U1}, 'damped', v_p[1], v_p[2], v_p[3], {UH}, {USIM}, {USEED + 1}, true);")
...
trap('umunze1_arps_di_per_month', 'the exponential forced', [f"v_wrong := (pg_temp.d4_arps({U1}, 'Exponential'))[2];"])
trap('umunze1_arps_di_per_month', 'the harmonic forced', [f"v_wrong := (pg_temp.d4_arps({U1}, 'Harmonic'))[2];"])
w('  v_cm := array[]::double precision[];')
for mt in METHODS:
    w(f"  v_cm := v_cm || (pg_temp.d4_bt({U2}, '{mt}', {UFO}, {UHZ}, {UST}, true, 1, 'Auto-Select'))[3];")
w('  v_rk := array(select v from unnest(v_cm) v order by v);')
trap('umunze2_compare_arps_mase', 'the best MASE quoted', ['v_wrong := v_rk[1];'])
trap('umunze2_compare_arps_mase', 'the arps MAE quoted', [f"v_wrong := (pg_temp.d4_bt({U2}, 'arps', {UFO}, {UHZ}, {UST}, true, 1, 'Auto-Select'))[2];"])
trap('umunze2_compare_arps_mase', 'an exponential baseline', [f"v_wrong := (pg_temp.d4_bt({U2}, 'arps', {UFO}, {UHZ}, {UST}, true, 1, 'Exponential'))[3];"])
