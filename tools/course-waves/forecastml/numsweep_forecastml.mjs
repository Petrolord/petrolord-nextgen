#!/usr/bin/env node
// THE D4 NUMSWEEP: the kit's numsweep.mjs, run per tier, with ONE exemption and
...
// digit (the digest shows EKENE-P1's Arps b as returned, 0.49999999999999994,
// beside its six-decimal field).
...
//
//   node numsweep_forecastml.mjs [--tier beginner|intermediate|advanced] [--content DIR]
//   node numsweep_forecastml.mjs --selftest     the negative controls
//
...
const KIT = process.env.D4_KIT || '/root/dc-wavekit';
...
const die = (m) => { console.log(`numsweep_forecastml REFUSES: ${m}`); process.exit(2); };
...
const run = (content, tiers, reasons) => {
  ...
  tiers.forEach((tier) => {
    ...
    const tmp = fs.mkdtempSync(path.join(os.tmpdir(), `d4ns-${tier}-`));
...
const selftest = () => {
  ...
  console.log(`numsweep_forecastml selftest ${pass ? 'OK' : 'FAILED'}`);
  return pass ? 0 : 1;
};

if (argv.includes('--selftest')) process.exit(selftest());
const content = opt('--content') || path.join(REPO, 'src/content/courses/forecastml');
const tier = opt('--tier');
...
console.log(`[numsweep_forecastml] ${reasons.size} engine reason and message strings read from digest.txt`);
