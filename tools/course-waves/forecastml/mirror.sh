#!/usr/bin/env bash
# MIRROR THE D4 WAVE INPUTS INTO THE NEXTGEN REPOSITORY, byte for byte. Every
# suite under src/components/course/panels/forecastml reads the committed copy by
...
DEST="$NG/tools/course-waves/forecastml"
mkdir -p "$DEST"
INPUTS=$(node -e "const w=require('$NG/tools/course-waves/waves.json');console.log(w.forecastml.inputs.join('\n'))")
n=0
for f in $INPUTS; do
