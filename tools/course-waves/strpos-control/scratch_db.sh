#!/usr/bin/env bash
# =============================================================================
# GENERIC FACILITIES SCRATCH DATABASE. A dry run's target, and never production.
#
# A copy of tools/course-waves/metering/scratch_db.sh with the container name
# and the excluded wave taken from the environment:
#   SCRATCH=<container> EXCLUDE=fc6_ scratch_db.sh [ref]
#
...
#
# Usage: SCRATCH=<name> EXCLUDE=fcN_ scratch_db.sh [ref]   (re)creates container <name>
# =============================================================================
set -eu
REF=${1:-HEAD}
...
SQL
n=0
for f in $(git -C "$REPO" ls-tree --name-only "$REF" migrations/ | grep -E '_fc[0-9]+_[a-z]+_course\.sql$' | grep -v "${EXCLUDE:?}"); do
  git -C "$REPO" show "$REF:$f" | P
  n=$((n + 1))
done
echo "$C ready: 4 academy tables from the repository's own DDL, $n committed Facilities course migrations loaded at $REF"
