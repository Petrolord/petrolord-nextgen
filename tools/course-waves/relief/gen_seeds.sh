#!/usr/bin/env bash
# =============================================================================
# FC5 SEEDS, GENERATED FROM THE COMMITTED TREE AND NEVER FROM A WORKING TREE.
#
...
# that matches nothing a reviewer can read. FC5 ran three tier audits that made
...
if [ -d "$HERE/../../../.git" ] || [ -f "$HERE/../../../.git" ]; then
  ...
  DEFAULT_REPO=/root/wt-fc5-nextgen
fi
...
STAGE=$(mktemp -d /tmp/fc5seed.XXXXXX)
OUTDIR="$STAGE/out"
...
FC5_WAVE="$STAGE" FC5_COURSE_OUT="$OUTDIR/${DATE}_${PREFIX}_${SLUG}_course.sql" \
...
FC5_WAVE="$STAGE" \
  FC5_COURSE_SQL="$OUTDIR/${DATE}_${PREFIX}_${SLUG}_course.sql" \
  FC5_GOLIVE_OUT="$OUTDIR/${DATE}_${PREFIX}_${SLUG}_go_live.sql" \
