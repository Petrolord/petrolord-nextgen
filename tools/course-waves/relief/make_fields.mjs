// fields.json and precision.json: the eighteen graded FC5 fields with the
...
// repository, which fc5_capstone.mjs and the teaching lab both import. This
...
const rows = JSON.parse(execFileSync('node', ['/root/fc-wip-relief/fc5_capstone.mjs', '--json'],
...
const out = rows.map((r) => {
  const { stated, cls } = { stated: GRADED_FIELDS.find(([, k]) => k === r.key)[3], cls: r.cls };
...
if (!BARE) {
  fs.writeFileSync('/root/fc-wip-relief/fields.json', `${JSON.stringify(out, null, 1)}\n`);
  const precision = precisionDeclaration();
  fs.writeFileSync('/root/fc-wip-relief/precision.json', `${JSON.stringify(precision, null, 1)}\n`);
