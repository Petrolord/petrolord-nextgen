#!/usr/bin/env bash
# =============================================================================
# FC5 DRY RUN: THE WHOLE LADDER, ROLLED BACK.
#
...
# apply_fc5_relief.sh and it is the owner's to run. The go-live is HELD
...
# fail. FC5's field keys ARE unique across tiers, so a key alone names the
...
CONTROL=${2:-}
TIER=${3:-intermediate}
FIELD=${4:-ogbainbiri_liquid_area_fraction}
# The size of the move. One part in 1e7 by default. SEVENTEEN OF THE EIGHTEEN
# ARE ASSERTED AT AN ABSOLUTE EPSILON FAR TIGHTER THAN THE FIELD'S OWN SHIPPED
# TOLERANCE, so a move of this size is caught on any of those seventeen. The
# eighteenth, the blowdown time, is asserted against the exact integral at
# 5e-5 s because the march sits 3.98e-5 s from it, and that is said in the
# go-live's header rather than hidden. The default field is deliberately the
# SMALLEST graded value on the wave, the liquid area fraction at about 0.3119,
# where one part in 1e7 is an absolute move of about 3.1e-8: a sixteenth of the
# field's own 5e-7 tolerance, which would pass a grader, and it is still refused
# here.
MULT=${5:-1.0000001}
...
if up=$(cd "$HERE/../../.." 2>/dev/null && pwd) && { [ -d "$up/.git" ] || [ -f "$up/.git" ]; }; then
  ...
  REPO=${REPO:-/root/wt-fc5-nextgen}
fi
LINKED=${LINKED:-/opt/petrolord-studio/workspaces/dev1/projects/petrolord-nextgen}
RUN=$(mktemp -d /tmp/fc5dry.XXXXXX)

FILES="20260925_fc5_relief_course
...
union all select 'path_order 43 held', count(*)::text from public.academy_apps where path_order = 43
...
union all select 'relief any',   count(*)::text from public.academy_apps where slug = 'relief'
...
# ------------------------------------------------- the ladder, then rollback
{
  ...
  for f in $FILES; do
    ...
    echo
    if [ "$CONTROL" = --control ] && [ "$f" = 20260925_fc5_relief_advanced_deep ]; then
...
select 'DRY RUN' as what,
       'relief ' || a.status
       || ' | ' || (select count(*) from public.academy_course_structures where app_slug='relief' and active)::text || ' tiers'
       || ' | ' || (select count(*) from public.academy_course_structures s,
                           lateral jsonb_array_elements(s.structure->'modules') m,
                           lateral jsonb_array_elements_text(m->'lesson_keys') lk
                     where s.app_slug='relief' and s.active)::text || ' lessons'
       || ' | ' || (select count(*) from public.academy_quiz_questions where app_slug='relief')::text || ' questions ('
       || (select string_agg(n::text, '/' order by ord) from (
             select case tier when 'beginner' then 1 when 'intermediate' then 2 else 3 end ord, count(*) n
               from public.academy_quiz_questions where app_slug='relief' group by tier) t) || ')'
       || ' | ' || (select count(*) from public.academy_capstones where app_slug='relief')::text || ' capstones'
       || ' | ' || (select count(*) from public.academy_capstones c, lateral jsonb_array_elements(c.fields) f
                     where c.app_slug='relief')::text || ' graded'
       ...
       || ' coming_soon' as v
  from public.academy_apps a where a.slug = 'relief';
rollback;
SQL
...
if [ "$CONTROL" = --control ]; then
  if [ $FAILED = 1 ] && grep -q "$FIELD" "$RUN/out.txt"; then
    ...
    grep -oE "FC5 go-live refused[^\"]*" "$RUN/out.txt" | head -2 | sed 's/^/    /'
    exit 0
  fi
