#!/usr/bin/env python3
"""FC9 lesson length gate: PROSE WORDS per lesson, against that lesson's own
...
THE DOCSTRING OF THE TWO PRIVATE COPIES USED TO SAY HEADINGS WERE EXCLUDED AND
NEITHER COPY EVER DID IT. LESSON_TASK.md, scaffold.py's stub text and
structure.py's comment all repeated the claim. A writer who trusted any of them
ran twenty to forty words LIGHT against the floor on every lesson. The words were
corrected to match the code rather than the other way round, because the counts
of every wave already written were produced by the code.
...
CONTENT = os.environ.get(
    'FC9_CONTENT', '/root/wt-fc9-nextgen/src/content/courses/corrosion')
...
def selftest():
    ...
    import tempfile
    root = tempfile.mkdtemp(prefix='fc9-lengths-')
