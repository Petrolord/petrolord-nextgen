...
union all select 'path_order 47 held', count(*)::text from public.academy_apps where path_order = 47
...
select 'DRY RUN',
       ...
       || ' | ' || (select count(*) from public.academy_course_structures s,
                     ...
                     where s.app_slug='corrosion' and s.active)::text || ' lessons'
       || ' | ' || (select count(*) from public.academy_quiz_questions where app_slug='corrosion')::text || ' questions ('
       || (select string_agg(n::text, '/' order by ord) from (
             select case tier when 'beginner' then 1 when 'intermediate' then 2 else 3 end ord, count(*) n
               from public.academy_quiz_questions where app_slug='corrosion' group by tier) t) || ')'
       || ' | ' || (select count(*) from public.academy_capstones where app_slug='corrosion')::text || ' capstones'
