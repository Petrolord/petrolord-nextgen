#!/usr/bin/env bash
# =============================================================================
# FC9 "Corrosion & Integrity" SEED LADDER, content-addressed.
#
...
# THE TWO STEPS.
#
#   seed      Step 1. Catalogue row goes in as `coming_soon` at path_order 47,
...
#
# =========================== THE UPLOAD GATE ================================
# DO NOT RUN `go-live` UNTIL A PRODUCTION UPLOAD CARRIES /dashboard/apps/corrosion.
#
# The 78 lessons, the teaching lab (corrosionLab.js) and its three explorer
...
# DashboardPage chunk and confirm it carries `apps/corrosion`. `go-live`
...
#
# USAGE
#   ./apply_fc9_corrosion.sh verify        content check only, no database
#   ./apply_fc9_corrosion.sh seed          step 1
#   ./apply_fc9_corrosion.sh go-live       step 2, ONLY after the upload
#   ./apply_fc9_corrosion.sh pin <ref>     reprint the digest table
#
...
# which PR moved it.
FILES="
20260925_fc9_corrosion_course
20260925_fc9_corrosion_beginner_deep
20260925_fc9_corrosion_intermediate_deep
...
digest_for() {
  case $1 in
    ...
    20260925_fc9_corrosion_go_live          ) echo 26fed9456b52a238c0fc2cab198360bb789b6eccb683bb2a5242124beba29a92 ;;
    *) echo UNPINNED ;;
  esac
}
SEEDS="20260925_fc9_corrosion_course
20260925_fc9_corrosion_beginner_deep
20260925_fc9_corrosion_intermediate_deep
20260925_fc9_corrosion_advanced_deep"
GOLIVE=20260925_fc9_corrosion_go_live

RUN=$(mktemp -d /tmp/fc9apply.XXXXXX)
...
resolve() {
  ...
  Either $REF does not yet carry the FC9 course PR, or a later change moved a seed.
...
select 'course' as what, status as v from public.academy_apps where slug = 'corrosion'
union all select 'tiers', count(*)::text from public.academy_course_structures where app_slug = 'corrosion' and active
...
union all select 'questions', count(*)::text from public.academy_quiz_questions where app_slug = 'corrosion'
union all select 'capstones', count(*)::text from public.academy_capstones where app_slug = 'corrosion'
...
case "${1:-verify}" in
  ...
  seed)
    resolve
    echo
    echo "STEP 1: seeding FC9 HIDDEN as coming_soon."
    ...
    echo "DO NOT run go-live until an upload carries /dashboard/apps/corrosion."
    ;;
  ...
  go-live)
    ...
    echo "'apps/corrosion' in it. Checking a hash is not this check."
    printf "Type exactly  apps/corrosion  to proceed: "
    read -r answer
    [ "$answer" = "apps/corrosion" ] || refuse "upload gate not confirmed"
    run_file "$GOLIVE"
    echo
    state
    echo
    echo "FC9 is available. Facilities now carries its ninth course."
