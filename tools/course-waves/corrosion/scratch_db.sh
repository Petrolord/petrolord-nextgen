#!/usr/bin/env bash
# =============================================================================
# FC9 SCRATCH DATABASE. The dry run's target, and never production.
#
...
#
# Usage: scratch_db.sh [ref]     (re)creates container fc9-scratch
# =============================================================================
set -eu
REF=${1:-HEAD}
...
SQL
n=0
for f in $(git -C "$REPO" ls-tree --name-only "$REF" migrations/ | grep -E '_fc[0-9]+_[a-z]+_course\.sql$' | grep -v fc9_); do
  git -C "$REPO" show "$REF:$f" | P
  n=$((n + 1))
done
echo "fc9-scratch ready: 4 academy tables from the repository's own DDL, $n committed Facilities course migrations loaded at $REF"
