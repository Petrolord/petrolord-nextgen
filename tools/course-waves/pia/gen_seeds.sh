#!/usr/bin/env bash
# =============================================================================
# EC7 SEEDS AND CASE FILES, GENERATED FROM THE COMMITTED TREE AND NEVER FROM A
# WORKING TREE. Adapted from tools/course-waves/procurement/gen_seeds.sh (SC2).
#
# This script exports the COMMITTED tree at REF with `git archive` into a
# throwaway directory: the wave mirror tools/course-waves/pia (the
...
# bank JSONs under tools/course-banks/pia, the three manifests, the
# tolerance module gradedTolerance.js and the whole of packages/engines (the
# vendored cashflow.ts at 3.12.0 and fiscalConventions.js, and
...
if [ -d "$HERE/../../../.git" ] || [ -f "$HERE/../../../.git" ]; then
  ...
  DEFAULT_REPO=/root/wt-ec7-nextgen
fi
...
STAGE=$(mktemp -d /tmp/ec7seed.XXXXXX)
OUTDIR="$STAGE/out"
...
export EC7_WAVE="$W" EC7_WAVE_DIR="$W" EC7_REPO="$STAGE/repo" EC7_ENGINES="$STAGE/repo/packages/engines" \
       EC7_TOLERANCE="$STAGE/repo/src/components/course/panels/$SLUG/gradedTolerance.js"
( cd "$W" && EC7_COURSE_OUT="$OUTDIR/${DATE}_${PREFIX}_${SLUG}_course.sql" EC7_CASES_OUT="$OUTDIR/cases" \
...
( cd "$W" && EC7_COURSE_SQL="$OUTDIR/${DATE}_${PREFIX}_${SLUG}_course.sql" \
    EC7_GOLIVE_OUT="$OUTDIR/${DATE}_${PREFIX}_${SLUG}_go_live.sql" \
