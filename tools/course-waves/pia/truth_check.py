#!/usr/bin/env python3
"""truth-pia.json MUST BE THE HARVEST OF THE digest.txt BESIDE IT.

numsweep resolves lesson literals against truth-*.json, which the wave's
harvest_pia.py (the kit's harvest_digest.py plus comma-grouped figures)
...
truth = os.path.join(HERE, 'truth-pia.json')
if not os.path.exists(truth):
    print('REFUSED: no truth-pia.json; run harvest_pia.py'); sys.exit(2)
tmp = tempfile.mkdtemp(prefix='ec7-truth-')
...
subprocess.run(['python3', os.path.join(HERE, 'harvest_pia.py'), tmp], check=True, capture_output=True)
same = filecmp.cmp(truth, os.path.join(tmp, 'truth-pia.json'), shallow=False)
n = len(__import__('json').load(open(truth)))
shutil.rmtree(tmp)
print(f'truth_check: truth-pia.json ({n} digest lines) {"IS" if same else "IS NOT"} the harvest of digest.txt')
if '--plant' in sys.argv:
