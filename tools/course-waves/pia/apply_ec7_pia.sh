#!/usr/bin/env bash
# =============================================================================
# EC7 pia "Petroleum Industry Act 2021 & Nigerian Fiscal Terms" SEED LADDER, content-addressed.
...
# sha256 below, pinned to the head of feat/ec7-pia-course, or the run REFUSES
...
#
# THE FIVE FILES.
#   20261107_ec7_pia_course      catalogue row (coming_soon, module
#                                         economics, path_order 72, no
#                                         prerequisite) + 3 capstones, 18
#                                         graded fields
#   20261107_ec7_pia_{beginner,intermediate,advanced}_deep
#                                         3 structures, 78 lesson keys, 396 questions
#   20261107_ec7_pia_go_live            HELD. coming_soon -> available, behind
#                                         the shape, grader, prompt, engine-ledger,
#                                         second-route, oracle and trap assertions.
#
# =========================== THE UPLOAD GATE ================================
# The 78 lessons, the teaching lab (piaLab.js), the three calculator panels
...
# site serves /dashboard/apps/pia: serve the built DashboardPage chunk and see
# `apps/pia` in it. `apply --prod --go-live` will not run until you type it.
# ============================================================================
#
# MODES
#   verify          content pins only. Touches no database.
#   dryrun          LOCAL SCRATCH ONLY: scratch_db.sh at $REF, then the whole
#                   ladder rolled back (dryrun_ec7.sh), the ladder run twice for
#                   idempotence, the six negative controls (ledger, route,
#                   oracle, trap, prompt, leak), and verify_sql.py with its
#                   canaries. The scratch container ec7-scratch is removed
#                   afterwards.
#   attempts        PRODUCTION, READ-ONLY: every learner row that already names
#                   pia (enrolments, lesson progress, quiz and capstone
...
#                   snapshotted before and after and compared (dryrun_ec7.sh
...
digest_for() {
  case $1 in
    20261107_ec7_pia_course               ) echo 723fb241707b040bc727db2d88ecd15497a2a6dac67e509a5cbd77ca944ff9a5 ;;
    20261107_ec7_pia_beginner_deep        ) echo 85717de1cbff7d06d6c4a4c1cd60d83fbe02e8d65525a54c201f9867aa930436 ;;
    20261107_ec7_pia_intermediate_deep    ) echo 4d76c6b959ad2235c2d50152ae30e64d920c8055227b50e15a9ebba5e5fd6048 ;;
    20261107_ec7_pia_advanced_deep        ) echo 591e4a11eb7d97b318d274fc606ccafdff54a81ba62a545969ee6e8b9f0bd3a3 ;;
    20261107_ec7_pia_go_live              ) echo 31e6fdc0d8ea3503e0405dc03db4a7faeb84eb3fb016ba0ebedc42237c1c87c5 ;;
    *) echo UNPINNED ;;
  esac
}
...
# The hash of every pia row the seeds write, so a seed inside BEGIN ... ROLLBACK
# can say whether it would change anything.
HASH_Q="md5(coalesce((select string_agg(md5(a::text), ',') from (select slug, name, module, path_order, prereq_slug from public.academy_apps where slug = 'pia') a), '')
 || coalesce((select string_agg(md5(s::text), ',' order by s.tier) from (select tier, structure, content_version, active from public.academy_course_structures where app_slug = 'pia') s), '')
 || coalesce((select string_agg(md5(q::text), ',' order by q.tier, q.scope, q.module_key, q.ord) from (select tier, scope, module_key, ord, prompt, options, answer_index, explanation, active from public.academy_quiz_questions where app_slug = 'pia') q), '')
 || coalesce((select string_agg(md5(c::text), ',' order by c.tier) from (select tier, cert_tier, dataset, title, prompt, fields from public.academy_capstones where app_slug = 'pia') c), ''))"

COURSE_SEED=20261107_ec7_pia_course
...
golive_state() {
  echo "select 'EC7|' || coalesce((select status from public.academy_apps where slug = 'pia'), 'absent') as ec7_golive;" > "$RUN/golive.sql"
  out=$(prod_q "$RUN/golive.sql")
  case "$(grep -oE 'EC7\|[a-z_]+' <<<"$out" | head -1)" in
    'EC7|available') echo APPLIED ;;
    'EC7|coming_soon') echo HELD ;;
    'EC7|absent') echo "HELD (the course row is not seeded yet)" ;;
    *) echo UNKNOWN ;;
  esac
}
...
select 'ATT|' || string_agg(t || ' ' || n, ' | ' order by t) as ec7_attempts from (
  select 'enrollments' t, count(*) n from public.academy_enrollments where app_slug = 'pia'
  union all select 'lesson_progress', count(*) from public.academy_lesson_progress where app_slug = 'pia'
  union all select 'quiz_attempts', count(*) from public.academy_quiz_attempts where app_slug = 'pia'
  union all select 'capstone_attempts', count(*) from public.academy_capstone_attempts where app_slug = 'pia'
  union all select 'certifications', count(*) from public.academy_certifications where app_slug = 'pia') x;
SQL
  out=$(prod_q "$RUN/attempts.sql")
  line=$(grep -oE 'ATT\|[^"]+' <<<"$out" | head -1)
  [ -n "$line" ] || { echo "$out" | tail -5; refuse "could not read the attempt counts"; }
  say "  ${line#ATT|}"
  say "  A new course must show 0 everywhere. A non-zero count means pia rows already exist in production: stop and read them before applying."
}

rows() {
  d=$(date -u +%F)
  for f in $SEEDS; do
    echo "| $d | \`$f.sql\` | EC7 Petroleum Industry Act 2021 & Nigerian Fiscal Terms (pia), seed | **APPLIED $d** by the owner via tools/course-waves/pia/apply_ec7_pia.sh apply --prod from $REF (content pinned by sha256; scratch dry run and prod rolled-back dry run first) |"
  done
  echo "| $d | \`$GOLIVE.sql\` | EC7 Petroleum Industry Act 2021 & Nigerian Fiscal Terms (pia), GO-LIVE: coming_soon to available | **APPLIED $d** via apply_ec7_pia.sh apply --prod --go-live, after the deployed site served /dashboard/apps/pia |"
}

dryrun() {
  local ok=1
  REPO="$NG_REPO" bash "$HERE/scratch_db.sh" "$REF" | tail -2 || refuse "scratch_db.sh failed"
  REPO="$NG_REPO" bash "$HERE/dryrun_ec7.sh" "$REF" | tail -3 | sed 's/^/  /'; [ "${PIPESTATUS[0]}" = 0 ] || ok=0
  REPO="$NG_REPO" bash "$HERE/dryrun_ec7.sh" "$REF" --idempotent | grep -E '^IDEMPOTENT' | sed 's/^/  /'; [ "${PIPESTATUS[0]}" = 0 ] || ok=0
  for c in ledger route oracle trap prompt leak; do
    REPO="$NG_REPO" bash "$HERE/dryrun_ec7.sh" "$REF" --control "$c" | grep -E '^CONTROL' | sed 's/^/  /'; [ "${PIPESTATUS[0]}" = 0 ] || ok=0
  done
  ...
  docker rm -f ec7-scratch >/dev/null 2>&1 && say "  scratch container ec7-scratch removed"
...
case "${1:-verify}" in
  ...
  prod-dryrun) fetch; say "target: PRODUCTION, the whole ladder rolled back"
               TARGET=linked LINKED="$NG_REPO" REPO="$NG_REPO" bash "$HERE/dryrun_ec7.sh" "$SHA" || refuse "the production rolled-back dry run did not come back clean" ;;
  apply)
    ...
    if [ "${3:-}" = "--go-live" ]; then
      ...
      say "THE GO-LIVE. Confirm you have SERVED the built DashboardPage chunk and SEEN 'apps/pia' in it."
      printf "Type exactly  apps/pia  to proceed: "; read -r answer
      [ "$answer" = "apps/pia" ] || refuse "upload gate not confirmed"
    ...
    say; say "SEEDED. pia is coming_soon and no learner can reach it."
    say "Now run: $0 rows, and paste the four seed rows over their NOT YET APPLIED rows in MIGRATIONS.md."
    say "The go-live stays HELD until the deployed site serves /dashboard/apps/pia." ;;
  rows)        rows ;;
