...
any lesson body or manifest title, and in the three EC7 calculator panels, their shared bits and the lab
...
<wave>/banks, EC7_BANKS to override). Headings included.
...
BANKS = os.environ.get('EC7_BANKS', os.path.join(HERE, 'banks'))
APP_TEXT = [os.path.join(REPO, 'src/components/course/panels/pia', f) for f in
...
def main():
    ...
    if os.path.isdir(BANKS):
        names = sorted(n for n in os.listdir(BANKS) if re.match(r'^ec7[bia]_.*\.json$', n))
        planted_only = False
        if '--plant-bank' in sys.argv and not names:
            ...
            names, planted_only = ['ec7b_planted.json'], True
        for n in names:
    ...
    if '--plant-bank' in sys.argv:
        caught = len([f for f in bad if f[0].startswith('ec7')])
