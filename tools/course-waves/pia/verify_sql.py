#!/usr/bin/env python3
"""Compare the EC7 pia seed ladder against its committed sources, FIELD BY FIELD:
...
REPO = os.environ.get(
    'REPO', UP if os.path.exists(os.path.join(UP, '.git')) else '/root/wt-ec7-nextgen')
SLUG, PREFIX, DATE = 'pia', 'ec7', '20261107'
...
cap_rows = re.split(r"\n\(\n  'pia', ", course.split('insert into public.academy_capstones', 1)[1])[1:]
got_fields = []
for row in cap_rows:
    tier = row.split("'", 2)[1]
