#!/usr/bin/env bash
# =============================================================================
# EC7 SCRATCH DATABASE. A local Postgres 16 the ladder is iterated on BEFORE any
...
# production).
#
# Adapted from tools/course-waves/procurement/scratch_db.sh (SC2). The four
...
# and 53 to 58 (the economics courses among them), and the TWENTY SEVEN FC,
# Commercial & Trading, Energy Transition, Assurance, HSE, Data & AI and SC2
# courses read out of the COMMITTED course migrations at REF (39 to 52, 59 to
# 71, SC2 procurement at 71), so the path_order, catalogue and prompt checks
# meet real neighbours and real capstone rows. Every path_order from 1 to 71 is
# asserted occupied, so a hole refuses rather than passing short. EC7 pia joins
# at 72.
#
...
# what `dryrun_ec7.sh` with TARGET=linked is for (rolled back, snapshotted
...
#
# Usage: scratch_db.sh [ref]     (re)creates container ec7-scratch; default HEAD
#        (remove it afterwards with: docker rm -f ec7-scratch)
# =============================================================================
set -eu
REF=${1:-HEAD}
...
for f in $(git -C "$REPO" ls-tree --name-only "$REF" migrations/ \
           | grep -E '_(fc[0-9]+_[a-z]+|cr_crude|rf_refinery|tds_supply|gv_gasvalue|cef_carbon|asrc_riskchange|cq_compliance|h[1-5]_[a-z]+|d1_dataqc|d2_mlcore|d3_facies|d4_forecastml|d5_appliedai|sc2_procurement)_course\.sql$'); do
  git -C "$REPO" show "$REF:$f" | P
  n=$((n + 1))
done
[ "$n" = 27 ] || { echo "REFUSED: expected the 27 seeded course migrations at $REF, found $n"; exit 2; }
# No hole: every path_order 1..71 must hold a row, stand-in or real, so a
# neighbour course the pattern above misses refuses instead of passing short.
holes=$(P -Atc "select coalesce(string_agg(g::text, ',' order by g), '') from generate_series(1, 71) g where not exists (select 1 from public.academy_apps a where a.path_order = g)")
