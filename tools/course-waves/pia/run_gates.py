#!/usr/bin/env python3
"""RUN EVERY EC7 FOUNDATION GATE, WITH ITS NEGATIVE CONTROL, AND PIN WHAT RAN.
...
count the digest does not have.
    ...
    EC7_STAGE=lessons python3 run_gates.py   the same, once lesson prose exists
    EC7_STAGE=banks python3 run_gates.py     and every bank gate over the 21 banks
...
any other reason still fails the run. EC7_STAGE=lessons makes both expect 0.
...
STAGE = os.environ.get('EC7_STAGE', 'foundation')
FOUNDATION = STAGE == 'foundation'
# At the foundation, the stub reason each lesson gate must print to count as met.
STUB_REASON = {
    'numsweep_pia.mjs': r'held NOT ONE literal of seven or more significant figures',
...
PY_ORACLE = 'python3'  # the EC7 oracle is standard library only
...
RUNS = [
    ...
    ('pia_capstone.mjs', f'{HERE}/pia_capstone.mjs', ['node', f'{HERE}/pia_capstone.mjs'], 0),
    ...
    ('digestpromise_pia.py', f'{HERE}/digestpromise_pia.py', ['python3', f'{HERE}/digestpromise_pia.py'], 0),
    ('digestpromise_pia.py NEGATIVE CONTROL', f'{HERE}/digestpromise_pia.py', ['python3', f'{HERE}/digestpromise_pia.py', '--plant'], 1),
    ...
    ('numsweep_pia.mjs', f'{HERE}/numsweep_pia.mjs', ['node', f'{HERE}/numsweep_pia.mjs'], 1 if FOUNDATION else 0),
    ('numsweep_pia.mjs --selftest', f'{HERE}/numsweep_pia.mjs', ['node', f'{HERE}/numsweep_pia.mjs', '--selftest'], 0),
