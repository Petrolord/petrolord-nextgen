#!/usr/bin/env python3
"""Lay out the EC7 pia content tree from structure.py: the three tier
...
it left alone.
    ...
    python3 scaffold.py [--repo /root/wt-ec7-nextgen] [--check]
...
content = os.path.join(repo, 'src/content/courses/pia')

wrote = kept = manifests = drift = 0
for tier, modules in S.TIERS.items():
    manifest = {'app_slug': 'pia', 'tier': tier, 'content_version': 1, 'modules': []}
