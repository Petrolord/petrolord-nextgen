// GATE: nothing the EC7 capstone grades, no input it is set on and no field it
...
//
// Adapted from SC2's gate_capstone_leak.mjs. The EC7 capstone inputs are
// SYNTHETIC LEASES OF THEIR OWN (ODOZI, NKEMDI, ALAKU): their own terms,
...
//      digest.txt, pia_dump.mjs or the golden inputs the lab imports; no
...
//   4. NAMES. No capstone name (ODOZI, NKEMDI, ALAKU) in the digest, the
...
//      <repo>/src/content/courses/pia), and THE BRIEFS (BRIEF.md,
...
const ENG = process.env.EC7_ENGINES || path.join(REPO, 'packages/engines');
const WAVES = process.env.EC7_WAVES || path.join(REPO, 'tools/course-waves');
...
const DUMP = read(path.join(HERE, 'pia_dump.mjs'));
const CAPSTONE = read(path.join(HERE, 'pia_capstone.mjs'));
const FIELDS = JSON.parse(read(path.join(HERE, 'fields.json')));
if (FIELDS.length !== 18) die(`fields.json carries ${FIELDS.length} fields`);
const INPUTS = JSON.parse(execFileSync('node', [path.join(HERE, 'pia_capstone.mjs'), '--inputs'],
  { encoding: 'utf8', stdio: ['ignore', 'pipe', 'ignore'], maxBuffer: 1e8, env: { ...process.env, EC7_ENGINES: ENG } }));
const NAMES = Object.keys(INPUTS);
if (NAMES.join() !== 'ODOZI,NKEMDI,ALAKU') die(`unexpected capstone datasets ${NAMES.join()}`);
...
const GOLDPATH = path.join(ENG, 'test-data/economics/goldens/pia2021_cases.json');
const GOLD = JSON.parse(read(GOLDPATH));
const GOLD_INPUTS = GOLD.cases.map((c) => ({ name: c.name, note: c.note, cfg: c.cfg, prodRows: c.prodRows, capexRows: c.capexRows, opexRows: c.opexRows }));
...
Object.entries(INPUTS).forEach(([name, o]) => {
  ...
  ['oil_bbl', 'gas_mscf', 'condensate_bbl'].forEach((col) => {
    ...
    if (v.some((x) => x !== 0)) series.push({ name, k: `${col} rows`, v });
  });
});
...
if (has('--plant-input')) DIGEST += `\nA planted label: ${INPUTS.NKEMDI.label}.\n`;
if (has('--plant-name')) DIGEST += '\nA planted line naming Alaku.\n';

if (strings.length < 3) die(`only ${strings.length} distinctive capstone strings parsed`);
if (series.length < 6) die(`only ${series.length} capstone production series parsed`);
...
[['digest.txt', DIGEST], ['pia_dump.mjs', DUMP], ['the golden inputs', LAB_DATA]].forEach(([label, text]) => {
...
// 4. NAMES
const nameRe = /\b(odozi|nkemdi|alaku)\b/i;
[['digest.txt', DIGEST], ['pia_dump.mjs', DUMP], ['the golden inputs', LAB_DATA]].forEach(([label, text]) => {
...
// 5. NEITHER FILE READS THE OTHER
[['pia_dump.mjs', DUMP, /pia_capstone|fields\.json/],
  ['pia_capstone.mjs', CAPSTONE, /pia_dump|digest\.txt/]].forEach(([label, text, re]) => {
...
if (fs.existsSync(WAVES)) {
  fs.readdirSync(WAVES).filter((w) => w !== 'pia').forEach((w) => {
...
// 8. THE APP
const APP = ['piaLab.js', 'RoyaltyCalculator.jsx', 'HctCalculator.jsx', 'LedgerCalculator.jsx', 'panelBits.jsx']
  .map((f) => path.join(REPO, 'src/components/course/panels/pia', f))
  .concat([path.join(REPO, 'src/pages/apps/PiaLearningPage.jsx')]);
APP.forEach((p) => {
  const text = read(p);
  count('8 app sources');
  const m = text.match(nameRe);
...
const lessonsDir = opt('--lessons') || path.join(REPO, 'src/content/courses/pia');
const lessonFiles = [];
let lessonLits = 0;
let plantedIn = null;
