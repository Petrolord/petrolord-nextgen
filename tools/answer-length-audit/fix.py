...
def text_checks(tag, old, new, course, errs, derived=None):
    ...
    extra = set(NUM.findall(new)) - set(NUM.findall(old or '')) - lesson_numbers(course) - set(derived or {})
    if extra:
...
def apply(rows, grp):
    ...
    for it in grp.get('items', []):
        ...
        n = new[k]
        course = k[0]
        derived = it.get('derived_numbers') or {}
        for num, how in derived.items():
            if not how or not all(t in lesson_numbers(course) for t in NUM.findall(how) if t != num):
                errs.append(f'{tag}: derived number {num} needs a derivation from figures the lessons print')
        ...
        for f in ('prompt', 'explanation'):
            if f in ch:
                ...
                text_checks(f'{tag} {f}', c['old'], c['new'], course, errs, derived)
                n[f] = c['new']
        for j, c in (ch.get('options') or {}).items():
            ...
            text_checks(f'{tag} option {j}', c['old'], c['new'], course, errs, derived)
            n['options'][j] = c['new']
    ...
    written = {key(it): {int(j) for j in ((it.get('changes') or {}).get('options') or {})} for it in grp.get('items', [])}
    for k in touched:
        o, a = new[k]['options'], new[k]['answer_index']
        if len(set(o)) != len(o):
            errs.append(f'{label(k)}: duplicate option text')
        # a tie is refused when this group creates it: on an insert, when the
        # correct option is rewritten, or on a distractor the group rewrites. A
        # tie the served row already carried between options the group leaves
        # alone (bare numbers of equal width) is not the group's to refuse.
        check = range(len(o)) if (k in ins_keys or a in written.get(k, set())) else written.get(k, set())
        was = idx.get(k)
        def created(j):
            if len(o[j]) != len(o[a]):
                return False
            # a rewritten distractor that already tied the untouched correct option
            # (a bare figure of the same width) keeps a tie; it does not create one
            return not (was and j in written.get(k, set()) and a not in written.get(k, set())
                        and len(was['options'][j]) == len(was['options'][a]))
        if any(created(j) for j in check if j != a):
...
def sql(g, container):
    ...
    for k in idx:
        counts[k[0]] += 1
    ins_keys = {key(i) for i in grp.get('inserts', [])}
    tag = grp.get('tag') or f'B4 fix {g}'
    after = grp.get('runs_after')
    L = [
        '-- ==========================================================================',
        f'-- {grp.get("banner") or "B4 FOLLOW-ON FIX"} ({g}): {grp.get("title", "")}',
        '--',
        ...
        *([f'-- tools/answer-length-audit/fix.py sql {g}. Runs AFTER {after}:',
           '-- every guard below is the text those leave behind.'] if after else
